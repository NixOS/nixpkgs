...
for line in "${autoLines[@]}"; do
    ...
    trap 'echo -e "\e[33m\e[1mRestart this script after resolving the merge conflict as described above\e[0m"' exit
    git rebase --onto "$parent" "$(git merge-base "$targetBranch" HEAD)"
    trap - exit
