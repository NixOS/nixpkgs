...
- Sometimes, there is no `pull_request.number`. That's the case for `push`, `schedule`, `workflow_dispatch` and `workflow_run` events. To ensure non-PR runs are never cancelled, we add a fallback of `github.run_id`. This is a unique value for each workflow run.
