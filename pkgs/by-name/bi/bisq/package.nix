...
stdenvNoCC.mkDerivation rec {
  ...
  src = fetchurl {
    url = "https://github.com/bisq-network/${pname}/releases/download/v${version}/Bisq-${version}.deb";
  ...
  signature = fetchurl {
    url = "https://github.com/bisq-network/${pname}/releases/download/v${version}/Bisq-${version}.deb.asc";
  ...
  installPhase = ''
    ...
    install -D -m 777 ${bisq-launcher ""} $out/bin/${pname}
    substituteAllInPlace $out/bin/${pname}

    install -D -m 777 ${bisq-launcher "-Dglass.gtk.uiScale=2.0"} $out/bin/${pname}-hidpi
    substituteAllInPlace $out/bin/${pname}-hidpi

    for n in 16 24 32 48 64 96 128 256; do
      size=$n"x"$n
      magick convert opt/${pname}/lib/Bisq2.png -resize $size ${pname}.png
