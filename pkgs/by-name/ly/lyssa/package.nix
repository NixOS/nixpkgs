...
stdenv.mkDerivation {
  ...
  buildInputs = [ cglm libclipboard libxcb glfw ];
  nativeBuildInputs = [ pkg-config taglib makeWrapper ];

  postFixup = ''
      wrapProgram $out/bin/lyssa \
      --prefix PATH : ${
      lib.makeBinPath [ jq exiftool stb miniaudio yt-dlp ffmpeg python3Packages.glad ]
    }
'';
