...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru = {
    updateScript = writeScript "update-font-bh-lucidatypewriter-75dpi" ''
      #!/usr/bin/env nix-shell
      #!nix-shell -i bash -p common-updater-scripts
      version="$(list-directory-versions --pname font-bh-lucidatypewriter-75dpi \
        --url https://xorg.freedesktop.org/releases/individual/font/ \
        | sort -V | tail -n1)"
      update-source-version font-bh-lucidatypewriter-75dpi "$version"
    '';
  };
