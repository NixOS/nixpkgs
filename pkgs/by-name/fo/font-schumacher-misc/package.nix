...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru = {
    updateScript = writeScript "update-font-schumacher-misc" ''
      #!/usr/bin/env nix-shell
      #!nix-shell -i bash -p common-updater-scripts
      version="$(list-directory-versions --pname font-schumacher-misc \
        --url https://xorg.freedesktop.org/releases/individual/font/ \
        | sort -V | tail -n1)"
      update-source-version font-schumacher-misc "$version"
    '';
  };
