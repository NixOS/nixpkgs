...
python.pkgs.pythonPackages.buildPythonPackage {
  ...
  patches = [
    # Allow configuring the sqlite db path and default to db.sqlite3 in the cwd, upstream
    # hardcodes a path relative to the source code
    ./sqlite-path.patch
    # Harcode version to the one set in the derivation
    ./version.patch
  ];
  postPatch = ''
    rm siteroot/settings/dev.py
    substituteInPlace bookmarks/views/settings.py --subst-var-by version ${common.version}
  '';
