...
stdenv.mkDerivation (finalAttrs: {
  ...
  # TODO: remove after 2.13.1
  patches = [
    # temporary fix for CMAKE_INSTALL_LIBDIR
    (fetchpatch {
      name = "fix-ep-install-lib-path.patch";
      url = "https://github.com/evaleev/libint/commit/f47820bd2b8eb1b71b3076249a8f516f1a5316e4.patch";
      hash = "sha256-igOQ+l5i1dJFE+RD4684lkpG4ZwYEVUvx8/q+Yo8XQE=";
    })
  ];
  ...
  cmakeFlags = [
    # libint only works with a relative path
  ...
  # fix the hard-coded path to the basis set directory
  # that is inserted from `LIBINT_DATADIR_ABSOLUTE`
  postInstall = ''
    mv $out$out/include/libint2/basis.h $out/include/libint2/
    rm -rf $out/nix
  '';
