...
assert lib.assertMsg (builtins.elem sslBackend [ "gnutls" "libressl" "openssl" "none" ]) "sslBackend must be one of 'gnutls', 'libressl', 'openssl', or 'none'";
let
  dbusOnDarwinWarning = lib.warnIf (enableDbus && stdenv.isDarwin) "DBus is not supported on Darwin. Disabling.";
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ zlib ]
    ...
    ++ lib.optionals useMdnsresponder (with darwin; [
  ...
  meta = {
    ...
    changelog = "https://github.com/OpenPrinting/libcups/releases/tag/v${version}";
