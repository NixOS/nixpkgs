    ...
    })
    {
      command = [
        (writeShellScript "update-hashes.sh" ''
          ...
          initialPath="${lib.concatStringsSep " " stdenv.initialPath} ${gitMinimal} ${nurl}"
          ...
          if [ x"$UPDATE_NIX_OLD_VERSION" != x"$packageVersion" ]; then
            ...
            yarnOfflineCacheHashNew=$(nurl -e "let pkgs = (import $nixpkgs/. { }); in pkgs.fetchYarnDeps { yarnLock = pkgs.$UPDATE_NIX_ATTR_PATH.src + \"/app/yarn.lock\"; hash = pkgs.lib.fakeHash; }")
