...
clangStdenv.mkDerivation (
  ...
  in
  {
    ...
    postUnpack = ''
      ...
      cp -r ${tt-metal}/include $NIX_BUILD_TOP/$sourceRoot/build/include
      chmod -R u+w $NIX_BUILD_TOP/$sourceRoot/build/include

      rm -rf $NIX_BUILD_TOP/$sourceRoot/build/include/spdlog
      rm -rf $NIX_BUILD_TOP/$sourceRoot/build/include/ttnn

      cp -r build/_deps/spdlog-src/include/spdlog $NIX_BUILD_TOP/$sourceRoot/build/include/spdlog
      cp build/_deps/reflect-src/reflect $NIX_BUILD_TOP/$sourceRoot/build/include/reflect
    ...
    preConfigure = ''
      ...
      export TT_METAL_HOME=$NIX_BUILD_TOP/$sourceRoot/third_party/tt-metal/src/tt-metal

      # tt-mlir cannot find its own headers
      export NIX_CFLAGS_COMPILE="$NIX_CFLAGS_COMPILE -I$NIX_BUILD_TOP/$sourceRoot/include -I$NIX_BUILD_TOP/$sourceRoot/build/include"

      cmakeFlagsArray+=(
        -DMODEL_EXPLORER_SOURCE_DIR=$NIX_BUILD_TOP/$sourceRoot/tools/explorer/model-explorer/src/model-explorer
        -DMETAL_SOURCE_DIR=$NIX_BUILD_TOP/$sourceRoot/third_party/tt-metal/src/tt-metal
      )
