...
newVer = subprocess.run(
    ["nix-instantiate", "--eval", "--raw", "-A", "nextcloud-talk-desktop.version"],
    capture_output=True,
    encoding="locale",
).stdout

darwinUrl = f"https://github.com/nextcloud-releases/talk-desktop/releases/download/v{newVer}/Nextcloud.Talk-macos-universal.dmg"
...
oldDarwinHash = subprocess.run(
    [
        ...
        "--raw",
        "-A",
        f"nextcloud-talk-desktop.passthru.hashes.darwin",
    ],
    capture_output=True,
    encoding="locale",
).stdout
