...
//!
//! Runs once, gated by `ConditionPathExists`. Will be removed once the perl
...
/// Append `lines` to `path`, inserting a newline first if the file lacks a
/// trailing one. Assumes the file already exists (see `touch`).
...
fn import_ids(args: &Args) -> Result<()> {
    ...
    // append_lines() does not create, and shadow must never be 0644.
    touch(&passwd, 0o644)?;
    touch(&group, 0o644)?;
    touch(&shadow, 0o640)?;
