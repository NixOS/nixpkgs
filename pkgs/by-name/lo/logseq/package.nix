...
let
  electron = electron-bin; # TODO: remove. This is here just so that we don't have to wait for electron builds.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  yarnOfflineCacheResources = fetchYarnDeps {
    name = "logseq-${finalAttrs.version}-yarn-deps-resources";
    inherit (finalAttrs) src;
  ...
  buildInputs = [
    # deps for the canvas npm dependency
    pixman
    cairo
    pango
  ];
  ...
  postConfigure = ''
    ...
    pushd static
    yarnOfflineCache="$yarnOfflineCacheResources" yarnConfigHook
    popd
    ...
    # the electron-rebuild deadlocks for some reason, let's just use normal npm rebuild (since we overrode the nodedir anyways)
    npm rebuild --verbose

    # remove references to the set nodedir
  ...
  installPhase =
    ''
      ...
      find static/out/*/resources/app/node_modules -type f -exec remove-references-to -t ${nodejs} '{}' \;
    ''
