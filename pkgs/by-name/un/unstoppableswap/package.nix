...
buildNpmPackage rec {
  ...
  src = fetchFromGitHub {
    ...
    rev = "v${version}";
    hash = "sha256-Ssz8GKvp8qJbWVKuQJKsvFcje0ZORkA/KjmYBUQBM/U=";
  };
  ...
  npmDepsHash = "sha256-MNVVh4+WCjN+h25RxQGqa5b00DxriPOPUspC/F3Es5I=";

  appNpmDeps = fetchNpmDeps {
    inherit src;
    sourceRoot = "${src.name}/release/app";
    hash = "sha256-ETHwO/scm68Sw1lP0ju7gav9sUvHZEdlmrcxLIDHeWs=";
  };
  ...
  postInstall = ''
    makeWrapper ${electron}/bin/electron $out/bin/UnstoppableSwap \
