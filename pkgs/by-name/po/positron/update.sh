...
positron_nix="$nixpkgs/pkgs/by-name/po/positron/package.nix"
...
if ! nix-build -A positron "$nixpkgs"; then
  echo "The updated positron failed to build."
  exit 1
fi

# Commit changes
git add "$positron_nix"
git commit -m "positron: ${current_version} -> ${new_version}"
