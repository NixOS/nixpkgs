...
python3.pkgs.buildPythonApplication (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-UIpZFUtjHXmZyAzW6oLdhgUk4E/EHEhd6hTI72jeFaI=";
  };
  ...
  postInstall =
    let
      sandboxDefault = if stdenv.hostPlatform.isLinux then "off" else "auto";
    in
    ''
      substituteInPlace $out/${python3.sitePackages}/kiro_crew/config/sections.py \
        --replace-fail \
        'sandbox: str = field(${"\n"}        default="auto",' \
      ...
      substituteInPlace $out/${python3.sitePackages}/kiro_crew/config/loader.py \
        ...
        'sandbox=agent_data.get("sandbox", "${sandboxDefault}"),'
    '';
  ...
  passthru = {
    ...
    updateScript = nix-update-script {
      extraArgs = [
        # kirocrew has some releases with "-insider" in the name; skip them.
        "--use-github-releases"
        # bump the dashboard's npmDepsHash too.
        "--subpackage=frontend"
      ];
    };
  };
