...
stdenv.mkDerivation (finalAttrs: {
  ...
  ] ++ lib.optionals (stdenv.hostPlatform != stdenv.buildPlatform) [ pkgsBuildBuild.guile_1_8 ];
  ...
  configureFlags =
    [
      ...
      (lib.enableFeature false "error-on-warning")
    ]
  ...
  postInstall =
    ...
    ''
    # XXX: See http://thread.gmane.org/gmane.comp.lib.gnulib.bugs/18903 for why
    # `--with-libunistring-prefix' and similar options coming from
    # `AC_LIB_LINKFLAGS_BODY' don't work on NixOS/x86_64.
    + ''
      sed -i "$out/lib/pkgconfig/guile"-*.pc -e "s|-lltdl|-L${lib.getLib libtool}/lib -lltdl|g"
    '';
  ...
  meta = {
    ...
    maintainers = lib.teams.guile.members ++ (with lib.maintainers; [ ludo ]);
