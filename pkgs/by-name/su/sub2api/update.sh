...
readonly owner="Wei-Shaw"
readonly repo="sub2api"
readonly package_path="pkgs/by-name/su/sub2api"
...
update_hashes_file pnpmDepsHash "${dummy_hash}"
update_hashes_file vendorHash "${dummy_hash}"

update_fixed_output_hash() {
  local attribute="$1"
  local key="$2"
  local output
  local status
  ...
  if [[ ${status} -eq 0 || -z "${dependency_hash}" ]]; then
    printf '%s\n' "${output}" >&2
    echo "Could not determine ${key} for ${attribute}" >&2
    exit 1
  fi

  update_hashes_file "${key}" "${dependency_hash}"
}

# Resolve the source first, then frontend dependencies, so the backend build
# reaches vendorHash only after its inputs have known hashes.
update_fixed_output_hash ".#sub2api.src" hash
update_fixed_output_hash ".#sub2api.frontend.pnpmDeps" pnpmDepsHash
update_fixed_output_hash ".#sub2api" vendorHash
