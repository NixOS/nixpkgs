...
LICENSE_FULL_NAME_RE = re.compile(
    r'(?P<indent>\s*)license\.fullName = "(?P<license>[^"]+)";'
)
...
def commit_files(repo, message: str, files: list[Path]) -> None:
    ...
    subprocess.run(
        ["git", "commit", "-m", message, "--", *paths], cwd=worktree, check=True
    )
    ...
    def get_update(
        ...
        def update() -> tuple[Redirects, list[tuple[str, str, str]]]:
            ...
            if to_update:
                specs_to_process = [
                    p
                    for p in sorted_all_specs
                    if p.normalized_name in to_update or p.name in to_update
                ]
            ...
            for plug in sorted_all_specs:
                ...
                if plug.normalized_name in results_map:
                    ...
                    if nix_expr:
                        ...
                        log.warning(
                            f"Update failed for {plug.name}, keeping existing version. Error: {error}"
                        )
                        ...
                        log.error(
                            f"Update failed for {plug.name} and no existing version found. Error: {error}"
                        )
                        errors.append((plug, error))
                else:
                ...
                if final_expr:
                    ...
                    track_version_change(
                        plug, final_expr, old_versions, updated_plugins
                    )
