...
let
  ...
  # - vector4 is enabled, because there is a Conformance test that does not compile
...
stdenv.mkDerivation (finalAttrs: {
  ...
  checkPhase = ''
    ...
    ${lib.optionalString (!enableStatic) ''export DYLD_LIBRARY_PATH="$(realpath .)"''}

    # enabling extern C fails this specific test case, so exclude it
