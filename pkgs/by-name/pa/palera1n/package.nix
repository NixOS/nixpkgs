...
let
  ...
  ramdisk = fetchurl {
    url = "https://web.archive.org/web/20260310113826id_/https://cdn.nickchan.lol/palera1n/c-rewrite/deps/ramdisk.dmg";
    sha256 = "sha256-4ubbTpLGUk0KqRXBJP6WPW+mfefC+jO7jbnk7kVGa2w=";
  };
  binpack = fetchurl {
    url = "https://web.archive.org/web/20260310113826id_/https://cdn.nickchan.lol/palera1n/c-rewrite/deps/binpack.dmg";
    sha256 = "sha256-JmxGdlE5OOXHOLpOa/xsyUyuxXOSyWDM22cmA+uk1HY=";
  };
  pongo = fetchurl {
    url = "https://web.archive.org/web/20260310113826id_/https://cdn.nickchan.lol/palera1n/artifacts/kpf/iOS15/Pongo.bin";
...
stdenv.mkDerivation {
  ...
  postPatch = ''
    substituteInPlace Makefile \
      --replace-fail 'CFLAGS += -isystem $(DEP)/include -I$(SRC)/include -I$(SRC) -D_XOPEN_SOURCE=500' \
                'CFLAGS += -I$(SRC)/include -I$(SRC) -D_XOPEN_SOURCE=500' \
      --replace-fail 'LIBS += $(DEP)/lib/libimobiledevice-1.0.a $(DEP)/lib/libirecovery-1.0.a $(DEP)/lib/libusbmuxd-2.0.a' \
                'LIBS += -limobiledevice-1.0 -lirecovery-1.0 -lusbmuxd-2.0' \
      --replace-fail 'LIBS += $(DEP)/lib/libimobiledevice-glue-1.0.a $(DEP)/lib/libplist-2.0.a -pthread -lm' \
                'LIBS += -limobiledevice-glue-1.0 -lplist-2.0 -pthread -lm' \
      --replace-fail 'LIBS += $(DEP)/lib/libmbedtls.a $(DEP)/lib/libmbedcrypto.a $(DEP)/lib/libmbedx509.a $(DEP)/lib/libreadline.a' \
                'LIBS += -lmbedtls -lmbedcrypto -lmbedx509 -lreadline' \
      --replace-fail 'LDFLAGS += -static -no-pie' 'LDFLAGS +='
    substituteInPlace src/Makefile \
      --replace-fail 'LIBS += $(DEP)/lib/libusb-1.0.a' 'LIBS += -lusb-1.0'
  '';
