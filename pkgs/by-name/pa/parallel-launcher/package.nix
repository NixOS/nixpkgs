...
let
  ...
  parallel-n64-core-default = (callPackage ./parallel-n64-next.nix { });

  parallel-n64-core' =
    if parallel-n64-core == null then parallel-n64-core-default else parallel-n64-core;
...
stdenv.mkDerivation (
  ...
  in
  {
    ...
    version = "8.1.0"; # Check ./parallel-n64-next.nix for updates when updating, too

    src = fetchFromGitLab {
      owner = "parallel-launcher";
      repo = "parallel-launcher";
      tag = reformatVersion finalAttrs.version;
      hash = "sha256-DD0pwBmehuDWlrws4qpq+J+/HAzpXnW6vpiq7kNdNZU=";
    };
    ...
    passthru = {
      parallel-n64-core = parallel-n64-core-default;
    };
