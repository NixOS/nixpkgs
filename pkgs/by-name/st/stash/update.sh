...
sed -i -E -e "s#gitHash = \".*\"#gitHash = \"$gitHash\"#" package.nix
sed -i -E -e "s#srcHash = \".*\"#srcHash = \"$srcHash\"#" package.nix
sed -i -E -e "s#yarnHash = \".*\"#yarnHash = \"$yarnHash\"#" package.nix

curl -s "https://raw.githubusercontent.com/stashapp/stash/v${version}/ui/v2.5/package.json" --output package.json
