...
rustPlatform.buildRustPackage rec {
  ...
  postPatch = ''
    ...
    substituteInPlace ${"$"}{cargoDepsCopy}/sudo-0.6.0/src/lib.rs \
      --replace 'Command::new("/usr/bin/sudo")' 'Command::new("sudo")'
  '';
  ...
  fixupPhase = ''
    ...
    substituteInPlace $out/share/applications/org.startup_disk.StartupDisk.desktop \
      --replace-fail "Exec=pkexec startup-disk" "Exec=pkexec $out/bin/startup-disk"
  '';
  ...
  meta = {
    ...
    changelog = "https://gitlab.gnome.org/davide125/startup-disk/-/tags/${version}";
