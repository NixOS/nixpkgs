...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    substituteInPlace src/binding/f77/buildiface \
      --replace-fail "/usr/bin/env perl" ${lib.getExe perl}
  '';

  configureFlags = [
    "--enable-static=${if enableStatic then "yes" else "no"}"
    "--enable-shared=${if enableStatic then "no" else "yes"}"
  ];
  ...
  checkTarget = lib.concatStringsSep " " [
    ...
    "check"
    # run parallel test programs on 4 MPI processes
    "ptest"
  ];

  # run parallel test programs on 3,4,6,8 MPI processes
  # cannot do parallel building otherwise failed
  # takes long time, disabled as we have ptest already
  # postCheck = ''
  #   make ptests
  # '';
