...
python3Packages.buildPythonApplication rec {
  ...
  postPatch = ''
    substituteInPlace dbus/meson.build \
      --replace-fail "dbus_policy_dir," "'${placeholder "out"}/share/dbus-1/system.d'," \
      --replace-fail "dbus_service_dir," "'${placeholder "out"}/share/dbus-1/system-services',"
    substituteInPlace systemd/meson.build \
      --replace-fail ": systemd_system_unit_dir" ": '${placeholder "out"}/lib/systemd/system'" \
      --replace-fail ": systemd_user_unit_dir" ": '${placeholder "out"}/lib/sysusers.d'"
