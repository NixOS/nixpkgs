...
attr="${UPDATE_NIX_ATTR_PATH:-openutau-beta}"
version="$(curl -sSL "https://api.github.com/repos/stakira/OpenUtau/releases" | jq -r 'map(select(.prerelease)) | first | .tag_name' | sed -e 's|^build/||')"
