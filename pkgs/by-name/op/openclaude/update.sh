...
COMMIT=$(git ls-remote "$REPO" "$BRANCH" | awk '{print $1}')
[ -z "$COMMIT" ] && error "Could not resolve latest commit SHA."
...
SRC_HASH=$(nix-prefetch-github Gitlawb openclaude --rev "$COMMIT" 2>/dev/null | jq -r .hash || true)
...
sed -i "s|tag = \"[^\"]*\";|rev = \"$COMMIT\";|" "$NIX_FILE"
