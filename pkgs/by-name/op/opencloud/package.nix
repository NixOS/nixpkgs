...
buildGoModule {
  ...
  postPatch = ''
    ...
    cat .bingo/Variables.mk

    # tries to build web assets, done separately
    substituteInPlace services/idp/Makefile \
      --replace-fail 'ci-node-generate: assets' 'ci-node-generate:'
    substituteInPlace services/web/Makefile \
      --replace-fail 'ci-node-generate: pull-assets' 'ci-node-generate:'

    # tries to build some random binaries off the internet and
    # no need to build protobuf bindings, they are in-repo
