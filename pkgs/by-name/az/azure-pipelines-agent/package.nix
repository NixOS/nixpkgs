...
buildDotnetModule (finalAttrs: {
  ...
  + lib.optionalString stdenv.hostPlatform.isDarwin ''
    export DYLD_LIBRARY_PATH="${finalAttrs.dotnet-sdk.icu}/lib''${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}"
  ''
  + lib.optionalString (stdenv.hostPlatform.isDarwin && stdenv.hostPlatform.isAarch64) ''
    echo "Signing freshly-built binaries so the darwin-aarch64 kernel will let them run..."
    signDarwinBinariesIn src
  '';
  ...
  ''
  + lib.optionalString stdenv.hostPlatform.isDarwin ''
    # On darwin LD_LIBRARY_PATH is ignored by dyld; the .NET runtime needs
    # DYLD_LIBRARY_PATH to locate the ICU bundled with the dotnet SDK,
    # otherwise it aborts at startup.
    makeWrapperArgs+=(
      --prefix DYLD_LIBRARY_PATH : ${finalAttrs.dotnet-sdk.icu}/lib
    )
  '';
  ...
  disabledTests =
    map (x: "Microsoft.VisualStudio.Services.Agent.Tests.${x}") [
      ...
      "LogPluginHost.LogPluginHostL0.LogPluginHost_RunMultiplePlugins"
    ...
    ++ lib.optionals stdenv.hostPlatform.isDarwin (
      map (x: "Microsoft.VisualStudio.Services.Agent.Tests.${x}") [
        "Worker.CodeCoverage.CoberturaSummaryReaderTests.VerifyCoberturaCoverageStatisticsForValidSummaryFile"
        "Worker.TestResults.XUnitResultReaderTests.ReadResultsReturnsCorrectValues"
        "ProcessInvokerL0.RedirectSTDINKeepStreamOpen"
        "ProcessExtensionL0.SuccessReadProcessEnv"
      ]
    );
