...
stdenv.mkDerivation (finalAttrs: {
  ...
  meta = {
    ...
    license = with lib.licenses; [
      # the project does not provide a top level license or copying file, so i scoured the wholec
      # codebase (which is quite small fortunately) and to find all licenses.
      ...
      isc
      sgi-b-20
      # luxi font license is listed in general/Licenses.xml.
      # However since threre is no font is in the repo, this license is not added here.
      # There is a high chance that some of the other licenses are not used in xorg-docs either
      # but there is no way to find out.

      # repo contains the X logo which was converted from Xmu/DrawLogo.c, which is licensed under:
      mitOpenGroup
    ];
    maintainers = [ ];
