...
stdenv.mkDerivation (finalAttrs: {
  ...
  meta = {
    ...
    license = with lib.licenses; [
      # The copyright notices are split between each protocol so to be able to valdiate this
      ...
      # omittable by spdx, which makes it technically a different license.
      # But its close enough I think.
      x11
      # x11proto and xextproto both contain licenses that are definitely free, but don't seem to
      # match any of the licenses known by spdx.
      # I checked with the spdx license checker and manually with a word diff tool, but all licenses
      # that seemed similar had a different wording or a sentence missing.
      free

      # fontsproto and x11proto both contain a license that is almost the X11 license, but with one
      # important difference: The sentence:
      # "Permission is hereby granted [...] to use, copy, modify, merge, publish, distribute ..."
      # is replaced with "All rights reserved."
