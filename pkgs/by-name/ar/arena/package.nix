...
stdenv.mkDerivation rec {
  ...
  unpackPhase = ''
    ...
    mkdir -p $out/lib/arena-${version}
    tar -C $out/lib/arena-${version} -xf ${src}

    # Remove executable bits from data files. This matters for the find command
    # we'll use below to find all bundled engines.
    chmod -x $out/lib/arena-${version}/Engines/*/*.{txt,bin,bmp,zip}
  '';
  ...
  buildPhase = ''
    ...
    exes=( $(find $out -name '*x86_64_linux')
           $(find $out/lib/arena-${version}/Engines -type f -perm /u+x)
         )
    for i in "''${exes[@]}"; do
  ...
  installPhase = ''
    mkdir -p $out/bin
    ln -s $out/lib/arena-${version}/Arena_x86_64_linux $out/bin/arena
  '';
