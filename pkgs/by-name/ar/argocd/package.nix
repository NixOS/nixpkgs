...
buildGoModule rec {
  ...
  ldflags =
    let
      ...
      kubectlVersion = builtins.getEnv "KUBECTL_VERSION";
    in
    [
      "-s"
      "-w"
      ...
      "-X ${packageUrl}.kubectlVersion=${kubectlVersion}"
  ...
  # setting environment variable for kubectl version to be consumed in the ldflags via builting.getEnv
  # Per https://github.com/search?q=repo%3Aargoproj%2Fargo-cd+%22KUBECTL_VERSION%3D%22+path%3AMakefile&type=code
  prePatch = ''
