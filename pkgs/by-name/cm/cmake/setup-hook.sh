...
# Bash associative array representing the CMake cache variable entries
# See the following documentation for how CMake handles the variables and how the `-D` flags work.
...
canonicalizeCMakeEntryAttrs() {
    local scriptName=".attrs.cmakeEntries.sh"
    cmake -DCMAKE_ENTRIES_OBJECT_STRING= -DOUTPUT_SCRIPT_NAME="$scriptName" -P @ParseCMakeEntryAttrs@
    source "./$scriptName"
}

if [[ -n "${__structuredAttrs-}" ]]; then
    appendToVar prePhases canonicalizeCMakeEntryAttrs
fi
...
cmakeConfigurePhase() {
    ...
    nop "${cmakeEntries[CMAKE_EXPORT_NO_PACKAGE_REGISTRY]=ON}"
    nop "${cmakeEntries[CMAKE_FIND_USE_PACKAGE_REGISTRY]=OFF}"
    nop "${cmakeEntries[CMAKE_FIND_USE_SYSTEM_PACKAGE_REGISTRY]=OFF}"
    ...
    if [[ -z "${separateCMakeEntries-}" ]]; then
