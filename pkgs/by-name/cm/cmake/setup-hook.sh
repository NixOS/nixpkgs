...
canonicalizeCMakeEntryAttrs() {
    ...
    for key in "${!cmakeEntries[@]}"; do
        if ! jq -e ".cmakeEntries | has(\"$key\")" "$NIX_ATTRS_JSON_FILE" > /dev/null; then
            continue
        fi
        typeOrig="$(jq -r ".cmakeEntries.$key|type" "$NIX_ATTRS_JSON_FILE")"
...
appendToVar prePhases canonicalizeCMakeEntryAttrsIfAvailable
