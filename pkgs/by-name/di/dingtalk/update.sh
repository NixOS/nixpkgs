...
LINUX_AARCH64_ACTUAL_URL=$(curl -sI "$LINUX_AARCH64_URL" | grep -i location | head -1 | sed 's/location: //i' | tr -d '\r')
LINUX_AARCH64_HASH=$(nix-prefetch-url "$LINUX_AARCH64_ACTUAL_URL")

# Linux x86_64
LINUX_X86_64_URL="https://www.dingtalk.com/win/d/qd=linux_amd64"
LINUX_X86_64_ACTUAL_URL=$(curl -sI "$LINUX_X86_64_URL" | grep -i location | head -1 | sed 's/location: //i' | tr -d '\r')
...
DARWIN_AARCH64_ACTUAL_URL=$(curl -sI "$DARWIN_AARCH64_URL" | grep -i location | head -1 | sed 's/location: //i' | tr -d '\r')
DARWIN_AARCH64_HASH=$(nix-prefetch-url "$DARWIN_AARCH64_ACTUAL_URL")

# Darwin x86_64 (Intel)
DARWIN_X86_64_URL="https://www.dingtalk.com/mac/d/"
DARWIN_X86_64_ACTUAL_URL=$(curl -sI "$DARWIN_X86_64_URL" | grep -i location | head -1 | sed 's/location: //i' | tr -d '\r')
