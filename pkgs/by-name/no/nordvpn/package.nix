    ...
    (final: prev: {
      ...
      openvpn = prev.openvpn.overrideAttrs (old: {
        patches =
          ...
          (old.patches or [ ])
          ++ [
            (patchPathFtn "02-tunnelblick-openvpn_xorpatch-a.diff")
            (patchPathFtn "03-tunnelblick-openvpn_xorpatch-b.diff")
            (patchPathFtn "04-tunnelblick-openvpn_xorpatch-c.diff")
            (patchPathFtn "05-tunnelblick-openvpn_xorpatch-d.diff")
            (patchPathFtn "06-tunnelblick-openvpn_xorpatch-e.diff")
          ];
      });
    })
  ];
...
buildGoModule (finalAttrs: {
  ...
  meta = {
    ...
    longDescription = ''
      ...
      If you enable the firewall with `networking.firewall.enable = true;`,
      then set `networking.firewall.checkReversePath = "loose";`.
