...
rustPlatform.buildRustPackage (finalAttrs: {
  ...
  cargoPatches = [
    # Patch cargo.lock because the one in the upstream repo has duplicates entries.
    # It should probably be removed in the next rustdesk update (if they fix their cargoLock)
    ./remove-duplicate-crate.patch
  ];

  useFetchCargoVendor = true;
  cargoHash = "sha256-xeuKH78sUINe0JCfVtVa/juetnjlVAVQ6S2PezNk+Qg=";
