...
DICTIONARIES_CHROMIUM_NIX = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "dictionaries-chromium.nix"
)
...
hunspell_dictionaries_commit = re.search(
    hunspell_dictionaries_pattern, deps.decode(), re.MULTILINE
).group(1)
...
hunspell_dictionaries_git = nix_prefetch_git(
...
with fileinput.FileInput(DICTIONARIES_CHROMIUM_NIX, inplace=True) as file:
    for line in file:
        result = re.sub(
            r'^      version = ".+";', f'      version = "{chromium_version}";', line
        )
        result = re.sub(
            r'^        rev = ".*";',
            f'        rev = "{hunspell_dictionaries_commit}";',
            result,
        )
        result = re.sub(
            r'^        hash = ".+";',
            f'        hash = "{hunspell_dictionaries_hash}";',
            result,
        )
        print(result, end="")
