{
  ...
  zig_0_13,
  llvmPackages_18,
  # these below are only used for tests
...
rustPlatform.buildRustPackage rec {
  ...
  # help roc find glibc files
  glibcPath = lib.optionalString stdenv.isLinux "${glibc.out}/lib";
  libGccSPath = lib.optionalString stdenv.isLinux "${stdenv.cc.cc.lib}/lib";

  postInstall =
    if stdenv.isLinux then
      ''
        wrapProgram $out/bin/roc --set NIX_GLIBC_PATH ${glibcPath} --set NIX_LIBGCC_S_PATH ${libGccSPath} --prefix PATH : ${lib.makeBinPath [ stdenv.cc ]}
      ''
    else
      ''
  ...
  checkPhase = ''
    NIX_GLIBC_PATH=${glibcPath} NIX_LIBGCC_S_PATH=${libGccSPath} cargo test --release --workspace --exclude test_mono --exclude uitest -- --skip glue_cli_tests
  '';
