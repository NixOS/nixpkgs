...
TAG="$(curl -sL https://api.github.com/repos/caelestia-dots/shell/releases/latest | jq -r .tag_name)"
VERSION="${TAG#v}"

REV="$(curl -sL "https://api.github.com/repos/caelestia-dots/shell/commits/$TAG" | jq -r .sha)"
...
CMAKE_TXT="$(curl -sL "https://raw.githubusercontent.com/caelestia-dots/shell/$REV/CMakeLists.txt")"
