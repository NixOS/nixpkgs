...
let
  ...
  libusb = stdenv.mkDerivation rec {
    name = "libusb";
    ...
    src =
      ...
      in
      fetchurl {
        url = "https://sourceforge.net/projects/libusb/files/libusb-${minorVersion}/libusb-${version}/libusb-${version}.tar.bz2";
    ...
    patches = [
      (fetchpatch {
        url = "https://raw.githubusercontent.com/ksoloti/ksoloti/${ksolotiVersion}/platform_linux/src/libusb.stdfu.patch";
...
stdenv.mkDerivation rec {
  ...
  buildPhase = ''
    ...
    ant -Dbuild.version=${version} -Dshort.version=${builtins.head (builtins.split "-" version)}
  ...
  installPhase = ''
    ...
    wrapProgram "$out/opt/ksoloti/Ksoloti.sh" \
      ...
      --prefix LD_LIBRARY_PATH : '${lib.makeLibraryPath [ gcc-arm-embedded-6 ]}' \
      --set JAVA_HOME '${zulu}' \
