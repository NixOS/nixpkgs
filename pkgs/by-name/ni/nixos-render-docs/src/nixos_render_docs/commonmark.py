...
class CommonMarkRenderer(Renderer):
    ...
    def __init__(
        ...
        admonition_style: AdmonitionStyle = AdmonitionStyle.PLAIN,
    ):
    ...
    def _maybe_parbreak(self) -> str:
        result = (
            f"\n{self._parstack[-1].indent}" * 2
            if self._parstack[-1].continuing
            else ""
        )
        self._parstack[-1].continuing = True
        return result
    ...
    def _fenced_div_open(self, classes: Sequence[str] = ()) -> str:
        fence_size = self._div_fence_sizes[-1] + 1 if self._div_fence_sizes else 3
        fence = ":" * fence_size
    ...
    def list_item_open(self, token: Token, tokens: Sequence[Token], i: int) -> str:
        ...
        lbreak = (
            "" if not lst.first_item_seen else self._break() * (1 if lst.compact else 2)
        )
    ...
    def fence(self, token: Token, tokens: Sequence[Token], i: int) -> str:
        ...
        if code.endswith("\n"):
            code = code[:-1]
        pbreak = self._maybe_parbreak()
        return pbreak + self._indent_raw(
            md_make_code(code, info=token.info, multiline=True)
        )
    ...
    def myst_role(self, token: Token, tokens: Sequence[Token], i: int) -> str:
        ...
        if token.meta["name"] == "manpage" and (
            url := self._manpage_urls.get(token.content)
        ):
