# Render a nixdoc export v1 collection
#
# Takes json of v1 export schema
# https://github.com/nix-community/nixdoc/blob/554a2e2676548f58f85d76b69308447a4e60450d/src/export.rs#L101
#
# Returns a commonmark flavour that nixos-render-docs can process
#

import re
from typing import Any, NamedTuple

# html anchors only allow certain anchors
_ANCHOR_HOSTILE = re.compile(r"[^A-Za-z0-9._-]")

def _anchor_id(s: str) -> str:
    return _ANCHOR_HOSTILE.sub("", s)
...
def render_group(group: Group, entries: list[Entry], revision: str) -> str:
    parts = [f"# {group.id} {{#sec-functions-library-{_anchor_id(group.id)}}}\n"]
    if group.description:
        parts.append(shift_headings(group.description, 1) + "\n")

    for entry in entries:
        parts.append(f"## `{entry.attr_path}` {{#function-library-{_anchor_id(entry.id)}}}\n")
