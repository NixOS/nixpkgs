...
# - evaluated vars configurations (the script will then be a NO-OP), i.e.
#   objects already matching the vars schema (TODO: document this)
#
# Note that this file is *not* a Nix(OS) module! This will automatically be
# evaluated by the vars CLI.
#
# Parameters:
# - `configuration`: The thing to actually turn into a vars configuration.
...
#
# People who want to write custom Nix(OS)-modules for use with the vars CLI
# should write an accompanying `jsonify`-esque function, and pass its output to
# the CLI. As long as the output matches the vars configuration schema (TODO:
