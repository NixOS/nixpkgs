...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("nix-vars -f /etc/nixos/config1.nix collect-garbage")
    machine.succeed("nix-vars -f /etc/nixos/config1.nix generate")
    ...
    machine.succeed("nix-vars -f /etc/nixos/config2.nix collect-garbage")
    machine.succeed("test ! -f /tmp/vars-demo/generators/derived/files/derived")

    machine.succeed("nix-vars -f /etc/nixos/config2.nix generate")
