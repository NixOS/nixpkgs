...
pub fn setup_etc(layout: &EtcLayout) -> Result<()> {
    ...
    if live {
        ...
        let etc_fd = open_tree(CWD, &layout.etc, OpenTreeFlags::empty())
        ...
        unmount(&layout.etc, UnmountFlags::DETACH)
...
fn transfer_submounts(
    ...
    for m in mounts.iter() {
        let Some(rel) = m.file.strip_prefix(&prefix) else {
            continue;
        };
        ...
        let src = open_tree(CWD, &m.file, OpenTreeFlags::OPEN_TREE_CLONE)
...
fn cleanup_legacy_metadata_mounts(mounts: &Mounts) {
    for m in mounts.iter() {
        ...
        if let Err(e) = unmount(&m.file, UnmountFlags::DETACH) {
