    ...
    match action:
        case Action.BUILD_IMAGE if flake:
            variants = nix.get_build_image_variants_flake(
                flake,
                eval_flags=grouped_nix_args.flake_build_flags | grouped_nix_args.flake_eval_flags,
            )
    ...
    match (build_host, flake):
        case (Remote(_), Flake(_)):
            path_to_config = nix.build_remote_flake(
                ...
                build_host,
                eval_flags=grouped_nix_args.flake_build_flags | grouped_nix_args.flake_eval_flags,
                flake_build_flags={"no_link": no_link, "dry_run": dry_run} | grouped_nix_args.flake_build_flags,
...
def edit(flake: Flake | None, grouped_nix_args: GroupedNixArgs) -> None:
    if flake:
        nix.edit_flake(flake, grouped_nix_args.flake_build_flags | grouped_nix_args.flake_eval_flags)
    else:
        nix.edit()
...
def repl(
    ...
    if flake:
        nix.repl_flake(flake, grouped_nix_args.flake_build_flags | grouped_nix_args.flake_eval_flags)
    else:
