...
The secrets associated with each generator must be handled by a _backend_. A single `nixos-secrets` configuration can contain multiple backends.

There are many ways to provide a nixos-secrets configuration to the CLI, but for now we'll start with the most basic. That is, by using the provided [NixOS module](../../../../nixos/modules/security/secrets.nix). Since `nixos-secrets` is still in its early stages, the module must be imported manually:
...
```
$ nixos-secrets generate -f /path/to/config
Rebuild order:
...
```
$ nixos-secrets generate -f /path/to/config -g example
Rebuild order:
- 'example' (forced)
Evaluating prompts:
Generating 'example'
...
Note that we did not manually specify a backend this time! In such scenarios, the module will automatically use the `secrets.defaultGeneratorBackend` backend. Running the CLI should generate the new secret:
...
```nix
{
  secrets.promptBackends.plain.script =
    pkgs:
...
```nix
{
  ...
  secrets.generatorBackends.plain = {
    get =
      pkgs:
...
Backends can also provide `deploy.local` and `deploy.remote` scripts. Note that each secret file has a `deploy` flag that is on by default. Users can choose to disable deployment for any of their secrets by setting said flag to `false`. This might be useful (for example) when handling generators that are meant to be used as inputs to other generators, but must not have their outputs deployed right away. A backend can choose to not provide either of the scripts above.
...
This is done by calling to the so-called [`jsonify.nix`](./nixos_secrets/nix/jsonify.nix) function. If we were to assign a type signature to the aforementioned function, it would look something like this:

```
jsonify : { config, pkgsDefault ? null, pkgsHost ? null, pkgsTarget ? null } -> SecretsConfiguration
```
