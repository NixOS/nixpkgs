...
The CLI does accept pre-evaluated configurations (like one would, for example, expect when using flakes). When given a non-evaluated configuration, the CLI evaluates it using the Nixpkgs available in the `NIX_PATH`. One should pre-evaluate their configuration if pinning of the Nixpkgs version is desired.

Once the configuration is evaluated, the CLI extracts the data it needs into a Nix attrset that can be directly serialized as JSON (this implies the package-set reliant script functions are evaluated with the host package set as an argument, for example) before being taken in by the Python code.
...
We've already seen what happens if `config` is given as a (possibly evaluated) NixOS configuration. If this function receives a `SecretsConfiguration` as the config argument, then the function directly returns the configuration it is given. This means one can side-step the `jsonify.nix` logic entirely and produce a Nix attrset containing the needed data by whichever means they desire (for example, as part of a tool that's totally disconnected from the NixOS module system).
...
One can pre-evaluate their NixOS configuration in order to pin the target's Nixpkgs instance. This implicitly also sets the host's Nixpkgs instance to the one used by the configuration. If this is not desired (for example, when the target and host architectures differ), then one must invoke `jsonify` themselves, passing its output to the CLI (one could also side-step `jsonify` entirely, as explained above; a person doing that is assumed to already know what they're doing though!).
