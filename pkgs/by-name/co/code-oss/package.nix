...
let
  ...
  nodejs = nodejs_22;
  electronInfo = lib.importJSON ./electron-info.json;
  ...
  ripgrep-scripts =
    let
      ...
      ripgrep-zip = runCommand "ripgrep-${ripgrep.version}.tar.gz" { } ''
    ...
    ''
      ...
      echo ln -s ${ripgrep-zip} $TMPDIR/vscode-ripgrep-cache-${vscode-ripgrep-package-version}/ripgrep-${version'}-${target}.${ext}
      ln -s ${ripgrep-zip} $TMPDIR/vscode-ripgrep-cache-${vscode-ripgrep-package-version}/ripgrep-${version'}-${target}.${ext}
    '';

  npm-install-script = writeShellScriptBin "npm-install-script" ''
    #!${bash}/bin/bash
    source $stdenv/setup
  ...
  code-oss-build = stdenv.mkDerivation (finalAttrs: {
    ...
    src = fetchFromGitHub {
      ...
      hash = "sha256-EQYIkqDa7O0GDx08XsgkUd2vztgQSW9M80lZv5uEAbQ=";
    };
    ...
    npmDeps = stdenvNoCC.mkDerivation {
      ...
      outputHash = "sha256-5xGgyDlm56uzG6nbT4IpOTLX9iVl655hRqJCVeh+fms=";
    };
    nativeBuildInputs = [
      writableTmpDirAsHomeHook
      nodejs
    ...
    buildPhase = ''
      ...
      export BUILD_DATE="$(cat ${./date})"
      export npm_install_command="npm-install-script"
      ...
      cp ${electron.src} ${cache-dir}/electron/$(printf 'https://github.com/electron/electron/releases/download/v${electron.version}' | sha256sum | cut -f1 -d' ')/electron-v${electron.version}-${platform}-${arch}.zip
      ...
      cp ${vscode-extensions.ms-vscode.js-debug-companion.src} ./ms-vscode.js-debug-companion.vsix
      cp ${vscode-extensions.ms-vscode.js-debug.src} ./ms-vscode.js-debug.vsix
      cp ${vscode-extensions.ms-vscode.vscode-js-profile-table.src} ./ms-vscode.vscode-js-profile-table.vsix

      substituteInPlace product.json \
        --replace-fail '"repo": "https://github.com/microsoft/vscode-js-debug-companion"' '"vsix": "./ms-vscode.js-debug-companion.vsix"' \
        --replace-fail '"repo": "https://github.com/microsoft/vscode-js-debug"' '"vsix": "./ms-vscode.js-debug.vsix"' \
        --replace-fail '"repo": "https://github.com/microsoft/vscode-js-profile-visualizer"' '"vsix": "./ms-vscode.vscode-js-profile-table.vsix"'
...
vscode-generic {
  ...
  sourceExecutableName = if stdenv.hostPlatform.isDarwin then "code" else "code-oss";
  longName = "Code - OSS";
  shortName = "code-oss";
