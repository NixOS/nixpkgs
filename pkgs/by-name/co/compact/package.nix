...
stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    sed -e 's;#!/usr/bin/env .*;'${chez}/bin/scheme' --program;' compiler/compactc.ss > obj/compiler/compactc.ss
    sed -e 's;#!/usr/bin/env .*;'${chez}/bin/scheme' --program;' compiler/format-compact.ss > obj/compiler/format-compact.ss
    sed -e 's;#!/usr/bin/env .*;'${chez}/bin/scheme' --program;' compiler/fixup-compact.ss > obj/compiler/fixup-compact.ss
    patchShebangs --host .

    ${chez}/bin/scheme -q << END
      (reset-handler abort)
      (optimize-level 2)
