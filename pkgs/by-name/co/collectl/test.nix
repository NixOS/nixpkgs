...
runCommand "collectl-test"
  ...
  ''
    ...
    timeout 5s ${collectl}/bin/collectl -c1 >/dev/null || true

    # Test that explicit config file option still works with original config
    timeout 5s ${collectl}/bin/collectl --config ${collectl}/etc/collectl.conf -c1 >/dev/null || true
    ...
    config_output=$(timeout 5s ${collectl}/bin/collectl --config "$custom_config_path" -c1 -d1 2>&1 | grep -i "Config File Search Path:" | head -1)
