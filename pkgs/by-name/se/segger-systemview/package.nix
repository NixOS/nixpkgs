...
, segger-jlink
, config
, acceptLicense ? config.segger-systemview.acceptLicense or false
, fontconfig
, xorg
, makeDesktopItem
, copyDesktopItems
...
let
  ...
  src =
    assert !acceptLicense -> throw ''
      Use of the "SEGGER SystemView real-time software analysis too" requires the
      acceptance of the following licenses:

        - SEGGER SystemView Installation and Activation [1]
      ...
      configuration.nix:
        ...
        nixpkgs.config.segger-systemview.acceptLicense = true;

      config.nix:
        allowUnfree = true;
        segger-systemview.acceptLicense = true;

      [1]: https://www.segger.com/products/development-tools/systemview/license/systemview-installation/
