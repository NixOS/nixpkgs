...
stdenv.mkDerivation {
  ...
  ++ (if withPcre then [ pcre2 ] else [ ])
  ++ (if withRe2 then [ re2 ] else [ ])
  ++ (if withUnwind then [ libunwind ] else [ ]);

  cmakeFlags = [
    "-DCMAKE_AR=${gcc-unwrapped}/bin/gcc-ar"
    "-DCMAKE_RANLIB=${gcc-unwrapped}/bin/gcc-ranlib"
    "-DENABLE_GIT_VERSION=OFF"
    "-DWITH_ASAN=${if withAsan then "ON" else "OFF"}"
    "-DWITH_AWS=${if withAws then "ON" else "OFF"}"
    "-DWITH_GCP=${if withGcp then "ON" else "OFF"}"
    "-DWITH_GPERF=${if withGperf then "ON" else "OFF"}"
    "-DWITH_SEARCH=${if withSearch then "ON" else "OFF"}"
    "-DWITH_USAN=${if withUsan then "ON" else "OFF"}"
  ];
