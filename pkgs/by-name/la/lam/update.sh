...
GIT_VERSION="$(curl --silent ${GITHUB_TOKEN:+-u ":$GITHUB_TOKEN"} "https://api.github.com/repos/LDAPAccountManager/lam/releases/latest" | jq '.tag_name' --raw-output)"
...
git clone --depth 1 --branch "${GIT_VERSION}" https://github.com/LDAPAccountManager/lam.git "${TMPDIR}/lam"
composer -d "${TMPDIR}/lam/lam" install
cp "${TMPDIR}/lam/lam/composer.lock" "${PACKAGE_DIR}/composer.lock"
