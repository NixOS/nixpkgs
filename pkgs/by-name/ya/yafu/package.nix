...
stdenv.mkDerivation {
  ...
  postPatch = ''
    ...
    substituteInPlace top/driver.c \
      --replace-fail 'fopen("docfile.txt","r")' "fopen(\"$out/share/yafu/docfile.txt\",\"r\")"

    substituteInPlace top/driver.c \
      --replace-fail 'ini_success = readINI("yafu.ini", options);' \
      "ini_success = readINI(\"yafu.ini\", options); if (!ini_success) ini_success = readINI(\"$out/share/yafu/yafu.ini\", options);"
