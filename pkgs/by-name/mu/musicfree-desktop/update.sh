...
new_version=$(curl -sL https://api.github.com/repos/maotoumao/MusicFreeDesktop/releases/latest | jq -r .tag_name | tr -d v)
...
if patch --batch --dry-run -p1 -i $patch; then
  ...
  npm update sharp node-abi --package-lock-only
