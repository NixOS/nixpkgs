...
let
  ...
  disabledJavascriptTests = lib.optionals withV8 [
    # V8 without ICU timezone data miscomputes some historical local offsets.
    "util_printd.in"
  ];
...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    printf '%s\n' ${
      lib.escapeShellArgs (map (test: "${test} * * * *") disabledJavascriptTests)
