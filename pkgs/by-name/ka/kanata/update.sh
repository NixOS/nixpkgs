NEW_VERSION="$(curl --silent https://api.github.com/repos/jtroo/kanata/releases/latest | jq '.tag_name | ltrimstr("v")' --raw-output)"
...
oldDarwinVersion=$(nix-instantiate --eval -E "with $importTree; kanata.darwinDriverVersion" | tr -d '"')
nixFile=$(nix-instantiate --eval --strict -A "kanata.meta.position" | sed -re 's/^"(.*):[0-9]+"$/\1/')
sed -i.cmp "$nixFile" -re "s|\"$oldDarwinVersion\"|\"$newKarabinerDkVersion\"|"
