NEW_VERSION="$(curl --silent https://api.github.com/repos/jtroo/kanata/releases/latest  --header "Authorization: Bearer $GITHUB_TOKEN" | jq '.tag_name | ltrimstr("v")' --raw-output)"
...
oldDarwinVersion=$(nix-instantiate --eval -E "with $importTree; kanata.passthrough.darwinDriverVersion" | tr -d '"')
