...
runCommandCC "kak-tree-sitter-parser-${lang}"
  ...
  (
    ...
    + lib.optionalString (grammarSrc != null) ''
      ...
      compile="$(tomlq --arg cc "$CC" -r '.grammar["${lang}"].compile? // "cc" as $result | if $result == "cc" then $cc else $result end' "${configFile}")"
      ...
      link="$(tomlq --arg cc "$CC" -r '.grammar["${lang}"].link? // "cc" as $result | if $result == "cc" then $cc else $result end' "${configFile}")"
