...
let
  ...
  src = fetchFromGitHub {
    ...
    repo = "Kazumi";
    rev = version;
    hash = "sha256-hKelMtHXPyboFw/xXDqpMkAWCpHAaEf9wKWGhGjyw5A=";
  };
  mdk-sdk = fetchurl {
  ...
  customSourceBuilders = {
    flutter_volume_controller =
      ...
      stdenv.mkDerivation rec {
        ...
        patchPhase = ''
          runHook prePatch
          substituteInPlace linux/CMakeLists.txt --replace-fail '# Include ALSA' 'find_package(PkgConfig REQUIRED)'
          substituteInPlace linux/CMakeLists.txt --replace-fail 'find_package(ALSA REQUIRED)' 'pkg_check_modules(ALSA REQUIRED alsa)'
          runHook postPatch
        '';
        installPhase = ''
          runHook preInstall
