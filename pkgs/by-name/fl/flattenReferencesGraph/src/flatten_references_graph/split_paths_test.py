...
from .lib import directed_graph, pick_keys
...
vertex_props_dict = {"Root1": {"a": 1, "b": 1}, "B": {"b": 2}, "X": {"x": 3}}
...
class Test(unittest.TestCase, CustomAssertions, th.CustomAssertions):
    def test_empty_paths(self):
        ...
        result = self.assertResultKeys(["rest"], split_paths([], input_graph))

        self.assertGraphEqual(result["rest"], input_graph)

    def test_empty_graph(self):
        empty_graph = directed_graph([])

        def test_empty(paths):
            result = self.assertResultKeys(["rest"], split_paths(paths, empty_graph))

            self.assertGraphEqual(result["rest"], empty_graph)
    ...
    def test_split_paths_single(self):
        result = self.assertResultKeys(
            ["main", "common", "rest"], split_paths(["B"], make_test_graph())
        )
        ...
        self.assertGraphEqual(result["common"], directed_graph([("D", "E")]))

    def test_split_paths_multi(self):
        result = self.assertResultKeys(
            ["main", "common", "rest"], split_paths(["B", "Root3"], make_test_graph())
        )
    ...
    def test_split_no_common(self):
        result = self.assertResultKeys(
            ["main", "rest"], split_paths(["D"], make_test_graph())
        )

        self.assertGraphEqual(result["main"], directed_graph([("D", "E")]))
