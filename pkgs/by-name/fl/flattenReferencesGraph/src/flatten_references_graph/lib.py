...
def debug_plot(graph, name, **kwargs):
    ...
    save_as = (
        None
        if DEBUG_PLOT_SAVE_BASE_NAME is None
...
def debug_plot_with_highligth(g, vs, layout):
    debug_plot(
        ...
        vertex_color=["green" if v.index in vs else "red" for v in g.vs],
    )
...
def subcomponent_multi(graph, vertices, mode="out"):
    ...
    vertices.
    """
    return tlz.mapcat(lambda vertex: graph.subcomponent(vertex, mode=mode), vertices)
...
reference_graph_node_keys_to_keep = ["closureSize", "narSize"]
...
def references_graph_to_igraph(references_graph):
    ...
    path_to_size_dict = {node["path"]: node["narSize"] for node in references_graph}
    ...
    return igraph.Graph.DictList(
        ...
        unnest_iterable(
            map(edges_for_reference_graph_node(path_to_size_dict), references_graph)
        ),
        directed=True,
    )
...
def split_every(count, graph):
    vs = graph.vs
    return [graph.induced_subgraph(vs[x : x + count]) for x in range(0, len(vs), count)]
