...
from .subcomponent import subcomponent_out, subcomponent_in

from .lib import pick_keys, directed_graph, empty_directed_graph
...
class CustomAssertions:
    def assertResultKeys(self, result):
        self.assertListEqual(list(result.keys()), ["main", "rest"])
...
class Test(unittest.TestCase, CustomAssertions, th.CustomAssertions):
    def test_empty_paths(self):
        def test(func):
            ...
            result = self.assertResultKeys(func([], input_graph))

            self.assertGraphEqual(result["main"], empty_directed_graph())
    ...
    def test_empty_graph(self):
        def test(func):
            ...
            def test_empty(paths):
                result = self.assertResultKeys(func(paths, empty_graph))

                self.assertGraphEqual(result["main"], empty_graph)
    ...
    def test_subcomponent_out(self):
        result = self.assertResultKeys(subcomponent_out(["B"], make_test_graph()))
    ...
    def test_subcomponent_in(self):
        result = self.assertResultKeys(subcomponent_in(["B"], make_test_graph()))
    ...
    def test_subcomponent_in_multi(self):
        ...
        self.assertGraphEqual(
            result["rest"],
            directed_graph(
                [], ["C", "D", "E", "X"], pick_keys(["X"], vertex_props_dict).items()
            ),
        )
