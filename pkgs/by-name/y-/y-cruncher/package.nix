...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    find Binaries/ -name "*~*" -printf '"Binaries/%f"\n' | xargs install -Dm755 -t "$out"/lib/y-cruncher/Binaries/
    ...
    mkdir -p "$out"/bin/
    echo "#!/bin/bash" > "$out"/bin/y-cruncher
    echo "exec ${placeholder "out"}/lib/y-cruncher/y-cruncher \$@" >> "$out"/bin/y-cruncher
    chmod 755 "$out"/bin/y-cruncher
    patchShebangs "$out"/bin/y-cruncher
