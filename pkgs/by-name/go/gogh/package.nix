...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    substituteInPlace {gogh.sh,installs/*.sh} \
      --replace-fail 'bash ' '${lib.getExe' bashNonInteractive "bash"} '
  ...
  meta = {
    description = "A collection of color schemes for terminal emulators";
    longDescription = ''
    ...
    changelog = with finalAttrs; "${meta.homepage}/releases/tag/v${version}";
