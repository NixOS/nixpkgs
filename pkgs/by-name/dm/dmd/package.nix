...
stdenv.mkDerivation (finalAttrs: {
  ...
  srcs = [
    (fetchFromGitHub {
      ...
      hash = "sha256-tlWcFgKtXzfqMMkOq4ezhZHdYCXFckjN5+m6jO4VH0U=";
      name = "dmd";
    })
    (fetchFromGitHub {
      owner = "dlang";
      repo = "phobos";
      rev = "v${finalAttrs.version}";
      hash = "sha256-uU8S4rABOfhpKh+MvSbclkbdf0hrsuKF8SIpWMnPpfU=";
      name = "phobos";
    })
  ];
  ...
  patches = lib.optionals (lib.versionOlder finalAttrs.version "2.088.0") [
  ...
  '' + lib.optionalString (lib.versionAtLeast finalAttrs.version "2.089.0" && lib.versionOlder finalAttrs.version "2.092.2") ''
    rm dmd/compiler/test/dshell/test6952.d
  '' + lib.optionalString (lib.versionAtLeast finalAttrs.version "2.092.2") ''
  ...
  nativeBuildInputs = [
    ...
    which
    installShellFiles
  ] ++ lib.optionals (lib.versionOlder finalAttrs.version "2.088.0") [
    git
  ];
  ...
  nativeCheckInputs = [
    gdb
  ] ++ lib.optionals (lib.versionOlder finalAttrs.version "2.089.0") [
    unzip
  ];
