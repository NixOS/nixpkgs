...
linux_loong_url=$(echo "$linux_urls" | jq -r '.LoongArch')
...
linux_loong_hash=$(nix-hash --type sha256 --to-sri $(nix-prefetch-url --name "wpsoffice-365-$linux_version.deb" "$linux_loong_url"))
update-source-version wpsoffice-365 $linux_version $linux_loong_hash $linux_loong_url --system=loongarch64-linux --ignore-same-version
