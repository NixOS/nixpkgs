...
{
  version = runCommand "mojo-bin-version" { } ''
    export HOME=$TMPDIR
    ${finalPackage}/bin/mojo --version | grep -F '${finalPackage.version}'
    touch $out
  '';

  hello-world = runCommand "mojo-bin-hello-world" { } ''
    export HOME=$TMPDIR
    cat > hello.mojo <<'EOF'
    def main():
  ...
  cuda-codegen = runCommand "mojo-bin-cuda-codegen" { } ''
    export HOME=$TMPDIR
  ...
  driver-runpath = runCommand "mojo-bin-driver-runpath" { nativeBuildInputs = [ patchelf ]; } ''
    elfCount=0
  ...
  no-gpu = runCommand "mojo-bin-no-gpu" { } ''
    export HOME=$TMPDIR
  ...
  tooling = runCommand "mojo-bin-tooling" { } ''
    export HOME=$TMPDIR
  ...
  formatter = runCommand "mojo-bin-formatter" { } ''
    export HOME=$TMPDIR
