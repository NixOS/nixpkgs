...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    # We run every step from `yarn dev` EXCEPT build-completions
    # TODO: Maybe `build:all` is all we need?
    yarn --offline run-s -sn sh:build-time build:all sh:relink
    ...
    makeWrapper ${lib.getExe nodejs} "$out/bin/fish-lsp" \
      ...
      --prefix PATH : "${
        lib.makeBinPath [
          fish # TODO: may be removable, seems to build and function fine without it
          which
        ]
      }"
