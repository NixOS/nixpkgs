...
buildNpmPackage {
  ...
  postPatch = ''
    # Allow the Electron version supplied by nixpkgs.
    substituteInPlace package.json \
      --replace-fail '"electron": "^43.0.0"' '"electron": "*"'

    # Check for the target architecture's bundled rclone binary.
    ...
    # Skip the upstream DMG notarization hook.
    substituteInPlace package.json \
      --replace-fail '"afterAllArtifactBuild": "build/notarize-dmg.js",' ""

    # Use a conventional macOS icon without Xcode 26's actool.
    substituteInPlace package.json \
  ...
  buildPhase = ''
    ...
    # Prepare nixpkgs Electron and rclone for packaging.
  ...
  meta = {
    ...
    longDescription = ''
      ...
      Network-drive mounting requires externally installed macFUSE or FUSE-T on macOS,
      or FUSE3 and a working fusermount3 helper on Linux.
