...
python3Packages.buildPythonPackage rec {
  ...
  postPatch = ''
    substituteInPlace requirements.txt \
      --replace-fail "psutil==6.1.0" "psutil==6.1.1" \
      --replace-fail "evdev==1.7.1" "evdev==1.9.0"
  '';
  ...
  meta = {
    ...
    longDescription = ''
      To install udev rules, you have to add `services.udev.packages = [ pkgs.boxflat ]` into your configuration.
      Additionally you will also need to install the `universal-pidff` kernel module.
    '';
