...
let
  ...
  makeOfficeDesktopItem =
    ...
    makeDesktopItem {
      name = "softmaker-office-nx-${name}";
      desktopName = "SoftMaker Office NX ${desktopName}";
      icon = "softmaker-office-nx-${icon}";
      categories = [ "Office" ];
      exec = "softmaker-office-nx-${name} %F";
      tryExec = "softmaker-office-nx-${name}";
...
stdenv.mkDerivation rec {
  ...
  unpackPhase = ''
    ...
    mkdir ${pname}
    tar -C ${pname} -xf installer/officenx.tar.lzma
    runHook postUnpack
  '';
  ...
  meta = {
    description = "Subscription-based office suite with a word processor, spreadsheet and presentation program";
    longDescription = ''
      SoftMaker Office NX is the subscription version of SoftMaker Office that includes:
      - TextMaker NX for word processing
      - PlanMaker NX for spreadsheets
      - Presentations NX for presentations
      This version always provides the latest features and updates through the subscription model.
    '';
