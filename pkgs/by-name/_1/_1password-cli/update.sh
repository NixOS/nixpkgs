...
CLI_VERSION="$(curl -Ls https://app-updates.agilebits.com/product_history/CLI2 | xq --no-pager -q 'article:not(.beta) h3' | head -n1)"
if [ -z "$CLI_VERSION" ]; then
  echo "ERROR: failed to determine latest CLI version"
  exit 1
fi
