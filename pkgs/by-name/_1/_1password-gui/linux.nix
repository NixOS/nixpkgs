...
stdenv.mkDerivation {
  ...
  installPhase =
    ...
    + ''
      ...
      patchelf --set-interpreter $interp $out/share/1password/{1password,1Password-BrowserSupport,1Password-LastPass-Exporter,op-ssh-sign,onepassword-mcp}
      patchelf --set-rpath ${rpath}:$out/share/1password $out/share/1password/{1password,1Password-BrowserSupport,1Password-LastPass-Exporter,op-ssh-sign,onepassword-mcp}
