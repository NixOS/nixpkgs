...
buildNpmPackage rec {
  ...
  postPatch = ''
    ...
    ${jq}/bin/jq '.version |= "${version}"' packages/bruno-electron/package.json | sponge packages/bruno-electron/package.json
    ${jq}/bin/jq '.version |= "${version}"' packages/bruno-app/package.json | sponge packages/bruno-app/package.json
  '';
