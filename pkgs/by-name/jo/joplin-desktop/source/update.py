...
#!nix-shell -i python3 -p 'python3.withPackages(ps: [ps.requests ps.plumbum])' nix-prefetch yarn-berry_3 yarn-berry_3.yarn-berry-fetcher prefetch-npm-deps
import json
import requests
import tempfile
import shutil
...
from plumbum.cmd import nix_prefetch, nix_build, yarn, chmod, yarn_berry_fetcher, prefetch_npm_deps
...
for key, value in plugin_repositories.items():
    ...
    plugin["hash"] = nix_prefetch.with_cwd(HERE)[
        ...
        "extra-experimental-features",
        "flakes",
        f"import ./buildPlugin.nix {dict_to_argstr(plugin)}"
    ]().strip()

    plugin_src = nix_build.with_cwd(HERE)[
        "--no-out-link",
        "-E",
        f"((import <nixpkgs> {{}}).callPackage (import ./buildPlugin.nix {dict_to_argstr(plugin)}) {{}}).src"
    ]().strip()
