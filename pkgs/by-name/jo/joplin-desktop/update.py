...
with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    yarn_lock = Path(tmp_dir).joinpath("yarn.lock")

    with HERE.joinpath("yarn.lock.diff").open("w") as fd:
        fd.write("generated by update.py\n")
        fd.flush()
        (diff["-u", Path(src_dir).joinpath("yarn.lock"), yarn_lock] > fd)(retcode=1)
