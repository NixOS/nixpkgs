...
stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    substituteInPlace scripts/pi-hole/php/gravity.sh.php \
      --replace 'sudo pihole -g' '${pihole}/bin/pihole -g 2>&1'

    substituteInPlace settings.php \
      --replace '/usr/bin/pihole-FTL' ${pihole-ftl}/bin/pihole-FTL

    substituteInPlace scripts/pi-hole/php/func.php \
      --replace 'pidof pihole-FTL' '${procps}/bin/pidof pihole-FTL' \
      --replace "ps -p" "${procps}/bin/ps -p" \
      --replace 'sudo pihole' ${pihole}/bin/pihole
  '';
