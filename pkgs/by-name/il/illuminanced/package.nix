...
rustPlatform.buildRustPackage (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    sha256 = "sha256-ZEVma0uj9rsWB+vfUL7w3dHxI/ppBCG23TirGE+RREk=";
  };
  ...
  preBuild = ''
    substituteInPlace src/main.rs \
      --replace-fail /usr/local/etc/illuminanced.toml $out/usr/local/etc/illuminanced.toml
  '';

  postInstall = ''
    install -Dm0644 illuminanced.toml $out/usr/local/etc/illuminanced.toml
  '';
