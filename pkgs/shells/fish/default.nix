{ stdenv, fetchurl, ncurses, nettools, python, which, groff, gettext, man,
...
stdenv.mkDerivation rec {
  ...
  propagatedBuildInputs = [ python which groff gettext ]
                          ++ stdenv.lib.optional (!stdenv.isDarwin) man
                          ++ [ bc coreutils ];
  ...
  '' + stdenv.lib.optionalString (!stdenv.isDarwin) ''
    ...
    sed -i "s|Popen(\['manpath'|Popen(\['${man}/bin/manpath'|" "$out/share/fish/tools/create_manpage_completions.py"
    sed -i "s|command manpath|command ${man}/bin/manpath|" "$out/share/fish/functions/man.fish"
  '' + ''
