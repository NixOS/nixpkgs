...
mkKdeDerivation {
  ...
  postFixup = ''
    mkdir -p $out/nix-support
     ${lib.optionalString (backend == "mysql") "echo '${mariadb}' >> $out/nix-support/depends"}
     ${lib.optionalString (backend == "postgres") "echo '${postgresql}' >> $out/nix-support/depends"}
     ${lib.optionalString (backend == "sqlite") "echo '${sqlite}' >> $out/nix-support/depends"}
  '';
}
