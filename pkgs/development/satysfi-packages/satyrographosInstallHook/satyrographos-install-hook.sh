...
satyrographosInstallHook() {
    ...
    local dstPath="$2"
    mkdir -p "$dstPath"
    cat <<EOF > ./satyrographos-install-hook.py
#!/usr/bin/env python3
...
EOF
    patchShebangs .
    python3 satyrographos-install-hook.py "$srcPath" "$dstPath"
}

defaultSatyrographosInstallPhase() {
  runHook preInstall
...
if [ -z "${installPhase-}" ]; then
    installPhase=defaultSatyrographosInstallPhase
fi
