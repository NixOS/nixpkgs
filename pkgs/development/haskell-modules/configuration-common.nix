...
self: super: {
  ...
  double-conversion = overrideCabal
    (drv: {
  ...
  inherit (let
    mkSeldaPackage = name: overrideCabal
      (drv: {
  ...
  monad-logger-extras = appendPatch
    (fetchpatch {
  ...
  ghcjs-dom = assert super.ghcjs-dom.version == "0.9.5.0"; overrideCabal
    (old: {
      postPatch = ''
  ...
  postgrest = lib.pipe
    (super.postgrest.overrideScope (lself: lsuper: {
