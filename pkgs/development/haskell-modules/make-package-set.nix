...
let
  ...
  mkDerivationImpl = pkgs.callPackage ./generic-builder.nix {
    ...
    cpphs = overrideCabal (self.cpphs.overrideScope
      (self: super: {
...
package-set { inherit pkgs stdenv callPackage; } self // {
      ...
      drv =
        (extensible-self.extend
          (pkgs.lib.composeExtensions
            ...
            overrides)).callCabal2nix name root { };
  ...
  shellFor = { packages, withHoogle ? false, ... } @ args:
    ...
    in ((combinedPackageFor packages).envFunc { inherit withHoogle; }).overrideAttrs
      (old: mkDerivationArgs // {
