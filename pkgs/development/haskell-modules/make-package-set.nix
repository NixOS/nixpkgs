...
let
  ...
  callPackageWithScope = scope: fn: manualArgs:
    ...
    in
    stdenv.lib.makeOverridable drvScope (auto // manualArgs);
    ...
    let
      combinedPackageFor = packages:
        let
          ...
          # new-style commands.
          combinedPackages = pkgs.lib.filter (input: pkgs.lib.all (p: input.outPath or null != p.outPath) selected);
        ...
        in
        self.mkDerivation genericBuilderArgs;

      mkDerivationArgs = builtins.removeAttrs args [ "packages" "withHoogle" ];
    in
    ((combinedPackageFor packages).envFunc { inherit withHoogle; }).overrideAttrs (old: mkDerivationArgs // {
