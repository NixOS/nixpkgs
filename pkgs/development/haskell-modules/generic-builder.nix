...
in
  {
    ...
    enableDeadCodeElimination ? (!stdenv.isDarwin)
    ...
    shellHook ? "",
    coreSetup ? false
    # Use only core packages to build Setup.hs.
    ,
    useCpphs ? false,
    ...
    maxBuildCores ? 16
    # more cores usually don't improve performance: https://ghc.haskell.org/trac/ghc/ticket/9221
    ,
    ...
    assert stdenv.hostPlatform.isWasm -> enableStaticLibraries == false; let
      ...
      defaultConfigureFlags =
        ...
        ++ [
          ...
          (enableFeature enableExecutableProfiling (if versionOlder ghc.version "8"
          then "executable-profiling"
          else "profiling"))
