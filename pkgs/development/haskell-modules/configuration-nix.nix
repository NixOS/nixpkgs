...
builtins.intersectAttrs super {
  ...
  ${if pkgs.stdenv.hostPlatform.isDarwin then null else "GLUT"} = overrideCabal
    (drv: {
  ...
  copilot-libraries = overrideCabal
    (drv: {
      testToolDepends = drv.testToolDepends or [ ] ++ [
        pkgs.z3
      ];
  ...
  haskell-bee-redis = overrideCabal
    (drv: {
      testToolDepends = drv.testToolDepends or [ ] ++ [
        pkgs.redisTestHook
      ];
