...
builtins.intersectAttrs super {
  ...
  al = appendConfigureFlag "--extra-include-dirs=${pkgs.openal}/include/AL"
    super.al;
  ...
  heist =
    overrideCabal (drv: { testToolDepends = [ pkgs.pandoc ]; }) super.heist;

  # https://github.com/NixOS/cabal2nix/issues/136 and https://github.com/NixOS/cabal2nix/issues/216
  gio = disableHardening [ "fortify" ] (addPkgconfigDepend pkgs.glib
    (addBuildTool self.buildHaskellPackages.gtk2hs-buildtools super.gio));
  ...
  gtk = let
    ...
    gtk4 = if pkgs.stdenv.isDarwin then
      appendConfigureFlag "-fhave-quartz-gtk" gtk3
    else
      gtk4;
  in gtk3;
  ...
  hasql-interpolate =
    dontCheck super.hasql-interpolate; # wants to connect to postgresql
  hasql-transaction =
  ...
  tz = overrideCabal (drv: {
    ...
    patches = [
      ...
      (pkgs.fetchpatch {
        ...
        url =
          "https://github.com/sternenseemann/haskell-tz/commit/64928f1a50a1a276a718491ae3eeef63abcdb393.patch";
  ...
  xmonad_0_17_0 = doDistribute
    (appendPatch ./patches/xmonad_0_17_0-nix.patch (super.xmonad_0_17_0));

  # Need matching xmonad version
  xmonad-contrib_0_17_0 = doDistribute
    (super.xmonad-contrib_0_17_0.override { xmonad = self.xmonad_0_17_0; });
  ...
  wxc =
    (addBuildDepend self.split super.wxc).override { wxGTK = pkgs.wxGTK30; };
  ...
  miniball = overrideCabal (drv: { librarySystemDepends = [ pkgs.miniball ]; })
    super.miniball;
  ...
  SDL-ttf =
    overrideCabal (drv: { librarySystemDepends = [ pkgs.SDL pkgs.SDL_ttf ]; })
    super.SDL-ttf;
  SDL-mixer =
    overrideCabal (drv: { librarySystemDepends = [ pkgs.SDL pkgs.SDL_mixer ]; })
    super.SDL-mixer;
  SDL-gfx =
    overrideCabal (drv: { librarySystemDepends = [ pkgs.SDL pkgs.SDL_gfx ]; })
    super.SDL-gfx;
  ...
  ghcide = overrideCabal
    (drv: { preCheck = ''export PATH="$PWD/dist/build/ghcide:$PATH"''; })
    super.ghcide;
  ...
  git-annex = let pathForDarwin = pkgs.lib.makeBinPath [ pkgs.coreutils ];
  ...
  spago = let
    ...
    spagoWithPatches = appendPatch (
      ...
      pkgs.fetchpatch {
        url =
          "https://github.com/purescript/spago/commit/dd4bf4413d9675c1c8065d24d0ed7b345c7fa5dd.patch";
  ...
  retrie =
    overrideCabal (drv: { testToolDepends = [ pkgs.git pkgs.mercurial ]; })
    super.retrie;
  ...
  pinboard-notes-backup = overrideCabal (drv: {
    ...
    '' + (drv.postInstall or "");
  }) (generateOptparseApplicativeCompletion "pnbackup"
  ...
  cpuid = overrideCabal { platforms = pkgs.lib.platforms.x86; } super.cpuid;
  ...
  hsignal = overrideCabal { platforms = pkgs.lib.platforms.x86; } super.hsignal;

  # uses x86 intrinsics
  blake3 = overrideCabal { platforms = pkgs.lib.platforms.x86; } super.blake3;

  # uses x86 intrinsics, see also https://github.com/NixOS/nixpkgs/issues/122014
  crc32c = overrideCabal { platforms = pkgs.lib.platforms.x86; } super.crc32c;

  # uses x86 intrinsics
  seqalign =
    overrideCabal { platforms = pkgs.lib.platforms.x86; } super.seqalign;

  # uses x86 intrinsics
  geomancy =
    overrideCabal { platforms = pkgs.lib.platforms.x86; } super.geomancy;
  ...
  # uses x86 assembler
  inline-asm =
    overrideCabal { platforms = pkgs.lib.platforms.x86; } super.inline-asm;

  # uses x86 assembler in C bits
  hw-prim-bits =
    overrideCabal { platforms = pkgs.lib.platforms.x86; } super.hw-prim-bits;
  ...
  cachix = generateOptparseApplicativeCompletion "cachix"
    (super.cachix.override { nix = pkgs.nix_2_4; });

  hercules-ci-agent = appendConfigureFlag "-fnix-2_4"
    (super.hercules-ci-agent.override { nix = pkgs.nix_2_4; });
  hercules-ci-cnix-expr = appendConfigureFlag "-fnix-2_4"
    (super.hercules-ci-cnix-expr.override { nix = pkgs.nix_2_4; });
  hercules-ci-cnix-store = appendConfigureFlag "-fnix-2_4"
    (super.hercules-ci-cnix-store.override { nix = pkgs.nix_2_4; });
