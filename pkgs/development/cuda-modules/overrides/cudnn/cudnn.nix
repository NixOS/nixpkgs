...
in
{
  ...
  postFixup =
    optionalString
      (versionAtLeast finalAttrs.version "8.0.0.0" && versionOlder finalAttrs.version "9.0.0.0")
      ''
        ${getExe' patchelf "patchelf"} $lib/lib/libcudnn.so --add-needed libcudnn_cnn_infer.so
        ${getExe' patchelf "patchelf"} $lib/lib/libcudnn_ops_infer.so --add-needed libcublas.so --add-needed libcublasLt.so
      '';
