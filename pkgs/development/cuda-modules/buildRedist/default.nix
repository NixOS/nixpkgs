  ...
  extendDrvArgs =
    ...
    }:
    {
      ...
      meta = meta // {
        ...
        # NOTE:
        #   Most redistributables include multiple licenses. We add licenses to `config.allowlistedLicenses` when
        #   `config.cudaSupport` is enabled. Because `pkgs/top-level/stdenv/generic/check-meta.nix` allows for
        #   `meta.license` to be both a single license or a list of licenses, and only requires that at least one
        #   license in that list is in `config.allowlistedLicenses`, we always include `lib.licenses.nvidiaCudaRedist`,
        #   ensuring pacakges can be built without allowing all unfree licenses.
        #   Since licenses are compared in their entirety, it's important that package-specific license information (like
        #   URL) is included in a separate license, allowing us to match on the first one.
        license = lib.toList meta.license or [ licenses.nvidiaCudaRedist ];
