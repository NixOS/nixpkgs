...
assert (runVend == true && proxyVendor == true)
  -> throw "can't use `runVend` and `proxyVendor` together";
...
let
  ...
  go-modules = if vendorSha256 != null then
  ...
  package = stdenv.mkDerivation (args // {
    ...
    buildPhase = args.buildPhase or ''
      ...
      buildGoDir() {
        ...
        local OUT
        if ! OUT="$(go $cmd $buildFlags "''${buildFlagsArray[@]}" ''${tags:+-tags=${
          lib.concatStringsSep "," tags
        }} ''${ldflags:+-ldflags="$ldflags"} -v -p $NIX_BUILD_CORES $d 2>&1)"; then
