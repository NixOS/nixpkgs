{ lib, stdenv, fetchFromGitHub, gcc-arm-embedded, binutils-arm-embedded, python2
, skipTargets ? [
...
stdenv.mkDerivation rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "betaflight";
    rev =
      "8e9e7574481b1abba9354b24f41eb31054943785"; # Always use a commit id here!
    sha256 = "1wyp23p876xbfi9z6gm4xn1nwss3myvrjjjq9pd3s0vf5gkclkg5";
  };

  nativeBuildInputs = [ gcc-arm-embedded binutils-arm-embedded python2 ];

  postPatch = ''
    sed -ri "s/REVISION.*=.*git log.*/REVISION = ${
  ...
  meta = with lib; {
    description =
      "Flight controller software (firmware) used to fly multi-rotor craft and fixed wing craft";
