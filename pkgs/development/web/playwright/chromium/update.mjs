...
for (const attr_path of Object.keys(lockfile)) {
  const version_nixpkgs = lockfile[attr_path].version
  const version_upstream = await get_latest_playwright_chromium(await get_current_playwright_version())
  ...
  if (version_greater_than(version_upstream, version_nixpkgs)) {
    ...
    const version_chromium = version_upstream

    const chromium_rev = await chromium_resolve_tag_to_rev(version_chromium)

    lockfile[attr_path] = {
      version: version_chromium,
      chromedriver: await fetch_chromedriver_binaries(version_chromium),
      deps: {
        depot_tools: {},
        gn: {},
        npmHash: dummy_hash,
...
async function get_current_playwright_version() {
    const version = JSON.parse(await $nixpkgs`nix-instantiate --eval -A playwright.version --json`);
    return version;
}
...
async function resolve_DEPS(depot_tools_checkout, chromium_rev) {
  const { stdout } = await $`${nixpkgs}/pkgs/applications/networking/browsers/chromium/depot_tools.py ${depot_tools_checkout} ${chromium_rev}`
  const deps = JSON.parse(stdout)
  return Object.fromEntries(Object.entries(deps).map(([k, { url, rev, hash }]) => [k,  { url, rev, hash }]))
}

async function get_latest_playwright_chromium(playwrightVersion) {
  const release = await (await fetch(`https://api.github.com/repos/microsoft/playwright/releases/tags/v${playwrightVersion}`)).json()
  const chromium_version = release.body.match(/Chromium ([\d.]+)/)[1];
  if (!chromium_version) {
    throw new Error("Couldn't fetch supported chromium version from playwright.");
  }
