...
It supports two modes:
  - `update` for updating a specific Electron release.
    Can also be used to initialize a new Electron release.
  - `update-all` for updating all releases at once.

The optional `--commit` flag allows to automatically commit the changes.
"""
import base64
import csv
import json
...
def save_info_json(path: str, content: dict) -> None:
    ...
    with open(path, "w") as f:
        f.write(json.dumps(content, indent=4, default=vars, sort_keys=True))
        f.write("\n")
...
def eval(version):
    ...
    print(json.dumps(tree, indent=4, default=vars, sort_keys=True))
