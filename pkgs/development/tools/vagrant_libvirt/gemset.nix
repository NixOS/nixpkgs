{
  builder = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "0qibi5s67lpdv1wgcj66wcymcr04q6j4mzws6a479n0mlrmh5wr1";
      type = "gem";
    };
    version = "3.2.3";
  };
  excon = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "05hpyh5ik2fwr02zbhjn14avifm71mnb4ip0nz3w18lrmlqam333";
      type = "gem";
    };
    version = "0.56.0";
  };
  fog-core = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "0ac25s2wv7m6av7h7vjzd2bg3srhnn5yrz3lq1xpimhnfmp82sw6";
      type = "gem";
    };
    version = "1.43.0";
  };
  fog-json = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "0advkkdjajkym77r3c0bg2rlahl2akj0vl4p5r273k2qmi16n00r";
      type = "gem";
    };
    version = "1.0.2";
  };
  fog-libvirt = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "1b138i28m5icd3yibf1xlj6mxjfl24z6w4lsqksvb1ap7d1nbqgh";
      type = "gem";
    };
    version = "0.4.0";
  };
  fog-xml = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "043lwdw2wsi6d55ifk0w3izi5l1d1h0alwyr3fixic7b94kc812n";
      type = "gem";
    };
    version = "0.1.3";
  };
  formatador = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "1gc26phrwlmlqrmz4bagq1wd5b7g64avpx0ghxr9xdxcvmlii0l0";
      type = "gem";
    };
    version = "0.2.5";
  };
  json = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "01v6jjpvh3gnq6sgllpfqahlgxzj50ailwhj9b3cd20hi2dx0vxp";
      type = "gem";
    };
    version = "2.1.0";
  };
  mini_portile2 = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "1y25adxb1hgg1wb2rn20g3vl07qziq6fz364jc5694611zz863hb";
      type = "gem";
    };
    version = "2.1.0";
  };
  multi_json = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "1wpc23ls6v2xbk3l1qncsbz16npvmw8p0b38l8czdzri18mp51xk";
      type = "gem";
    };
    version = "1.12.1";
  };
  nokogiri = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "045xdg0w7nnsr2f2gb7v7bgx53xbc9dxf0jwzmh2pr3jyrzlm0cj";
      type = "gem";
    };
    version = "1.6.8.1";
  };
  ruby-libvirt = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "00y71gdjk30x40j14g8lq7lmfq30x3f6jyia8m1fl66x00ajdwb1";
      type = "gem";
    };
    version = "0.7.0";
  };
  vagrant-libvirt = {
    source = {
      remotes = ["https://rubygems.org"];
      sha256 = "03vvzckc7wbgb5zp8h9yxcp7s8kzy17nf417m6229f0hl5xdgw80";
      type = "gem";
    };
    version = "0.0.40";
  };
}