{ lib, fetchurl, fetchzip, ocaml-ng, version
, tarballName ? "ocamlformat-${version}.tbz", }:

let
  src = if version == "0.11.0" then
    fetchzip {
  ...
  ocamlPackages = if lib.versionAtLeast version "0.17.0" then
    ocaml-ng.ocamlPackages
  else if lib.versionAtLeast version "0.14.3" then
...
buildDunePackage {
  ...
  minimumOCamlVersion =
    if lib.versionAtLeast version "0.17.0" then "4.08" else "4.06";

  useDune2 = true;

  buildInputs = if lib.versionAtLeast version "0.19.0" then [
    base
    cmdliner
    fpath
    re
    ...
    dune-build-info
    odoc-parser
  ] else if lib.versionAtLeast version "0.18.0" then [
    base
    cmdliner
    fpath
    odoc
  ...
  ] else if lib.versionAtLeast version "0.17.0" then [
    base
    cmdliner
    fpath
    odoc
  ...
  ] else if lib.versionAtLeast version "0.15.1" then [
    base
    cmdliner
    fpath
    odoc
  ...
  ] else if lib.versionAtLeast version "0.14" then [
    base
    cmdliner
    fpath
