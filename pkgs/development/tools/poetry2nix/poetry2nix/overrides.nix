...
{
  automat = super.automat.overridePythonAttrs (old: rec {
  ...
  argcomplete = super.argcomplete.overridePythonAttrs (old: rec {
  ...
  astroid = super.astroid.overridePythonAttrs (old: rec {
  ...
  backports-entry-points-selectable =
    super.backports-entry-points-selectable.overridePythonAttrs (old: {
      postPatch = ''
  ...
  bcrypt = super.bcrypt.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ pkgs.libffi ]; });

  black = super.black.overridePythonAttrs (old: { dontPreferSetupPy = true; });
  ...
  cftime = super.cftime.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.cython ]; });

  cheroot =
    super.cheroot.overridePythonAttrs (old: { dontPreferSetupPy = true; });
  ...
  cryptography = super.cryptography.overridePythonAttrs (old:
    {
      nativeBuildInputs = (old.nativeBuildInputs or [ ])
        ++ lib.optional (lib.versionAtLeast old.version "3.4")
        [ self.setuptools-rust ]
        ++ lib.optional (stdenv.buildPlatform != stdenv.hostPlatform)
        self.python.pythonForBuild.pkgs.cffi;
  ...
  gdal = super.gdal.overridePythonAttrs (old: {
  ...
  jsonpickle =
    super.jsonpickle.overridePythonAttrs (old: { dontPreferSetupPy = true; });
  ...
  jupyter = super.jupyter.overridePythonAttrs (old: rec {
  ...
  jupyterlab-widgets = super.jupyterlab-widgets.overridePythonAttrs (old: rec {
  ...
  licensecheck =
    super.licensecheck.overridePythonAttrs (old: { dontPreferSetupPy = true; });
  ...
  lockfile = super.lockfile.overridePythonAttrs (old: {
    propagatedBuildInputs = (old.propagatedBuildInputs or [ ]) ++ [ self.pbr ];
  });

  lxml = super.lxml.overridePythonAttrs (old: {
    nativeBuildInputs = with pkgs;
      (old.nativeBuildInputs or [ ]) ++ [ pkg-config libxml2.dev libxslt.dev ]
  ...
  matplotlib = super.matplotlib.overridePythonAttrs (old:
    ...
    in {
      NIX_CFLAGS_COMPILE =
        lib.optionalString stdenv.isDarwin "-I${pkgs.libcxx}/include/c++/v1";

      XDG_RUNTIME_DIR = "/tmp";

      buildInputs = (old.buildInputs or [ ])
        ++ lib.optional enableGhostscript pkgs.ghostscript
        ++ lib.optional stdenv.isDarwin [ Cocoa ] ++ [ self.certifi ];
      ...
      propagatedBuildInputs = (old.propagatedBuildInputs or [ ])
        ++ [ pkgs.libpng pkgs.freetype pkgs.qhull ]
  ...
  mongomock = super.mongomock.overridePythonAttrs
    (oa: { buildInputs = oa.buildInputs ++ [ self.pbr ]; });

  moto = super.moto.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.sshpubkeys ]; });
  ...
  munch = super.munch.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pbr ]; });
  ...
  opencv-python = super.opencv-python.overridePythonAttrs (old: rec {
    buildInputs = (old.buildInputs or [ ]) ++ [ self.scikit-build ];
  });

  openexr = super.openexr.overridePythonAttrs (old: rec {
  ...
  parsel = super.parsel.overridePythonAttrs (old: rec {
    nativeBuildInputs = (old.nativeBuildInputs or [ ])
      ++ [ self.pytest-runner ];
  });
  ...
  pillow = super.pillow.overridePythonAttrs (old: {
    nativeBuildInputs = [ pkgs.pkg-config self.pytest-runner ]
  ...
  pip = if lib.versionAtLeast super.pip.version "20.3" then
    super.pip.overridePythonAttrs (old:
      let
        pname = "pip";
        version = "20.2.4";
      in {
  ...
  pyarrow = if lib.versionAtLeast super.pyarrow.version "0.16.0" then
    super.pyarrow.overridePythonAttrs (old:
      ...
      in if arrowCppVersion != pyArrowVersion then
        ...
        nativeBuildInputs = (old.nativeBuildInputs or [ ])
          ++ [ self.cython pkgs.pkg-config pkgs.cmake ];
  ...
  pycairo = (drv:
    ...
    (old: {
      ...
      nativeBuildInputs = (old.nativeBuildInputs or [ ])
        ++ [ pkgs.meson pkgs.ninja pkgs.pkg-config ];

      propagatedBuildInputs = (old.propagatedBuildInputs or [ ])
        ++ [ pkgs.cairo pkgs.xlibsWrapper ];
  ...
  pycocotools = super.pycocotools.overridePythonAttrs (old: {
    buildInputs = (old.buildInputs or [ ]) ++ [ self.cython self.numpy ];
  });
  ...
  pygame = super.pygame.overridePythonAttrs (old: rec {
    nativeBuildInputs = [ pkgs.pkg-config pkgs.SDL ];
  ...
  pytezos = super.pytezos.override
  ...
  rockset = super.rockset.overridePythonAttrs (old: rec {
    postPatch = ''
  ...
  requests-unixsocket = super.requests-unixsocket.overridePythonAttrs (old: {
    nativeBuildInputs = (old.nativeBuildInputs or [ ]) ++ [ self.pbr ];
  });

  requestsexceptions = super.requestsexceptions.overridePythonAttrs
    (old: { nativeBuildInputs = old.nativeBuildInputs ++ [ self.pbr ]; });
  ...
  scikit-learn = super.scikit-learn.overridePythonAttrs (old: {
    buildInputs = (old.buildInputs or [ ])
      ++ [ pkgs.gfortran pkgs.glibcLocales ]
  ...
  vispy = super.vispy.overrideAttrs (old: {
    ...
    nativeBuildInputs = (old.nativeBuildInputs or [ ])
      ++ [ self.cython self.setuptools-scm-git-archive ];
  });
  ...
  bootstrapped-pip = super.bootstrapped-pip.override {
    wheel = (pkgs.python3.pkgs.override { python = self.python; }).wheel;
  };
  ...
  wheel = let
    ...
    sourcePackage =
      ...
      }).overrideAttrs (old: { inherit (super.wheel) pname name version src; });
  in if isWheel then wheelPackage else sourcePackage;
  ...
  packaging = super.packaging.overridePythonAttrs (old: {
    buildInputs = (old.buildInputs or [ ]) ++
      ...
      lib.optional (lib.versionAtLeast old.version "20.5"
        && lib.versionOlder old.version "20.8") [ self.flit-core ];
  });
  ...
  sentencepiece = super.sentencepiece.overridePythonAttrs (old: {
    dontUseCmakeConfigure = true;
    nativeBuildInputs = (old.nativeBuildInputs or [ ])
      ++ [ pkgs.pkg-config pkgs.cmake pkgs.gperftools ];
  ...
  pyjsg = super.pyjsg.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pbr ]; });

  pyshex = super.pyshex.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pbr ]; });

  pyshexc = super.pyshexc.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pbr ]; });

  shexjsg = super.shexjsg.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pbr ]; });

  sparqlslurper = super.sparqlslurper.overridePythonAttrs
    (old: { buildInputs = (old.buildInputs or [ ]) ++ [ self.pbr ]; });
