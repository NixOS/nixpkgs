{
  ...
  preferWheels ? false,
  __isBootstrap ? false
  # Hack: Always add Poetry as a build input unless bootstrapping
  ,
  ...
}:
  pythonPackages.callPackage
  (
    {preferWheel ? preferWheels, ...} @ args: let
      inherit (pkgs) stdenv;
      ...
      fileInfo = let
        ...
        entries =
          (if preferWheel
          then binaryDist ++ sourceDist
          else sourceDist ++ binaryDist)
          ++ eggs;
        lockFileEntry = (
          if lib.length entries > 0
      ...
      in rec {
        ...
        format =
          if _isEgg
          then "egg"
          else if lib.strings.hasSuffix ".whl" name
          then "wheel"
          else "pyproject";
        kind =
          if _isEgg
          then python.pythonVersion
          else if format == "pyproject"
          then "source"
      ...
      format =
        if isDirectory || isGit || isUrl
        then "pyproject"
        else fileInfo.format;
    in
      buildPythonPackage {
        ...
        src =
          ...
          then (poetryLib.cleanPythonSources {src = localDepPath;})
          else if isFile
          then localDepPath
          else if isLegacy
          then
