{ autoPatchelfHook, pkgs, lib, python, buildPythonPackage, poetryLib, evalPep508
}:
{ name, version, files, source, dependencies ? { }, pythonPackages
, python-versions, pwd, sourceSpec
, supportedExtensions ? lib.importJSON ./extensions.json, preferWheels ? false
, __isBootstrap ?
  false # Hack: Always add Poetry as a build input unless bootstrapping
, ... }:
...
pythonPackages.callPackage ({ preferWheel ? preferWheels, ... }@args:
  let
    inherit (pkgs) stdenv;
    inherit (poetryLib)
      isCompatible getManyLinuxDeps fetchFromLegacy fetchFromPypi moduleName;
    ...
    baseBuildInputs = lib.optional (!lib.elem name skipSetupToolsSCM)
      pythonPackages.setuptools-scm;
    format =
      if isDirectory || isGit || isUrl then "pyproject" else fileInfo.format;
  in buildPythonPackage {
    ...
    nativeBuildInputs = [ pythonPackages.poetry2nixFixupHook ] ++ lib.optional
      (!isSource && (getManyLinuxDeps fileInfo.name).str != null)
      autoPatchelfHook ++ lib.optional (format == "pyproject")
    ...
    in builtins.map (n: pythonPackages.${moduleName n}) depAttrs;
    ...
    src = if isGit then
      (builtins.fetchGit {
        ...
        ref = sourceSpec.branch or sourceSpec.rev or (if sourceSpec ? tag then
          "refs/tags/${sourceSpec.tag}"
        else
          "HEAD");
      })
    else if isUrl then
      builtins.fetchTarball { inherit (source) url; }
