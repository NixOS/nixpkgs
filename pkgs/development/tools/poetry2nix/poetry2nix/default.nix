{ pkgs ? import <nixpkgs> { }, lib ? pkgs.lib, poetry ? null, poetryLib ?
  import ./lib.nix {
    inherit lib pkgs;
    stdenv = pkgs.stdenv;
  } }:
let
  ...
  mkInputAttrs = { py, pyProject, attrs, includeBuildSystem ? true }:
    let
    ...
    in {
      ...
      propagatedBuildInputs =
        mkInput "propagatedBuildInputs" (getDeps "dependencies")
        ++ ([ py.pkgs.setuptools ]);
...
in lib.makeScope pkgs.newScope (self: {
  ...
  */
  ...
  mkPoetryPackages = { projectDir ? null
    ...
    , pwd ? projectDir, preferWheels ? false
      # Example: { my-app = ./src; }
    , editablePackageSources ? { }, __isBootstrap ?
    ...
    let
      ...
      specialAttrs =
        [ "overrides" "poetrylock" "projectDir" "pwd" "preferWheels" ];
      ...
      baseOverlay = self: super:
        let
          ...
          lockPkgs = builtins.listToAttrs (builtins.map (pkgMeta: rec {
            ...
            value = self.mkPoetryDep (pkgMeta // {
              ...
              pythonPackages = self;
              sourceSpec =
                pyProject.tool.poetry.dependencies.${name} or pyProject.tool.poetry.dev-dependencies.${name} or { };
            });
          }) (lib.reverseList compatible));
        in lockPkgs;
      overlays = builtins.map getFunctorFn ([
        (self: super:
          let hooks = self.callPackage ./hooks { };
          in {
            ...
            poetry-core = if __isBootstrap then
              null
            else
              poetryPkg.passthru.python.pkgs.poetry-core;
            poetry = if __isBootstrap then null else poetryPkg;

            __toPluginAble = toPluginAble self;

            inherit (hooks)
              pipBuildHook removePathDependenciesHook poetry2nixFixupHook
              wheelUnpackHook;
  ...
  */
  mkPoetryEnv = { projectDir ? null, pyproject ? projectDir + "/pyproject.toml"
    ...
    in poetryPython.python.withPackages (_: poetryPackages);
  ...
  */
  defaultPoetryOverrides = self.mkDefaultPoetryOverrides
    (import ./overrides.nix { inherit pkgs lib; });
  ...
  overrides = {
    ...
    */
    withDefaults = overlay: [ self.defaultPoetryOverrides overlay ];
  };
})
