{
  ...
  getFunctorFn = fn:
    if builtins.typeOf fn == "set"
    then fn.__functor
    else fn;

  # Map SPDX identifiers to license names
  spdxLicenses = lib.listToAttrs (lib.filter (pair: pair.name != null) (builtins.map (v: {
    name =
      if lib.hasAttr "spdxId" v
      then v.spdxId
      else null;
    value = v;
  ...
  in {
    buildInputs = mkInput "buildInputs" (if includeBuildSystem
    then buildSystemPkgs
    else []);
  ...
  lib.makeScope pkgs.newScope (self: {
    ...
    mkPoetryPackages = {
      ...
      partitions = let
        supportsPythonVersion = pkgMeta:
          if pkgMeta ? marker
          then (evalPep508 pkgMeta.marker)
      ...
      (
            ...
            in
              {
                ...
                poetry-core =
                  if __isBootstrap
                  then null
                  else poetryPkg.passthru.python.pkgs.poetry-core;
                poetry =
                  if __isBootstrap
                  then null
                  else poetryPkg;
        ...
        (if builtins.typeOf overrides == "list"
        then overrides
        else [overrides])
      );
    ...
    mkPoetryApplication = {
      ...
      src ? (
        ...
        if lib.isDerivation projectDir
        then projectDir
        else self.cleanPythonSources {src = projectDir;}
      ),
      pyproject ? projectDir + "/pyproject.toml",
      poetrylock ? projectDir + "/poetry.lock",
      overrides ? self.defaultPoetryOverrides,
      ...
      app = py.pkgs.buildPythonPackage (
        ...
        // {
          ...
          passthru = {
            python = py;
            dependencyEnv = (
              lib.makeOverridable ({app, ...} @ attrs: let
                args =
                  builtins.removeAttrs attrs ["app"]
                  // {
                    extraLibs = [app];
                  };
              in
