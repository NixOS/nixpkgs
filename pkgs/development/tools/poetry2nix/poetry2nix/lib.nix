...
let
  ...
  getPythonVersion = python:
    ...
    in joinVersion (if major pyVer == major ver && minor pyVer == minor ver then
      ver
    else
      pyVer);
  ...
  #
  getManyLinuxDeps = f:
    let ml = pkgs.pythonManylinuxPackages;
  ...
  findGitIgnores = path:
    ...
    in lib.optionals (builtins.toString path != "/" && !isGitRoot)
...
in {
  inherit fetchFromPypi fetchFromLegacy getManyLinuxDeps isCompatible readTOML
    getBuildSystemPkgs satisfiesSemver cleanPythonSources moduleName
    getPythonVersion targetMachine;
}
