...
#! nix-shell -p python3 dotnet-sdk_5 -i bash

# Run this script to generate deps.nix
# ./create_deps.sh /path/to/netcoredbg/source/checkout
...
dotnet restore -v normal --no-cache managed/ManagedPart.csproj --use-lock-file -r linux-x64
...
cat all_versions.txt | sed '/^$/d' | sort -u > tmp
