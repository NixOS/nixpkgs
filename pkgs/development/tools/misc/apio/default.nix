...
buildPythonApplication rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "apio";
    rev = "v${version}";
    sha256 = "sha256-U1g5rvapklGtduvw0dOYUJpvE+KUHg6AYLFnjMRfOok=";
  };

  postPatch = ''
    substituteInPlace pyproject.toml \
      --replace 'pyjwt>=1.5.3,<2' 'pyjwt>=2'

    # API is different in pyjwt>=2
    substituteInPlace apio/util.py --replace \
      'return jwt.decode(text, "secret", algorithm="HS256")' \
      'return jwt.decode(text, "secret", algorithms=["HS256"])'
