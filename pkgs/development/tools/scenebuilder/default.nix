...
maven'.buildMavenPackage rec {
  ...
  patches = [
    ...
    # maven.build.timestamp was not able to be set for some reason, so instead we patch out its usage
    (substituteAll {
      src = ./build-timestamp.patch;
      release_date = "2023.10.30"; # v21.0.1 release date in yyyy-MM-dd format
    })
  ];

  mvnParameters = "-Dmaven.test.skip -Dproject.build.outputTimestamp=1980-01-01T00:00:02Z";
