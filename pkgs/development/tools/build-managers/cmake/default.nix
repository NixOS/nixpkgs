{ stdenv, lib, fetchurl, pkg-config, bzip2, curlMinimal, expat, libarchive, xz
, zlib, libuv, rhash, buildPackages
# darwin attributes
, SystemConfiguration, ps, isBootstrap ? false
, useSharedLibraries ? (!isBootstrap && !stdenv.isCygwin)
, useOpenSSL ? !isBootstrap, openssl, useNcurses ? false, ncurses
...
stdenv.mkDerivation rec {
  ...
  patches = [
    ...
    ./remove-systemconfiguration-dep.patch
  ...
  buildInputs = lib.optionals useSharedLibraries [
    ...
    libuv
    rhash
  ] ++ lib.optional useOpenSSL openssl ++ lib.optional useNcurses ncurses
  ...
  configureFlags = [ "--docdir=share/doc/${pname}${version}" ]
    ...
    ++ lib.optional withQt5 "--qt-gui" ++ lib.optionals buildDocs [
