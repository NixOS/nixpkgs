...
let
  ...
  common = { scala, deps }:
    stdenv.mkDerivation rec {
      ...
      nativeBuildInputs = [ zip gnused scala makeWrapper jre ];
      postPatch = ''
      ...
      buildPhase = ''
        SCALASOURCES=$(find . -type f -name '*.scala' | egrep -v '/test/|/out/|/readme/' | ${filterSourceDirs} ${scala.version})
        JAVASOURCES=$(find . -type f -name '*.java'  | egrep -v '/test/|/out/|/readme/' | ${filterSourceDirs} ${scala.version})
...
*/
    deps = [
      ...
      (artifact { sha1 = "d7a8a219ba99d3b2e492a1e64625ffdcb4183b4e"; url = "mirror://maven/org/slf4j/slf4j-nop/1.7.16/slf4j-nop-1.7.16.jar"; })
    ];
  };
