...
{ lib, ncurses, which, graphviz, lua, fetchzip, mkCoqDerivation
, recurseIntoAttrs, withDoc ? false, single ? false, coqPackages, coq
, ocamlPackages, version ? null }@args:
with builtins // lib;
let
  repo = "math-comp";
  owner = "math-comp";
  ...
  mathcomp_ = package:
    let
      mathcomp-deps = if package == "single" then
        ...
        map mathcomp_ (head (splitList (pred.equal package) packages));
      pkgpath =
