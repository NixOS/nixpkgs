    ...
    if isFunction overrides then overrides else old:
      let
          ...
          attrs =
            ...
            else  if isAttrs overrides then overrides // {
              meta     = old.meta // overrides.meta;
              passthru = old.passthru // overrides.passthru;
            }
    ...
    let
      ...
      fixdeps = attrs:
        let version = attrs.version or "master";
            mcdeps  = mkMathcompGenSet (filter isString attrs.passthru.mathcompDeps) overrides;
