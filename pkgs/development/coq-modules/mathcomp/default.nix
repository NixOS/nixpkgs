...
let
  ...
  default-mathcomp-version =
    let v = head (
      filter
        (mc: mathcomp-coq-versions.${mc} coq.coq-version)
      ...
      attrs = {
        ...
        passthru = {
          ...
          overrideMathcomp = moreOverrides:
            (mkMathcompGen
              (old:
                let new = overrides old; in new // moreOverrides new)
              mathcomp-pkg
              mathcomp-version
            ).${mathcomp-pkg};
  ...
  mathcompCorePkgs =
    recurseIntoAttrs (mapDerivationAttrset dontDistribute (mathcompGen default-mathcomp-version));
...
in
{
  ...
  mathcomp_single =
    dontDistribute (getAttrOr (mathcompGenSingle default-mathcomp-version) "single");
