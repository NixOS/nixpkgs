...
for pkg in desired_packages:
    ...
    if pkg["uuid"] in stdlib_infos["stdlibs"]:
        path = (
            stdlib_infos["stdlib_root"]
            + "/"
            + stdlib_infos["stdlibs"][pkg["uuid"]]["name"]
        )
        isStdLib = True
    ...
    if path:
        if (Path(path) / "Project.toml").exists():
            ...
            print(
                f"""WARNING: package {pkg["name"]} didn't have a Project.toml in {path}"""
            )
        ...
        result["deps"][pkg["name"]].append(
            {
                "version": pkg["version"],
                "uuid": pkg["uuid"],
                "git-tree-sha1": (tree_hash if tree_hash != "nothing" else None)
                or None,
                "deps": deps or None,
