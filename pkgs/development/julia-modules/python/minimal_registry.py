...
registry["packages"] = {
    k: v for k, v in registry["packages"].items() if k in uuid_to_versions
}
...
for uuid, versions in uuid_to_versions.items():
    if uuid in package_overrides:
        ...
        versions_to_keep = (
            {k: v for k, v in all_versions.items() if k in versions}
            if versions != None
            else all_versions
        )
        ...
        if versions is None:
            ...
            shutil.copy2(
                registry_path / path / "Package.toml", out_path / path / "Package.toml"
            )
...
for pkg in desired_packages:
    for dep in pkg.get("deps", []) or []:
        ...
        if not uuid in uuid_to_versions:
            ...
            if not entry:
                print(
                    f"""WARNING: found missing UUID but couldn't resolve it: {uuid}"""
                )
                continue
