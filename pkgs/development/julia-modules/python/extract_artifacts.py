...
def get_archive_derivation(
    ...
    if closure_dependencies_dag.has_node(uuid):
        depends_on = set(closure_dependencies_dag.get_dependencies(uuid)).intersection(
            dependency_uuids
        )
...
def process_item(args):
    ...
    artifacts = toml.loads(
        subprocess.check_output(
            [julia_path, extract_artifacts_script, uuid, src]
        ).decode()
    )
    if not artifacts:
        return f"  uuid-{uuid} = {{}};\n"
    ...
    for artifact_name, details in artifacts.items():
        ...
        if any(parsed_url.path.endswith(x) for x in archive_extensions):
            derivation = get_archive_derivation(
                uuid,
                artifact_name,
                url,
                sha256,
    ...
    with open(closure_yaml_path, "r") as f:
        ...
        for uuid, contents in closure_yaml.items():
            if contents.get("depends_on"):
                closure_dependencies_dag.add_node(
                    uuid, dependencies=contents["depends_on"].values()
                )
