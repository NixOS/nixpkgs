...
with open(out_path, "w") as f:
    ...
    treehash = "{treehash}";
            ...
            if (not ("tree_hash" in pkg)) or pkg["tree_hash"] == "nothing":
                continue
            ...
            if not pkg["version"] in all_versions:
                continue
            version_to_use = all_versions[pkg["version"]]

            if not "nix-sha256" in version_to_use:
                raise KeyError(
                    f"""Couldn't find nix-sha256 hash for {pkg["name"]} {pkg["version"]} in {versions_toml}. This might indicate that we failed to prefetch the hash when computing the augmented registry. Was there a relevant failure in {registry_path / "failures.yml"}?"""
                )
