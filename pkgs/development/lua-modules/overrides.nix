...
with prev; {
  ...
  lgi = prev.lib.overrideLuarocks prev.lgi (drv: {
    ...
    buildInputs = [ pkgs.glib pkgs.gobject-introspection ];
    patches = [
      (pkgs.fetchpatch {
  ...
  lrexlib-gnu = prev.lib.overrideLuarocks prev.lrexlib-gnu
    (drv: { buildInputs = [ pkgs.gnulib ]; });
  ...
  lrexlib-posix = prev.lib.overrideLuarocks prev.lrexlib-posix
    (drv: { buildInputs = [ pkgs.glibc.dev ]; });

  lua-iconv = prev.lib.overrideLuarocks prev.lua-iconv
    (drv: { buildInputs = [ pkgs.libiconv ]; });
  ...
  luadbi-mysql = prev.lib.overrideLuarocks prev.luadbi-mysql (drv: {
    ...
    buildInputs = [ pkgs.mariadb.client pkgs.libmysqlclient ];
  });

  luadbi-postgresql = prev.lib.overrideLuarocks prev.luadbi-postgresql
    (drv: { buildInputs = [ pkgs.postgresql ]; });
  ...
  luasystem = prev.lib.overrideLuarocks prev.luasystem
    (drv: { buildInputs = [ pkgs.glibc.out ]; });

  luazip = prev.lib.overrideLuarocks prev.luazip
    (drv: { buildInputs = [ pkgs.zziplib ]; });

  lua-yajl = prev.lib.overrideLuarocks prev.lua-yajl
    (drv: { buildInputs = [ pkgs.yajl ]; });
  ...
  luuid = (prev.lib.overrideLuarocks prev.luuid (drv: {
    ...
    meta = oa.meta // { platforms = pkgs.lib.platforms.linux; };
    ...
    patchFlags = [ "-p2" ];
    patches = [ ./luuid.patch ];
    postConfigure = let inherit (prev.luuid) version pname;
    in ''
    ...
    passthru = {
      libluv = final.luv.overrideAttrs (oa: {
        preBuild = final.luv.preBuild + ''
          ...
          sed -i 's,${"\${.*INSTALL_INC_DIR}"},${
            placeholder "out"
          }/include/luv,' CMakeLists.txt
        '';
  ...
  lyaml = prev.lib.overrideLuarocks prev.lyaml
    (oa: { buildInputs = [ pkgs.libyaml ]; });
  ...
  readline = (prev.lib.overrideLuarocks prev.readline (drv: {
    unpackCmd = ''
      ...
      tar xf *.tar.gz
    '';
    propagatedBuildInputs = prev.readline.propagatedBuildInputs
      ++ [ pkgs.readline.out ];
    extraVariables = rec {
