...
then throw "vg is not available for OCaml ${ocaml.version}"
  ...
  stdenv.mkDerivation {
    ...
    buildPhase = topkg.buildPhase + " --with-uutf ${boolToString pdfBackend}" + " --with-otfm ${boolToString pdfBackend}" + " --with-js_of_ocaml ${boolToString htmlcBackend}" + " --with-cairo2 false";
