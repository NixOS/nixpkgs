{ lib, fetchurl, stdenv, ocaml, ocamlbuild, findlib, ocaml_extlib, glib, perl
...
stdenv.mkDerivation {
  ...
  buildFlags = [ "all" "opt" ];
  nativeBuildInputs = [ findlib ocaml ocamlbuild pkg-config ];
  buildInputs = [ glib perl stdlib-shims ];
