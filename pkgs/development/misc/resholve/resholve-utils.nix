  ...
  */
  ...
  makeEnvVal = solution: env: val:
    if env == "inputs" then
      ...
      spaces (makeDirectives solution env val)
    else
  ...
  removeCliArgs = value: removeAttrs value [ "scripts" "flags" ];
  ...
  makeArgs = { flags ? [ ], scripts, ... }: spaces (flags ++ scripts);
  ...
  makeInvocation = solution: value:
    ...
    # we pass resholve a directory
      "RESHOLVE_LORE=${binlore.collect { drvs = value.inputs; }} ${
        makeEnvs solution value
      } ${resholve}/bin/resholve --overwrite ${makeArgs value}"
    else
      throw "invalid solution"; # shouldn't trigger for now

  # Build resholve invocation for each solution.
  makeCommands = solutions: lib.mapAttrsToList makeInvocation solutions;
}
