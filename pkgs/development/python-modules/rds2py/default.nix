...
buildPythonPackage rec {
  ...
  # setuptools drives the build, not the cmake hook, so cmakeFlags/CMAKE_ARGS
  # have no effect. Upstream's CMake tries to find zlib, which fails unless we
  # inject include/lib paths. To solve this, we wrap cmake in a script:
  #   - If invoked with "--build", we forward arguments unmodified.
  #   - Otherwise (the configure step), we add ZLIB_* paths explicitly.
  # This ensures only the configure step is patched, while the build step works
  # normally.
  preConfigure = ''
    mkdir -p .bin
    cat > .bin/cmake <<EOF
    ...
    if [ "\$is_build_step" -eq 1 ]; then
        exec ${cmake}/bin/cmake "\$@"
    else
        exec ${cmake}/bin/cmake \\
          -DZLIB_INCLUDE_DIR=${lib.getDev zlib}/include \\
          -DZLIB_LIBRARY=${lib.getLib zlib}/lib/libz.so \\
          "\$@"
    fi
    EOF
    chmod +x .bin/cmake
