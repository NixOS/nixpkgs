{ stdenv
, buildPythonPackage
, fetchPypi
, isPy3k
, elasticsearch_5
, ipaddress
, python-dateutil
, pytz
}:

buildPythonPackage rec {
  pname = "elasticsearch-dsl";
  version = "5.4.0";

  src = fetchPypi {
    inherit pname version;
    sha256 = "1a0vn3zk94d0pcrn0smq5s1bi9bvdjchsidwf34fnzax11kn9vyb";
  };

  propagatedBuildInputs = [ elasticsearch_5 python-dateutil pytz ] ++ stdenv.lib.optional (!isPy3k) ipaddress;

  # ImportError: No module named test_elasticsearch_dsl
  # Tests require a local instance of elasticsearch
  doCheck = false;

  meta = with stdenv.lib; {
    description = "High level Python client for Elasticsearch";
    longDescription = ''
      Elasticsearch DSL is a high-level library whose aim is to help with
      writing and running queries against Elasticsearch. It is built on top of
      the official low-level client (elasticsearch-py).
    '';
    homepage = https://github.com/elasticsearch/elasticsearch-dsl-py;
    license = licenses.asl20;
    maintainers = with maintainers; [ desiderius ];
  };
}
