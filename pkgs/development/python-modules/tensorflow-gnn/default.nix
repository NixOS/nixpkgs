...
let
  ...
  tensorflowProtoSrc = fetchzip {
    url = "https://github.com/tensorflow/tensorflow/archive/refs/tags/v${tfVersion}.tar.gz";
...
buildPythonPackage rec {
  ...
  # The upstream build expects TF_USE_LEGACY_KERAS=1 to be set before importing
  # the package. Propagate it to build, check, and downstream consumers.
  ...
  preBuild = ''
    ...
    proto_includes=(--proto_path=. --proto_path=${tensorflowProtoSrc}/tensorflow-${tfVersion})
    for proto in \
  ...
  disabledTests = [
    # Fail with newer numpy: "only 0-dimensional arrays can be converted to Python scalars".
    "test_three_components"
    "test_two_node_sets"
    "test_triples_from_array"
  ];
