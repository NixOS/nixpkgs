...
{ addOpenGLRunpath, autoPatchelfHook, buildPythonPackage, config, fetchurl
, isPy39, lib, stdenv
...
let device = if cudaSupport then "gpu" else "cpu";
in buildPythonPackage rec {
  ...
  src = {
    cpu = fetchurl {
      url =
        "https://storage.googleapis.com/jax-releases/nocuda/jaxlib-${version}-cp39-none-manylinux2010_x86_64.whl";
      sha256 = "sha256:0rqhs6qabydizlv5d3rb20dbv6612rr7dqfniy9r6h4kazdinsn6";
    };
    gpu = fetchurl {
      url =
        "https://storage.googleapis.com/jax-releases/cuda111/jaxlib-${version}+cuda111-cp39-none-manylinux2010_x86_64.whl";
  ...
  preInstallCheck = lib.optional cudaSupport ''
    ...
    for file in $out/**/*.so; do
      ...
      patchelf --set-rpath "$rpath:${cudatoolkit_11}/lib:${
        lib.makeLibraryPath [ cudatoolkit_11.lib ]
      }" $file
    done
  '';
  ...
  propagatedBuildInputs = [ absl-py flatbuffers scipy ]
    ++ lib.optional cudaSupport cudatoolkit_11;
