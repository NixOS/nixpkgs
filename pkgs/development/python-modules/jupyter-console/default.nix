...
buildPythonPackage rec {
  ...
  postPatch = ''
    ...
    substituteInPlace jupyter_console/tests/test_console.py \
      --replace-fail "args = ['-m', 'jupyter_console', '--colors=NoColor']" "args = ['--colors=NoColor']" \
      --replace-fail "cmd = sys.executable" "cmd = '${placeholder "out"}/bin/jupyter-console'" \
      --replace-fail "check_output([sys.executable, '-m', 'jupyter_console'," "check_output(['${placeholder "out"}/bin/jupyter-console',"
  '';
