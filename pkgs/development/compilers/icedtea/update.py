...
DROP_URL = 'http://icedtea.classpath.org/download/drops/icedtea{}/{}'
...
def get_new_bundle_attrs(jdk, version, path):
	drop_url = DROP_URL.format(jdk, version)
	...
	for bundle in BUNDLES:
		...
		if bundle == 'hotspot':
			...
			if btype == 'hg':
				battrs['url'] = '{}/archive/{}.tar.gz'.format(m.group(2), m.group(3))
			elif btype == 'drop':
				battrs['url'] = '{}/hotspot.tar.bz2'.format(m.group(2))
			else:
				raise Exception('Unknown HotSpot bundle type: {}'.format(btype))
		...
		battrs['url'] = '{}/{}.tar.bz2'.format(drop_url, bundle)
...
def generate_jdk(jdk):
	...
	drop_url = DROP_URL.format(jdk, version)
	src_drop_url = DROP_URL.format(jdk, '${version}')
	...
	for bundle in BUNDLES:
		...
		b_url = battrs['url']
		b_url = b_url.replace(drop_url, '${drop_url}')
