{ minor_version, major_version, patch_version, ... }@args:
let
...
in { lib, stdenv, fetchurl, ncurses, buildEnv, libunwind, libX11, xorgproto
...
in let
  ...
  name = "ocaml${optionalString aflSupport "+afl"}${
      optionalString spaceTimeSupport "+spacetime"
    }${optionalString flambdaSupport "+flambda"}-${version}";
