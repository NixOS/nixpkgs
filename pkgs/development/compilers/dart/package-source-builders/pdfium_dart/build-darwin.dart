...
// pdfium-binaries ships the library under lib/ on both platforms; only the
// shared-library suffix differs.
const _linuxPdfium = '@pdfium-binaries@/lib/libpdfium.so';
const _macosPdfium = '@pdfium-binaries@/lib/libpdfium.dylib';
...
void main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (target.targetOS == OS.linux || target.targetOS == OS.macOS) {
      final pdfiumPath = target.targetOS == OS.linux
          ? _linuxPdfium
          : _macosPdfium;
      final source = File(pdfiumPath);
      if (!await source.exists()) {
        throw Exception('nixpkgs PDFium library not found at $pdfiumPath');
      }
      ...
      throw UnsupportedError(
        'pdfium_dart is patched for linux/macos offline builds in nixpkgs',
      );
    }
...
final class _PdfiumTarget {
  ...
  static _PdfiumTarget fromCodeConfig(CodeConfig config) {
    return switch (config.targetOS) {
      OS.linux => const _PdfiumTarget(
        targetOS: OS.linux,
        libraryFileName: 'libpdfium.so',
      ),
      OS.android => const _PdfiumTarget(
        targetOS: OS.android,
        libraryFileName: 'libpdfium.so',
      ),
      OS.windows => const _PdfiumTarget(
        targetOS: OS.windows,
        libraryFileName: 'pdfium.dll',
      ),
      OS.macOS => const _PdfiumTarget(
        targetOS: OS.macOS,
