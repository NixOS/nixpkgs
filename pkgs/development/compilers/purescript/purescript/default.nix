...
in stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://github.com/${pname}/${pname}/releases/download/v${version}/" +
      (if stdenv.hostPlatform.isDarwin then
        if stdenv.hostPlatform.isAarch64 then "macos-arm64" else "macos"
      else
        if stdenv.hostPlatform.isAarch64 then "linux-arm64" else "linux64"
      ) +
      ".tar.gz";
    sha256 =
      if stdenv.hostPlatform.isDarwin && stdenv.hostPlatform.isAarch64
      then "0bi231z1yhb7kjfn228wjkj6rv9lgpagz9f4djr2wy3kqgck4xg0"
      else if stdenv.hostPlatform.isDarwin
      then "178ix54k2yragcgn0j8z1cfa78s1qbh1bsx3v9jnngby8igr6yn3"
      else if stdenv.hostPlatform.isAarch64
      then "1ws5h337xq0l06zrs9010h6wj2hq5cqk5ikp9arq7hj7lxf43vn5"
      else "1w4jgjpfhaw3gkx9sna64lq9m030x49w4lwk01ik5ci0933imzj3";
  };
