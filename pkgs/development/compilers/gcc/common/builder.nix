...
(stdenv.mkDerivation (
  ...
  // lib.optionalAttrs (stdenv.targetPlatform.isFreeBSD || stdenv.hostPlatform.isFreeBSD || stdenv.buildPlatform.isFreeBSD) {
    # the version number must be passed or gcc gets very mad. Flags are in configure-flags.nix
    configurePlatforms = [ "build" "host" ] ++ lib.optional (!stdenv.targetPlatform.isFreeBSD) "target";
  }
))
