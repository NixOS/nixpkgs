{
  ...
  texinfo ? null,
  perl ? null
  # optional, for texi2pod (then pod2man); required for Java
  ,
  gmp,
  mpfr,
  libmpc,
  ...
  patchelf,
  libelf
  # optional, for link-time optimizations (LTO)
  ,
  cloog ? null,
  isl ? null
  ...
  enablePlugin ? stdenv.hostPlatform == stdenv.buildPlatform
...
with builtins; let
  ...
  patches =
    ...
    ++ [../struct-ucontext.patch ../struct-sigaltstack-4.9.patch]
    # glibc-2.26
    ...
    stdenv.mkDerivation (
      {
        ...
        postPatch =
          if targetPlatform != hostPlatform || stdenv.cc.libc != null
          then
            ...
            in ''              echo "fixing the \`GLIBC_DYNAMIC_LINKER' and \`UCLIBC_DYNAMIC_LINKER' macros..."
        ...
        meta = {
          ...
          description =
            "GNU Compiler Collection, version ${version}"
            + (if stripped
            then ""
            else " (with debugging info)");
