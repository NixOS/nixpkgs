...
, bootBintoolsNoLibc ? if stdenv.targetPlatform.linker == "lld"
  then null
  else pkgs.bintoolsNoLibc
, bootBintools ? if stdenv.targetPlatform.linker == "lld"
  then null
  else pkgs.bintools
, darwin
  ...
  libraries = lib.makeExtensible (libraries:
    ...
    in
    {
      ...
      compiler-rt-libc = callPackage ./compiler-rt {
        inherit llvm_meta;
        stdenv =
          if stdenv.hostPlatform.useLLVM or false || (stdenv.hostPlatform.isDarwin && stdenv.hostPlatform.isStatic)
      ...
      compiler-rt =
        if stdenv.hostPlatform.isAndroid || stdenv.hostPlatform.isDarwin
