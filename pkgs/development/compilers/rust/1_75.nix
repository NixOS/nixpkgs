...
let
  toolchain = import ./default.nix
    ...
    # patch the output of the rustc.nix expression
    # with changes for the rust_1_75 expression from e9ca6038eb5f4398fc57feb4084b7df4afaa6d7d
    overwrittenRustc =
      toolchain.packages.stable.rustc.overrideAttrs (previousAttrs: {
        configureFlags =
          [ "--sysconfdir=${placeholder "out"}/etc" ] ++ previousAttrs.configureFlags;
        postConfigure = null;
      });
in
lib.attrsets.recursiveUpdate toolchain {
  packages.stable.rustc = overwrittenRustc;
}
