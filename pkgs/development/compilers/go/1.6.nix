...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    ...
    sha256 = "10rsacdf5549fz7c9cjn6s64mswls7vlmpasjppdq9s2y9b6f6id";
  };
  ...
  prePatch = ''
    ...
    # chroot builds)
    rm src/net/{listen_test.go,parse_test.go}
    ...
    # Disable /etc/protocols lookup tests
    ...
    sed -i '/src\/cmd\/api\/run.go/ireturn nil' src/cmd/dist/test.go
    sed -i 's/which /type -P /' misc/cgo/testcarchive/test.bash
    sed -i 's/.*testcshared.*//' src/cmd/dist/test.go
