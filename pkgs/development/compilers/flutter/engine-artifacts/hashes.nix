{
  "54a7145303f0dd9d0f93424a2e124eb4abef5091" = {
    skyNotice = "sha256-1rkOkTJwwfFRh1r3DjylUN4mV93UGKyfbu/r5Tsdi4g=";
    flutterNotice = "sha256-pZjblLYpD/vhC17PkRBXtqlDNRxyf92p5fKJHWhwCiA=";
    android-arm = {
      "artifacts.zip" = "sha256-mpMtAyKMotZz1PDXMuFkLBU7nXZZUEE+ZvJC/0HT19I=";
    };
    android-arm-profile = {
      "artifacts.zip" = "sha256-CrMWldUmYk1vd4wzYhnphtAdrah4JrSdPSl53iUoJ0M=";
      "linux-x64.zip" = "sha256-etVFjBI2Ip/Z9fSXsgIHEHZZ8ufL/l74NEOHnqy4BS4=";
      "darwin-x64.zip" = "sha256-a/Iujytz2whX8wzZo2HANEcDEF3J5/zMy0rVFHHXQ3g=";
    };
    android-arm-release = {
      "artifacts.zip" = "sha256-4+iTBR+O353sSwXU+hHbfY9sj1stjObviEHkhRyBTzg=";
      "linux-x64.zip" = "sha256-2JbLrWZQOm+sNVmum+tbL4AUdJ4ASnKrEM1v/WtcEqA=";
      "darwin-x64.zip" = "sha256-OVrVU7yNAxVzh6RNWJGKEqw52k1evDY6Ju+8Se2WecE=";
    };
    android-arm64 = {
      "artifacts.zip" = "sha256-XnY0a+rmj1VCBOTxieRghVScAgUsXEjXqBepOEdy88s=";
    };
    android-arm64-profile = {
      "artifacts.zip" = "sha256-R+tnoRAkDxo33EeT5JrdgWwXpIVSBtywn8q2MGiSTlc=";
      "linux-x64.zip" = "sha256-FiWcnYDyEgTYT+I46M4dN7mURTb6UZP74yfVZyUfa2I=";
      "darwin-x64.zip" = "sha256-lHNs3NFE3nRJg1ZhkTNcP+WZauVNCInfs06CK3jeG4c=";
    };
    android-arm64-release = {
      "artifacts.zip" = "sha256-KgK3Wk38bZy9RxutaVuk6HwznrxS4GBjqo/g9Y6RI5E=";
      "linux-x64.zip" = "sha256-BvI6p1SHf3ablWXefiu1Cfm3rWbghnDuBgPny5G8DWg=";
      "darwin-x64.zip" = "sha256-51oNX2FGj06yKgW3piwtvjvl0gdyboeOaoiR/1wAY/M=";
    };
    android-x64 = {
      "artifacts.zip" = "sha256-6mdJ29C67rL5y6OpHvmdPcVZzQRHzLN/0D4eld50D68=";
    };
    android-x64-profile = {
      "artifacts.zip" = "sha256-aPmXIPWj81Jj+Wbw8mxYyBFaqoTaGk0HliigPOhyEEU=";
      "linux-x64.zip" = "sha256-gJAvF4KincOHUu408GgZIkJHtPF7KaEDczIpCp+GYow=";
      "darwin-x64.zip" = "sha256-i4XyehtqnNJYLtgwJx7Df1ZEeF4zxp0EltIQHvnv/WA=";
    };
    android-x64-release = {
      "artifacts.zip" = "sha256-zWOAREw+rj6L9I+wZxRWzKr1cwivvVequUurkjTbeUc=";
      "linux-x64.zip" = "sha256-l+xrwGVNiZAC53Tn8aIfiMZNKOMXGhIAQaelRCCodI8=";
      "darwin-x64.zip" = "sha256-O25gxbI5+1s7bM10vnBiBb8bqppbcr5myFP2WBG9494=";
    };
    android-x86 = {
      "artifacts.zip" = "sha256-SfCn92Al4O8BKbPdWp2FbaAaadOub9IrIkjDXlfEkes=";
    };
    android-x86-jit-release = {
      "artifacts.zip" = "sha256-nwE1zCqO4LhEt1b3dAYBStDs6iVyDCwWWPIZ9FPetzU=";
    };
    darwin-arm64 = {
      "artifacts.zip" = "sha256-kdgsj89zA17OernmXyomI2wfTSUJxb+lubr4AHXrYik=";
      "font-subset.zip" = "sha256-NEN4x96RyRvJnQMuQjmQddIm+3aWzuIwmmg1jRmwmOA=";
    };
    darwin-arm64-profile = {
      "artifacts.zip" = "sha256-Rqp5b3StavBF+pwn8MPgZgUNOYtizxp0y+jP2hlPETw=";
    };
    darwin-arm64-release = {
      "artifacts.zip" = "sha256-QsnGpVlSWvP1HB66+iv1CcyuFSnTDfOIRVahwVpSReU=";
    };
    darwin-x64 = {
      "FlutterEmbedder.framework.zip" = "sha256-I+HWKU3Y3ifB6lsIewKEG8MFHMTuK2fq+JofVFBnv00=";
      "FlutterMacOS.framework.zip" = "sha256-s/593FHymgsJyjyGQiwNMC8nybdFCPg5XjV26unSMco=";
      "artifacts.zip" = "sha256-1TjPYPGPw9AJ/mvB7dO7kfvfNgGyOe5DX8WDPC9JWS0=";
      "font-subset.zip" = "sha256-BL7xU+pyQMW/EB05X5A3Dkrz+46v8PPUauGoO+KeyBc=";
      "gen_snapshot.zip" = "sha256-HSRasNmoQLnhNdvtKKNeJn4x3EAvltLb0M+IzoDsWnw=";
    };
    darwin-x64-profile = {
      "FlutterMacOS.framework.zip" = "sha256-kw4esoj9mubPEGE4OokP7a7QEMFzUPAmutxORJWZndM=";
      "artifacts.zip" = "sha256-iUX2Psy8n199O/jVHEhhzuZ7RcQBTbdMKMGn5Dtc3SI=";
      "gen_snapshot.zip" = "sha256-aj2jDnFCFcwoiAND/K1Vjec7QiUb4Bs48HYvfeHm6pE=";
    };
    darwin-x64-release = {
      "FlutterMacOS.dSYM.zip" = "sha256-7OlJ3wiciY1JBpoCCQ+pKOivhIlL+M0D4Zx9p/bXUbk=";
      "FlutterMacOS.framework.zip" = "sha256-uafJ7X//jUyLwWBD9PFyc8q7ECMH1OaQGzMk4KkvcX0=";
      "artifacts.zip" = "sha256-hSqcnf68Hv1H8AEk8x0X7m8bSWw8GNbb6We8mgw4M18=";
      "gen_snapshot.zip" = "sha256-y2zl/Z6hWgqQ3D4uk2CHqtrw/wjix1un+fAf53gzzA8=";
    };
    "flutter_patched_sdk.zip" = "sha256-SjGccH51cUF8EEmzSQRxHrgHhB27QniptNOoSB4kaLQ=";
    "flutter_patched_sdk_product.zip" = "sha256-BBqeN34ZATXH9A56eg4xN38TfpXdUeTfh0bYkY/Pyp0=";
    ios = {
      "artifacts.zip" = "sha256-jLc6AlV8ZGWNgZ8Ss4NS2jp5HUH2RJLS1qpJNyqWWXM=";
    };
    ios-profile = {
      "artifacts.zip" = "sha256-ENeZF3H0X+3YZMvPRFgl+CiiSiFyvgaQvBSCZphMUVY=";
    };
    ios-release = {
      "Flutter.dSYM.zip" = "sha256-2Jkuo+sdTCVSspi2HVHoRG0qZ5KWumKD8WFWDiDGMNA=";
      "artifacts.zip" = "sha256-8z+TrpqWeyzZqV3fHGmzA1dVJHq8D/ORcWZ2v+C6g8g=";
    };
    linux-arm64 = {
      "artifacts.zip" = "sha256-PFs4h/jufdPedgO8szoD2uTUUvYecKJaM1Lx2UcAJIw=";
      "font-subset.zip" = "sha256-h8jd3aGp/mdn/u2A21fqIovyNK5oGGKmVh4QCb6+xIw=";
    };
    linux-arm64-debug = {
      "linux-arm64-flutter-gtk.zip" = "sha256-T8VunH262m0pyDeoEaJ1gpwqcqQurmhygO6+A9polgw=";
    };
    linux-arm64-profile = {
      "linux-arm64-flutter-gtk.zip" = "sha256-jhs9XzM9trhLJ/wa/ySmQoxYiTfYpMUenI4dtwn83zE=";
    };
    linux-arm64-release = {
      "linux-arm64-flutter-gtk.zip" = "sha256-KOMuSJ4LJSgMFA8HoBuzx2T+I4Xi0F0uqKGnC8SK60s=";
    };
    linux-x64 = {
      "artifacts.zip" = "sha256-UtwMVBNvXZYY/v7i61FBhRSd6BlXj0mL9aMm+wcs9Sw=";
      "font-subset.zip" = "sha256-8HXNeaOtt7Ia/Y4sr82Z6YSDKRvznAGyherT21fu8aU=";
    };
    linux-x64-debug = {
      "linux-x64-flutter-gtk.zip" = "sha256-HKkRWnVhGZ1EFlhRrxxCCAH6P79iU9uIhehZLplJBEE=";
    };
    linux-x64-profile = {
      "linux-x64-flutter-gtk.zip" = "sha256-ujTwiLlLGlT9OEhabYR8euFV4ZRm+TWfmuDSKLP6Kwg=";
    };
    linux-x64-release = {
      "linux-x64-flutter-gtk.zip" = "sha256-7AhY2nZLK5WPoVvNZk4WHjHs7zE27/HsUD33l4UNmuY=";
    };
  };
}
