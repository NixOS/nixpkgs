  ...
  builderArgs = rec {
    ...
    linux = universal // {
      ...
      installPhase = universal.installPhase or ''
        ...
        mkdir -p $out/bin
        mkdir -p $out/app
        mv $built $out/app/$name

        for f in $(find $out/app/$name -iname "*.desktop" -type f); do
          install -D $f $out/share/applications/$(basename $f)
        done

        for f in $(find $out/app/$name -maxdepth 1 -type f); do
          ln -s $f $out/bin/$(basename $f)
        done

        # make *.so executable
        find $out/app/$name -iname "*.so" -type f -exec chmod +x {} +

        # remove stuff like /build/source/packages/ubuntu_desktop_installer/linux/flutter/ephemeral
        for f in $(find $out/app/$name -executable -type f); do
