...
# build-tools
, bootPkgs, autoconf, autoreconfHook, automake, coreutils, fetchgit, perl
...
, enableDwarf ? stdenv.targetPlatform.isx86 && !stdenv.targetPlatform.isDarwin
...
enableNativeBignum ? !(lib.meta.availableOn stdenv.hostPlatform gmp), gmp
...
let
  ...
  libDeps = platform:
    [ libffi ncurses ] ++ lib.optional (!enableNativeBignum) gmp
    ++ lib.optional (platform.libc != "glibc" && !targetPlatform.isWindows)
    libiconv ++ lib.optional enableDwarf elfutils;
...
stdenv.mkDerivation (rec {
  ...
  meta = {
    ...
    hydraPlatforms =
      builtins.filter (p: p != "aarch64-linux") ghc.meta.platforms;
  };
