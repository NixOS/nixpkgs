...
let
  # Some eggs declare dependencies on modules which are part of chicken itself and thus need not
  # (and cannot) be installed as eggs. Instead of marking such eggs as broken, we remove these
  # invalid dependencies. Note that r7rs is one of them in CHICKEN 6, where it is no longer an
  # egg, and that cond-expand shows up because read-egg.scm flattens conditional dependencies
  # to their head; the only egg using one restricts it to CHICKEN 5.
  invalidDependencies = [
    "cond-expand"
    "r7rs"
  ];
