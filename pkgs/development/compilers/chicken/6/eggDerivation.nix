...
let
  ...
  # chicken-install takes a lock in its egg cache even when building an egg from
  # the current directory, and the cache defaults to a location under HOME,
  # which is not writable in the sandbox. The cache itself stays empty: with
  # -cached, chicken-install builds the unpacked egg in place.
  eggCacheSetup = ''
...
(stdenv.mkDerivation (
  {
    ...
    installPhase = ''
      ...
      # Patching generated .egg-info instead of original .egg to work around
      # https://bugs.call-cc.org/ticket/1855, which is still unfixed in 6.0.0:
      # eggs without a version property in their .egg get installed without one.
