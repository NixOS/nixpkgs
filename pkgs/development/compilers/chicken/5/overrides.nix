...
in
{
  ...
  # cond-expand has no chicken-5 clause any more
  slib-wt-tree = broken;
  srfi-137 = broken;
  srfi-38 = broken;
  # depends on srfi-38, which is broken
  magic-pipes = broken;
  srfi-48 = broken;
  # imports scheme.base, which CHICKEN 5 does not provide
  matchertext = broken;
  # csc: file `lsp-server.private.chicken.import.scm' does not exist
  lsp-server = broken;
  canvas-draw = broken;
  gemini = broken;
