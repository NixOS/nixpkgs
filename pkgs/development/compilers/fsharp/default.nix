...
{ stdenv, fetchurl, pkgconfig, autoconf, automake, which, mono46, dotnetbuildhelpers, dotnetPackages }:
...
stdenv.mkDerivation rec {
  ...
  postInstall = ''
    substituteInPlace $out/bin/fsharpc --replace " mono " " ${mono46}/bin/mono "
    substituteInPlace $out/bin/fsharpi --replace " mono " " ${mono46}/bin/mono "
    substituteInPlace $out/bin/fsharpiAnyCpu --replace " mono " " ${mono46}/bin/mono "
