...
{ toolsVersion ? "26.1.1", platformToolsVersion ? "31.0.3"
, buildToolsVersions ? [ "31.0.0" ], includeEmulator ? false
, emulatorVersion ? "30.6.3", platformVersions ? [ ], includeSources ? false
, includeSystemImages ? false, systemImageTypes ? [ "google_apis_playstore" ]
, abiVersions ? [ "armeabi-v7a" ], cmakeVersions ? [ ], includeNDK ? false
, ndkVersion ? "22.1.7171670", ndkVersions ? [ ndkVersion ]
...
let
  ...
  mkRepoJson = { packages ? [ ], images ? [ ], addons ? [ ] }:
    let
      mkRepoRuby =
        (pkgs.ruby.withPackages (pkgs: with pkgs; [ slop nokogiri ]));
...
in rec {
  ...
  system-images = lib.flatten (map (apiVersion:
    map (type:
      map (abiVersion:
        if lib.hasAttrByPath [ apiVersion type abiVersion ]
        system-images-packages then
          deployAndroidPackage {
            inherit os;
            package =
  ...
  makeNdkBundle = ndkVersion:
    import ./ndk-bundle {
      inherit deployAndroidPackage os autoPatchelfHook makeWrapper pkgs
        pkgsHostHost lib platform-tools;
  ...
  androidsdk = if !licenseAccepted then
    ...
    import ./tools.nix {
      inherit deployAndroidPackage requireFile packages toolsVersion
        autoPatchelfHook makeWrapper os pkgs pkgs_i686 lib;
        ...
        ${lib.concatMapStrings (licenseName:
          let
            licenseHashes =
              builtins.concatStringsSep "\n" (mkLicenseHashes licenseName);
            licenseHashFile =
              pkgs.writeText "androidenv-${licenseName}" licenseHashes;
          in ''
