...
in rec {
    ...
    apiVersion:
      map (
        type:
          map (
            abiVersion:
              if lib.hasAttrByPath [apiVersion type abiVersion] system-images-packages
              then
                deployAndroidPackage {
                  inherit os;
  ...
  ndk-bundles =
    if includeNDK
    then map makeNdkBundle ndkVersions
    else [];

  # The "default" NDK bundle.
  ndk-bundle =
    if includeNDK
    then lib.findFirst (x: x != null) null ndk-bundles
    else null;
    ...
    then
      ...
      import ./tools.nix {
        ...
        postInstall = ''
          ...
          # Link extras
          ${lib.concatMapStrings (identifier: let
          ...
          # Write licenses
          mkdir -p licenses
          ${lib.concatMapStrings (licenseName: let
