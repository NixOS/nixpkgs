...
, libffi, jemalloc, autoreconfHook, bison, autoconf, libiconv, libobjc
, libunwind, Foundation, buildEnv, bundler, bundix, makeWrapper, buildRubyGem
, defaultGemConfig, removeReferencesTo }@args:
  ...
  generic = { version, sha256 }:
    let
      ...
      self = lib.makeOverridable ({ stdenv, buildPackages, lib, fetchurl
        ...
        , rubygemsSupport ? true, zlib, zlibSupport ? true, openssl
        , opensslSupport ? true, gdbm, gdbmSupport ? true, ncurses, readline
        , cursesSupport ? true, groff, docSupport ? true, libyaml
        , yamlSupport ? true, libffi, fiddleSupport ? true, jemalloc
        ...
        , removeReferencesTo, jitSupport ? false, autoreconfHook, bison
        , autoconf, buildEnv, bundler, bundix, libiconv, libobjc, libunwind
        , Foundation, makeWrapper, buildRubyGem, defaultGemConfig }:
        stdenv.mkDerivation rec {
          ...
          buildInputs = [ autoconf ] ++ (op fiddleSupport libffi)
            ...
            ++ (op yamlSupport libyaml) ++ (op jemallocSupport jemalloc)
          ...
          patches = (import ./patchsets.nix {
          ...
          }).${ver.majMinTiny} ++ [ ./do-not-regenerate-revision.h.patch ]
            ++ op (atLeast30 && useRailsExpress)
            ...
            ++ op (!atLeast30 && rubygemsSupport) (fetchpatch {
              url =
          ...
          configureFlags = [
            "--enable-shared"
            "--enable-pthread"
            "--with-soname=ruby-${version}"
          ] ++ op useRailsExpress "--with-baseruby=${baseruby}/bin/ruby"
            ++ op (!jitSupport) "--disable-jit-support"
            ++ op (!docSupport) "--disable-install-doc"
            ++ op (jemallocSupport) "--with-jemalloc" ++ ops stdenv.isDarwin [
