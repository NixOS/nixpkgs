...
let
  generic = { callPackage, lib, stdenv, nixosTests, tests, fetchurl, makeWrapper
    ...
    , system-sendmail, valgrind, xcbuild

    , version, sha256, extraPatches ? [ ], packageOverrides ? (final: prev: { })
    ...
    , pearSupport ? true, pharSupport ? true, phpdbgSupport ? true
      ...
      # Misc flags
    , apxs2Support ? !stdenv.isDarwin, argon2Support ? true
    , cgotoSupport ? false, embedSupport ? false, ipv6Support ? true
    , systemdSupport ? stdenv.isLinux, valgrindSupport ? !stdenv.isDarwin
      ...
      mkBuildEnv = prevArgs: prevExtensionFunctions:
        lib.makeOverridable ({ extensions ? ({ enabled, ... }: enabled)
          ...
          let
            ...
            enabledExtensions = builtins.foldl' (enabled: f:
              f {
                inherit enabled;
            ...
            getExtName = ext:
              lib.removePrefix "php-" (builtins.parseDrvName ext.name).name;
            ...
            phpWithExtensions = symlinkJoin {
              ...
              passthru = php.passthru // {
                ...
                overrideAttrs = f:
                  ...
                  in php.buildEnv { inherit extensions extraConfig; };
          ...
          in phpWithExtensions);

      mkWithExtensions = prevArgs: prevExtensionFunctions: extensions:
        mkBuildEnv prevArgs prevExtensionFunctions { inherit extensions; };
    in stdenv.mkDerivation (let
      attrs = {
        pname = "php";
        ...
        configureFlags =
          ...
          # PCRE
          ++ lib.optionals (lib.versionAtLeast version "7.4")
