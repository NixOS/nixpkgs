...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ stdenv gcc openssl ];
  ...
  buildCommand = ''
    ...
    make all
    cd ..
    # picolisp doesn't have an 'install' target. The steps below follow the
    # debian install layout with workarounds for shbang paths being too long.
    cp bin/* "$out"/bin
