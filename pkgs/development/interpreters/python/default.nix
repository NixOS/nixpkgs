...
(let
  ...
  passthruFun = { implementation, libPrefix, executable, sourceVersion
    ...
    , pythonOnHostForHost, pythonOnTargetForTarget
      ...
      pythonPackages = callPackage ({ pkgs, stdenv, python, overrides }:
        let
          ...
          keep = self: {
            ...
            inherit (self)
              isPy27 isPy35 isPy36 isPy37 isPy38 isPy39 isPy310 isPy3k isPyPy
              pythonAtLeast pythonOlder python bootstrapped-pip
              buildPythonPackage buildPythonApplication fetchPypi
              hasPythonModule requiredPythonModules makePythonPath disabledIf
              toPythonModule toPythonApplication buildSetupcfg
              ...
              setuptoolsBuildHook setuptoolsCheckHook venvShellHook
              wheelUnpackHook
        ...
        in lib.makeScopeWithSplicing pkgs.splicePackages pkgs.newScope
        otherSplices keep extra
        (lib.extends (lib.composeExtensions aliases extensions)
          pythonPackagesFun)) {
