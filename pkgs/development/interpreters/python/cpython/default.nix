...
with lib; let
  ...
  with passthru; let
    ...
    pythonAbiName =
      ...
      then replaceStrings ["musl"] ["gnu"] parsed.abi.name
    ...
    stdenv.mkDerivation {
      ...
      patches =
        ...
        ++ optionals (pythonOlder "3.8") [
          ...
          (
            if isPy36 || isPy37
            then ./3.6/profile-task.patch
            else ./3.5/profile-task.patch
          )
        ]
        ...
        ++ optionals (isPy3k && hasDistutilsCxxPatch) [
          ...
          (
            if isPy35
            then ./3.5/python-3.x-distutils-C++.patch
            else if pythonAtLeast "3.7"
        ...
        ++ [
          ...
          (
            if isPy36
            then ./3.6/fix-finding-headers-when-cross-compiling.patch
            else ./3.7/fix-finding-headers-when-cross-compiling.patch
          )
        ]
        ++ optionals (isPy36) [
