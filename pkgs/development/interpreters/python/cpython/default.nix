{
  ...
  bzip2,
  withExpat ? !withMinimal,
  expat,
  libffi,
  libuuid,
  withLibxcrypt ? !withMinimal,
  libxcrypt,
  withMpdecimal ? !withMinimal,
  mpdecimal,
  ncurses,
  withOpenssl ? !withMinimal,
  openssl,
  withSqlite ? !withMinimal,
  sqlite,
  xz,
  zlib,
  zstd,
  ...
  bluezSupport ? false,
  bluez,
  mimetypesSupport ? !withMinimal,
  mailcap,
  tzdata,
  withGdbm ? !withMinimal && !stdenv.hostPlatform.isWindows,
  gdbm,
  withReadline ? !withMinimal && !stdenv.hostPlatform.isWindows,
  readline,
  x11Support ? false,
  tcl,
  tk,
  ...
  hash,
  passthruFun,
  stripConfig ? withMinimal,
  stripIdlelib ? withMinimal,
  stripTests ? withMinimal,
  stripTkinter ? withMinimal,
  rebuildBytecode ? !withMinimal,
  stripBytecode ? true,
  includeSiteCustomize ? !withMinimal,
  ...
  # python3Minimal
  allowedReferenceNames ? if withMinimal then [ "bashNonInteractive" ] else [ ],
...
let
  ...
  libxcrypt = if stdenv.hostPlatform.isFreeBSD && withMinimal then null else inputs.libxcrypt;

  buildPackages = pkgsBuildHost;
  inherit (passthru) pythonOnBuildForHost;

  tzdataSupport = !withMinimal && tzdata != null && passthru.pythonAtLeast "3.9";
