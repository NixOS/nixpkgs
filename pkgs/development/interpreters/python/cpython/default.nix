{ lib, stdenv, fetchurl, fetchpatch, bzip2, expat, libffi, gdbm, xz
, mime-types ? null, mimetypesSupport ? true, ncurses, openssl, readline, sqlite
, tcl ? null, tk ? null, tix ? null, libX11 ? null, xorgproto ? null
, x11Support ? false, bluez ? null, bluezSupport ? false, zlib, tzdata ? null
, self, configd, autoreconfHook, autoconf-archive, python-setup-hook
, nukeReferences
...
, static ? stdenv.hostPlatform.isStatic, enableOptimizations ? false
...
let
  ...
  nativeBuildInputs = optionals (!stdenv.isDarwin) [ autoreconfHook ]
    ...
    ++ optionals (stdenv.cc.isClang && enableLTO) [ stdenv.cc.cc.libllvm.out ];
...
stdenv.mkDerivation {
  ...
  postInstall = let
    ...
    keep-references = concatMapStringsSep " " (val: "-e ${val}")
      ([ (placeholder "out") ] ++ optionals tzdataSupport [ tzdata ]);
  in ''
