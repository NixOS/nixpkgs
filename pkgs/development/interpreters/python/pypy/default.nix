...
in with passthru; stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/${libPrefix}-c/${if isPy38OrNewer then sitePackages else "lib/${libPrefix}/${sitePackages}"}
    cp ${../sitecustomize.py} $out/${libPrefix}-c/${if isPy38OrNewer then sitePackages else "lib/${libPrefix}/${sitePackages}"}/sitecustomize.py
