...
in stdenv.mkDerivation {
  ...
  outputs = [
    "out"
    # OpenCL drivers get a separate output, because they depend on a bunch of
    # Clang/libclc stuff that adds about a gigabyte of closure size
    "opencl"
  ...
  mesonFlags = [
    ...
    (lib.mesonBool "gallium-nine" false) # Direct3D9 in Wine, no users currently
                                         # This can be reenabled if someone packages wine-nine-standalone, I guess?
    (lib.mesonBool "gallium-xa" false) # "X Acceleration" Gallium frontend, only user is xf86-video-vmware, which is dead
  ...
  postFixup = ''
    ...
    rm -f $out/lib/pkgconfig/{gl,egl}.pc

    # remove libgbm, we ship it as a separate package
    rm -f $out/include/gbm.h \
          $out/lib/pkgconfig/gbm.pc

    # remove dri.pc, things should use dri-pkgconfig-stub
