{
  ...
  ,
  gplLicensing ? true
  # GPL: fdkaac,openssl,frei0r,cdio,samba,utvideo,vidstab,x265,x265,xavs,avid,zvbi,x11grab
  ,
  version3Licensing ? true
  # (L)GPL3: libvmaf,opencore-amrnb,opencore-amrwb,samba,vo-aacenc,vo-amrwbenc
  ,
  nonfreeLicensing ? false
  # NONFREE: openssl,fdkaac,blackmagic-design-desktop-video
  /*
   *  Build options
   */
  ,
  ...
  ,
  safeBitstreamReaderBuild ? true
  # Buffer boundary checking in bitreaders
  ,
  multithreadBuild ? true
  # Multithreading via pthreads/win32 threads
  ,
  networkBuild ? true
  # Network support
  ,
  pixelutilsBuild ? true
  # Pixel utils in libavutil
  ,
  enableLto ? false
  ...
  ,
  avcodecLibrary ? true
  # Build avcodec library
  ,
  avdeviceLibrary ? true
  # Build avdevice library
  ,
  avfilterLibrary ? true
  # Build avfilter library
  ,
  avformatLibrary ? true
  # Build avformat library
  ,
  avutilLibrary ? true
  # Build avutil library
  ,
  postprocLibrary ? true
  # Build postproc library
  ,
  swresampleLibrary ? true
  # Build swresample library
  ,
  swscaleLibrary ? true
  # Build swscale library
  /*
  ...
  # Man documentation pages
  ,
  podpagesDocumentation ? false
  # POD documentation pages
  ,
  ...
  ,
  libjack2 ? null
  # Jack audio (only version 2 is supported in this build)
  ,
  ladspaH ? null
  ...
  ,
  libass ? null
  # (Advanced) SubStation Alpha subtitle rendering
  ,
  libaom ? null
  # AV1 encoder
  ,
  ...
  ,
  libcaca ? null
  # Textual display (ASCII art)
  #, libcdio-paranoia ? null # Audio CD grabbing
  ,
  libdc1394 ? null,
  libraw1394 ? null
  # IIDC-1394 grabbing (ieee 1394)
  ,
  libdrm ? null
  # libdrm support
  ,
  ...
  ,
  libmfx ? null
  # Hardware acceleration vis libmfx
  ,
  libmodplug ? null
  # ModPlug support
  ,
  ...
  ,
  libvmaf ? null
  # Netflix's VMAF (Video Multi-Method Assessment Fusion)
  ,
  libvorbis ? null
  # Vorbis de/encoding, native encoder exists
  ,
  libvpx ? null
  # VP8 & VP9 de/encoding
  ,
  ...
  ,
  libxml2 ? null
  # libxml2 support, for IMF and DASH demuxers
  ,
  xz ? null
  # xz-utils
  ,
  ...
  ,
  opencore-amr ? null
  # AMR-NB de/encoder & AMR-WB decoder
  ...
  ,
  xvidcore ? null
  # Xvid encoder, native encoder exists
  ,
  zeromq4 ? null
  # Message passing
  ,
...
in
  ...
  # configure flag since 0.6
  assert avdeviceLibrary
  -> avformatLibrary
  && avcodecLibrary
  && avutilLibrary;
  # configure flag since 0.6
  assert avformatLibrary -> avcodecLibrary && avutilLibrary;
  # configure flag since 0.6
    ...
    stdenv.mkDerivation rec {
      ...
      configureFlags =
        [
          ...
          }" #mingw32 and mingw64 doesn't have a difference here, it is internally rewritten as mingw32
          ...
          then
            (
              ...
              then "--disable-pthreads --enable-w32threads"
              else
                # Use POSIX threads by default
                "--enable-pthreads --disable-w32threads"
            )
          else "--disable-pthreads --disable-w32threads")
          ...
          (enableFeature (if isLinux
          then libdc1394 != null && libraw1394 != null
          else false) "libdc1394")
          ...
          (enableFeature (if isLinux && !isAarch64
          then libmfx != null
          else false) "libmfx")
      ...
      meta = with lib; {
        ...
        license = (
          ...
          else if version3Licensing
          then licenses.gpl3
          else if gplLicensing
          then licenses.gpl2Plus
          else licenses.lgpl21Plus
        );
