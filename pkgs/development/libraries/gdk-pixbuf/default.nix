...
, gettext, python3, libxslt, docbook-xsl-nons, docbook_xml_dtd_43, gi-docgen
, glib, libtiff, libjpeg, libpng, gnome, doCheck ? false, makeWrapper, lib
, withIntrospection ? (stdenv.buildPlatform == stdenv.hostPlatform)
, gobject-introspection }:

let withGtkDoc = stdenv.buildPlatform == stdenv.hostPlatform;
in stdenv.mkDerivation rec {
  pname = "gdk-pixbuf";
  version = "2.42.6";

  outputs = [ "out" "dev" "man" ] ++ lib.optional withGtkDoc "devdoc"
  ...
  passthru = {
    ...
    tests = { installedTests = nixosTests.installed-tests.gdk-pixbuf; };
