...
let
  ...
  #   pname = "libahp-xc";
  #   buildInputs = [ libusb-compat-0_1 urjtag libftdi ];
  #   meta = with lib; {
  #     license = licenses.unfreeRedistributable;
  #     platforms = with platforms; x86_64 ++ aarch64 ++ i686 ++ arm;
  #   };
  ...
  libapogee = buildIndi3rdParty {
    pname = "libapogee";
    buildInputs = [ curl indilib libusb1 ];
  ...
  libatik = buildIndi3rdParty {
    pname = "libatik";
    buildInputs = [ stdenv.cc.cc.lib libusb1 systemd libdc1394 ];
    nativeBuildInputs = [ autoPatchelfHook ];
    meta = with lib; {
      license = licenses.unfreeRedistributable;
      platforms = with platforms; x86_64 ++ aarch64 ++ i686 ++ arm;
  ...
  libinovasdk = buildIndi3rdParty {
    pname = "libinovasdk";
    buildInputs = [ stdenv.cc.cc.lib libusb1 ];
    nativeBuildInputs = [ autoPatchelfHook ];
    meta = with lib; {
  ...
  libpigpiod = buildIndi3rdParty {
    ...
    meta = with lib; {
      ...
      platforms = with platforms; aarch64 ++ arm;
    };
  };
  ...
  libplayerone = buildIndi3rdParty {
    ...
    buildInputs = [ stdenv.cc.cc.lib libusb1 systemd ];
    nativeBuildInputs = [ autoPatchelfHook ];
    meta = with lib; {
      license = lib.licenses.unfreeRedistributable;
      platforms = with platforms; x86_64 ++ aarch64 ++ arm;
...
in
{
  ...
  indi-aok = buildIndi3rdParty {
    pname = "indi-aok";
    buildInputs = [ indilib libnova ];
  };

  indi-apogee = buildIndi3rdParty {
    pname = "indi-apogee";
    buildInputs = [ cfitsio indilib libapogee zlib ];
  ...
  indi-asi = buildIndi3rdParty {
    pname = "indi-asi";
    buildInputs = [ cfitsio indilib libasi libusb1 zlib ];
  ...
  #  inherit src version;
  #  name = "indi-asi-power";
  #  buildInputs = [ indilib libpigpiod ];
  #};

  indi-astroasis = buildIndi3rdParty {
    pname = "indi-astroasis";
    buildInputs = [ cfitsio indilib libastroasis libusb1 zlib ];
  ...
  indi-astromechfoc = buildIndi3rdParty {
    ...
    buildInputs = [ indilib zlib ];
  };

  indi-atik = buildIndi3rdParty {
    pname = "indi-atik";
    buildInputs = [ cfitsio indilib libusb1 zlib ];
  ...
  indi-avalonud = buildIndi3rdParty {
    pname = "indi-avalonud";
    buildInputs = [ indilib libnova zeromq ];
  };

  indi-beefocus = buildIndi3rdParty {
    pname = "indi-beefocus";
    buildInputs = [ gtest indilib ];
  };
  ...
  indi-celestronaux = buildIndi3rdParty {
    ...
    buildInputs = [ indilib gsl libnova zlib ];
  };
  ...
  indi-dsi = buildIndi3rdParty {
    pname = "indi-dsi";
    buildInputs = [ gtest cfitsio indilib libusb1 zlib ];
  ...
  indi-duino = buildIndi3rdParty {
    pname = "indi-duino";
    buildInputs = [ curl indilib ];
  };

  indi-eqmod = buildIndi3rdParty {
    pname = "indi-eqmod";
    buildInputs = [ indilib gsl gtest libahp-gt libnova zlib ];
    meta.platforms = libahp-gt.meta.platforms;
  };

  indi-ffmv = buildIndi3rdParty {
    pname = "indi-ffmv";
    buildInputs = [ cfitsio indilib libdc1394 zlib ];
  };
  ...
  indi-fli = buildIndi3rdParty {
    pname = "indi-fli";
    buildInputs = [ cfitsio indilib zlib ];
  ...
  indi-gige = buildIndi3rdParty {
    pname = "indi-gige";
    buildInputs = [ aravis cfitsio indilib glib zlib ];
  };

  indi-gphoto = buildIndi3rdParty {
    pname = "indi-gphoto";
    buildInputs = [ cfitsio libjpeg libraw libusb1 indilib zlib ];
    propagatedBuildInputs = [ libgphoto2 ];
  };

  indi-gpsd = buildIndi3rdParty {
    pname = "indi-gpsd";
    buildInputs = [ indilib gpsd libnova zlib ];
  };

  indi-gpsnmea = buildIndi3rdParty {
    pname = "indi-gpsnmea";
    buildInputs = [ indilib libnova zlib ];
  };
  ...
  #   name = "indi-libcamera";
  #   buildInputs = [ boost cfitsio indilib libcamera libexif libdrm libpng libusb1 libraw zlib ];
  # };

  indi-limesdr = buildIndi3rdParty {
    pname = "indi-limesdr";
    buildInputs = [ cfitsio indilib limesuite zlib ];
  };

  indi-maxdomeii = buildIndi3rdParty {
    pname = "indi-maxdomeii";
    buildInputs = [ gtest indilib libnova zlib ];
  };

  indi-mgen = buildIndi3rdParty {
    pname = "indi-mgen";
    buildInputs = [ cfitsio indilib libftdi1 zlib ];
  };
  ...
  indi-nightscape = buildIndi3rdParty {
    ...
    buildInputs = [ cfitsio indilib libusb1 libftdi1 zlib ];
  };

  indi-nut = buildIndi3rdParty {
    pname = "indi-nut";
    buildInputs = [ indilib nut zlib ];
  };
  ...
  indi-pentax = buildIndi3rdParty {
    pname = "indi-pentax";
    buildInputs = [ cfitsio indilib libraw zlib ];
    propagatedBuildInputs = [ libpktriggercord libricohcamerasdk ];
  ...
  indi-qhy = buildIndi3rdParty {
    pname = "indi-qhy";
    buildInputs = [ libqhy cfitsio indilib libnova libusb1 zlib ];
  ...
  indi-qsi = buildIndi3rdParty {
    pname = "indi-qsi";
    buildInputs = [ cfitsio indilib libqsi zlib ];
  ...
  indi-rpi-gpio = buildIndi3rdParty {
   ...
   indi-rtklib = buildIndi3rdParty {
     pname = "indi-rtklib";
     buildInputs = [ indilib libnova libpng zlib ];
   };
   ...
   indi-starbook = buildIndi3rdParty {
     pname = "indi-starbook";
     buildInputs = [ curl indilib gtest libnova ];
   };
   ...
   indi-sx = buildIndi3rdParty {
     pname = "indi-sx";
     buildInputs = [ cfitsio indilib libusb1 ];
     postPatch = ''
       substituteInPlace CMakeLists.txt --replace "/lib/udev/rules.d" "lib/udev/rules.d"
     '';
   };
   ...
   indi-toupbase = buildIndi3rdParty {
     pname = "indi-toupbase";
     buildInputs = [ cfitsio indilib zlib ];
     propagatedBuildInputs = [ libaltaircam libbressercam libmallincam libmeadecam libnncam libogmacam libomegonprocam libstarshootg libtoupcam libtscam ];
   ...
   indi-webcam = buildIndi3rdParty {
     pname = "indi-webcam";
     buildInputs = [ cfitsio indilib ffmpeg-headless zlib ];
   };
