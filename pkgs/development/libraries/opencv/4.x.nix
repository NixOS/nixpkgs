...
let
  ...
  ippicv = {
    src = fetchFromGitHub {
      ...
      rev = if effectiveStdenv.hostPlatform.isDarwin then "0cc4aa06bf2bef4b05d237c69a5a96b9cd0cb85a" else "7f55c0c26be418d494615afca15218566775c725";
      hash = if effectiveStdenv.hostPlatform.isDarwin then "sha256-/kHivOgCkY9YdcRRaVgytXal3ChE9xFfGAB0CfFO5ec=" else "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    } + "/ippicv";
    files = let name = platform: "ippicv_2021.10.0_${platform}_20230919_general.tgz"; in
      if effectiveStdenv.hostPlatform.system == "x86_64-linux" then
        { ${name "lnx_intel64"} = "d06e6d44ece88f7f17a6cd9216761186"; }
      else if effectiveStdenv.hostPlatform.system == "i686-linux" then
        { ${name "lnx_ia32"} = "85ffa2b9ed7802b93c23fa27b0097d36"; }
