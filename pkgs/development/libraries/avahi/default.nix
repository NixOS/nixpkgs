...
, nixosTests, gtk3Support ? false, gtk3 ? null, qt4 ? null, qt4Support ? false
, qt5 ? null, qt5Support ? false, withLibdnssdCompat ? false, python ? null
, withPython ? false }:
...
let
  ...
  pkg-config-helper =
    writeShellScriptBin "pkg-config" ''exec $PKG_CONFIG "$@"'';
...
in stdenv.mkDerivation rec {
  ...
  buildInputs = [ libdaemon dbus glib expat libiconv libevent ]
    ...
    ++ (lib.optional gtk3Support gtk3) ++ (lib.optional qt4Support qt4)
