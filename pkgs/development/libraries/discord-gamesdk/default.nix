...
let arch = if stdenv.hostPlatform.isAarch64 then "aarch64" else "x86_64";
in stdenv.mkDerivation rec {
  ...
  installPhase =
    ''runHook preInstall
    mkdir -p $out/lib
    install -Dm555 lib/${arch}/discord_game_sdk${stdenv.hostPlatform.extensions.sharedLibrary} $out/lib/discord_game_sdk${stdenv.hostPlatform.extensions.sharedLibrary}
    runHook postInstall'';
