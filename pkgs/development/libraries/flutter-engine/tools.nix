{ callPackage,
  ...
  python3,
  hostPlatform,
  depot_toolsCommit ? "25cf78395cd77e11b13c1bd26124e0a586c19166",
  depot_toolsHash ? "sha256-Qn0rqX2+wYpbyfwYzeaFsbsLvuGV6+S9GWrH3EqaHmU=",
  ...
  cipdHashes ? {
    ...
    "mac-amd64" = "41d05580c0014912d6c32619c720646fd136e4557c9c7d7571ecc8c0462733a1";
    "mac-arm64" = "dc672bd16d9faf277dd562f1dc00644b10c03c5d838d3cc3d3ea29925d76d931";
...
in
{
  ...
  cipd = runCommand "cipd-${cipdCommit}" {
    unwrapped = fetchurl {
  ...
  vpython = runCommand "vpython3" {}
    "mkdir -p $out/bin && install -m755 ${python3}/bin/python $out/bin/vpython3";
}
