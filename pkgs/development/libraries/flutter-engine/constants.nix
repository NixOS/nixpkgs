{ targetPlatform }:
rec {
  os = if targetPlatform.isLinux then
    "linux"
  else if targetPlatform.isDarwin then
    "mac"
  else if targetPlatform.isWindows then
    "windows"
  else throw "Unsupported OS \"${targetPlatform.parsed.kernel.name}\"";

  arch = if targetPlatform.isx86_64 then
    "amd64"
    ...
    "mips64${if targetPlatform.parsed.cpu.significantByte == "littleEndian" then "le" else ""}"
  else if targetPlatform.isPower64 then
    "ppc64${if targetPlatform.parsed.cpu.significantByte == "littleEndian" then "le" else ""}"
  else if targetPlatform.isS390x then
    "s390x"
  else throw "Unsupported CPU \"${targetPlatform.parsed.cpu.name}\"";

  alt-arch = if targetPlatform.isx86_64 then
    "x64"
  else if targetPlatform.isAarch64 then
    "arm64"
