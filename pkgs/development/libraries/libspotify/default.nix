{ lib, stdenv, fetchurl, libspotify, alsa-lib, readline, pkg-config
, apiKey ? null, unzip, gnused }:
...
in if (stdenv.hostPlatform.system != "x86_64-linux"
  ...
  stdenv.mkDerivation {
    ...
    # darwin-specific
    nativeBuildInputs =
      lib.optional (stdenv.hostPlatform.system == "x86_64-darwin") unzip;

    # linux-specific
    installFlags = lib.optional isLinux "prefix=$(out)";
    patchPhase =
      lib.optionalString isLinux "${gnused}/bin/sed -i 's/ldconfig//' Makefile";
    postInstall = lib.optionalString isLinux "mv -v share $out";
