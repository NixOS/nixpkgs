...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ python cmake  ]
    ++ optionals (!stdenv.isDarwin) ([ mesa libX11 xproto libXt ] ++ optional (qtLib != null) qtLib)
    ++ optionals stdenv.isDarwin [ qtLib xpc CoreServices DiskArbitration IOKit cf-private
  ...
  cmakeFlags = [ "-DCMAKE_C_FLAGS=-fPIC" "-DCMAKE_CXX_FLAGS=-fPIC" ]
    ++ optional withPython "-DVTK_PYTHON_VERSION=2"
    ++ optional (qtLib != null) "-DVTK_USE_QT:BOOL=ON"
    ++ optionals stdenv.isDarwin [ "-DBUILD_TESTING:BOOL=OFF"
                                   "-DCMAKE_OSX_SYSROOT="
