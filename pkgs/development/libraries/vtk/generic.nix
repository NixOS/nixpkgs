...
stdenv.mkDerivation (finalAttrs: {
  ...
  srcs =
    ...
    ++ lib.optionals finalAttrs.finalPackage.doCheck [
      (fetchurl {
        ...
        hash = sourceDataSha256;
      })
    ];

  patches = patchesToFetch;
  ...
  cmakeFlags =
    ...
    ++ lib.optionals finalAttrs.finalPackage.doCheck [
      # external viskores does not install testing libs/headers
      (lib.cmakeBool "VTK_MODULE_USE_EXTERNAL_VTK_vtkviskores" false)
  ...
  # doing check requires xvfb backend built with glx/dri support
  # see https://github.com/NixOS/nixpkgs/pull/423961
  doCheck = false;
