...
stdenv.mkDerivation (finalAttrs: {
  ...
  ++ lib.optional (stdenv.isx86_64 && (precision == "single"))
    "--enable-sse"
  ...
  ++ lib.optional enableMpi "--enable-mpi" # doc generation causes Fortran wrapper generation which hard-codes gcc
  ++ lib.optional (!withDoc) "--disable-doc";

  # fftw builds with mtune=native by default
  makeFlags = [ "CFLAGS=-mtune=generic" ];
