{
  ...
  useVendoredLibxml ? libxml2 == null, # needed for bootstrap
}:

let
  # use system libxml2, not vendored 2.9.9 version from 2019
  # checks fail during stdenv bootstrap due to missing locales,
  # but vendor libxml2 didn't build checks either
  libxml = libxml2.overrideAttrs { doCheck = false; };
in
...
stdenv.mkDerivation (finalAttrs: {
  ...
  ''
  + lib.optionalString (!useVendoredLibxml) ''
    # Remove vendored libxml 2.9.9 from 2019
    rm -rf gettext-tools/gnulib-lib/libxml
  ...
  buildInputs =
    lib.optionals (!useVendoredLibxml) [
      libxml
    ]
  ...
  // lib.optionalAttrs (!useVendoredLibxml) {
    NIX_CFLAGS_COMPILE = "-I${lib.getDev libxml}/include/libxml2";
  }
