...
let
  ...
  ifMinVer = minVer: flag:
    if reqMin minVer
    then flag
    else null;

  ifVerOlder = maxVer: flag:
    if (lib.versionOlder branch maxVer)
    then flag
    else null;
  ...
  disDarwinOrArmFix = origArg: minVer: fixArg:
    if ((isDarwin || isAarch32) && reqMin minVer)
    then fixArg
    else origArg;
  ...
  assert libaomSupport -> libaom != null;
    stdenv.mkDerivation rec {
      ...
      ++ [
        ...
        (ifMinVer "2.5" (if sdlSupport && reqMin "3.2"
        then "--enable-sdl2"
        else if sdlSupport
        then "--enable-sdl"
        else null)) # autodetected before 2.5, SDL1 support removed in 3.2 for SDL2
      ...
      buildInputs =
        ...
        ++ optionals (!isDarwin && !isAarch32) [libpulseaudio]
        # Need to be fixed on Darwin and ARM
        ++ optional ((isLinux || isFreeBSD) && !isAarch32) libva
        ++ optional ((isLinux || isFreeBSD) && !isAarch32) libdrm
        ++ optional isLinux alsa-lib
        ++ optionals isDarwin darwinFrameworks
        ++ optional vdpauSupport libvdpau
        ++ optional sdlSupport (if reqMin "3.2"
        then SDL2
        else SDL)
