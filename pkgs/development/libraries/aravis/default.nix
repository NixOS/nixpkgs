...
, audit, glib, libusb1, libxml2, wrapGAppsHook, gstreamer ? null
, gst-plugins-base ? null, gst-plugins-good ? null, gst-plugins-bad ? null
, libnotify ? null, gnome ? null, gtk3 ? null, enableUsb ? true
...
let
  gstreamerAtLeastVersion1 = lib.all
...
in assert enableGstPlugin -> lib.all (pkg: pkg != null) [
  gstreamer
  gst-plugins-base
  gst-plugins-good
  gst-plugins-bad
...
stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = [ autoreconfHook pkg-config intltool gtk-doc ]
    ++ lib.optional enableViewer wrapGAppsHook;

  buildInputs = [ glib libxml2 ] ++ lib.optional enableUsb libusb1
  ...
  configureFlags = lib.optional enableUsb "--enable-usb"
    ++ lib.optional enablePacketSocket "--enable-packet-socket"
    ++ lib.optional enableViewer "--enable-viewer"
    ++ lib.optional enableGstPlugin (if gstreamerAtLeastVersion1 then
      "--enable-gst-plugin"
    else
      "--enable-gst-0.10-plugin")
