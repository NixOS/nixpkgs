...
stdenv.mkDerivation rec {
  ...
  cmakeFlags = [
    "-DARROW_BUILD_SHARED=${lib.boolToCMakeString enableShared}"
    "-DARROW_BUILD_STATIC=${lib.boolToCMakeString (!enableShared)}"
    ...
    "-Dxsimd_SOURCE=AUTO"
    "-DARROW_DEPENDENCY_USE_SHARED=${lib.boolToCMakeString enableShared}"
    "-DARROW_COMPUTE=ON"
    "-DARROW_CSV=ON"
    "-DARROW_DATASET=ON"
    "-DARROW_ENGINE=ON"
    "-DARROW_FILESYSTEM=ON"
    "-DARROW_FLIGHT_SQL=${lib.boolToCMakeString enableFlight}"
    "-DARROW_HDFS=ON"
    "-DARROW_IPC=ON"
    "-DARROW_JEMALLOC=${lib.boolToCMakeString enableJemalloc}"
    "-DARROW_JSON=ON"
    "-DARROW_PLASMA=ON"
    # Disable Python for static mode because openblas is currently broken there.
    "-DARROW_PYTHON=${lib.boolToCMakeString enableShared}"
    "-DARROW_USE_GLOG=ON"
    ...
    "-DARROW_FLIGHT=${lib.boolToCMakeString enableFlight}"
    "-DARROW_FLIGHT_TESTING=${lib.boolToCMakeString enableFlight}"
    "-DARROW_S3=${lib.boolToCMakeString enableS3}"
    "-DARROW_GCS=${lib.boolToCMakeString enableGcs}"
  ] ++ lib.optionals (!enableShared) [
