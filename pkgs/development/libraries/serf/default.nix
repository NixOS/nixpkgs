{ stdenv, fetchurl, apr, scons, libssl, aprutil, zlib, kerberos, pkgconfig, gnused }:
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ apr scons libssl aprutil zlib ]
  ...
  buildPhase = ''
    scons PREFIX="$out" OPENSSL="${libssl}" ZLIB="${zlib}" APR="$(echo "${apr}"/bin/*-config)" \
