{
  ...
  enableShared ? !(with stdenv.hostPlatform; isStatic || libc == "msvcrt")
  # problems for now
  ,
...
with lib; let
  ...
  jobs =
    if versionOlder version "1.58"
    then "$(($NIX_BUILD_CORES<=64 ? $NIX_BUILD_CORES : 64))"
    else if versionOlder version "1.65"
    then "$(($NIX_BUILD_CORES<=256 ? $NIX_BUILD_CORES : 256))"
