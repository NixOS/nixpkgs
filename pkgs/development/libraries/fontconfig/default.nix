{ stdenv, fetchurl, fetchpatch, pkgconfig, freetype, expat, libxslt, dejavu
, substituteAll }:
...
stdenv.mkDerivation rec {
  ...
  configureFlags = [
    ...
    "--with-default-fonts=${dejavu.minimal}"
  ];
  ...
  postInstall = ''
    cd "$out/etc/fonts"
    "${libxslt.bin}/bin/xsltproc" --stringparam fontDirectories "${dejavu.minimal}" \
