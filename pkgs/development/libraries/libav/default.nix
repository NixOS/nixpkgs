{
  ...
  freetype ? null
  # it's small and almost everywhere
  ,
  SDL
  # only for avplay in $bin, adds nontrivial closure to it
  ,
  enableGPL ? true
  # ToDo: some additional default stuff may need GPL
  ,
  enableUnfree ? faacSupport,
}:
assert faacSupport -> enableUnfree; let
  inherit (lib) optional hasPrefix enableFeature;
in
  /*
   ToDo:
