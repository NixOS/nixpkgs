...
, libXxf86vm, gtk2, gtk3, xorgproto, gst_all_1, setfile, libGLSupported ?
  lib.elem stdenv.hostPlatform.system lib.platforms.mesaPlatforms
, withMesa ? libGLSupported, libGLU, libGL, compat24 ? false, compat26 ? true
, unicode ? true, withGtk2 ? true, withWebKit ? false, webkitgtk, AGL, Carbon
, Cocoa, Kernel, QTKit }:

with lib;

assert assertMsg (withGtk2 -> withWebKit == false)
  "wxGTK30: You cannot enable withWebKit when using withGtk2.";
...
stdenv.mkDerivation rec {
  ...
  ] ++ optional withGtk2 gtk2 ++ optional (!withGtk2) gtk3
