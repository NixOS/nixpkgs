...
, pkg-config, python2, ruby, ICU, OpenGL }:
...
in qtModule {
  pname = "qtwebkit";
  qtInputs = [ qtbase qtdeclarative qtlocation qtsensors ] ++ lib.optional
    (stdenv.isDarwin && lib.versionAtLeast qtbase.version "5.9.0") qtmultimedia
  ...
  nativeBuildInputs = [ bison flex gdb gperf perl pkg-config python2 ruby ]
  ...
  meta = { maintainers = with lib.maintainers; [ abbradar periklis ]; };
}
