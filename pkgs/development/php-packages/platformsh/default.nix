{ mkDerivation, fetchurl, makeWrapper, writeShellScript, lib, php, curl, jq, common-updater-scripts }:
let
  pname = "platformsh";
  version = "v3.78.0";
in
mkDerivation {
  ...
  passthru = {
      updateScript = writeShellScript "update-${pname}" ''
        ...
        update-source-version "php.packages.platformsh" "$NEW_VERSION"
      '';
    };
  ...
  meta = with lib; {
    ...
    maintainers = with maintainers; [ shyim ] ++ teams.php.members;
  };
}
