{ stdenv, lib, pkgArches, callPackage, name, version, src, mingwGccs, monos
, geckos, platforms, bison, flex, fontforge, makeWrapper, pkg-config, autoconf
, hexdump, perl, supportFlags, patches, buildScript ? null, configureFlags ? [ ]
}:
...
((lib.optionalAttrs (buildScript != null) { builder = buildScript; }) // rec {
  ...
  buildInputs = toBuildInputs pkgArches (with supportFlags;
    (pkgs:
      ...
      ++ lib.optional vkd3dSupport vkd3d ++ lib.optionals gstreamerSupport
      (with pkgs.gst_all_1; [
        gstreamer
        gst-plugins-base
        gst-plugins-good
