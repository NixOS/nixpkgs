...
, runtimeShell, hasLuaModule, python3, callPackage, makeSetupHook }:
...
let
  ...
  packDir = packages:
    let
      ...
      linkVimlPlugin = plugin: packageName: dir: ''
        ...
        ln -sf ${plugin}/${rtpPath} $out/pack/${packageName}/${dir}/${
          lib.getName plugin
        }
      '';

      link = pluginPath:
        if hasLuaModule pluginPath then
          linkLuaPlugin pluginPath
        else
          linkVimlPlugin pluginPath;
  ...
  */
  vimrcContent = { packages ? null, vam ? null, pathogen ? null, plug ? null
    , beforePlugins ? ''
    ...
    let
      ...
      */
      vamImpl = lib.optionalString (vam != null) (let
        ...
        toVimL = x:
          if builtins.isString x then
            ...
            toVimL x # a derivation
          else if builtins.isAttrs x then
            "{${
              lib.concatStringsSep ", "
              (lib.mapAttrsToList (n: v: "${toVimL n}: ${toVimL v}") x)
            }}"
          else if builtins.isList x then
            "[${lib.concatMapStringsSep ", " toVimL x}]"
          else if builtins.isInt x || builtins.isFloat x then
            builtins.toString x
      ...
      entries = [ beforePlugins vamImpl ]
        ++ lib.optional (packages != null && packages != [ ])
        (nativeImpl packages) ++ lib.optional (pathogen != null) pathogenImpl
