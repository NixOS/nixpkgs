  ...
  vimrcContent = {
    ...
    (let
      ...
      toVimL = x:
        ...
        else if builtins.isAttrs x && builtins ? out
        then toVimL x
        # a derivation
        else if builtins.isAttrs x
        then "{${lib.concatStringsSep ", " (lib.mapAttrsToList (n: v: "${toVimL n}: ${toVimL v}") x)}}"
        else if builtins.isList x
        then "[${lib.concatMapStringsSep ", " toVimL x}]"
        else if builtins.isInt x || builtins.isFloat x
...
in rec {
  ...
  inherit
    (import ./build-vim-plugin.nix {inherit lib stdenv rtpPath vim vimGenDocHook;})
    buildVimPlugin
  ...
  requiredPlugins = {
    ...
    knownPlugins =
      if givenKnownPlugins != null
      then givenKnownPlugins
      else if vam != null && vam ? knownPlugins
      then vam.knownPlugins
      else if pathogen != null && pathogen ? knownPlugins
      then pathogen.knownPlugins
      else vimPlugins;
    ...
    nonNativePlugins =
      (lib.optionals (pathogen != null) pathogenPlugins)
