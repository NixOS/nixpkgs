...
let
  defaultVersion = "3.21.0";
  defaultSrc = fetchFromGitHub {
    owner = "OP-TEE";
    repo = "optee_os";
    rev = defaultVersion;
    hash = "sha256-0Xqjlb5NIKo1WaYfKiNbYoRISkLMMGDmKIkgQ7/uW0c=";
  };
  buildOptee = lib.makeOverridable ({ version ? null
                                    , src ? null
                                    , platform
                                    , extraMakeFlags ? [ ]
                                    , extraMeta ? { }
    ...
    stdenv.mkDerivation ({
      ...
      nativeBuildInputs = [
        dtc
        (pkgsBuildBuild.python3.withPackages (p: with p; [ pyelftools cryptography ]))
      ];

      makeFlags = [ "O=out" "PLATFORM=${platform}" "CFG_USER_TA_TARGETS=${taTarget}" ] ++
