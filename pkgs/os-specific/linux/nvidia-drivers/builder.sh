...
nvidiaUserspace() {
  ...
  #
    ...
    # X-Video Motion Compensation library
      installNvidiaLib 0 304 'libXvMCNVIDIA'
    ...
    # Framebuffer capture library
      installNvidiaLib 340 0 'libnvidia-fbc'

    # NVENC video encoding library
      if test -z "${libsOnly}" ; then
        installNvidiaLib 340 0 'libnvidia-encode'
      fi
  ...
  #
    ...
    if test -z "${libsOnly}" ; then
      installNvidiaMan 340 0 'nvidia-cuda-mps-control'
      installNvidiaMan 304 304 'nvidia-cuda-proxy-control'
      ###installNvidiaMan 0 0 'nvidia-installer'
      ###installNvidiaMan 0 0 'nvidia-modprobe'
      installNvidiaMan 340 0 'nvidia-persistenced'
      if test -n "${nvidiasettingsSupport}" ; then
