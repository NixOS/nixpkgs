...
with (lib.kernel.whenHelpers version); let
  ...
  options = {
    debug = {
      # Necessary for BTF
      DEBUG_INFO = mkMerge [
        (whenOlder "5.2" (if (features.debug or false)
        then yes
        else no))
        (whenAtLeast "5.2" yes)
      ];
    ...
    sound =
      ...
      // optionalAttrs (stdenv.hostPlatform.system
      == "x86_64-linux"
    ...
    filesystem = {
      ...
      TMPFS_POSIX_ACL = yes;
      FS_ENCRYPTION =
        if (versionAtLeast version "5.1")
        then yes
        else whenAtLeast "4.9" (option module);
    ...
    criu =
      if (versionAtLeast version "4.19")
      then
        {
