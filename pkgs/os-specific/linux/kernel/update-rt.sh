...
update-if-needed() {
    ...
    khash=$(nix-prefetch-url "$mirror/v${major}.x/linux-${kversion}.tar.xz" | xargs nix-hash --type sha256 --to-sri)
    phash=$(nix-prefetch-url "$mirror/projects/rt/${branch}/older/patch-${new}.patch.xz" | xargs nix-hash --type sha256 --to-sri)
    if [ "$cur" ]; then
        msg="$nixattr: $cur -> $new"
    else
