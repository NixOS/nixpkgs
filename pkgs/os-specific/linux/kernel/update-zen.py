...
def read_file(relpath, variant):
    file_path = os.path.join(DIR, relpath)
    re_version = re.compile(rf'^\s*version = "(.+)"; # {variant}')
    re_suffix = re.compile(rf'^\s*suffix = "(.+)"; # {variant}')
    version = None
    suffix = None
...
if __name__ == "__main__":
    ...
    pattern = re.compile(rf"v(\d+\.\d+\.?\d*)-({variant}\d+)")
    ...
    if old_version != zen_version or old_suffix != zen_suffix:
        zen_hash = nix_prefetch_git(
            "https://github.com/zen-kernel/zen-kernel.git", zen_tag
        )
