...
let
  ...
  drvAttrs = config_: platform: kernelPatches: configfile:
    ...
    in (optionalAttrs isModular { outputs = [ "out" "dev" ]; }) // {
      ...
      installTargets = [
        (
          if platform ? kernelInstallTarget
          then platform.kernelInstallTarget
          else
            if platform.kernelTarget == "uImage"
            then "uinstall"
            else
              if platform.kernelTarget == "zImage" || platform.kernelTarget == "Image.gz"
              then "zinstall"
              else "install"
        )
      ];
      ...
      '') + (
        if (platform ? kernelDTB && platform.kernelDTB)
        then ''
