  ...
  stdenv.mkDerivation {
    ...
    installTargets = [
      (stdenv.hostPlatform.linux-kernel.installTarget or (
        if
          stdenv.hostPlatform.linux-kernel.target == "uImage" && stdenv.hostPlatform.linuxArch == "arm"
        then
          "uinstall"
        else if
          (
            stdenv.hostPlatform.linux-kernel.target == "zImage"
            || stdenv.hostPlatform.linux-kernel.target == "Image.gz"
            || stdenv.hostPlatform.linux-kernel.target == "vmlinuz.efi"
          )
