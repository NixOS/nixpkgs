  ...
  stdenv'.mkDerivation {
    ...
    postPatch = optionalString buildKernel ''
      ...
      substituteInPlace ./module/os/linux/zfs/zfs_ctldir.c \
        --replace-warn '"/usr/bin/env", "umount"' '"${util-linux}/bin/umount", "-n"' \
        --replace-warn '"/usr/bin/env", "mount"'  '"${util-linux}/bin/mount", "-n"'
    '' + optionalString buildUser ''
      substituteInPlace ./lib/libshare/os/linux/nfs.c --replace-warn "/usr/sbin/exportfs" "${
        # We don't *need* python support, but we set it like this to minimize closure size:
        # If it's disabled by default, no need to enable it, even if we have python enabled
        # And if it's enabled by default, only change that if we explicitly disable python to remove python from the closure
        nfs-utils.override (old: { enablePython = old.enablePython or true && enablePython; })
      }/bin/exportfs"
      substituteInPlace ./lib/libshare/smb.h        --replace-warn "/usr/bin/net"            "${samba}/bin/net"
      # Disable dynamic loading of libcurl
      substituteInPlace ./config/user-libfetch.m4   --replace-warn "curl-config --built-shared" "true"
      substituteInPlace ./config/user-systemd.m4    --replace-warn "/usr/lib/modules-load.d" "$out/etc/modules-load.d"
      substituteInPlace ./config/zfs-build.m4       --replace-warn "\$sysconfdir/init.d"     "$out/etc/init.d" \
                                                    --replace-warn "/etc/default"            "$out/etc/default"
      substituteInPlace ./contrib/initramfs/Makefile.am \
        --replace-warn "/usr/share/initramfs-tools" "$out/usr/share/initramfs-tools"
    '' + optionalString isAtLeast22Series ''
      substituteInPlace ./udev/vdev_id \
        --replace-warn "PATH=/bin:/sbin:/usr/bin:/usr/sbin" \
         "PATH=${makeBinPath [ coreutils gawk gnused gnugrep systemd ]}"
    '' + optionalString (!isAtLeast22Series) ''
      substituteInPlace ./etc/zfs/Makefile.am --replace-warn "\$(sysconfdir)/zfs" "$out/etc/zfs"
    ...
    '' + ''
      substituteInPlace ./config/zfs-build.m4 \
        --replace-warn "bashcompletiondir=/etc/bash_completion.d" \
