...
sources // {
  # select the package with the greatest version number; this is usually the
  # beta driver, but sometimes stable is more recent than beta
  latest = if (lib.versionOlder sources.beta.version sources.stable.version) then sources.stable else sources.beta;
}
