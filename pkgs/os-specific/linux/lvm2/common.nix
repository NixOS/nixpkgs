...
{ lib, stdenv, fetchpatch, fetchurl, pkg-config, util-linux, libuuid, libaio
, enableCmdlib ? false, enableDmeventd ? false, udev ? null, nixosTests }:
...
stdenv.mkDerivation rec {
  ...
  outputs = [ "out" "dev" "man" ]
    ++ lib.optionals (enableCmdlib != true) [ "bin" "lib" ];
