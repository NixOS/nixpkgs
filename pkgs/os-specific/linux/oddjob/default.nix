{ lib, pkgs, stdenv, libxml2, dbus, pam, systemd, pkg-config, autoreconfHook }:
stdenv.mkDerivation rec {
  ...
  src = pkgs.fetchurl {
     url =
       "https://pagure.io/oddjob/archive/${pname}-${version}/oddjob-${pname}-${version}.tar.gz";
     sha256 = "sha256-SUOsMH55HtEsk5rX0CXK0apDObTj738FGOaL5xZRnIM=";
  };

  nativeBuildInputs = [ autoreconfHook pkg-config ];
  buildInputs =
    [ libxml2 dbus pam systemd ];
  ...
  meta = with lib; {
    ...
    changelog = "https://pagure.io/oddjob/blob/master/f/ChangeLog";
