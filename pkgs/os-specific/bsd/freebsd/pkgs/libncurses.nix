{ lib, versionInfo, mkDerivation, libncurses-tinfo, ...}:
mkDerivation {
  path = "lib/ncurses/ncurses";
  extraPaths = ["lib/ncurses" "contrib/ncurses" "lib/Makefile.inc"];
  MK_TESTS = "no";
  preBuild = lib.optionalString (versionInfo.major == 14) ''
    make -C ../tinfo $makeFlags curses.h ncurses_dll.h ncurses_def.h
  '';
  buildInputs = lib.optionals (versionInfo.major == 14) [libncurses-tinfo];
