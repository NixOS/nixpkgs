...
mkDerivation {
  ...
  preBuild = ''
    ...
    make -C $BSDSRCDIR/bin/sh $makeFlags "CC=${pkgsBuildBuild.stdenv.cc}/bin/cc" CFLAGS="-D__unused= -D__printf0like\(a,b\)= -D__dead2=" ${lib.optionalString (!stdenv.isFreeBSD) "MK_PIE=no "}mkbuiltins mksyntax mktokens mknodes
    make -C $BSDSRCDIR/bin/csh $makeFlags "CC=${pkgsBuildBuild.stdenv.cc}/bin/cc" CFLAGS="-D__unused= -D__printf0like\(a,b\)= -D__dead2= -I$BSDSRCDIR/contrib/tcsh -I." ${lib.optionalString (!stdenv.isFreeBSD) "MK_PIE=no "}gethost
  '';
