...
appleDerivation' (if headersOnly then stdenvNoCC else stdenv) (
  ...
  in
  {
    ...
    MIGCC =
      if stdenv.isx86_64 && lib.versionAtLeast stdenv.hostPlatform.darwinSdkVersion "11" then
        "${lib.getBin pkgsBuildBuild.stdenv.cc.cc}/bin/clang"
      else
        "cc";
    ARCHS = arch;
    ARCH_CONFIGS = arch;
    ...
    appleHeaders = builtins.readFile (
      ./. + "/headers-${stdenv.hostPlatform.darwinSdkVersion}-${arch}.txt"
    );
  }
