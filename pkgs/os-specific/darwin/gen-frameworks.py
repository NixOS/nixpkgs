...
# Frameworks to skip scanning.
SKIP_FRAMEWORKS = set([
    # Swift builtins.
    "Swift", "SwiftOnoneSupport", "ObjectiveC",
    # Part of libSystem.
    "Darwin", "os", "Dispatch", "XPC",
    # Packaged separately in swiftPackages.
    "XCTest",
])
...
def scan_sdk(sdk):
    ...
    for entry in os.listdir(f"{sdk}/System/Library/Frameworks"):
        if not entry.startswith("_") and entry.endswith(".framework"):
            entry = entry.removesuffix(".framework")
            frameworks.add(entry)

    # Find Swift-only modules.
    for entry in os.listdir(f"{sdk}/usr/lib/swift"):
        if not entry.startswith("_") and entry.endswith(".swiftmodule"):
            entry = entry.removesuffix(".swiftmodule")
            if entry not in frameworks:
                frameworks.add(entry)

    frameworks = list(frameworks - SKIP_FRAMEWORKS)
    frameworks.sort()
    ...
    for framework in frameworks:
        ...
        deps = set([dep for dep in deps if dep in frameworks])
