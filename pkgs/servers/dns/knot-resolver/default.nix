...
# build+runtime deps.
, knot-dns, luajitPackages, libuv, gnutls, lmdb, systemd, libcap_ng
...
let # un-indented, over the whole file
  ...
  unwrapped = stdenv.mkDerivation rec {
    ...
    doInstallCheck = with stdenv;
      hostPlatform == buildPlatform && !(isDarwin
        && isAarch64); # avoid luarocks, as it's broken ATM on the platform
    installCheckInputs =
