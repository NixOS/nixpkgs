...
in {
  analysis-icu = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "1sz858m9963xqr5kzjlwnq7k0a146rn60v6xijyfbp8y3brg618p"
        else if version == "6.8.21"
        then "06b1pavyggzfp4wwdql0q9nm3r7i9px9cagp4yh4nhxhnk4w5fiq"
  ...
  analysis-lemmagen = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "0yjy9yhw77lmalivxnmv2rq8fk93ddxszkk73lgmpffladx2ikir"
        else if version == "6.8.21"
        then "0m80cn7vkcvk95v4pdmi6vk5ww7p01k0hj2iqb9g870vs6x2qjzv"
        else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  analysis-phonetic = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "1w5ndgffqzj5ijglmykifrk1jsgh7qwn8m7sbpiv0r7n3aayhz1x"
        else if version == "6.8.21"
        then "07w8s4a5gvr9lzjzf629y8rx3kvs6zd1vl07ksw1paghp42yb354"
        else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  discovery-ec2 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "16mv7b9nl96bcygabvjqidxp2sjk340va19mrmliblpq3mxa2sii"
        else if version == "6.8.21"
        then "1kdpbrasxwr3dn21zjrklp1s389rwa51fairygdwl8px9liwwfa5"
        else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  ingest-attachment = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "0bf8f8cybsp6s2ai3j04yay9kbhsafpgxivxjvzn2iy9qgc84ls4"
        else if version == "6.8.21"
        then "0v31yyhjcdlqnjw1f9kihh7z3c6d31whc57hqqd1dn579n4s9rlz"
        else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-s3 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "0sfa0ql3hh8jmha230dyhr51bvsvwmazyycf36ngpmxsysm8ccml"
        else if version == "6.8.21"
        then "0sfh1az30q4f34zxig2fz8wn9gk53fmmxyg5pbi1svn9761p5awq"
        else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-gcs = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.16.1"
        then "1b95hjr4qhiavm7r7k19bwk5c64r00f1g5s0ydnb6gzym9hdb5s1"
        else if version == "6.8.21"
        then "00lwj00rfdk6850gk1n86chiz2w6afpqn7jn588jdbwv41qh5mrv"
        else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  in
    esPlugin rec {
      ...
      version =
        ...
        if esVersion == "7.16.1"
        then "${esVersion}-52.5.0"
        else if esVersion == "6.8.21"
        then "${esVersion}-25.6"
