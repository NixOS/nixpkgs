...
in
{
  ...
  analysis-icu = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.5.1"
        then "0v6ynbk34g7pl9cwy8ga8bk1my18jb6pc3pqbjl8p93w38219vi6"
        else
          if version == "6.8.3"
          then "0vbaqyj0lfy3ijl1c9h92b0nh605h5mjs57bk2zhycdvbw5sx2lv"
  ...
  analysis-lemmagen = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.5.1"
        then "0js8b9a9ma797448m3sy92qxbwziix8gkcka7hf17dqrb9k29v61"
        else
          if version == "6.8.3"
          then "12bshvp01pp2lgwd0cn9l58axg8gdimsh4g9wfllxi1bdpv4cy53"
          else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  discovery-ec2 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.5.1"
        then "09wl2bpng4xx384xns960rymnm64b5zn2cb1sp25n85pd0isp4p2"
        else
          if version == "6.8.3"
          then "0pmffz761dqjpvmkl7i7xsyw1iyyspqpddxp89rjsznfc9pak5im"
          else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  ingest-attachment = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.5.1"
        then "0hhwxkjlkw1yv5sp6pdn5k1y8bdv4mnmb6nby1z4367mig6rm8v9"
        else
          if version == "6.8.3"
          then "0kfr4i2rcwinjn31xrc2piicasjanaqcgnbif9xc7lnak2nnzmll"
          else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-s3 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.5.1"
        then "1j1rgbha5lh0a02h55zqc5qn0mvvi16l2m5r8lmaswp97px056v9"
        else
          if version == "6.8.3"
          then "1mm6hj2m1db68n81rzsvlw6nisflr5ikzk5zv9nmk0z641n5vh1x"
          else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-gcs = esPlugin rec {
    ...
    src = fetchurl {
      ...
      sha256 =
        if version == "7.5.1"
        then "15g438zpxrcmsgddwmk3sccy92ha90cyq9c61kcw1q84wfi0a7jl"
        else
          if version == "6.8.3"
          then "1s2klpvnhpkrk53p64zbga3b66czi7h1a13f58kfn2cn0zfavnbk"
          else throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  search-guard =
    ...
    in esPlugin rec {
      ...
      src = fetchurl {
        ...
        sha256 =
          if version == "7.5.1-38.0.0"
          then "1a1wp9wrmz6ji2rnpk0b9jqnp86w0w0z8sb48giyc1gzcy1ra9yh"
          else
            if version == "6.8.3-25.5"
            then "0a7ys9qinc0fjyka03cx9rv0pm7wnvslk234zv5vrphkrj52s1cb"
            else throw "unsupported version ${version} for plugin ${pluginName}";
      };
      meta = with lib; {
