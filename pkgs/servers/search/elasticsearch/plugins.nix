...
in
{
  ...
  analysis-icu = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "8.19.16" then
          "sha256-e0cgSZ0xIwwERkThqNvsKnqW5X5SueweWJUPX0T7/cQ="
        else
  ...
  analysis-kuromoji = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "8.19.16" then
          "sha256-vrAufO060SQ7wYMbi9kr8LQu+4LEm8SGtZRLyqTTxXg="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = {
  ...
  analysis-phonetic = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "8.19.16" then
          "sha256-QjlJ85BPGWGnjFsHjRg2GfAwHqhUwKOfPExOr09N/g8="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = {
  ...
  analysis-smartcn = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "8.19.16" then
          "sha256-9usDzsQNmiqD7UjduCx0LvwWBRof03kFNcOpVsnLPvk="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = {
  ...
  discovery-ec2 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "8.19.16" then
          "sha256-Biw3ikxGABu/YfDZ9MJk//LCmxE3i3NI86HVyPtxs3g="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = {
...
// lib.optionalAttrs config.allowAliases {
  ...
  ingest-attachment = throw "elasticsearchPlugins.ingest-attachment was merged into Elasticsearch core in 8.0 and no longer ships separately."; # Added 2026-05-30
  repository-gcs = throw "elasticsearchPlugins.repository-gcs was merged into Elasticsearch core in 8.0 and no longer ships separately."; # Added 2026-05-30
  repository-s3 = throw "elasticsearchPlugins.repository-s3 was merged into Elasticsearch core in 8.0 and no longer ships separately."; # Added 2026-05-30
  search-guard = throw "elasticsearchPlugins.search-guard does not yet have a release compatible with Elasticsearch ${esVersion}; latest Search Guard FLX supports up to 8.19.14."; # Added 2026-05-30
}
