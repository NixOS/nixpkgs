let
  ...
  generic =
      ...
      base = stdenv'.mkDerivation (finalAttrs: {
        ...
        passthru =
          ...
          in
          {
            ...
            withJIT =
              if jitSupport then
                wrapPostgresql (extensions this.pkgs ++ [ finalAttrs.finalPackage.jit ])
              else
                null;
            withoutJIT = this;
            ...
            withPackages = f: wrapPostgresql (extensions this.pkgs ++ f this.pkgs);
      ...
      wrapPostgresql =
        ...
        let
          finalPackage = buildEnv {
            ...
            postBuild =
              let
                wrapperArgs = lib.concatMap (ext: ext.wrapperArgs or [ ]) exts;
              in
              ''
    ...
    in
    if installedExtensions == [ ] then base else wrapPostgresql installedExtensions;
