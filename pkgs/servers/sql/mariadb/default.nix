...
# Common components
, curl, libiconv, ncurses, openssl, pcre2, libkrb5, liburing, systemd
, CoreServices, cctools, perl, jemalloc, less
# Server components
, bzip2, lz4, lzo, snappy, xz, zlib, zstd, cracklib, judy, libevent, libxml2
, linux-pam, numactl, pmdk, withStorageMroonga ? true, kytea, libsodium, msgpack
, zeromq, withStorageRocks ? true }:
...
let # in mariadb # spans the whole file
  ...
  common = rec { # attributes common to both builds
    ...
    buildInputs = [ curl libiconv ncurses openssl pcre2 zlib ]
  ...
  server = stdenv.mkDerivation (common // {
    ...
    buildInputs = common.buildInputs
      ...
      ++ optional
      (stdenv.hostPlatform.isLinux && !stdenv.hostPlatform.isAarch32) numactl
      ++ optionals stdenv.hostPlatform.isLinux [ linux-pam ]
      ++ optional (stdenv.hostPlatform.isLinux && stdenv.hostPlatform.isx86_64)
      pmdk.dev ++ optional (!stdenv.hostPlatform.isDarwin) mytopEnv
    ...
    cmakeFlags = common.cmakeFlags ++ [
      ...
      [ "-DWITH_NUMA=ON" ]
      ++ optional (!withStorageMroonga) [ "-DWITHOUT_MROONGA=1" ]
      ++ optional (!withStorageRocks) [ "-DWITHOUT_ROCKSDB=1" ]
      ++ optional (!stdenv.hostPlatform.isDarwin && withStorageRocks)
      [ "-DWITH_ROCKSDB_JEMALLOC=ON" ]
      ++ optional (!stdenv.hostPlatform.isDarwin) [ "-DWITH_JEMALLOC=yes" ]
      ++ optionals stdenv.hostPlatform.isDarwin [
        "-DPLUGIN_AUTH_PAM=OFF"
        "-DWITHOUT_OQGRAPH=1"
