{ stdenv, lib, fetchFromGitHub, openssl, zlib, cmake, python2, perl, snappy, lzo
, which }:
...
let
  common = { version, sha256 }:
    stdenv.mkDerivation {
      ...
      NIX_CFLAGS_COMPILE =
        lib.optionalString (lib.versionAtLeast version "3.5") "-Wno-error";
      preConfigure = lib.optionalString (lib.versionAtLeast version "3.5")
        "patchShebangs utils";
      ...
      cmakeFlags = [
        ...
        "-DHAVE_SSE42=${
          if stdenv.hostPlatform.sse4_2Support then "ON" else "OFF"
        }"
        "-DASM_OPTIMIZATIONS=${
          if stdenv.hostPlatform.sse4_2Support then "ON" else "OFF"
        }"
