{ abiCompat ? null, lib, stdenv, makeWrapper, fetchurl, fetchpatch
, fetchFromGitLab, buildPackages, automake, autoconf, gettext, libiconv, libtool
, intltool, freetype, tradcpp, fontconfig, meson, ninja, ed, fontforge, libGL
, spice-protocol, zlib, libGLU, dbus, libunwind, libdrm, mesa, udev
, bootstrap_cmds, bison, flex, clangStdenv, autoreconfHook, mcpp, libepoxy
, openssl, pkg-config, llvm, libxslt, ApplicationServices, Carbon, Cocoa
, Xplugin }:
...
in self: super:
{
  bdftopcf = super.bdftopcf.overrideAttrs
    (attrs: { buildInputs = attrs.buildInputs ++ [ self.xorgproto ]; });
  ...
  encodings = super.encodings.overrideAttrs
  ...
  libXdmcp =
    super.libXdmcp.overrideAttrs (attrs: { outputs = [ "out" "dev" "doc" ]; });

  libXfont = super.libXfont.overrideAttrs (attrs: {
    outputs = [ "out" "dev" ];
    propagatedBuildInputs = attrs.propagatedBuildInputs or [ ]
      ++ [ freetype ]; # propagate link reqs. like bzip2
  ...
  libICE =
    super.libICE.overrideAttrs (attrs: { outputs = [ "out" "dev" "doc" ]; });
  ...
  libXcursor =
    super.libXcursor.overrideAttrs (attrs: { outputs = [ "out" "dev" ]; });

  libXdamage =
    super.libXdamage.overrideAttrs (attrs: { outputs = [ "out" "dev" ]; });

  libXft = super.libXft.overrideAttrs (attrs: {
    outputs = [ "out" "dev" ];
    propagatedBuildInputs = attrs.propagatedBuildInputs or [ ]
      ++ [ self.libXrender freetype fontconfig ];
    configureFlags = attrs.configureFlags or [ ] ++ malloc0ReturnsNullCrossFlag;
    ...
    patches = [
      ...
      (fetchpatch {
        url =
          "https://gitlab.freedesktop.org/xorg/lib/libxft/commit/723092ece088559f1af299236305911f4ee4d450.patch";
        sha256 = "1y5s6x5b7n2rqxapdx65zlcz35s7i7075qxkfnj859hx7k5ybx53";
      })
      (fetchpatch {
        url =
          "https://gitlab.freedesktop.org/xorg/lib/libxft/commit/e0fc4ce7e87ab9c4b47e5c8e693f070dfd0d2f7b.patch";
        sha256 = "1x7cbhdrprrmngyy3l3b45bz6717dzp881687h5hxa4g2bg5c764";
      })
      (fetchpatch {
        url =
          "https://gitlab.freedesktop.org/xorg/lib/libxft/commit/d385aa3e5320d18918413df0e8aef3a713a47e0b.patch";
  ...
  libXext = super.libXext.overrideAttrs (attrs: {
    outputs = [ "out" "dev" "man" "doc" ];
    propagatedBuildInputs = attrs.propagatedBuildInputs or [ ]
      ++ [ self.xorgproto self.libXau ];
    configureFlags = attrs.configureFlags or [ ] ++ malloc0ReturnsNullCrossFlag;
  });

  libXfixes =
    super.libXfixes.overrideAttrs (attrs: { outputs = [ "out" "dev" ]; });
  ...
  libXp = super.libXp.overrideAttrs (attrs: { outputs = [ "out" "dev" ]; });
  ...
  libXpresent = super.libXpresent.overrideAttrs (attrs: {
    buildInputs = with self;
      attrs.buildInputs ++ [ libXext libXfixes libXrandr ];
  });
  ...
  libpciaccess = super.libpciaccess.overrideAttrs
    (attrs: { meta = attrs.meta // { platforms = lib.platforms.linux; }; });
  ...
  x11perf = super.x11perf.overrideAttrs
    (attrs: { buildInputs = attrs.buildInputs ++ [ freetype fontconfig ]; });

  xcbutil = super.xcbutil.overrideAttrs (attrs: { outputs = [ "out" "dev" ]; });
  ...
  xf86videoomap = super.xf86videoomap.overrideAttrs
    (attrs: { NIX_CFLAGS_COMPILE = [ "-Wno-error=format-overflow" ]; });
  ...
  xf86videoati = super.xf86videoati.overrideAttrs (attrs: {
    NIX_CFLAGS_COMPILE =
      "-I${self.xorgserver.dev or self.xorgserver}/include/xorg";
  });

  xf86videovmware = super.xf86videovmware.overrideAttrs (attrs: {
    buildInputs = attrs.buildInputs
      ++ [ mesa mesa.driversdev llvm ]; # for libxatracker
    meta = attrs.meta // { platforms = [ "i686-linux" "x86_64-linux" ]; };
  });
  ...
  xvinfo = super.xvinfo.overrideAttrs
    (attrs: { buildInputs = attrs.buildInputs ++ [ self.libXext ]; });

  xkbcomp = super.xkbcomp.overrideAttrs (attrs: {
    configureFlags =
      [ "--with-xkb-config-root=${self.xkeyboardconfig}/share/X11/xkb" ];
  });
    ...
    let
      patchIn = name: layout:
        ...
        with lib; ''
          ...
          EOF
        '';
    in self.xkeyboardconfig.overrideAttrs (old: {
      buildInputs = old.buildInputs ++ [ automake ];
      postPatch = with lib; concatStrings (mapAttrsToList patchIn layouts);
    });

  xload = super.xload.overrideAttrs
    (attrs: { nativeBuildInputs = attrs.nativeBuildInputs ++ [ gettext ]; });
      ...
      let
        ...
        attrs = if (abiCompat == null || lib.hasPrefix abiCompat version) then
          ...
          postPatch = lib.optionalString stdenv.isLinux
            "sed '1i#include <malloc.h>' -i include/os.h";
          meta.platforms = lib.platforms.unix;
        } else
          throw
      ...
      in if (!isDarwin) then {
        ...
        patches = [
          ...
          (fetchpatch {
            name = "stdbool.patch";
            url =
              "https://gitlab.freedesktop.org/xorg/xserver/-/commit/454b3a826edb5fc6d0fea3a9cfd1a5e8fc568747.diff";
            sha256 = "1l9qg905jvlw3r0kx4xfw5m12pbs0782v2g3267d1m6q4m6fj1zy";
          })
        ];
        buildInputs = commonBuildInputs ++ [ libdrm mesa ];
        propagatedBuildInputs = attrs.propagatedBuildInputs or [ ]
          ++ [ libpciaccess libepoxy ] ++ commonPropagatedBuildInputs
        ...
        nativeBuildInputs = attrs.nativeBuildInputs
          ++ [ autoreconfHook self.utilmacros self.fontutil ];
        buildInputs = commonBuildInputs
          ++ [ bootstrap_cmds automake autoconf Xplugin Carbon Cocoa ];
  ...
  twm = super.twm.overrideAttrs
    (attrs: { nativeBuildInputs = attrs.nativeBuildInputs ++ [ bison flex ]; });
  ...
  xinit = (super.xinit.override {
    ...
    depsBuildBuild = [ buildPackages.stdenv.cc ];
    configureFlags = [ "--with-xserver=${self.xorgserver.out}/bin/X" ]
  ...
  xf86videointel = super.xf86videointel.overrideAttrs (attrs: {
    ...
    buildInputs = attrs.buildInputs
      ++ [ self.libXScrnSaver self.libXfixes self.libXv self.pixman ];
    nativeBuildInputs = attrs.nativeBuildInputs
      ++ [ autoreconfHook self.utilmacros ];
  ...
  xf86videoopenchrome = super.xf86videoopenchrome.overrideAttrs
    (attrs: { buildInputs = attrs.buildInputs ++ [ self.libXv ]; });
...
// (let
  ...
  mapNamesToAttrs = f: names:
    with lib;
    listToAttrs (zipListsWith nameValuePair names (map f names));

in mapNamesToAttrs (setLicense lib.licenses.unfreeRedistributable) redist
