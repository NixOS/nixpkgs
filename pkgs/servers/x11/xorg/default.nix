...
lib.makeScope newScope (self: with self; {
  ...
  xkeyboardconfig = callPackage ({ stdenv, pkg-config, fetchurl, libX11, xorgproto, python3 }: stdenv.mkDerivation rec {
    pname = "xkeyboard_config";
    version = "2.32";
    builder = ./builder.sh;
    src = fetchurl {
      url = "mirror://xorg/individual/data/xkeyboard-config/xkeyboard-config-${version}.tar.bz2";
  ...
  xorgproto = callPackage ({ stdenv, pkg-config, fetchurl, libXt }: stdenv.mkDerivation rec {
    pname = "xorgproto";
    version = "2021.4";
    builder = ./builder.sh;
    src = fetchurl {
      url = "mirror://xorg/individual/proto/${pname}-${version}.tar.bz2";
