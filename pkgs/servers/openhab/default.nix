...
let
  url = { pname, version, ext, isStable }:
    let
      base = if isStable
      then "https://bintray.com/openhab/mvn/download_file?file_path="
      else "https://openhab.jfrog.io/openhab/libs-milestone-local/";
    in
      "${base}org/openhab/distro/${pname}/${version}/${pname}-${version}.${ext}";

  addon = { pname, version, sha256, isStable }: stdenv.mkDerivation rec {
    inherit pname version;

    src = fetchurl {
      url = url { inherit pname version isStable; ext = "kar"; };
      inherit sha256;
    };
  ...
  generic = { version, sha256, isStable }: stdenv.mkDerivation rec {
    pname = "openhab";
    inherit version;

    src = fetchurl {
      url = url { inherit pname version isStable; ext = "zip"; };
      inherit sha256;
    };
  ...
  openhab = {
    version = "2.5.3";
    sha256 = "1hp8lcsc3hf3xb20yy259jvq0qd1cdx5bv9b8mcs550czz3b78lg";
    isStable = true;
  };

  openhab-v1-addons = {
    pname = "openhab-addons-legacy";
    version = openhab.version;
    sha256 = "0pmil6bq790xymp621khz1xzc75prq27vj34zrgf3zxz6aal4v6w";
    isStable = openhab.isStable;
  };

  openhab-v2-addons = {
    pname = "openhab-addons";
    version = openhab.version;
    sha256 = "0kxhqg4cknsj2ylgirlfg4mkf6jyyj455xdbbs8r86zkrfjc8h98";
    isStable = openhab.isStable;
  };
  ...
  openhab-milestone = {
    ...
    isStable = openhab.stable && openhab.version == openhab-milestone.version;
  };

  openhab-milestone-v1-addons = {
    pname = "openhab-addons-legacy";
    sha256 = "0vywzm9iq5d7rsjpp63jc2fin52f2g3jmmibb3rxg039hr8fi0a6";
    inherit (openhab-milestone) version isStable;
  };
