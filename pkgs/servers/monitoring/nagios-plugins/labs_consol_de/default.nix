    ...
    stdenv.mkDerivation {
      ...
      prePatch = with lib; ''
        ...
        substituteInPlace plugins-scripts/Makefile.am \
          --replace-fail /bin/cat  ${getBin coreutils}/bin/cat \
          --replace-fail /bin/echo ${getBin coreutils}/bin/echo \
          --replace-fail /bin/grep ${getBin gnugrep}/bin/grep \
          --replace-fail /bin/sed  ${getBin gnused}/bin/sed
      '';
