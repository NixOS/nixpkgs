...
for version in "${versions[@]}"; do
  ...
  for repo in "${repos[@]}"; do
    ...
    jq -r ".\"$version\".hashes.\"$repo\".src=\"$(nix-hash --type sha256 --sri "$tmp/$repo")\"" versions.json > "$tmp/versions.json"
    mv "$tmp/versions.json" versions.json

    if [ -f "$tmp/$repo/package-lock.json" ]; then
      jq -r ".\"$version\".hashes.\"$repo\".npmDeps=\"$(prefetch-npm-deps "$tmp/$repo/package-lock.json")\"" versions.json > "$tmp/versions.json"
      mv "$tmp/versions.json" versions.json
    fi
    ...
    if [ -f "$tmp/$repo/composer.lock" ]; then
      ...
      jq -r ".\"$version\".hashes.\"$repo\".vendor=\"$hash\"" versions.json > "$tmp/versions.json"
      mv "$tmp/versions.json" versions.json
    fi
  done
done
