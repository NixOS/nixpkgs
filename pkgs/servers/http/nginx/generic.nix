...
, nixosTests, substituteAll, gd, geoip, perl, withDebug ? false
, withStream ? true, withMail ? false, withPerl ? true, modules ? [ ], ... }:

{ pname ? "nginx", version, nginxVersion ? version
, src ? null # defaults to upstream nginx ${version}
, sha256 ? null # when not specifying src
, configureFlags ? [ ], buildInputs ? [ ], fixPatch ? p: p, preConfigure ? ""
, postInstall ? null, meta ? null, passthru ? { tests = { }; } }:
...
in stdenv.mkDerivation {
  ...
  ] ++ optionals withDebug [ "--with-debug" ] ++ optionals withStream [
    "--with-stream"
  ...
  postInstall = if postInstall != null then
    postInstall
  else ''
    mv $out/sbin $out/bin
  '';
  ...
  passthru = {
    ...
    tests = {
      inherit (nixosTests)
        nginx nginx-auth nginx-etag nginx-pubhtml nginx-sandbox nginx-sso;
