  ...
  buildPackage =
    ...
    stdenv.mkDerivation ({
      ...
      installPhase = ''
        ...
        # Allow storage outside of read-only nix storage
        # TODO: Apply these changes using a patch!
        sed -i "$ d " bootstrap/app.php
        echo '$app->useStoragePath( env( "DATA_DIR", base_path() ) . "/storage" );' >> bootstrap/app.php
        echo '$app->useEnvironmentPath(env( "DATA_DIR", base_path() ) );' >> bootstrap/app.php
        echo 'return $app;' >> bootstrap/app.php
