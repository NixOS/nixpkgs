...
mkDerivation rec {
  ...
  postPatch = ''
    for f in *.pro; do
      substituteInPlace $f \
        --replace-fail '$$'{PREFIX}/include/libircclient ${libircclient.dev}/include/libircclient \
        --replace-fail 'LIB_DIRS =' 'LIB_DIRS = ${boost.out}/lib' \
        --replace-fail /opt/gsasl ${gsasl}
    done
  '';
