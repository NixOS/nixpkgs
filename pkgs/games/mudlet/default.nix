{ fetchurl, pkgs, stdenv, makeWrapper, qt5, yajl, libzip, hunspell, lua5_1, boost, lua5_filesystem }:
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ pkgs.unzip qt5 lua5_1 hunspell libzip yajl boost makeWrapper lua5_filesystem ];
  ...
  installPhase = ''
    ...
    makeWrapper $out/mudlet $out/bin/mudlet \
      --set LUA_CPATH "${lua5_filesystem}/lib/lua/5.2/?.so"
  '';
