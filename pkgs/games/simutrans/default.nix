{ lib, stdenv, fetchurl, fetchsvn, fetchzip, pkg-config, zlib, zstd, libpng, bzip2
, SDL2, SDL2_mixer, fontconfig, miniupnpc, symlinkJoin, makeWrapper, runCommand, cabextract }:
let
  fetchcab = args:
    let src = fetchurl args;
    in runCommand "source-cab" { nativeBuildInputs = [ cabextract ]; } ''
      cabextract ${src} -d $out
    '';
  ...
  pakSpec = lib.mapAttrs (pakName:
    {
      ...
      url ? "mirror://sourceforge/simutrans/${builtins.concatStringsSep "/" (
        map lib.escapeURL ([ basePath ] ++ srcPath)
      )}",
    ...
    {
      "pak32.comic" = {
        srcPath = [ "pak32.comic for 102-0" "pak32.comic_102-0.zip" ];
        prefix = "/pak32";
      ...
      pak64 = {
        srcPath = [ "124-0" "simupak64-124-0.zip" ];
        prefix = "/pak";
        hash = "sha256-jnEjsHjABxq/cyNOTAgES7SxDKjLfEeNhXzh5tchDsU=";
      };
      "pak64.classic" = {
        basePath = "pakHAJO";
        srcPath = [ "pakHAJO_102-2-2" "pakHAJO_0-102-2-2.zip" ];
        prefix = "/pakHAJO";
      ...
      "pak64.japan" = {
        srcPath = [ "123-0" "simupak64.japan-123-0.zip" ];
        prefix = "/pak.japan";
      ...
      "pak96.comic" = {
        srcPath = [ "pak96.comic for 111-3" "pak96.comic-0.4.10-plus.zip" ];
        prefix = "/pak96.comic";
      ...
      pak128 = {
        srcPath = [ "pak128 for ST 124up (2.9)" "simupak128-2.9-for124.zip" ];
        prefix = "/pak128";
        hash = "sha256-qjgnlIz7aQ73W7gFJpqBiE8BXabSrVgHwWVQotGwseE=";
      };
      "pak128.britain" = {
        srcPath = [ "pak128.Britain for 120-3" "pak128.Britain.1.18-120-3.zip" ];
      ...
      "pak128.cs" = {
        ...
        srcPath = [ "nightly builds" "pak128.CS-r2096.zip" ];
        hash = "sha256-vQ8HZqwICB3CUGQW8NwEWGVA3anUbCOdj3BEV3lLLpk=";
      };
      "pak128.german" = {
        basePath = "PAK128.german";
        srcPath = [ "PAK128.german_2.2_for_ST_124.0" "PAK128.german_2.2_for_ST_124.0.zip" ];
  ...
  mkPak = { pakName, prefix, src, inspect }:
  ...
  binaries = stdenv.mkDerivation {
    ...
    buildInputs = [ zlib zstd libpng bzip2 SDL2 SDL2_mixer fontconfig miniupnpc ];
