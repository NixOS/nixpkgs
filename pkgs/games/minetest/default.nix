{ lib, stdenv, fetchFromGitHub, cmake, irrlicht, libpng, bzip2, curl, libogg
, jsoncpp, libjpeg, libXxf86vm, libGLU, libGL, openal, libvorbis, sqlite, luajit
...
let
  ...
  generic = { version, rev ? version, sha256, dataRev ? version, dataSha256
    ...
    in stdenv.mkDerivation {
      ...
      ] ++ optionals buildClient [ "-DOpenGL_GL_PREFERENCE=GLVND" ];
      ...
      buildInputs = [
        ...
        ++ optionals buildClient [
          ...
          xorg.libX11
          libXxf86vm
        ] ++ optionals buildServer [ leveldb postgresql hiredis ];
...
in {
  minetestclient_4 = generic (v4 // {
    buildClient = true;
    buildServer = false;
  });
  minetestserver_4 = generic (v4 // {
    buildClient = false;
    buildServer = true;
  });

  minetestclient_5 = generic (v5 // {
    buildClient = true;
    buildServer = false;
  });
  minetestserver_5 = generic (v5 // {
    buildClient = false;
    buildServer = true;
  });
}
