...
let
  ...
  xonotic-unwrapped = stdenv.mkDerivation rec {
    ...
    buildPhase = (lib.optionalString withDedicated ''
      make -j $NIX_BUILD_CORES -l $NIX_MAX_CORES sv-${target}
    '' + lib.optionalString withGLX ''
      make -j $NIX_BUILD_CORES -l $NIX_MAX_CORES cl-${target}
    '' + lib.optionalString withSDL ''
      make -j $NIX_BUILD_CORES -l $NIX_MAX_CORES sdl-${target}
    '') + ''
      pushd ../d0_blind_id
      make -j $NIX_BUILD_CORES -l $NIX_MAX_CORES
      popd
    '';
