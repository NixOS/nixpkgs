...
def install_binaries(arch, binarylist):
    ...
    for stanza in deb822.Packages.iter_paragraphs(urllib.urlopen(packages_url)):
        ...
        if p in installset:
            ...
            yield download_file(p, os.path.splitext(os.path.basename(stanza['Filename']))[0], file_url)
            installset.remove(p)
...
resJSON = OrderedDict()
resJSON['__WARNING'] = 'This file is autogenerated! Do not edit it yourself, use update-runtime.py for regeneration.'
resJSON['date'] = date.today().isoformat()
...
print ("Creating runtime-generated.json")
...
for arch in arches:
    resJSON[arch] = list(install_binaries(arch, binary_pkgs))

    if args.symbols:
        resJSON[arch] += list(install_symbols(arch, binary_pkgs))

with open('runtime-generated.json', 'w') as out:
    json.dump(resJSON, out, indent=2)
