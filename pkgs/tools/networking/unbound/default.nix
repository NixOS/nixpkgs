...
, hiredis, dns-root-data, pkg-config, makeWrapper, symlinkJoin, bison
, nixosTests
#
...
stdenv.mkDerivation rec {
  ...
  configureFlags = [
    ...
    "--enable-pie"
    "--enable-relro-now"
  ] ++ lib.optional stdenv.hostPlatform.isStatic [ "--disable-flto" ]
  ...
  postConfigure = let inherit (builtins) storeDir;
  in ''
