...
let
  ...
  yojson =
    ocamlPackages.yojson.overrideAttrs (o: {
      src = fetchFromGitHub {
        owner = "gfngfn";
        repo = "yojson";
        rev = "v1.4.1+satysfi";
...
in
  stdenv.mkDerivation rec {
    ...
    buildInputs = [ camlpdf otfm yojson ] ++ (with ocamlPackages; [
      ocaml findlib menhir
      batteries camlimages core_kernel cppo ppx_deriving uutf omd
    ]);
    ...
    installPhase = ''
      ...
      for libpath in unidata fonts hash hyph packages md; do
        mkdir -p $out/share/satysfi/dist/$libpath
        cp lib-satysfi/dist/$libpath/* $out/share/satysfi/dist/$libpath/
      done
