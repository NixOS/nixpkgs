...
lib.makeOverridable ({ extraName ? "env"
, extraVersion ? ""
, extraOutputsToInstall ? [ "man" ]
, packages ? ps: [ ps.scheme-infraonly ]
, meta ? rec {
    description = "TeX Live environment";
    platforms = lib.platforms.all;
  }
# texlive.combine compatibility: preserve package order
# to avoid rebuilds (for instance, do not resolve dependencies)
, __calledFromCombine ? false
}:

let
  withMan = lib.elem "man" extraOutputsToInstall;
  withDocs = lib.elem "texdoc" extraOutputsToInstall;
  withSource = lib.elem "texsource" extraOutputsToInstall;

  pkgList = rec {
    # resolve dependencies of the packages that affect the runtime
    all =
      let
        list = packages self.__pkgs;
        runtime = builtins.partition (p: p.outputSpecified or false -> builtins.elem p.tlOutputName [ "out" "tex" "tlpkg" ]) list;
        keySet = p: { key = p.outPath or ((p.name or "${p.pname}-${p.version}") + "-" + p.tlOutputName or p.outputName or ""); inherit p; };
      in
      if __calledFromCombine then packages null else
        with lib; lib.catAttrs "p" (genericClosure {
          startSet = map keySet runtime.right;
          operator = s: map keySet (s.p.tlDeps or [ ]);
        }) ++ runtime.wrong;
    ...
    bin = lib.catAttrs "out" specified.wrong ++ groups.out or [ ];
    tlpkg = lib.catAttrs "tlpkg" specified.wrong ++ groups.tlpkg or [ ];

    # same build as texlive.combine: put all tex* outputs under the same tree
    # in the order tex texdoc texsource
    # if called from texlive.combine, preserve ordering inside all
    nonbin = if __calledFromCombine
      then lib.filter (p: lib.elem p.tlOutputName [ "tex" "texdoc" "texsource" ]) all
      else lib.concatMap (p:
        lib.optional (p ? tex) p.tex
        ++ lib.optional ((withDocs || (withMan && p ? man)) && p ? texdoc) p.texdoc
        ++ lib.optional (withSource && p ? texsource) p.texsource) specified.wrong
        ++ groups.tex or [ ] ++ groups.texdoc or [ ] ++ groups.texsource or [ ];
  ...
  name = "texlive-${extraName}-${self.bin.texliveYear}${extraVersion}";
  ...
  texmfdist = (buildEnv {
    ...
    # mktexlsr
    nativeBuildInputs = [ self.__pkgs."texlive.infra" ];
...
in (buildEnv {
  ...
  nativeBuildInputs = [
    makeWrapper
    libfaketime
    self.__pkgs."texlive.infra" # mktexlsr
    self.__pkgs.texlive-scripts # fmtutil, updmap
    self.__pkgs.texlive-scripts-extra # texlinks
    perl
  ];
