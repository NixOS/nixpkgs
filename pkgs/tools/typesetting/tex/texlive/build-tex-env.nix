...
let
  ...
  pkgList = rec {
    ...
    hyphenPatterns = lib.filter (p: (p.hasHyphens or false) && (p.outputSpecified or false -> p.tlOutputName or p.outputName == "tex")) all;
    formatPkgs = lib.filter (p: p ? formats && (p.outputSpecified or false -> p.tlOutputName or p.outputName == "tex")) all;
    formats = map (p: self { requiredTeXPackages = ps: [ ps.scheme-infraonly p ] ++ hyphenPatterns; __formatsOf = p.pname; }) formatPkgs;
  };
  ...
  name = if __combine then "texlive-${__extraName}-${bin.texliveYear}${__extraVersion}" # texlive.combine: old name name
    else "texlive-${bin.texliveYear}-" + (if __formatsOf != null then "${__formatsOf}-fmt" else "env");
...
(buildEnv {
  ...
  lib.optionalString (__formatsOf == null) ''
  ...
  (let
    ...
    formatPNames = if __formatsOf == null then map (p: p.pname) formats else [ __formatsOf ];
  ...
  lib.optionalString (__combine || __formatsOf != null) ''
  ...
  lib.optionalString (__formatsOf != null) ''
    ...
    find "$TEXMFSYSVAR" -type f -not -name '*.mem' -not -name '*.base' -not -name '*.fmt' -delete
    find "$TEXMFSYSVAR" -empty -delete
    rm -fr "$TEXMFSYSCONFIG" "$out"/bin
  '' +
    # if not generating formats only, check that there are no missing formats
  lib.optionalString (! __combine && __formatsOf == null) ''
    if fmtutil --sys --missing --dry-run | grep running ; then
      echo "error: some formats were not found" >&2
      exit 1
    fi
  '' +
    # if generating formats only, stop here, otherwise continue
  lib.optionalString (__formatsOf == null) (
    # generate font maps
  ''
  ...
  // lib.optionalAttrs (__formatsOf == null && ! __combine) ({
