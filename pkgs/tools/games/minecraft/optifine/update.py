...
def nix_prefetch_sha256(name):
    return subprocess.run(
        [
            ...
            "--type",
            "sha256",
            "https://optifine.net/download?f=" + name,
        ],
        capture_output=True,
        text=True,
    ).stdout.strip()
...
href = tree.xpath(
    '//tr[@class="downloadLine downloadLineMain"]/td[@class="colMirror"]/a/@href'
)
...
for i, r in enumerate(result):
    ...
    catalogue[index] = {"version": version, "sha256": nix_prefetch_sha256(r.group(0))}
...
for key, value in sorted(
    {**prev, **catalogue}.items(),
    key=lambda item: [
        int(s) if s.isdigit() else s for s in re.split(r"(\d+)", item[0])
    ],
):
    if key not in prev:
        ...
        changes[0]["commitMessage"] += "optifinePackages.{}: {} -> {}\n".format(
            key, prev[key]["version"], value["version"]
        )
