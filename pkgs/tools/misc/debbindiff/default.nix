...
, acl, binutils, bzip2, cdrkit, cpio, diffutils, e2fsprogs, file, gettext, gnupg, gzip, pdftk, poppler_utils, rpm, squashfsTools, unzip, vim, xz
}:
...
pythonPackages.buildPythonPackage rec {
  ...
  patchPhase = ''
    sed -i setup.py -e "s@'magic'@'Magic-file-extensions'@"   # Different pkg name in debian
    sed -i setup.py -e "/'rpm',/d"                            # Upstream doesn't provide a PKG-INFO file
  '';
