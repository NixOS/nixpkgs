{
  ...
  libsecret,
  enabledFlavors ?
    ["curses" "tty" "gtk2" "emacs"]
    ++ lib.optionals stdenv.isLinux ["gnome3"]
    ++ lib.optionals (!stdenv.isDarwin) ["qt"],
}:
with lib;
assert isList enabledFlavors && enabledFlavors != []; let
...
in
  pinentryMkDerivation rec {
    ...
    buildInputs =
      [libgpg-error libassuan libsecret]
    ...
    postInstall =
      concatStrings (flip map enabledFlavors (f: let
