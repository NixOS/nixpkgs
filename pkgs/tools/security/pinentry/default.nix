...
, gcr, libcap, libsecret, enabledFlavors ? [ "curses" "tty" "gtk2" "emacs" ]
...
in pinentryMkDerivation rec {
  ...
  patches = [ ./autoconf-ar.patch ] ++ optionals (elem "gtk2" enabledFlavors) [
    (fetchpatch {
      url =
  ...
  postInstall = concatStrings (flip map enabledFlavors (f:
    ...
    '')) + ''
      ln -sf ${placeholder (head enabledFlavors)}/bin/pinentry-${
        flavorInfo.${head enabledFlavors}.bin
      } $out/bin/pinentry
    '';
