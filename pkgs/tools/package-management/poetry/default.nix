...
let
  ...
  unwrapped = python.pkgs.buildPythonApplication rec {
    pname = "poetry";
    version = "1.3.2";
    format = "pyproject";
  ...
  wrapPoetry = { plugins ? [] }: let
    wrapNoPlugins = ''
      ln -s ${unwrapped}/bin/poetry $out/bin/poetry
    '';
    ...
    wrap = if plugins == [] then wrapNoPlugins else wrapWithPlugins;

    wrapper = runCommand "poetry-wrapped" {
      pname = "poetry-wrapped";
      ...
      passthru = {
        ...
        withPlugins = selector: let
          ...
          chosenPlugins = selector pluginsSet;
        in wrapPoetry { plugins = chosenPlugins; };
      };
    ...
    # Only include the binary and the completions in the wrapper,
    # so we don't accidentally drag in unrelated Python dependencies.
    #
    # Also, generate completions with the wrapped poetry so plugin commands are included.
    ''
      mkdir -p $out/bin
      ${wrap}
