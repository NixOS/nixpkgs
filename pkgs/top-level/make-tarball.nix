...
pkgs.releaseTools.sourceTarball {
  ...
  checkPhase = ''
    ...
    NIX_STATE_DIR=$TMPDIR NIX_PATH= nix-instantiate --eval --raw --expr "(import ${./packages-info.nix} { lib = import $src/lib; }) (import $src { config = import ${./packages-config.nix};})" | sed "s|$src/||g" | jq -c > packages.json
