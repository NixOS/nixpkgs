...
, # Fallback: The contents of the configuration file found at $NIXPKGS_CONFIG or
  ...
  in
    if configFile != "" && pathExists configFile then import configFile
      ...
      if homeDir != "" && pathExists configFile2
      then import configFile2
      else
        if homeDir != "" && pathExists configFile3
...
, # Overlays are used to extend Nixpkgs collection with additional
  ...
  in
    if pathOverlays != "" && pathExists pathOverlays then overlays pathOverlays
      ...
      if pathExists homeOverlaysFile && pathExists homeOverlaysDir
      then
        throw ''
...
import ./. (builtins.removeAttrs args [ "system" "platform" ] // {
  ...
  localSystem =
    if builtins.isString localSystem
    then localSystem
    else (
      if args ? localSystem
      then { }
