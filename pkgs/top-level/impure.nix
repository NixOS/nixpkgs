...
let
  ...
  try = x: def:
    let res = tryEval x; in if res.success then res.value else def;
    ...
    overlays = path:
      ...
      # it's a directory, so the set of overlays from the directory, ordered lexicographically
        ...
        map
          (n: import (path + ("/" + n)))
          (builtins.filter
            (n: builtins.match ".*\\.nix" n != null || pathExists (path + ("/" + n + "/default.nix")))
            (attrNames content)
          )
      else
...
import ./. (builtins.removeAttrs args [ "system" "platform" ] // {
  ...
  localSystem =
    if builtins.isString localSystem then localSystem
    else (
      if args ? localSystem then { }
      else { system = builtins.currentSystem; }
    ) // localSystem;
})
