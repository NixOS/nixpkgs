{ lib, buildPackages, pkgs, targetPackages, pkgsBuildBuild, pkgsBuildHost
, pkgsBuildTarget, pkgsHostHost, pkgsTargetTarget, stdenv, splicePackages
, newScope, preLibcCrossHeaders }:
...
in lib.makeScopeWithSplicing splicePackages newScope otherSplices (_: { })
(spliced: spliced.apple_sdk.frameworks) (self:
  let
    ...
    selectAttrs = attrs: names:
      lib.listToAttrs (lib.concatMap (n:
        if attrs ? "${n}" then [ (lib.nameValuePair n attrs."${n}") ] else [ ])
        names);
  ...
  in impure-cmds // appleSourcePackages // chooseLibs // {
    ...
    checkReexportsHook =
      pkgs.makeSetupHook { deps = [ pkgs.darwin.print-reexports ]; }
    ...
    autoSignDarwinBinariesHook =
      pkgs.makeSetupHook { deps = [ self.signingUtils ]; }
    ...
    inherit (pkgs.callPackages ../os-specific/darwin/xcode { })
      xcode_8_1 xcode_8_2 xcode_9_1 xcode_9_2 xcode_9_4 xcode_9_4_1 xcode_10_2
      xcode_10_2_1 xcode_10_3 xcode_11 xcode;
