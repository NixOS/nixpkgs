...
in
  makeScopeWithSplicing
  (generateSplicesForMkScope "darwin")
  (_: {})
  (spliced: spliced.apple_sdk.frameworks)
  (self: let
    # NOTE: Must use pkgs.callPackage to avoid infinite recursion.
    ...
    # macOS 10.12 SDK
    appleSDKs =
      (pkgs.callPackage ../os-specific/darwin/apple-sdks {})
      // {
        # Special case the old, pre-11.x SDKs, which are not in the same format as
        # the newer ones.
        apple_sdk_10_12 = pkgs.callPackage ../os-specific/darwin/apple-sdk-10-12 {
          inherit (buildPackages.darwin) print-reexports;
          inherit (self) darwin-stubs;
        };
      }
      // {
        # Pick the default SDK
        apple_sdk =
          if stdenv.hostPlatform.isAarch64
          then appleSDKs.apple_sdk_11_1
          else appleSDKs.apple_sdk_10_12;
      };

    # All supported Xcode releases
    xcodeReleases = pkgs.callPackages ../os-specific/darwin/xcode {};
    ...
    chooseLibs =
      ...
      // {
        inherit
          (
            if useAppleSDKLibs
            then appleSDKs.apple_sdk.frameworks
            else appleSourcePackages
          )
          Security
          ;
  ...
  in
    ...
    // {
      ...
      maloader =
        callPackage ../os-specific/darwin/maloader {
        };
      ...
      # TODO(@connorbaker): Currently does not compile.
      moltenvk = pkgs.darwin.apple_sdk_13_3.callPackage ../os-specific/darwin/moltenvk {
        inherit (pkgs.darwin) cctools sigtool;
        inherit (pkgs.darwin.apple_sdk_13_3) frameworks MacOSX-SDK;
      };

      opencflite = callPackage ../os-specific/darwin/opencflite {};

      openwith = pkgs.darwin.apple_sdk_13_3.callPackage ../os-specific/darwin/openwith {
        inherit (pkgs.darwin.apple_sdk_13_3.frameworks) AppKit Foundation UniformTypeIdentifiers;
      };
      ...
      # TODO: make swift-corefoundation build with apple_sdk_11_1.Libsystem
      CF =
        if useAppleSDKLibs
        then
          ...
          lib.overrideDerivation appleSDKs.apple_sdk.frameworks.CoreFoundation (drv: {
            setupHook = null;
          })
