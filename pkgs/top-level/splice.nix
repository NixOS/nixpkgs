...

{
  ...
  /**
    ...
    In another file, call `say-hello.nix` it without worrying about passing all
    the arguments found in `pkgs`. `name` must be passed though as it is not in
    the `pkgs` attribute set.
