{ stdenv, lib, rustPlatform, fetchgit, runCommand, symlinkJoin, pkg-config
, minijail, dtc, libusb1, libcap, linux }:
...
let
  ...
  arch = with stdenv.hostPlatform;
    if isAarch64 then
      ...
      throw "no seccomp policy files available for host platform";
...
in rustPlatform.buildRustPackage rec {
  ...
  patches = [ ./default-seccomp-policy-dir.diff ];
