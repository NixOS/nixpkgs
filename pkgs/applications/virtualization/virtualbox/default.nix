...
symlinkJoin {
  ...
  # Wrap manually because we wrap just a small number of executables.
  nativeBuildInputs = optional (!headless) wrapQtAppsHook;
  dontWrapQtApps = true;
  ...
  postBuild = ''
    ...
    wrapProgram "$libexec/VBoxExtPackHelperApp" \
      --argv0 "$(readlink -f "$libexec/VBoxExtPackHelperApp")"
