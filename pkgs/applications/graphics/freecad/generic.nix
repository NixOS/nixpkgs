{ pname, version, owner, rev, hash, patches }:
...
let
  inherit (python3Packages)
    boost gitpython matplotlib pivy ply pycollada pyside2 pyside2-tools python
...
stdenv.mkDerivation (finalAttrs: {
  ...
  passthru.tests = {
    # Check that things such as argument parsing still work correctly with the
    # PYTHONPATH patch. Previously the patch changed the
    # `PyConfig_InitIsolatedConfig` to `PyConfig_InitPythonConfig`, which caused
    # the built-in interpreter to attempt (and fail) to doubly parse argv. This
    # should catch if that ever regresses and also ensures that PYTHONPATH is
    # still respected enough for the FreeCAD console to successfully run and
    # check that it was included in `sys.path`.
    python-path = runCommand "freecad-test-console"
      ...
      HOME="$(mktemp -d)" \
      PYTHONPATH="$(pwd)/test" \
      FreeCADCmd --log-file $out \
      -c "if not '$(pwd)/test' in sys.path: sys.exit(1)" </dev/null
    '';
  };
