{ stdenv, lib, fetchurl, runtimeShell, buildPackages
...
stdenv.mkDerivation {
  ...
  # If cross compilation: Build "sane-desc" tool for the host-architecture.
  preConfigure = lib.optional (stdenv.hostPlatform != stdenv.buildPlatform) ''
    export CC=${buildPackages.stdenv.cc}/bin/${buildPackages.stdenv.cc.targetPrefix}cc
    ./configure $configureFlags --host=${stdenv.buildPlatform.config}
    unset CC
    make -C lib liblib.la
    make -C sanei libsanei.la
    make -C tools sane-desc
    cp ./tools/sane-desc ./tools/sane-desc.${stdenv.buildPlatform.system}
    make distclean
  '';
  ...
  buildFlags = lib.optionals (stdenv.hostPlatform != stdenv.buildPlatform) [ "CFLAGS=-DHAVE_MMAP=1" ];
  ...
  in ''
    ...
    cp ./tools/sane-desc ./tools/sane-desc.${stdenv.targetPlatform.system}
    ./tools/sane-desc.${stdenv.buildPlatform.system} -m udev+hwdb -s doc/descriptions:doc/descriptions-external > $out/etc/udev/rules.d/49-libsane.rules
    ./tools/sane-desc.${stdenv.buildPlatform.system} -m udev+hwdb -s doc/descriptions -m hwdb > $out/etc/udev/hwdb.d/20-sane.hwdb
    ...
    if test -f $out/etc/sane.d/net.conf; then
