...
# ~/.nix-profile/glimpse-version-plugins to your plugin list you can find at
...
let
  ...
  inherit (glimpse) targetPluginDir targetScriptDir;
  ...
  in
    stdenv.mkDerivation (
      ...
      // a
      // {
        name = "glimpse-plugin-${name}";
        buildInputs = [ glimpse glimpse.gtk glib ] ++ (a.buildInputs or []);
...
stdenv.lib.makeScope pkgs.newScope (
  self: with self; {
    gap = pluginDerivation {
      ...
      */
      ...
      NIX_LDFLAGS = [ "-lm" ];
      patchPhase = ''
        sed -e 's,^\(GIMP_PLUGIN_DIR=\).*,\1'"$out/${glimpse.name}-plugins", \
         -e 's,^\(GIMP_DATA_DIR=\).*,\1'"$out/share/${glimpse.name}", -i configure
      '';
    ...
    resynthesizer = pluginDerivation rec {
      ...
      */
      ...
      configureFlags = [ "--libdir=${placeholder "out"}/lib/glimpse/2.0" ];
      makeFlags = [ "GIMP_LIBDIR=${placeholder "out"}/lib/glimpse/2.0" ];
      src = fetchFromGitHub {
        owner = "bootchk";
        repo = "resynthesizer";
        rev = "v${version}";
