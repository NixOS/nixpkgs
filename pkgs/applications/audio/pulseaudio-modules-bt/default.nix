{ stdenv, runCommand, fetchFromGitHub, pulseaudio, pkg-config, ffmpeg, patchelf
...
in stdenv.mkDerivation rec {
  ...
  nativeBuildInputs = [ pkg-config patchelf cmake ];

  buildInputs = [ pulseaudio ffmpeg fdk_aac libtool ldacbt bluez dbus sbc ];
    ...
    for so in $out/lib/pulse-${pulseaudio.version}/modules/*.so; do
      ...
      patchelf \
        --set-rpath "${ldacbt}/lib:${
          lib.getLib ffmpeg
        }/lib:$out/${pulseaudio.pulseDir}/modules:$orig_rpath" \
        "$so"
    done
  '';
  ...
  meta = with lib; {
    ...
    description =
      "LDAC, aptX, aptX HD, AAC codecs (A2DP Audio) support for Linux PulseAudio";
