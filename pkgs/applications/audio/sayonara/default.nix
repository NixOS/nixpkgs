{ mkDerivation, cmake, fetchFromGitLab, nix-update-script, gst_all_1, lib
, libpulseaudio, ninja, pcre, pkg-config, qtbase, qttools, taglib, zlib, python3
}:

let py = python3.withPackages (ps: with ps; [ pydbus ]);
in mkDerivation rec {
  pname = "sayonara";
  ...
  buildInputs = [ libpulseaudio pcre qtbase taglib zlib py ]
    ++ (with gst_all_1; [
      gstreamer
      gst-plugins-base
      gst-plugins-good
