...
in
  with builtins; with lib; let
    ...
    coq-version =
      args.coq-version
      or (if version != "dev"
      then versions.majorMinor version
      else "dev");
    ...
    self = stdenv.mkDerivation {
      ...
      postPatch = ''
        ...
        ${
          if !versionAtLeast "8.7"
          then "substituteInPlace configure.ml --replace \"md5 -q\" \"md5sum\""
          else ""
        }
        ${csdpPatch}
      '';
