...
, ocamlPackages_4_10, ocamlPackages_4_12, ncurses, buildIde ? true, glib, gnome
, wrapGAppsHook, makeDesktopItem, copyDesktopItems, csdp ? null, version
, coq-version ? null, }@args:
let lib' = lib;
...
let
  ...
  coq-version = args.coq-version or (if version != "dev" then
    ...
    "dev");
  versionAtLeast = v:
    (coq-version == "dev") || (lib.versionAtLeast coq-version v);
  ...
  '' else
    "";
  ocamlPackages = if !isNull customOCamlPackages then
    customOCamlPackages
  else
    with versions;
    switch coq-version [
      {
        case = range "8.14" "8.14";
  ...
  self = stdenv.mkDerivation {
    ...
    nativeBuildInputs = [ pkg-config ] ++ optional buildIde copyDesktopItems
      ++ optional (!versionAtLeast "8.6") gnumake42;
    buildInputs = [ ncurses ] ++ ocamlBuildInputs ++ optionals buildIde
    ...
    buildFlags = [ "revision" "coq" "coqide" ]
      ++ optional (!versionAtLeast "8.14") "bin/votour";
    ...
    postInstall = let suffix = if versionAtLeast "8.14" then "-core" else "";
    in ''
