...
#!nix-shell -i python3 -p 'python3.withPackages (ps: [ ps.nixpkgs-updaters-library ])'
# run with:
# $ nix run .\#vimPluginsUpdater
# docs: https://nixos.org/manual/nixpkgs/unstable/#adding-new-plugins-to-nixpkgs
#
...
ROOT = Path("pkgs/applications/editors/vim/plugins")
...
class VimEntryInfo(EntryInfo, frozen=True):
    ...
    async def fetch(self) -> VimEntry:
        ...
        assert result.has_submodules is not None
...
class VimEntry(Entry[EntryInfo, VimMiniEntry], frozen=True):
    ...
    def minify(self) -> VimMiniEntry:
        assert self.fetched.commit is not None, (
            "This plugin was not prefetched!"
        )
        ...
        return VimMiniEntry(
            ...
            homepage=self.fetched.meta.homepage or self.fetched.url,
...
class VimImpl(ABCBase[VimEntry, VimEntryInfo]):
    ...
    _default_input_file: os.PathLike[str] = dataclasses.field(
        init=False,
        default=utils.NIXPKGS_PLACEHOLDER / ROOT / "vim-plugin-names.csv",
    )
    _default_output_file: os.PathLike[str] = dataclasses.field(
        init=False,
        default=utils.NIXPKGS_PLACEHOLDER / ROOT / "generated.json",
    )
    ...
    def write_entries_info(
        self, entries_info: c.Iterable[VimEntryInfo]
    ) -> None:
