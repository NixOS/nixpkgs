...
def get_dependencies(plugin, rockspec) -> str:
...
def look_for_rockspec(store_path: str) -> Tuple[bool, str]:
    ...
    print("Checking for rockspec in path %s" % store_path)
...
class VimEditor(pluginupdate.Editor):
    ...
    def plugin2nix(self, pdesc: pluginupdate.PluginDesc, plugin: pluginupdate.Plugin) -> str:
        ...
        if has_rockspec:
            content += "let "
            content += "   luaDrv = luaPackages.callPackage(" + get_dependencies(plugin, rockspec_path).rstrip() + ") {};"
            content += f'''
                in buildVimPluginFrom2Nix {{
                    pname = "{plugin.name}";
                    version = luaDrv.version;
...
""".format(plugin=plugin, src_nix = src_nix, repo=repo)
        print(content)
        return content
