{ lib, stdenv, fetchurl, copper, ruby, python3, qtbase, gtk3, pkg-config
, withQt ? false, withGtk ? false, wrapQtAppsHook ? null }:
stdenv.mkDerivation rec {
  ...
  postPatch = ''
    ...
    patchShebangs .
  '';
  nativeBuildInputs = [ copper python3 ruby qtbase gtk3 pkg-config ]
  ...
  ] ++ lib.optionals withQt [ "UI=qt" ] ++ lib.optionals withGtk [ "UI=gtk" ];
