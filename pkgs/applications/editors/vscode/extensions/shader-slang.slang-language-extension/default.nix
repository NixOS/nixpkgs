...
vscode-utils.buildVscodeMarketplaceExtension {
  ...
  postInstall =
    let
      ...
      binaries = {
        ...
        "x86_64-darwin" = {
          ...
          files = [
            "lib/libslang.dylib"
            "lib/libslang-glsl-module.dylib"
            "bin/slangd"
          ];
        };
        "aarch64-darwin" = {
    ...
    lib.strings.concatLines (
      ...
      ++ (
        if builtins.hasAttr stdenv.system binaries then
          with binaries.${stdenv.system};
          [ "mkdir -p $bin/${name}" ] ++ (map (path: "cp ${shader-slang}/${path} $bin/${name}/") files)
        else
          [ ]
      )
    );
