{ stdenv, symlinkJoin, lib, makeWrapper, writeText, bundlerEnv, ruby, nodejs
, nodePackages, python3, python3Packages, callPackage }:
with lib;
...
let
  wrapper = { extraName ? ""
    ...
    , wrapperArgs ? [ ], manifestRc ? null, withPython2 ? false
    , withPython3 ? true, python3Env ? python3, withNodeJs ? false
    , rubyEnv ? null, vimAlias ? false, viAlias ? false
    ...
    , wrapRc ? true, neovimRcContent ? "", ... }@args:
    let

      wrapperArgsStr = if isString wrapperArgs then
        wrapperArgs
      else
