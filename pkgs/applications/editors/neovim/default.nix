...
, libuv, lua, ncurses, pkg-config, unibilium, gperf, libvterm-neovim
, tree-sitter, glibcLocales ? null, procps ? null
...
let
  neovimLuaEnv = lua.withPackages (ps:
    (with ps;
      [ lpeg luabitop mpack ] ++ optionals doCheck [
        nvim-client
        luv
        coxpcall
        busted
  ...
  luvpath = with builtins;
    if stdenv.isDarwin then
      "${luv.libluv}/lib/lua/${lua.luaversion}/libluv.${
        head (match "([0-9.]+).*" luv.version)
      }.dylib"
    else
