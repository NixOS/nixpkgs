{ lib, vimUtils, nodejs, neovim-unwrapped, bundlerEnv, ruby, python3Packages
, writeText, wrapNeovimUnstable }:
let
  ...
  makeNeovimConfig = { withPython2 ? false
      # the function you would have passed to python.withPackages
    , extraPython2Packages ? (_: [ ]), withPython3 ? true
    ...
    let
      ...
      makeWrapperArgs = let
        ...
        flags = lib.concatLists (lib.mapAttrsToList
          (prog: withProg: [ "--cmd" (genProviderSettings prog withProg) ])
          hostprog_check_table);
      in [ "--argv0" "$0" "--add-flags" (lib.escapeShellArgs flags) ]
      ...
      manifestRc =
        vimUtils.vimrcContent (configurePatched // { customRC = ""; });
      neovimRcContent = vimUtils.vimrcContent configurePatched;
    in assert withPython2 -> throw
      "Python2 support has been removed from neovim, please remove withPython2 and extraPython2Packages.";
  ...
  legacyWrapper = neovim:
    { extraMakeWrapperArgs ? "", withPython ? false
      # the function you would have passed to python.withPackages
    ...
    in assert withPython -> throw
      "Python2 support has been removed from neovim, please remove withPython and extraPythonPackages.";
