...
def kotlinc_info(root_path: str) -> (str, str):
    file_contents = parse(open(root_path + "/.idea/kotlinc.xml").read())
    components = file_contents['project']['component']
    for component in components:
        if component['@name'] != 'KotlinJpsPluginSettings':
        ...
        print(f"* Prefetching Kotlin compiler version {version}...")
        prefetch = subprocess.run(["nix-prefetch-url", "--type", "sha256", f"https://cache-redirector.jetbrains.com/maven.pkg.jetbrains.space/kotlin/p/kotlin/kotlin-ide-plugin-dependencies/org/jetbrains/kotlin/kotlin-dist-for-ide/{version}/kotlin-dist-for-ide-{version}.jar"], capture_output=True, check=True, text=True)
...
def main():
    ...
    result['idea-community']['kotlinc'] = {}
    result['idea-community']['kotlinc']['version'], result['idea-community']['kotlinc']['hash'] = kotlinc_info(ideaOutPath)
    ...
    result['pycharm-community']['kotlinc'] = {}
    result['pycharm-community']['kotlinc']['version'], result['pycharm-community']['kotlinc']['hash'] = kotlinc_info(pycharmOutPath)
