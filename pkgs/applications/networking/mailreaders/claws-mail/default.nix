...
, enableLibSM ? true, libSM, enableGnuTLS ? true, gnutls
...
, enablePluginDillo ? true, enablePluginFancy ? useGtk3, libsoup, webkitgtk
, enablePluginFetchInfo ? true, enablePluginLibravatar ? enablePluginRavatar
...
, libcanberra-gtk2, libcanberra-gtk3, libnotify
, enablePluginPdfViewer ? enablePluginPdf, poppler, enablePluginPerl ? true
, perl, enablePluginPython ? true, enablePluginPgp ? enablePgp, gnupg, gpgme
, enablePluginRssyl ? true, libxml2, enablePluginSmime ? true
, enablePluginSpamassassin ? true, enablePluginSpamReport ? true
, enablePluginTnefParse ? true, libytnef, enablePluginVcalendar ? true, libical
}:
...
let
  ...
  features = [
    ...
    }
    {
      ...
      deps = [ libnotify ]
        ++ [ (if useGtk3 then libcanberra-gtk3 else libcanberra-gtk2) ];
    }
    {
...
in stdenv.mkDerivation rec {
  ...
  buildInputs = [ curl gsettings-desktop-schemas glib-networking ]
    ++ [ (if useGtk3 then gtk3 else gtk2) ]
    ++ concatMap (f: optionals f.enabled f.deps)
    (filter (f: f ? deps) features);
  ...
  configureFlags = [
    ...
    map (flag: strings.enableFeature feature.enabled flag) feature.flags)
    features);
