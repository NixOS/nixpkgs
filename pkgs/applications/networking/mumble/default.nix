{ lib, stdenv, fetchFromGitHub, pkg-config, qt5, avahi, boost, libopus
, libsndfile, protobuf, speex, libcap, alsa-lib, python3, rnnoise
, jackSupport ? false, libjack2, speechdSupport ? false, speechd
, pulseSupport ? false, libpulseaudio, iceSupport ? false, zeroc-ice
, grpcSupport ? false, grpc, which, nixosTests }:

let
  generic = overrides: source:
    qt5.mkDerivation (source // overrides // {
      pname = overrides.type;
      version = source.version;

      patches = (source.patches or [ ]) ++ [ ./fix-rnnoise-argument.patch ];

      nativeBuildInputs = [ pkg-config python3 qt5.qmake ]
        ++ (overrides.nativeBuildInputs or [ ]);

      buildInputs = [ boost protobuf avahi ] ++ (overrides.buildInputs or [ ]);
  ...
  client = source:
    generic {
      ...
      NIX_CFLAGS_COMPILE =
        lib.optional speechdSupport "-I${speechd}/include/speech-dispatcher";
