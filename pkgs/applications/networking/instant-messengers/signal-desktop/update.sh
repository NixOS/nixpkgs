...
case "$UPDATE_NIX_ATTR_PATH" in
  signal-desktop)
    latestTag=$(curl https://api.github.com/repos/signalapp/Signal-Desktop/releases/latest | jq -r ".tag_name")
    latestVersion="$(expr "$latestTag" : 'v\(.*\)')"
    latestVersionAarch64=$(curl "https://api.github.com/repos/0mniteck/Signal-Desktop-Mobian/releases/latest" | jq -r ".tag_name")

    echo "Updating signal-desktop for x64 Linux"
    ...
    ;;
  signal-desktop-beta)
    latestTagBeta=$(curl https://api.github.com/repos/signalapp/Signal-Desktop/releases | jq -r ".[0].tag_name")
    latestVersionBeta="$(expr "$latestTagBeta" : 'v\(.*\)')"
    echo "Updating signal-desktop-beta for x64 Linux"
