...
let
  ...
  getDebURL = channelName: version: arch: mirror:
    ...
    in
    "${mirror}/${packageName}/${packageName}_${version}-1_${arch}.deb";
...
in
{
  ...
  update =
    let
      ...
      channels = lib.fold lib.recursiveUpdate
        { } (map
        (attrs: {
          ${attrs.os}.${attrs.channel} = attrs // {
            history =
              ...
              in
              map (lib.flip removeAttrs [ "os" "channel" ]) sorted;
    ...
    in
    writeText "chromium-new-upstream-info.nix" ''
