    ...
    let
        ...
        ${attrs.os}.${attrs.channel} = attrs // {
          history =
            let
              ...
              cond = h: attrs.os == h.os && attrs.channel == h.channel && lib.versionOlder h.version attrs.current_version;
      ...
      */
      ...
      fetchLatest = channel:
        ...
        in
          if result.success then result.value else fetchLatest (channel // {
            version =
              if channel.history != [ ]
              then (lib.head channel.history).version
              else throw "Unfortunately there's no older version than " + "${channel.version} available for channel " + "${channel.channel} on ${channel.os}.";
      ...
      isLatest = channel: version:
        ...
        in
          if ourVersion == null then false
          else lib.versionOlder version sources.${channel}.version || version == sources.${channel}.version;
      ...
      dumpAttrs = indent: attrs:
        let
          mkVal = val:
            if lib.isAttrs val then dumpAttrs (indent + 1) val
