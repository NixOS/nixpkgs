  ...
  if (ungoogled_rev || version_greater_than(version_upstream, version_nixpkgs)) {
    ...
    if (is_helium) {
      ...
      if (ov) {
        ...
        lockfile[attr_path].DEPS['src/components/helium_onboarding'] = { url: ou, hash: oh, stripRoot: false }
      }
      ...
      const sv = hi_deps?.search_engines_data?.version
      if (sv) {
	    const su = hi_deps.search_engines_data.url.replaceAll('%(version)s', sv)
        const sh = await fetch_file_hash(su, false)
        lockfile[attr_path].DEPS['src/third_party/search_engines_data/resources_internal'] = { url: su, hash: sh, stripRoot: false }
      }
    }
