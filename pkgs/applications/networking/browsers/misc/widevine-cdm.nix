...
stdenv.mkDerivation (finalAttrs: {
  ...
  widevineInstaller = if stdenv.targetPlatform.isAarch64 then fetchFromGitHub {
      owner = "AsahiLinux";
  ...
  src = if stdenv.targetPlatform.isAarch64 then fetchurl {
  ...
  nativeBuildInputs = lib.optionals stdenv.targetPlatform.isAarch64 [
    squashfsTools curl python3
  ];

  unpackPhase = if stdenv.targetPlatform.isAarch64 then ''
    unsquashfs -q $src 'WidevineCdm/*'
    python3 ${finalAttrs.widevineInstaller}/widevine_fixup.py squashfs-root/WidevineCdm/_platform_specific/cros_arm64/libwidevinecdm.so libwidevinecdm.so
...
# Accoring to widevine-installer: "Hack because Chromium hardcodes a check for this right now..."
  postInstall = lib.optionalString stdenv.targetPlatform.isAarch64 ''
