...
, zip, unzip, jq, xdg-utils, writeText
...
, pipewire, tridactyl-native, fx_cast_bridge, udev, libkrb5, libva
...
let
  wrapper = { applicationName ? browser.applicationName or (lib.getName browser)
    ...
    , icon ? applicationName, extraNativeMessagingHosts ? [ ]
    , pkcs11Modules ? [ ], forceWayland ? false, useGlvnd ? true, cfg ?
      config.${applicationName} or { }
    ...
    let
      ...
      libs = lib.optionals stdenv.isLinux [
        ...
        pipewire ++ lib.optional ffmpegSupport ffmpeg
        ...
        ++ lib.optional smartcardSupport opensc ++ pkcs11Modules;
      ...
      extensions = if nameArray != (lib.unique nameArray) then
        ...
        builtins.map (a:
          if !(builtins.hasAttr "extid" a) then
            ...
            a) (if usesNixExtensions then nixExtensions else [ ]);

      enterprisePolicies = {
        policies =
          lib.optionalAttrs usesNixExtensions { DisableAppUpdate = true; }
      ...
      pluginsError = "Your configuration mentions ${
          ...
          configPlugins
        }. All plugin related options have been removed, since Firefox from version 52 onwards no longer supports npapi plugins (see https://support.mozilla.org/en-US/kb/npapi-plugins).";

    in if configPlugins != [ ] then
      throw pluginsError
    else
      (stdenv.mkDerivation {
