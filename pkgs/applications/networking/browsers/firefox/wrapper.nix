{
  ...
  libva,
  mesa
  # firefox wants gbm for drm+dmabuf
  ,
  cups,
}:
...
browser: let
  wrapper = {
    ...
    extraPolicies ? {},
    extraPoliciesFiles ? [],
    libName ? browser.libName or "firefox"
    ...
    nameArray = builtins.map (a: a.name) (if usesNixExtensions
    then nixExtensions
    else []);
    ...
    extensions =
      ...
      else if !(lib.hasSuffix "esr" browser.name)
      then throw "Nix addons are only supported in Firefox ESR"
      else
        builtins.map (
          a:
