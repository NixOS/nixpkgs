...
flutter324.buildFlutterApplication rec {
  ...
  postInstall = ''
    ...
    ln -fs "${passthru.helper}/bin/authenticator-helper" "$out/app/$name/helper/authenticator-helper"

    # Move the icon.
    mkdir $out/share/icons
    mv $out/app/$name/linux_support/com.yubico.yubioath.png $out/share/icons

    # Cleanup.
    rm -rf \
      "$out/app/$name/README.adoc" \
      "$out/app/$name/desktop_integration.sh" \
      "$out/app/$name/linux_support" \
      $out/bin/* # We will repopulate this directory later.

    # Symlink binary.
    ln -sf "$out/app/$name/authenticator" "$out/bin/yubioath-flutter"
