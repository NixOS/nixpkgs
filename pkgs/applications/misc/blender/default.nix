...
, pythonDeps ? ps: [] # To add additional Python packages for plugins, similar to python.withPackages
, makeWrapperArgs ? []
}:
...
let
  ...
  in pythonPackages.makePythonPath (defaultPythonPackages ++ packages);
  ...
  unwrapped = stdenv.mkDerivation rec {
    ...
    postFixup = optionalString cudaSupport ''
      for program in $out/bin/blender; do
  ...
  wrapped = let
    blenderExecutable = (if stdenv.isDarwin then "Blender.app/Contents/MacOS/Blender" else "bin/blender");
