{ lib, fetchFromSourcehut, buildPythonPackage, buildGoModule, python, srht
, pygit2, scmsrht }:
let
  version = "0.72.8";
...
in buildPythonPackage rec {
  ...
  postInstall = ''
    mkdir -p $out/bin
    cp ${
      buildShell "${src}/gitsrht-shell"
    }/bin/gitsrht-shell $out/bin/gitsrht-shell
    cp ${
      buildDispatcher "${src}/gitsrht-dispatch"
    }/bin/gitsrht-dispatch $out/bin/gitsrht-dispatch
