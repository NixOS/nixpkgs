...
in
  ...
  // rec {
    ...
    name =
      if supportFlags.waylandSupport
      then "${prevName}-wayland"
      else prevName;
    ...
    buildInputs = toBuildInputs pkgArches (with supportFlags; (pkgs:
      ...
      ++ lib.optional vulkanSupport (if stdenv.isDarwin
      then moltenvk
    ...
    meta = {
      ...
      description =
        if supportFlags.waylandSupport
        then "An Open Source implementation of the Windows API on top of OpenGL and Unix (with experimental Wayland support)"
        else "An Open Source implementation of the Windows API on top of X, OpenGL, and Unix";
      platforms =
        if supportFlags.waylandSupport
        then (lib.remove "x86_64-darwin" prevPlatforms)
        else prevPlatforms;
