...
def construct_url(x):
    ...
    if x["name"].startswith("FONT_NOTO_") and not probe_url(url):
        return "https://noto-website-2.storage.googleapis.com/pkgs/{}".format(
            x["tarball"]
        )

    if x["name"] == "FONT_OPENDYSLEXIC":
        return "https://github.com/antijingoist/opendyslexic/releases/download/v0.91.12/{}".format(
            x["tarball"]
        )
...
def get_packages():
    ...
    plus our additions.
    """
    return apply_additions(get_packages_from_download_list(), get_additions())
...
def apply_additions(xs, additions):
    for x in xs:
        yield dict_merge([x, additions.get(x["name"], {})])
...
def get_packages_from_download_list():
    ...
    def lines():
        for x in sub_symbols(parse_lines(get_lines())):
            ...
            if interpretation == "unrecognized":
                ...
                yield dict_merge([x, interpretation])
    ...
    for x in sorted(packages(), key=lambda x: x["index"]):
        yield dict_merge([{"name": x["name"]}, x["attrs"]])
...
def print_skipped_line(x):
    print("Skipped line {}: {}".format(x["index"], x["original"]), file=sys.stderr)
...
def parse_line(line):
    ...
    if match:
        return {"key": match.group(1), "value": match.group(2).strip()}
    else:
        return "unrecognized"
...
def sub_symbols(xs):
    ...
    for x in xs:
        yield dict_merge([{"value": sub_str(x["value"], get_value)}, x])
...
def interpret_sha256(x):
    ...
    if match:
        return {"name": match.group(1), "attrs": {"sha256": x["value"], "md5": ""}}
...
def interpret_tarball(x):
    ...
    if match:
        return {"name": match.group(1), "attrs": {"tarball": x["value"], "brief": True}}
...
def interpret_tarball_with_md5(x):
    ...
    match = {
        "key": re.match("^(.*)_(TARBALL|JAR)$", x["key"]),
        "value": re.match("(?P<md5>[0-9a-fA-F]{32})-(?P<tarball>.+)$", x["value"]),
    }

    if match["key"] and match["value"]:
        return {
            "name": match["key"].group(1),
            "attrs": {
                "tarball": match["value"].group("tarball"),
