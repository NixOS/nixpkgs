...
stdenv.mkDerivation rec {
  ...
  src = requireFile rec {
    ...
    message = ''
      This nix expression requires that ${name} is already part of the
      store. Download the TWS from "https://download2.interactivebrokers.com/
      download/unixmacosx_latest.jar", rename the file to ${name}, and
      add it to the nix store with "nix-prefetch-url file://${name}".
    '';
    sha256 = "0axmifhcv07zg0cc7xbs8jpxnwy93z9yimbfbwy370x2rsyracwy";
  };
