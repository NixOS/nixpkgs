...
let
  cygwinDllLink = derivation {
    inherit (localSystem) system;
    name = "cygwin-dll-link.sh";
    src = ../../os-specific/cygwin/cygwin-dll-link.sh;
    builder = "${bootstrapFiles.unpack}/bin/bash";
    args = [
      "-c"
      "sed s:@objdump@:${bootstrapFiles.unpack}/bin/objdump:g $src > $out"
    ];
...
[
  ...
  (prevStage: {
    ...
    stdenv = import ../generic rec {
      ...
      extraNativeBuildInputs = [
        prevStage.cygwin.cygwinDllLinkHook
      ];
