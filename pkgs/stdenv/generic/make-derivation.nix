...
, passthru ? { }, pos ? # position used in error messages and for meta.position
...
, __impureHostDeps ? [ ], __propagatedImpureHostDeps ? [ ], sandboxProfile ? ""
...
let
  ...
  !((stdenv.hostPlatform.isAarch64 || stdenv.hostPlatform.isAarch32)
    && stdenv.hostPlatform.isStatic) then
    supportedHardeningFlags
  else
...
in if builtins.length erroneousHardeningFlags != 0 then
  ...
  let
    ...
    references = nativeBuildInputs ++ buildInputs ++ propagatedNativeBuildInputs
    ...
    derivationArg = (removeAttrs attrs [
      ...
      (attrs ? name || (attrs ? pname && attrs ? version)) {
        ...
        mesonFlags = if mesonFlags == null then
          null
        else
          let
            ...
            cpuFamily = platform:
              ...
              if isAarch32 then
                ...
                platform.parsed.cpu.family
                + builtins.toString platform.parsed.cpu.bits;
      ...
      (stdenv.hostPlatform.isx86_64 && stdenv.hostPlatform ? gcc.arch) {
        ...
        __sandboxProfile = let
          profiles = [ stdenv.extraSandboxProfile ] ++ computedSandboxProfile
            ++ computedPropagatedSandboxProfile
