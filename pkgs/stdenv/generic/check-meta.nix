...
let
  ...
  showLicense = license:
    toString (map (l: l.shortName or "unknown") (lib.lists.toList license));
  ...
  checkMetaAttr = k: v:
    if metaTypes ? ${k} then
      if metaTypes.${k}.check v then
        ...
        "key '${k}' has a value ${toString v} of an invalid type ${
          builtins.typeOf v
        }; expected ${metaTypes.${k}.description}"
    else ''
  ...
  checkOutputsToInstall = attrs:
    let
      ...
      missingOutputs =
        builtins.filter (output: !builtins.elem output actualOutputs)
        expectedOutputs;
    in if shouldCheckMeta then builtins.length missingOutputs > 0 else false;
  ...
  assertValidity = { meta, attrs }:
    ...
    in validity // {
      ...
      handled = if !validity.valid then
