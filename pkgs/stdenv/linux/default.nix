...
# ensuring purity of components produced by it.
{
  ...
  bootstrapFiles ? let
    ...
    getCompatibleTools = lib.foldl (v: system:
      if v != null
      then v
      else if localSystem.isCompatible (lib.systems.elaborate {inherit system;})
...
assert crossSystem == localSystem; let
  ...
  commonPreHook = ''
    ...
    ${
      ...
      then "NIX_LIB64_IN_SELF_RPATH=1"
      else ""
    }
    ${
      if system == "mipsel-linux"
      then "NIX_LIB32_IN_SELF_RPATH=1"
      else ""
    }
  '';
  ...
  bootstrapTools = import (if localSystem.libc == "musl"
  then ./bootstrap-tools-musl
  else ./bootstrap-tools) {
