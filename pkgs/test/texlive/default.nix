...

{
  ...
  hyphen-base = runCommand "texlive-test-hyphen-base" {
    hyphenBase = texlive.hyphen-base;
    ...
    for fname in language.{dat,def,dat.lua} ; do
      diff --ignore-matching-lines='^\(%\|--\) Generated by ' -u \
        {"$hyphenBase","$schemeFull"/share/texlive/texmf-var}/tex/generic/config/"$fname" \
        | tee "$out/scheme-full/$fname.patch"
      diff --ignore-matching-lines='^\(%\|--\) Generated by ' -u \
        {,"$schemeInfraOnly"/share/texlive/texmf-var/tex/generic/config/}"$fname" \
  ...
  fmtutilCnf = runCommand "texlive-test-fmtutil.cnf" {
    kpathsea = texlive.kpathsea;
    schemeFull = texlive.combined.scheme-full;
  } ''
    mkdir -p "$out"

    diff --ignore-matching-lines='^# Generated by ' -u \
      {"$kpathsea","$schemeFull"/share/texlive/texmf-var}/web2c/fmtutil.cnf \
  ...
  binaries = let
      ...
      allPackages = lib.filter (p: p ? tlOutputName) (builtins.attrValues texlive);
      binPackages = lib.catAttrs "out" allPackages;
      binaries = buildEnv { name = "texlive-binaries"; extraOutputsToInstall = [ "out" ]; paths = binPackages; };
    in
  ...
  shebangs = let
      allPackages = lib.filter (p: p ? tlOutputName) (builtins.attrValues texlive);
      binPackages = lib.filter (p: p ? outPath) (lib.catAttrs "out" allPackages);
    in
