...
, glib, gtk, libgnomeui
, pkgconfig, dbus_glib, realCurl, pcre, libsexy, gtkspell
}:
...
stdenv.mkDerivation {
  ...
  buildInputs = [
    zlib libxml2 libxslt firefox openssl curl
    glib gtk pkgconfig dbus_glib realCurl pcre libsexy libgnomeui gtkspell
  ];

  configureFlags="--with-libcurl-libraries=${curl}/lib --with-libcurl-headers=${curl}/include --with-pcre_libraries=${pcre}/lib --with-pcre_headers=${pcre}/include --without-libnotify";  
}
