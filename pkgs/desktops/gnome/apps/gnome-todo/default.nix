...
, wrapGAppsHook, gettext, gnome, glib, gtk4, wayland, libadwaita, libpeas
, gnome-online-accounts, gsettings-desktop-schemas, libportal
, evolution-data-server, libical, librest, json-glib, itstool }:
...
stdenv.mkDerivation rec {
  ...
  patches = [
    ...
    (fetchpatch {
      url =
        "https://cgit.freebsd.org/ports/plain/deskutils/gnome-todo/files/patch-src_meson.build?id=a4faaf6cf7835014b5f69a337b544ea4ee7f9655";
  ...
  nativeBuildInputs =
    [ meson ninja pkg-config gettext python3 wrapGAppsHook itstool ];
