{ mkDerivation, lib, extra-cmake-modules, plymouth, nixos-icons, imagemagick
, netpbm, perl, logoName ? null, logoFile ? null, osName ? null
, osVersion ? null, topColor ? "black", bottomColor ? "black" }:
...
let
  ...
  resolvedLogoName = if (logoFile != null && logoName == null) then
    ...
    logoName;
in assert lib.asserts.assertOneOf "topColor" topColor validColors;
...
mkDerivation {
  ...
  cmakeFlags = [ ] ++ lib.optional (osName != null) "-DDISTRO_NAME=${osName}"
