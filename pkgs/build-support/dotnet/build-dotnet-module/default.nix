...
, nugetDeps ? null
  # The NuGet configuration file for `passthru.fetch-deps` script. If not specified, the hierarchy of configuration files from the source directory will be used (that is, dotnet default behavior).
  # Useful for packages that don’t have nuget.config at the source root.
      ...
      writeShellScript "fetch-${pname}-deps" ''
        ...
        dotnetRestore() {
            ...
            dotnet restore ''${project-} \
                ...
                ${lib.optionalString (nugetConfigFile != null) "--configfile=${lib.escapeShellArg nugetConfigFile}"} \
        ...
        if [ -d "$HOME/.nuget/packages" ]; then
          cp -rT "$HOME/.nuget/packages" "$tmp/nuget_pkgs"
        fi

        for rid in ${lib.escapeShellArgs runtimeIds}; do
            echo "Restoring project for $rid..."
        ...
        nuget-to-nix \
          ${lib.optionalString (nugetConfigFile != null) "--nuget-config ${lib.escapeShellArg nugetConfigFile}"} \
          -- "$tmp/nuget_pkgs" "$tmp/excluded_list" >> "$tmpFile"
        mv "$tmpFile" "$depsFile"
