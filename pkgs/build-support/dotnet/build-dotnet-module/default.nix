...
, dotnet-test-sdk ? dotnet-sdk
  ...
  # Dont set the Nuget source when running `dotnet configure`, this is used for generating the nuget lockfile.
, dontDotnetSetNugetSource ? false
, ...
} @ args:
...
assert (nugetDeps == null && !dontDotnetSetNugetSource) -> throw "Defining the `nugetDeps` attribute is required, as to lock the NuGet dependencies. This file can be generated by running the `passthru.fetch-deps` script.";
...
let
  ...
  _nugetDeps = lib.optionals (!dontDotnetSetNugetSource)
    (if lib.isDerivation nugetDeps
    then nugetDeps
    else mkNugetDeps { inherit name; nugetDeps = import nugetDeps; });

  nuget-source = lib.optionals (!dontDotnetSetNugetSource)
    (mkNugetSource
      {
  ...
  fetch-deps-impure = callPackage ./fetch-deps.nix {
    inherit name src meta dotnet-sdk projectFile testProjectFile dotnetFlags dotnetRestoreFlags enableParallelBuilding;
  };
...
stdenvNoCC.mkDerivation (args // {
  ...
  passthru = {
    ...
    # A script which fetches the dependencies and generates a lockfile for "nugetDeps".
    fetch-deps = writeScript "fetch-${pname}-deps" ''
      ...
      echo "# This file was automatically generated by a script created with \"nix-build -A attrpath.passthru.fetch-deps\". Please do not manually modify it, your changes might get overwritten.
      " > $deps_file
    ...
    # A derivation which fetches the dependencies and writes a lockfile to use for "nugetDeps".
    # This is preferable over passthru.fetch-deps as it mimics the behavior of `dotnet restore` used in the parent derivation.
    # Note that this process requires networking, so the sandbox must be disabled with "--option sandbox false".
    inherit fetch-deps-impure;
  } // args.passthru or { };
})
