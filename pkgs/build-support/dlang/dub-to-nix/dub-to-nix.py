...
if not os.path.exists("dub.selections.json"):
    eprint(
        "The file `dub.selections.json` does not exist in the current working directory"
    )
...
depsDict = {
    pname: (
        versionOrDepDict
        if isinstance(versionOrDepDict, dict)
        else {"version": versionOrDepDict}
    )
    for (pname, versionOrDepDict) in depsDict.items()
}

# Don't process path-type selections
depsDict = {
    pname: depDict for (pname, depDict) in depsDict.items() if "path" not in depDict
}
...
for pname in depsDict:
    ...
    if version.startswith("~"):
        ...
        eprint(
            "When packaging, you might also need to patch the version value in the appropriate places (`dub.selections.json`, dub.sdl`, `dub.json`)"
        )
        sys.exit(1)
    if "repository" in depDict:
        ...
        if (
            len(version) < 7
            or len(version) > 40
            or not all(c in string.hexdigits for c in version)
        ):
            eprint(
                f'Expected version field of "{pname}" to begin be a valid git revision'
            )
            eprint(f'Found: "{version}"')
            sys.exit(1)
...
for pname in depsDict:
    ...
    if "repository" in depDict:
        ...
        lockedDepsDict[pname] = {
            "version": version,
            "repository": strippedRepo,
            "sha256": sha256,
        }
    else:
