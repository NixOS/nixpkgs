...
lib.extendMkDerivation {
  ...
  extendDrvArgs =
    ...
    in
    {
          ...
          ${lib.concatStringsSep "\n" (
            builtins.map (
              ...
              ''
                ...
                # cp -rs creates a symlink tree so artifacts remain as
                ...
                cp -rs "${dep}" ".lake/packages/${name}"
                find ".lake/packages/${name}" -type d -exec chmod u+w {} +
              ''
            ) allLeanDeps
          )}
      ...
      meta = meta // {
        platforms = meta.platforms or lean4.meta.platforms or lib.platforms.all;
      };
    };
}
