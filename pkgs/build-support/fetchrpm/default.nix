...
let
  ...
  name = args.name or (
    if lib.match ".*\\.rpm$" basename != null
...
fetchurl (
  ...
  // {
    ...
    postFetch = ''
      ...
      # Step 1: Extract rpm
      pushd $out
      rpm2cpio $downloadedFile | cpio -idmv
      popd

      # Step 2: Decompress
      find $out -type f \( -iname "*.tar" -o -iname "*.tar.gz" -o -iname "*.tar.xz" -o -iname "*.tar.bz2" -o -iname "*.tar.zst" \) -exec sh -c 'for archive; do tar -xf "$archive" -C "$(dirname "$archive")"; done' sh {} +

      # Step 3: Fix dangling symlinks
