...
# compiler and the linker just "work".
{
  ...
  sharedLibraryLoader ?
    ...
    then
      if !(targetPackages ? netbsdCross)
      then netbsd.ld_elf_so
      else if libc != targetPackages.netbsdCross.headers
      then targetPackages.netbsdCross.ld_elf_so
      else null
...
assert (noLibc || nativeLibc) == (libc == null); let
  ...
  dynamicLinker =
    ...
    else if targetPlatform.system == "powerpc64le-linux"
    then "${sharedLibraryLoader}/lib/ld64.so.2"
    ...
    else if lib.hasSuffix "pc-gnu" targetPlatform.config
    then "ld.so.1"
    else null;
  ...
  stdenv.mkDerivation {
    ...
    emulation = let
      ...
      arch =
        ...
        else if targetPlatform.isAarch32
        then endianPrefix + "arm"
        else if targetPlatform.isx86_64
        then "x86-64"
        else if targetPlatform.isx86_32
        then "i386"
        else if targetPlatform.isMips
        then
          {
        ...
        else if targetPlatform.isSparc
        then "sparc"
        else if targetPlatform.isMsp430
        then "msp430"
        else if targetPlatform.isAvr
        then "avr"
        else if targetPlatform.isAlpha
        then "alpha"
        else if targetPlatform.isVc4
        then "vc4"
        else if targetPlatform.isOr1k
        then "or1k"
        else if targetPlatform.isM68k
        then "m68k"
        else if targetPlatform.isS390
        then "s390"
        else if targetPlatform.isRiscV
        then "lriscv"
        else throw "unknown emulation for platform: ${targetPlatform.config}";
    in
      if targetPlatform.useLLVM or false
      then ""
    ...
    in
      (if bintools_ ? meta
      then removeAttrs bintools.meta ["priority"]
      else {})
      // {
        description =
