...
, extraBuildCommands ? "", buildPackages ? { }, targetPackages ? { }
...
let
  ...
  expand-response-params = if buildPackages ? stdenv
  && buildPackages.stdenv.hasCC && buildPackages.stdenv.cc != "/dev/null" then
...
in stdenv.mkDerivation {
  ...
  emulation = let
    ...
    arch = if targetPlatform.isAarch64 then
    ...
    else if targetPlatform.isMmix then
      "mmix"
      ...
      "alpha"
    else if targetPlatform.isVc4 then
      "vc4"
    else if targetPlatform.isOr1k then
      "or1k"
    else if targetPlatform.isM68k then
      "m68k"
    else if targetPlatform.isS390 then
      "s390"
    else if targetPlatform.isRiscV then
      "lriscv"
    else
  ...
  in if targetPlatform.useLLVM or false then
    ...
    targetPlatform.bfdEmulation or (fmt + sep + arch);
