...
let
  ...
  gradleScript = writeShellScript "gradle-commands.sh" ''
    ...
    gradleFlagsArray+=(--init-script ${./init-deps.gradle})
    runHook preBuild
    ${preBuild}
    ${script}
    ${postBuild}
  ...
  source = srcOnly (pkg.overrideAttrs (old: { mitmCache = ""; }));
...
writeTextFile {
  ...
  text = ''
    ...
    outPath="${
      ...
      if lib.hasPrefix "${builtins.storeDir}/" (toString depsPath)
      then builtins.concatStringsSep "/" (lib.drop 1 (lib.splitString "/" (lib.removePrefix "${builtins.storeDir}/" (toString depsPath))))
      # if this is an absolute path, just use that path
      else if lib.hasPrefix "/" (toString depsPath)
      then toString depsPath
      # otherwise, use a path relative to the package
      else "${dirOf pkg.meta.position}/${depsPath}"
    }"

    pushd "$(${coreutils}/bin/mktemp -d)" >/dev/null
    MITM_CACHE_DIR="$PWD"
    trap "${coreutils}/bin/rm -rf '$MITM_CACHE_DIR'" SIGINT SIGTERM ERR EXIT
    ...
    export MITM_CACHE_PORT="''${mitmCachePort:-$(${python3.pkgs.ephemeral-port-reserve}/bin/ephemeral-port-reserve "$MITM_CACHE_HOST")}"
    ...
    for i in {0..20}; do
      ${procps}/bin/ps -p "$MITM_CACHE_PID" >/dev/null || (echo "Failed to start mitm-cache" && exit 1)
      ${curl}/bin/curl -so/dev/null "$MITM_CACHE_ADDRESS" && break
    ...
    for i in {0..20}; do
      ...
      if ${jq}/bin/jq -e 1 "$MITM_CACHE_DIR/out.json" >/dev/null 2>/dev/null; then
        cp "$MITM_CACHE_DIR/out.json" _tmp_deps.json
        ${python3.interpreter} ${./compress-deps-json.py} _tmp_deps.json "$outPath"
        exec ${coreutils}/bin/rm _tmp_deps.json
      fi
      sleep 1
    done
    exit 1
