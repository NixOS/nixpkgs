...
rec {
  ...
  tests = lib.recurseIntoAttrs (
    ...
    // {
      ...
      # buildDocs should produce $out/share/doc/<crate>/index.html instead of
      # an rlib, and the derivation must have a single "out" output (no $lib).
      buildDocs =
        let
          crate = mkHostCrate {
            crateName = "doc-test";
        ...
        in
        assert crate.outputs == [ "out" ];
        runCommand "buildRustCrate-buildDocs-test" { } ''
          test -f ${crate}/share/doc/doc_test/index.html
          touch $out
        '';

      # buildDocs must still compile and run build.rs with real rustc so that
      # OUT_DIR is populated before rustdoc resolves the source. A crate whose
      # public API is `include!`d from a build-script-generated file documents
      # the generated items; if rustdoc ran with an empty/missing OUT_DIR the
      # `include!` would fail and there would be no `fn.generated.html`.
      buildDocsWithBuildScript =
        let
          crate = mkHostCrate {
            crateName = "doc-build-script";
            src = symlinkJoin {
              name = "doc-build-script-src";
              paths = [
                (mkFile "src/lib.rs" ''
                  //! Crate-level docs.
                ...
                (mkFile "build.rs" ''
                  ...
                  fn main() {
                    let out_dir = env::var_os("OUT_DIR").expect("OUT_DIR not set");
                    let out_file = Path::new(&out_dir).join("gen.rs");
        ...
        in
        assert crate.outputs == [ "out" ];
        runCommand "buildRustCrate-buildDocsWithBuildScript-test" { } ''
          test -f ${crate}/share/doc/doc_build_script/index.html
          test -f ${crate}/share/doc/doc_build_script/fn.generated.html
          grep -q 'Generated by build.rs' ${crate}/share/doc/doc_build_script/fn.generated.html
          touch $out
        '';
    }
  );
