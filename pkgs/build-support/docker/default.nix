...
rec {
  ...
  streamNixShellImage =
    ...
    let
      extraContentsList =
        if builtins.isNull extraContents then
          # Legacy convenience derivations for backwards compatibility.
          [
            binSh
            usrBinEnv
          ]
        else if builtins.isList extraContents then
          extraContents
        else
          [ extraContents ];
      ...
      # https://github.com/NixOS/nix/blob/2.26.2/src/libstore/globals.hh#L689-L696
      sandboxBuildDir = "/build";

      # https://github.com/NixOS/nix/blob/2.26.2/src/libstore/unix/build/local-derivation-goal.cc#L1184
      initEnv =
        {
          # Not setting `PATH` since it's set by `$stdenv/setup` which is invoked on shell startup.
          HOME = homeDirectory;
          NIX_STORE = storeDir;
      ...
      # https://github.com/NixOS/nix/blob/2.26.2/src/libstore/unix/build/local-derivation-goal.cc#L1783
      runChildEnv = {
        # https://github.com/NixOS/nix/blob/2.26.2/src/libstore/globals.hh#L1052-L1067
      ...
      # https://github.com/NixOS/nix/blob/2.26.2/src/nix-build/nix-build.cc#L600-L634
      nixShell = writeScript "nix-shell" ''
        #!${shell}

        unset PATH
        dontAddDisableDepTrack=1
        # TODO: https://github.com/NixOS/nix/blob/2.26.2/src/nix-build/nix-build.cc#L611
    ...
    streamLayeredImage {
      ...
      contents = [
        (fakeNss.override {
          ...
          #
          # https://github.com/NixOS/nix/blob/2.26.2/src/libstore/unix/build/local-derivation-goal.cc#L1061-L1067
          #
      ...
      config = {
        ...
        Entrypoint = [ nixShell ];
        Cmd =
          # https://github.com/NixOS/nix/blob/2.26.2/src/nix-build/nix-build.cc#L221-L225
