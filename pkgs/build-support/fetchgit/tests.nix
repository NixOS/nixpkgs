    ...
    testers.invalidateFetcherByDrvHash
      (
        ...
        runCommand name
          ...
          ''
            prefetch() { NIX_REMOTE='/tmp/store' nix-prefetch-git $@ "$url" --rev "$rev" | jq -r .path; }
            path="$(prefetch --no-add-path)"
            if test -e /tmp/store/$path; then
              echo "$path exists in /tmp/store when it shouldn't" >&2
              exit 1
            fi
            path_added="$(prefetch)"
            if ! test -e /tmp/store/$path; then
              echo "$path_added doesn't exist in /tmp/store when it should" >&2
              exit 1
            fi
