...
let
  inherit (lib)
    ...
    revOrTag
    ;
  inherit (lib.fetchers) withNormalizedHash proxyImpureEnvVars;
  inherit (lib.strings) sanitizeDerivationName;

  urlToName =
    let
      inherit (lib.sources) urlToName shortRev;
    in
    {
      url,
      rev,
    ...
    let
      rev' = shortRev rev;
      appendShort = optionalString ((match "[a-f0-9]*" rev) != null) "-${rev'}";
    in
    "${urlToName url}${if append == "" then appendShort else append}";
    ...
    withNormalizedHash { } (
      ...
      #   ../../../doc/build-helpers/fetchers.chapter.md
      {
        ...
        name ? urlToName {
          ...
          append = if rootDir != "" then "-${sanitizeDerivationName rootDir}" else "";
        },
