{
  ...
  writers = with lib; rec {
    ...
    makeScriptWriter = {
      ...
      assert lib.or (types.path.check content) (types.str.check content); let
        name = last (builtins.split "/" nameOrPath);
      in
        pkgs.runCommandLocal name (if (types.str.check content)
        then
          {
    ...
    makeBinWriter = {
      ...
      assert lib.or (types.path.check content) (types.str.check content); let
        name = last (builtins.split "/" nameOrPath);
      in
        pkgs.runCommand name (if (types.str.check content)
        then
          {
            inherit content;
            passAsFile = ["content"];
    ...
    makePythonWriter = python: pythonPackages: name: {
      libraries ? [],
      flakeIgnore ? [],
    }: let
