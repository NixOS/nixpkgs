{ lib, stdenvNoCC, linkFarmFromDrvs, nuget2nix, writeScript, makeWrapper, fetchurl, xml2, dotnetCorePackages, dotnetPackages, cacert }:
...
, nugetDeps ? null
# The package's NuGet.config file, which contains external package repositories used. This is used in the fetch-deps script described above.
, nugetConfig ? null
  ...
  package = stdenvNoCC.mkDerivation (args // {
    ...
    passthru = {
      fetch-deps = writeScript "fetch-${args.pname}-deps" ''
        ...
        ${nuget2nix}/bin/nuget2nix --directory "$HOME/nuget_pkgs" ${lib.optionalString (nugetConfig != null) "--nuget-config ${lib.escapeShellArg nugetConfig}"} > "$deps_file"
