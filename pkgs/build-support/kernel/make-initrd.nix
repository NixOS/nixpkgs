...
in { stdenvNoCC, perl, cpio, ubootTools, lib, pkgsBuildHost
...
, compressor ? "gzip", _compressorFunction ? if lib.isFunction compressor then
  compressor
else if !builtins.hasContext compressor
&& builtins.hasAttr compressor compressors then
  compressors.${compressor}.executable
else
  _: compressor, _compressorExecutable ? _compressorFunction pkgsBuildHost
, _compressorName ? compressorName _compressorExecutable, _compressorMeta ?
  compressors.${_compressorName} or { }

  # List of arguments to pass to the compressor program, or null to use its defaults
, compressorArgs ? null, _compressorArgsReal ? if compressorArgs == null then
...
in stdenvNoCC.mkDerivation rec {
  ...
  nativeBuildInputs = [ perl cpio ] ++ lib.optional makeUInitrd ubootTools;
