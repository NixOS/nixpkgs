...
lib.extendMkDerivation {
  ...
  extendDrvArgs =
    ...
    mirrorArgs (
      args:
      let
        useFetchGitCurrent = lib.defaultTo (
          !(lib.all lib.id (
            lib.attrValues (
              lib.mapAttrs (
                n: default:
                let
                  arg = args.${n};
                in
                !(args ? ${n}) || arg == default || useFetchGitArgsDefaultNullable ? ${n} && arg == null
              ) useFetchGitArgsDefaultNonNull
            )
          ))
        ) args.useFetchGit or null;
      ...
      // (
        if enableUseFetchGitFinal then
          ...
          // getOptionalAttrs excludeUseFetchGitArgNames args
          // {
            useFetchGit = lib.any lib.id (
              lib.attrValues (
                lib.mapAttrs (
                  n: default: finalAttrs ? ${n} && finalAttrs.${n} != default
                ) useFetchGitArgsDefaultNonNull
              )
            );
          }
