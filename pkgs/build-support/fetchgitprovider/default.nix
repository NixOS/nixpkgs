...
let
  ...
  fetchFromGitProviderSimple =
    ...
    let
      ...
      useFetchGitArgsResolved =
        useFetchGitArgsWD
        // fetchgit.resolveNullableFetchGitArgs (args // useFetchGitArgsWD) useFetchGitArgsWD;
      drvArgsKept = useFetchGitArgsResolved // {
        inherit useFetchGit;
      };
      choice = if useFetchGit then fetchgitChoiceRawChecked else fetchzipChoiceRaw;
    in
    if lib.isFunction fpArgs then
      choice (
        finalAttrs:
        let
          args = fpArgs finalAttrs;
        in
        args // { derivationArgs = args.derivationArgs or { } // drvArgsKept; }
      )
    else
      let
        args = fpArgs;
      in
      choice (args // { derivationArgs = args.derivationArgs or { } // drvArgsKept; });
in
...
// {
  __functor =
    ...
    resultSimple
    // {
      inherit (resultDynamic)
        drvAttrs
        overrideAttrs
        overrideDerivation
        ;
      realDrvAttrs = resultSimple.drvAttrs;
    };
