  ...
  postArgsFunc =
    ...
    (
      ...
      // {
        ...
        renameImports =
          prevAttrs.renameImports or (
            let
              inputsWithAliases = lib.filter (x: x ? goPackageAliases) (
                buildInputs ++ (prevAttrs.propagatedBuildInputs or [ ])
              );
        ...
        doCheck = prevAttrs.doCheck or false;
        checkPhase =
...
stdenv.mkDerivation (
  finalAttrs:
  let
    prevAttrs = (if builtins.isAttrs args then args else args finalAttrs);
  in
  prevAttrs // (postArgsFunc prevAttrs)
)
