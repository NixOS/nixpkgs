...
with builtins; with lib; recursiveUpdate lib (rec {
  ...
  */
  ...
  /**
    ...
    ```
    overrideCoqDerivation :: AttrSet -> CoqLibraryDerivation -> CoqLibraryDerivation
    ```

    # Examples
    :::{.example}
    ## `overrideCoqDerivation` usage example
