...
, releaseRev ? (v: v), displayVersion ? { }, release ? { }
, extraBuildInputs ? [ ], namePrefix ? [ "coq" ], enableParallelBuilding ? true
, extraInstallFlags ? [ ], setCOQBIN ? true, mlPlugin ? false
, useMelquiondRemake ? null, dropAttrs ? [ ], keepAttrs ? [ ]
, dropDerivationAttrs ? [ ], useDune2ifVersion ? (x: false), useDune2 ? false
, opam-name ? (concatStringsSep "-" (namePrefix ++ [ pname ])), ... }@args:
let
  ...
  var-coqlib-install = (optionalString
    (versions.isGe "8.7" coq.coq-version || coq.coq-version == "dev") "COQMF_")
    + "COQLIB";
...
in stdenv.mkDerivation (removeAttrs ({
  ...
  buildInputs = [ coq ] ++ optionals mlPlugin coq.ocamlBuildInputs
  ...
  // (optionalAttrs (!args ? installPhase && !args ? useMelquiondRemake) {
    installFlags =
      ...
      "DOCDIR=$(out)/share/coq/${coq.coq-version}/" ++ extraInstallFlags;
