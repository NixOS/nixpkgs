{
  ...
  assert sshSupport -> openssh != null; let
    repoName = with lib; let
      fst = head;
      snd = l: head (tail l);
    ...
    in
      ...
      else if snd path == "branches"
      then "${trd path}-${fst path}"
      # ../repo/tags/tag -> repo-tag
      else if snd path == "tags"
      then "${trd path}-${fst path}"
      # ../repo (no trunk) -> repo
      else fst path;
    ...
    name_ =
      if name == null
      then "${repoName}-r${toString rev}"
      else name;
  in
    if md5 != ""
    then throw "fetchsvn does not support md5 anymore, please use sha256"
    else
