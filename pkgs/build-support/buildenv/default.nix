...
makeOverridable (extendMkDerivation {
  ...
  extendDrvArgs =
    ...
    let
      compatArgs =
        optionalAttrs (args ? nativeBuildInputs) {
          inherit nativeBuildInputs;
        }
        // optionalAttrs (args ? buildInputs) {
          inherit buildInputs;
        };
    in
    compatArgs
    ...
    // {
      ...
      chosenOutputs = map (drv: {
        paths =
          ...
          ++ filter (p: p != null) (map (outName: drv.${outName} or null) finalAttrs.extraOutputsToInstall);
      ...
      extraPathsFrom = optionalString finalAttrs.includeClosures (
        let
          pathsForClosure = pipe finalAttrs.chosenOutputs [
            (map (p: p.paths))
            flatten
            (remove null)
          ];
