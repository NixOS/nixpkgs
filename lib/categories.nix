# Categories for use in pkgs.<name>.meta.categories.
# Based on [Debian package tags](https://packages.debian.org/about/debtags):
#
# - The top-level attrset contains what is called "facets", which group
#   categories.
# - Each facet has a `name` (mandatory plain text string) and `description`
#   (mandatory CommonMark Markdown string). The rest of the attrset
#   entries are categories.
# - Each category only has a `name` (mandatory plain text string) and
#   `description` (optional CommonMark Markdown string).
#
