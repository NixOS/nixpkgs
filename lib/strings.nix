...
in rec {
  ...
  removePrefix =
    ...
    in
      ...
      then substring preLen (sLen - preLen) str
      else str;
  ...
  enableFeature = enable: feat:
    assert isString feat;
    # e.g. passing openssl instead of "openssl"
      "--${
        if enable
        then "enable"
        else "disable"
   ...
   */
  isStorePath = x:
    if !(isList x) && isCoercibleToString x
    then
      let
        str = toString x;
      in
