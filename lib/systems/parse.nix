...
lib.makeExtensible (
  self: with self; {
    ...
    types.significantByte = enum (attrValues self.significantBytes);

    significantBytes = setTypes self.types.openSignificantByte {
      bigEndian = { };
      littleEndian = { };
    };
    ...
    types.openCpuType = mkOptionType {
      ...
      check =
        x:
        self.types.bitWidth.check x.bits
        && (
          if 8 < x.bits then self.types.significantByte.check x.significantByte else !(x ? significantByte)
        );
    };
    ...
    cpuTypes =
      ...
      in
      setTypes self.types.openCpuType {
        arm = {
          bits = 32;
    ...
    types.vendor = enum (attrValues self.vendors);

    vendors = setTypes self.types.openVendor {
      apple = { };
      pc = { };
      knuth = { };
    ...
    types.execFormat = enum (attrValues self.execFormats);

    execFormats = setTypes self.types.openExecFormat {
      aout = { }; # a.out
      elf = { };
      macho = { };
      pe = { };
    ...
    types.kernelFamily = enum (attrValues self.kernelFamilies);

    kernelFamilies = setTypes self.types.openKernelFamily {
      bsd = { };
      darwin = { };
    };
    ...
    types.openKernel = mkOptionType {
      ...
      check =
        x:
        self.types.execFormat.check x.execFormat
        && all self.types.kernelFamily.check (attrValues x.families);
    };

    types.kernel = enum (attrValues self.kernels);
    ...
    kernels =
      ...
      in
      setTypes self.types.openKernel {
    ...
    types.abi = enum (attrValues self.abis);

    abis = setTypes self.types.openAbi {
      cygnus = { };
      msvc = { };
    ...
    types.parsedPlatform = mkOptionType {
      ...
      check =
        ...
        self.types.cpuType.check cpu
        && self.types.vendor.check vendor
        && self.types.kernel.check kernel
        && self.types.abi.check abi;
    };
    ...
    mkSystem =
      components:
      assert self.types.parsedPlatform.check components;
