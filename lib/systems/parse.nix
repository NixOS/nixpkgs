...
with lib.attrsets;
with lib.strings;
with (import ./inspect.nix {inherit lib;}).predicates; let
...
in rec {
  ...
  mkSkeletonFromList = l:
    {
      ...
      "3" =
        ...
        else if (elemAt l 2 == "mingw32")
        # autotools breaks on -gnu for window
        then
          {
            cpu = elemAt l 0;
            vendor = elemAt l 1;
  ...
  tripleFromSystem = {
    ...
    assert isSystem sys; let
      optExecFormat =
        lib.optionalString (kernel.name
        == "netbsd"
