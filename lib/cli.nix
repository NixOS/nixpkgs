...

{
  ...
  */
  ...
  /**
    Similar to `toCommandLineGNU`, with the exception that this returns a string of shell-escaped arguments instead of a list.
  */
  toCommandLineShellGNU =
    options: attrs: lib.escapeShellArgs (lib.cli.toCommandLineGNU options attrs);

  /**
    Convert an attribute set into a list of GNU-style command line options.
  ...
  */
  toCommandLineGNU =
    {
      isLong ? (optionName: lib.stringLength optionName > 1),
      explicitBool ? false,
  ...
  /**
    Similar to `toCommandLine`, with the exception that this returns a string of shell-escaped arguments instead of a list.
  */
  toCommandLineShell =
  ...
  toCommandLine =
    ...
    let
      ...
      renderOption =
        ...
        k: v:
        if v == null || v == false && !explicitBool then
          [ ]
        else if v == true && !explicitBool then
          [ option ]
        else
          let
            arg = formatArg v;
