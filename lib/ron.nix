{ lib, ... }:
{
  /**
    ...
    - Enums: `{ _type = "ron-enum"; variant = "..."; value = [...]; }`
  ...
  */
  ...
  /**
    ...
    lib.ron.mkEnum { variant = "Point"; value = [ 10 20 ]; }
    => { _type = "ron-enum"; variant = "Point"; value = [ 10 20 ]; }
    ```

    # Type

    ```
    mkEnum :: { variant :: String, value :: [Any] } -> RonEnum
    ```
    ...
    value
    : Optional list of values associated with the variant (defaults to empty list)
  */
  mkEnum =
    {
      value ? [ ],
  ...
  /**
    ...
    => { _type = "ron-map"; value = [ { key = 1; value = "one"; } { key = 2; value = "two"; } ]; }
    ```
  ...
  */
  ...
  /**
    Creates a RON raw value.

    Raw values are used to represent RON syntax that cannot be automatically
    converted to Nix values, or when you need precise control over the output format.

    # Example

    ```nix
    lib.ron.mkRaw "CustomType::new()"
    => { _type = "ron-raw"; value = "CustomType::new()"; }
    ```
    ...
    value
    : The raw RON string to include in the output
  */
  mkRaw = value: {
    _type = "ron-raw";
    inherit value;
  ...
  /**
    ...
    => { _type = "ron-tuple"; value = [ 1 "hello" true ]; }
    ```
          ...
          set =
            if value ? _type then
              if value._type == "ron-raw" then
                ...
                if value ? value then
                  assert lib.assertMsg (builtins.isList value.value) "enum type must have a list of values.";
                ...
                assert lib.assertMsg (value ? value) "map type must have a value.";
                assert lib.assertMsg (builtins.isList value.value) "map type value must be a list.";
                assert lib.assertMsg (builtins.all builtins.isAttrs value.value)
                ...
                assert lib.assertMsg (value ? value) "tuple type must have a value.";
                assert lib.assertMsg (builtins.isList value.value) "tuple type value must be a list.";
