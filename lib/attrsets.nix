...
rec {
  ...
  */
  ...
  /**
    Apply a function to each leaf (non‐attribute‐set attribute) of a tree built
    out nested attribute sets, returning the results of the function as a list,
    ...
    :::
  */
  mapAttrsToListRecursive = f: set: mapAttrsToListRecursiveCond (path: set: true) f set;

  /**
    Determine the nodes of a tree built out of nested attribute sets by
    applying a predicate, then apply a function to the leaves, returning the
    results as a list, ordered lexicographically by their attribute paths.
