            ...
            type = submodule (contract: {
              options = {
                ...
                instances = mkOption {
                  description = ''
                    ...
                    ```
                  '';
                  # `mergeableRaw` (not plain `raw`) at the leaf: `instances` is
                  # the one `contracts.*` tree that takes definitions from multiple
                  # modules at once, so it is the only one that needs leaves to
                  # merge rather than reject a second definition. A combined node
                  # co-locating several apps has each app's routing module route a
                  # shared instance (e.g. `mail.<group>`) to the same provider;
              ...
              config.instances =
                let
                  ...
                  # `want` with `mkOptionDefault`-wrapped leaves - `instances`'s
                  # `apply` then resolves each leaf path-aware.
                  #
