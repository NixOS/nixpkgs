...
let
  ...
  # Used for the single-reference `defaultProvider` option; the nested
  # `instances` tree uses `mergeableRaw` (see below) at its leaves instead.
  ...
  # `raw` for a provider-reference tree, but with a proper leaf merge: agreeing
  # duplicate definitions collapse, genuine conflicts throw. `raw`'s own merge
  # is `mergeOneOption`, which rejects any second definition -- even an identical
  # one -- so a combined node whose co-located apps each route a shared instance
  # to the same provider would spuriously fail. `mergeEqualOption` is the
  # standard nixpkgs "merge properly" for such opaque leaves. `instances` leaves
  # are provider references that `nestedAttrsOf` recurses into (their sub-tree
  # keys `module`/`contract` are opaque to `raw`), so this merge only fires at
  # the genuine scalar leaves, where identical references are byte-equal and a
  # real conflict differs.
  mergeableRaw = raw // {
              ...
              in
              {
                options = {
                  ...
                  instances = mkOption {
                    ...
                    # this ref tree, so the routing decision has a single source
                    # of truth that retains the reference (hence its `module.loc`,
                    # needed to recover the provider name).
                    #
                    # `mergeableRaw` (not plain `raw`) at the leaf: this is the
                    # only `contracts.*` tree that takes definitions from multiple
                    # modules at once (the others -- `requests`,
                    # `resolvedInstances`, `providerRequests` -- are computed from a
                    # single default), so it is the only one that needs leaves to
                    # merge rather than reject a second definition. A combined node
                    # co-locating several apps has each app's routing module route
                    # a shared instance (e.g. `mail.<group>`) to the same provider;
                    # `raw`'s `mergeOneOption` would reject the identical second
                    # definition. `mergeEqualOption` (via `mergeableRaw`) collapses
                    # agreeing definitions and still throws on a genuine conflict.
                    type = nestedAttrsOf mergeableRaw;
