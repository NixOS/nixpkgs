{ lib, ... }:
{
  options =
    let
      ...
      # `__uncall__` testing for different types
      simpleUncall = simpleType.__uncall__;
      descriptionBasedUncall = descriptionBasedType.__uncall__;
      parametricUncall = parametricType.__uncall__ "parametricUncall" "-too-name-suffix";

      # extend testing on `__uncall__`'ed parametric type
      extendedParametricType = (parametricType.__uncall__ "extendedParametric" "fake").extend (
        final: prev: {
      ...
      # Testing `__uncall__` after extend
      extendedParametricUncall = extendedParametricType.__uncall__ "extendedParametricUncall" "fake";
