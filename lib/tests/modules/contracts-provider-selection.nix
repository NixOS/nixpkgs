...
{
  imports = [
    ./contracts-arithmetic-contract.nix

    # -- Duplicate `instances` leaf definitions (mergeableRaw) --
    # Two modules routing the SAME instance to the SAME provider must collapse to
    # one (no spurious "defined multiple times"). This is what lets a combined
    # node co-locating several apps have each app's routing module route a shared
    # domain to the same provider. `raw`'s `mergeOneOption` would reject the
    # identical second definition; `mergeableRaw`'s `mergeEqualOption` collapses.

    # 1. duplicate agree: both route `x` to `increment`.
    ({ config, ... }: { contracts.dupAgree.instances.consumer.x = config.contracts.dupAgree.providers.increment; })
    ({ config, ... }: { contracts.dupAgree.instances.consumer.x = config.contracts.dupAgree.providers.increment; })

    # 2. distinct thunks agree: two references built independently (each its own
    # `module.value` thunk) still collapse -- the merge fires at the scalar
    # sub-leaves under `nestedAttrsOf` recursion, where identical references are
    # equal. This is the real combined-node case a byte-identity check would
    # spuriously reject.
    ({ config, ... }: { contracts.dupAgree.instances.consumer.thunky = { inherit (config.contracts.dupAgree.providers.increment) module; }; })
    ({ config, ... }: { contracts.dupAgree.instances.consumer.thunky = { inherit (config.contracts.dupAgree.providers.increment) module; }; })

    # 3. genuine conflict: two DIFFERENT providers at one leaf still throws -- the
    # merge tolerates agreement, not disagreement. The conflict is on `module.loc`
    # (the field routing reads), so forcing `dupConflict.providerRequests`
    # triggers it. Asserted by demanding that path directly (not via an exposed
    # `attrsOf`-typed attr, whose type check would force the error into unrelated
    # assertions) -- same isolation pattern as `noProvider`'s `unrouted`.
    ({ config, ... }: { contracts.dupConflict.instances.consumer.x = config.contracts.dupConflict.providers.increment; })
    ({ config, ... }: { contracts.dupConflict.instances.consumer.x = config.contracts.dupConflict.providers.double; })
  ];
  ...
  config = {
    ...
    # `dupAgree`/`dupConflict` exercise duplicate `instances` leaf definitions:
    # `mergeableRaw` (via `mergeEqualOption`) collapses agreeing duplicates and
    # still throws on a genuine conflict.
    contractDefinitions.dupAgree = arithmeticInterface;
    contractDefinitions.dupConflict = arithmeticInterface;
    ...
    services.increment.arithmetic = lib.mkMerge [
      ...
      # dupConflict needs no wiring: its error fires during `instances` merge,
      # before provider computation, and laziness keeps it from affecting the
      # other assertions (same pattern as `noProvider`'s `unrouted`).
      config.contracts.dupAgree.requests
    ];
    ...
    # Both providers registered for the duplicate-merge contract types.
    contracts.dupAgree.providers.increment.module = options.services.increment.arithmetic;
    contracts.dupAgree.providers.double.module = options.services.double.arithmetic;
    contracts.dupConflict.providers.increment.module = options.services.increment.arithmetic;
    contracts.dupConflict.providers.double.module = options.services.double.arithmetic;
    ...
    # Duplicate-merge consumers: each `want` leaf is routed by two modules (see
    # the `imports` below). `x` uses `value = 5` so a collapsed route yields 6.
    contracts.dupAgree.want.consumer.x.request.value = 5;
    contracts.dupAgree.want.consumer.thunky.request.value = 5;
    contracts.dupConflict.want.consumer.x.request.value = 5;
    ...
    result = {
      ...
      # duplicate agree: two modules routing `x` to `increment` collapse; the
      # single surviving route resolves (5 + 1 = 6).
      dupAgree = config.contracts.dupAgree.results.consumer.x.value;
      # distinct thunks agree: independently-built references still collapse.
      dupAgreeThunks = config.contracts.dupAgree.results.consumer.thunky.value;
    };
