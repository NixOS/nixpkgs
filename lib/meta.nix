...
rec {
  ...
  */
  ...
  /**
    `tryEval` version of [`lib.meta.cpeFullVersionWithVendor`](#function-library-lib.meta.cpeFullVersionWithVendor). Always returns `{success = true; value = cpeFullVersionWithVendor vendor version;}`, because cpeFullVersionWithVendor always succeeds.
    ...
    ```
    tryCPEFullVersionWithVendor :: string -> string -> AttrSet
    ```

    # Examples
    :::{.example}
    ## `lib.meta.tryCPEFullVersionWithVendor` usage example

    ```nix
    lib.meta.tryCPEFullVersionWithVendor "gnu" "1.2.3"
    => {
      success = true;
      value = {
        vendor = "gnu";
  ...
  */
  ...
  /**
    `tryEval` version of [`lib.meta.cpePatchVersionInUpdateWithVendor`](#function-library-lib.meta.cpePatchVersionInUpdateWithVendor). If `cpePatchVersionInUpdateWithVendor` succeeds, returns an attribute set with `success` set to `true` and `value` set to the result. Otherwise, `success` is set to `false` and `error` is set to the string representation of the error.
