...
let
  inherit (lib)
    elem flip functionArgs isAttrs isBool isDerivation isFloat isFunction isInt
    isList isString isStorePath setFunctionArgs toDerivation toList;
  inherit (lib.lists)
  ...
  inherit (lib.options)
    ...
    showFiles showOption;
  inherit (lib.strings)
    concatMapStringsSep concatStringsSep escapeNixString isCoercibleToString;
