...
rec {
  ...
  */
  ...
  # `nestedAttrsOf elemType`: an attribute set nested to arbitrary depth whose
  # leaves are of `elemType`. Namespace nodes recurse; leaves delegate to
  # `elemType.merge`, so the element type owns how duplicate definitions at one
  # leaf combine. A leaf that must tolerate agreeing duplicates simply uses an
  # element type whose `merge` does so (e.g. `mergeEqualOption` instead of `raw`'s
  # `mergeOneOption`) -- the nested type stays generic and hard-codes no duplicate
  # handling of its own.
  nestedAttrsOf =
    elemType:
    let
