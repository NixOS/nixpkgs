    ...
    defaultFunctor =
      ...
      lib.warnIf (!isAttrs self)
        ...
        ''
        {
          ...
          type = if isAttrs self then self.__uncall__ else types.${self} or null;
          wrapped = null;
          payload = null;
          binOp = a: b: null;
        };
          ...
          lib.fix' (
            ...
            // {
              extend =
                ...
                makeExtensibleOptionType (
                  self':
                  lib.extends f rattrs self'
                  // lib.optionalAttrs (!(f self' (rattrs self') ? __uncall__)) {
                    __uncall__ =
                      let
                        applyExtendRecursively =
                          value: if isFunction value then arg: applyExtendRecursively (value arg) else value.extend f;
                      in
                      applyExtendRecursively self.__uncall__;
                  }
                );
            }
          );
        ...
        recurseIntoArgs =
          ...
          if isFunction shape then
            ...
            makeExtensibleOptionType (
              ...
              mkOptionType' self (
                {
                  __uncall__ = mkOptionType argsWithSelf;
                }
                // (acc (argsWithSelf self))
              )
            );
      in
    ...
    types = rec {
      ...
      strMatching = mkOptionType (
        self: pattern: {
          ...
          inherit (str) merge;
          functor = defaultFunctor self // {
            type = payload: self.__uncall__ payload.pattern;
      ...
      separatedString = mkOptionType (
        self: sep: {
          ...
          functor = defaultFunctor self // {
            type = payload: self.__uncall__ payload.sep;
            payload = { inherit sep; };
      ...
      attrTag = mkOptionType (
        ...
        in
        {
          ...
          nestedTypes = tags;
          functor = defaultFunctor self // {
            type = { tags, ... }: self.__uncall__ tags;
      ...
      coercedTo = mkOptionType (
        ...
        ) "coercedTo: coercedType must not have submodules (it’s a ${coercedType.description})";
        {
          ...
          substSubModules = m: self.__uncall__ coercedType coerceFunc (finalType.substSubModules m);
          typeMerge = t: null;
          functor = defaultFunctor self // {
            ...
            final: prev: {
              ...
              merge = {
                ...
                v2 =
                  { loc, defs }:
                  let
                    orig = prev.merge.v2 { inherit loc defs; };
