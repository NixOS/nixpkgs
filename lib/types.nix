...
let
  ...
  elemTypeFunctor = name:
    { elemType, ... }@payload: {
      inherit name payload;
      wrappedDeprecationMessage = makeWrappedDeprecationMessage payload;
      type = outer_types.types.${name};
      binOp = a: b:
        let merged = a.elemType.typeMerge b.elemType.functor;
        in if merged == null then null else { elemType = merged; };
    };
  makeWrappedDeprecationMessage = payload:
    { loc }:
    lib.warn ''
      The deprecated `${
        lib.optionalString (loc != null) "type."
  ...
  outer_types = rec {
    ...
    mkOptionType = {
      ...
      nestedTypes ? { }, }: {
        ...
        inherit name check merge emptyValue getSubOptions getSubModules
          substSubModules typeMerge deprecationMessage nestedTypes
          descriptionClass;
        functor = if functor ? wrappedDeprecationMessage then
          functor // {
            wrapped = functor.wrappedDeprecationMessage { loc = null; };
          }
        else
          functor;
    ...
    types = rec {
      ...
      separatedString = sep:
        mkOptionType rec {
          ...
          functor = (defaultFunctor name) // {
            ...
            binOp = lhs: rhs:
              if lhs.sep == rhs.sep then { inherit (lhs) sep; } else null;
          };
        };
      ...
      pathWith = { inStore ? null, absolute ? null, }:
        ...
        "In pathWith, inStore means the path must be absolute" mkOptionType {
          name = "path";
          description = ((if absolute == null then
            ""
          else
            (if absolute then "absolute " else "relative ")) + "path"
            + (if inStore == null then
              ""
            else
              (if inStore then
                " in the Nix store"
          ...
          check = x:
            let
              #                isInStore = builtins.match "${builtins.storeDir}/[^.].*" (toString x) != null; # broken with content-addressed derivations
              isInStore = lib.path.hasStorePathPrefix (if builtins.isPath x then
                x
              ...
              isExpectedType = (if inStore == null || inStore then
                isStringLike x
              else
                isString
                x # Do not allow a true path, which could be copied to the store later on.
              );
            in isExpectedType && (inStore == null || inStore == isInStore)
      ...
      attrsWith = let
        ...
        binOp = lhs: rhs:
          let
            ...
            placeholder = if lhs.placeholder == rhs.placeholder then
              lhs.placeholder
              ...
              null;
          in if elemType == null || lazy == null || placeholder == null then
            null
          else {
            inherit elemType lazy placeholder;
          };
      in { elemType, lazy ? false, placeholder ? "name", }:
      mkOptionType {
        name = if lazy then "lazyAttrsOf" else "attrsOf";
        description = (if lazy then "lazy attribute set" else "attribute set")
          + " of ${
             optionDescriptionPhrase
             (class: class == "noun" || class == "composite") elemType
           }";
        ...
        emptyValue = { value = { }; };
        getSubOptions = prefix:
          elemType.getSubOptions (prefix ++ [ "<${placeholder}>" ]);
        getSubModules = elemType.getSubModules;
        substSubModules = m:
          attrsWith {
        ...
        mkOptionType rec {
          ...
          functor = (defaultFunctor name) // {
            wrappedDeprecationMessage =
              makeWrappedDeprecationMessage { elemType = finalType; };
          };
    ...
    */
    mergeTypes = a: b:
      ...
      let merged = a.typeMerge b.functor;
      in if merged == null then
