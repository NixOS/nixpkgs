...
in rec {
  ...
  mkPackageOption =
    ...
    in
      mkOption {
        ...
        (throw "${concatStringsSep "." default'} cannot be found in pkgs")
        pkgs;
        ...
        (if isList example
        then "pkgs." + concatStringsSep "." example
        else example);
      };
  ...
  mergeEqualOption = loc: defs:
    ...
    then (head defs).value
      ...
      (foldl' (first: def:
        ...
        then throw "The option `${showOption loc}' has conflicting definition values:${showDefs [first def]}"
        else first) (head defs) (tail defs))
      .value;
  ...
  literalDocBook = text:
    if !isString text
    then throw "literalDocBook expects a string."
    else
      {
