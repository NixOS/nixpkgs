...
in rec {
    ...
    with builtins; let
      ...
      gitFlattenAttrs = let
        recurse = path: value:
          ...
          then lib.mapAttrsToList (name: value: recurse ([name] ++ path) value) value
  ...
  withRecursion = args @ {
    ...
    assert builtins.isInt depthLimit; let
      transform = depth:
      ...
      mapAny = with builtins; depth: v: let
        evalNext = x: mapAny (depth + 1) (transform (depth + 1) x);
      in
        if isAttrs v
        then mapAttrs (const evalNext) v
        else if isList v
        then map evalNext v
  ...
  toPretty = {
    ...
    in
      ...
      then
        ...
        in
          if multiline && length newlineSplits > 1
          then
      ...
      then
        ...
        then "<derivation ${v.drvPath or "???"}>"
        else
          "{"
          + introSpace
          + libStr.concatStringsSep introSpace (libAttr.mapAttrsToList
          (name: value: "${libStr.escapeNixIdentifier name} = ${go (indent + "  ") value};")
          v)
          + outroSpace
          + "}"
