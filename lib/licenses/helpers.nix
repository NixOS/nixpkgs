...
rec {
  ...
  */
  evaluateProperty =
    ...
    let
      OR = if permissive then lib.or else lib.and;
      AND = if permissive then lib.and else lib.or;
    in
    if license.licenseType == "simple" then
      predicate license
    else if license.licenseType == "compound" then
      if license.operator == "OR" then
        OR (evaluateProperty predicate permissive license.base) (
          evaluateProperty predicate permissive license.mod
        )
      else if license.operator == "AND" then
        AND (evaluateProperty predicate permissive license.base) (
          evaluateProperty predicate permissive license.mod
        )
      else if license.operator == "WITH" then
        AND (evaluateProperty predicate permissive license.base) (
          evaluateProperty predicate permissive license.mod
        )
      else
  ...
  */
  toSPDX =
    license:
    let
      mkBracket = x: if x.type == "compound" then "(${toSPDX x})" else toSPDX x;
    in
