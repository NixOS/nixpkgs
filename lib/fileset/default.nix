/**
  ...
  All functions accepting file sets as arguments can also accept [paths](https://nix.dev/manual/nix/stable/language/values.html#type-path) as arguments.
  ...
  This is in contrast to using [paths in string interpolation](https://nix.dev/manual/nix/stable/language/values.html#type-path), which does add the entire referenced path to the store.
  :::
...
in
{
  ...
  */
  ...
  /**
    Add the local files contained in `fileset` to the store as a single [store path](https://nix.dev/manual/nix/stable/glossary#gloss-store-path) rooted at `root`.
    ...
    : The local directory [path](https://nix.dev/manual/nix/stable/language/values.html#type-path) that will correspond to the root of the resulting store path.
      Paths in [strings](https://nix.dev/manual/nix/stable/language/values.html#type-string), including Nix store paths, cannot be passed as `root`.
  ...
  /**
    ...
    : The predicate function to call on all files contained in given file set.
      ...
      - `type` (String, one of `"regular"`, `"symlink"` or `"unknown"`): The type of the file.
        This matches result of calling [`builtins.readFileType`](https://nix.dev/manual/nix/stable/language/builtins.html#builtins-readFileType) on the file's path.
  ...
  */
  ...
  /**
    ...
    : The [path](https://nix.dev/manual/nix/stable/language/values#type-path) to the working directory of a local Git repository.
  ...
  */
  ...
  /**
    ...
    `gitTrackedWith` does not perform any filtering when the path is a [Nix store path](https://nix.dev/manual/nix/stable/store/store-path.html#store-path) and not a repository.
    ...
    :::{.warning}
    Currently this function is based on [`builtins.fetchGit`](https://nix.dev/manual/nix/stable/language/builtins.html#builtins-fetchGit)
