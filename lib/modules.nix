...
let
  inherit (lib)
    all any attrByPath attrNames catAttrs concatLists concatMap count elem
    filter findFirst flip foldl foldl' getAttrFromPath head id imap1 isAttrs
    isBool isFunction isList isString length mapAttrs mapAttrsToList
    mapAttrsRecursiveCond min optional optionalAttrs optionalString
    recursiveUpdate reverseList sort setAttrByPath toList types warnIf;
  inherit (lib.options)
...
in rec {
      ...
      byName = attr: f: modules:
        foldl' (acc: module:
          if !(builtins.isAttrs module.${attr}) then
            throw ''
              You're trying to declare a value of type `${
                builtins.typeOf module.${attr}
              }'
            ...
            acc // (mapAttrs (n: v: (acc.${n} or [ ]) ++ f module v)
              module.${attr})) { } modules;
  ...
  */
  ...
  mkDerivedConfig = opt: f:
    mkOverride (opt.highestPrio or defaultPriority) (f opt.value);
