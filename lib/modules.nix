...
in rec {
  ...
  collectModules = let
    ...
    loadModule = args: fallbackFile: fallbackKey: m:
      ...
      then unifyModuleSyntax fallbackFile fallbackKey (applyIfFunction fallbackKey m args)
      else if isList m
      then
        let
          defs = [
    ...
    in
      parentFile: parentKey: initialModules: args: collectResults (imap1 (n: x: let
  ...
  in
    ...
    then
      let
        badAttrs = removeAttrs m ["_file" "key" "disabledModules" "imports" "options" "config" "meta" "freeformType"];
      in
        if badAttrs != {}
  ...
  mergeModules' = prefix: options: configs: let
    /*
      ...
      (map (
        ...
        in
          ...
          then
            ...
            ''
          else mapAttrs (n: f module) subtree
      )
      modules);
  ...
  mergeOptionDecls = let
    coerceOption = file: opt:
      if isFunction opt
      then setDefaultModuleLocation file opt
          ...
          then throw "The option `${showOption loc}' in `${opt._file}' is already declared in ${showFiles res.declarations}."
            ...
            let
              ...
              submodules =
                ...
                else if opt.options ? options
                then map (coerceOption opt._file) options' ++ res.options
                else res.options;
            in
              opt.options
              // res
  ...
  /*
   ...
   eventually.
   */
  fixupOptionType = loc: opt: let
    options =
      opt.options
      or (throw "Option `${showOption loc}' has type optionSet but has no option attribute, in ${showFiles opt.declarations}.");
    f = tp: let
      optionSetIn = type: (tp.name == type) && (tp.functor.wrapped.name == "optionSet");
    in
      if tp.name == "option set" || tp.name == "submodule"
      then throw "The option ${showOption loc} uses submodules without a wrapping type, in ${showFiles opt.declarations}."
      else if optionSetIn "attrsOf"
      then types.attrsOf (types.submodule options)
      else if optionSetIn "listOf"
      then types.listOf (types.submodule options)
      else if optionSetIn "nullOr"
      then types.nullOr (types.submodule options)
      else tp;
  in
