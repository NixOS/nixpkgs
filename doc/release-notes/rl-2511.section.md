...
- `pytestCheckHook` now default to reference installed Python site packages inside `$out` rather than the same packages in the source directory. To opt out this behaviour, set `dontCheckWithSitePackages = true`.
