...
- `buildEnv` now takes fixed-point arguments (`finalAttrs: { }`).
  ...
  The custom overrider `<env-pkg>.override` is now deprecated in favour of the unified `<pkg>.overrideAttrs`.
  `<env-pkg>.override` is preserved in this release for backward-compatibility purposes,
  and will be removed in future releases.

- `buildEnv` now takes `derivationArgs` for additional arguments to pass to `stdenv.mkDerivation`.
  A compatibility layer is added for directly-specified arguments `nativeBuildInputs`, `buildInputs`, `passthru`, and `meta`.

- `buildEnv` now enforces `__structuredAttrs = true` for all result derivations. `postBuild`'s commands referencing other attributes should be adjusted accordingly.
