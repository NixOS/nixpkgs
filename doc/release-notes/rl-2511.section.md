...
- The `ghcInfo` and `controlPhases` functions have been removed from `haskell.lib.compose` and `haskell.lib`. They were unused and would return incorrect results.
  ...
  - To find a suitable `nativeGhc`, `buildHaskellPackages` should be used. If that is not possible, users can fall back to `ghc.bootPkgs.ghc` unless not cross compiling.
