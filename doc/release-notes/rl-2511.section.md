...
- `services.forgejo.dump.age` is now enabled by default with a value of `4w`. This default implies the deletion of dumps older than 4 weeks.
  Differing from the previous behaviour, which was not to clean up the dump directory. This new behaviour could result in data loss, depending on the use case.
