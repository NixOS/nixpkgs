...
- `joplin-desktop` has been migrated to a from source build. No state migration is necessary. In case there's no working source build available (like on Darwin at the moment), the binary build is still available at `joplin-desktop-bin`.
