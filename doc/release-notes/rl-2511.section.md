...
- The `x86_64-darwin` platform will be deprecated within the next few years, in light of Apple’s announcement that macOS 26 will be the final version to support Intel Macs.
  When support is fully dropped, we will no longer build packages for the platform or guarantee that it can build at all.
  ...
  Based on these factors, we project that support for `x86_64-darwin` will be dropped in 26.11.
