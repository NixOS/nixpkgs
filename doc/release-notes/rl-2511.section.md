...
- Metadata identifier CPE has been added, which enables vulnerability search and partial SBOM generation. Maintainers are urged to enrich their drv.meta in order to contain a matchable `drv.meta.identifiers.cpeParts.vendor` and to cross check their `drv.meta.identifiers.v1.cpe` for completeness.

- Metadata identifier pURL has been added, which enables a SBOM generation. Maintainers are urged to check their `drv.meta.identifiers.v1.purl` for completeness.
