...
- Nixpkgs configuration, specified for NixOS using `nixpkgs.config`, or using the `config` argument when importing nixpkgs, has learned to validate and merge configuration options.  If you are referencing licenses in `lib.licenses` by having this configuration be a function taking a `pkgs` arg, this may result in infinite recursion errors.  To avoid this, the configuration function has also learned the `lib` argument, which can be referenced without needing to check the licenses that form the Nixpkgs bootstrap derivations.

    If you currently have configuration that looks like this:
    ...
    You will need to update it to something like this:
    ...
    Or, if you need configuration that works with both 25.11 and 25.05:

        { pkgs, ... }@args:
        let
          lib = args.lib or pkgs.lib;
        in
        {
          allowlistedLicenses = [ lib.licenses.nasa13 ];
          blocklistedLicenses = with lib.licenses; [ gpl3Only gpl3Plus ];
