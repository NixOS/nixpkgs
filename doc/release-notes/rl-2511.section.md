...
- New hardening flags, `libcxxhardeningfast` and `libcxxhardeningextensive` were made available, corresponding to the compiler options `-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_FAST` and `-D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_EXTENSIVE` respectively.
