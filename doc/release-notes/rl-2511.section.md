...
- `buildPythonPackage` and `buildPythonApplication` now sets `enabledTestPaths = [ "." ]` by default if not specified otherwise.

  When running install checks with `pytestCheckHook`, such default value resembles `pytest`'s default behaviour of discovering tests in the current working directory. This change makes overriding `enabledTestPaths` easier by making `enabledTestPaths` always specified as a non-empty list.
