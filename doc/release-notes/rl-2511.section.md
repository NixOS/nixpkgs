...
- Added `gitConfig` option to the nixpkgs `config`, to allow for setting a default `gitConfig` for all `fetchgit` invocations.
...
- `fetchgit`: Add `gitConfig` argument to set a git config (via `$GIT_CONFIG_GLOBAL`) for the fetcher.
