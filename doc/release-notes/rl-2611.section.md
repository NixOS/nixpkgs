...
- Festival is now available as a text to speech engine.
  It can be configured in `programs.festival` with many different voices available.
  For use with Speech-dispatcher, Festival can be run as a server with `services.festival`.
  `services.festival` inherits voices and configurations from `programs.festival`.
