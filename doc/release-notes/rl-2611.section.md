...
- With the following changes, `pytestCheckHook` now prefer the installed module over the current working directory one:
  - Invoke the `pytest` command directly instead of using `python3 -m pytest`, preventing Python from prepending the current working directory to `sys.path`.
  - Take attribute/variable `pytestDefaultMode` and default it to `"importlib"` so that pytest imports the test modules without touching `sys.path`.
