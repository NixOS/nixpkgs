...
- `google-fonts` has been split into individual font packages under the namespace `google-fonts`. `google-fonts-full` has been added as a convenience package that provides all fonts in one package. The locations of font files have changed from `$out/share/fonts/truetype/*.ttf` to `$out/share/fonts/truetype/google-fonts/<font-name>/*.ttf`.
