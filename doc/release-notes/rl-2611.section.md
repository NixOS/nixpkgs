...
- `cmake`'s setup hook now takes `cmakeEntries` for structured CMake cache entry access (previously through `cmakeFlags` and `-DKEY=VALUE`).
  The Nixpkgs-provided default CMake entry values are now prepended to `cmakeEntries`.

  `cmakeConfigurePhase` used to prepend default flags to `cmakeFlags`.
  With `cmakeEntries`, an optional attribute `joinCMakeFlags` decides whether or not to prepend the constructed flags to `cmakeFlags`.
  `joinCMakeFlags` is not set by default, which makes such behaviour backward-incompatible.
  Packages that depends on the joint `cmakeFlags` after the main `cmakeConfigurePhase` should set `joinCMakeFlags = true`.

- `cmake`'s setup hook now treats `$NIX_ATTRS_JSON_FILE` as the source of truth for both `cmakeEntries` and `cmakeFlags` upon `__structuredAttrs = true`,
  providing helper commands to operate both the JSON file and the shell variables simultaneously.
