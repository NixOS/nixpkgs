...
- `fedimint` has been updated to `0.12.1`. This removes the legacy Bitcoin
  RPC configuration and Electrum support. Direct package users must migrate
  their deployment configuration following the [upstream release
  guidance](https://github.com/fedimint/fedimint/releases/tag/v0.12.1) and
  review the [0.8.0](https://github.com/fedimint/fedimint/releases/tag/v0.8.0),
  [0.9.0](https://github.com/fedimint/fedimint/releases/tag/v0.9.0), and
  [0.10.0](https://github.com/fedimint/fedimint/releases/tag/v0.10.0),
  [0.11.0](https://github.com/fedimint/fedimint/releases/tag/v0.11.0), and
  [0.12.0](https://github.com/fedimint/fedimint/releases/tag/v0.12.0) release
  notes. Do not treat the old Nixpkgs `0.7.1` package as a directly tested
  upgrade source: first move to `0.10.1`, whose release notes document
  compatibility with earlier 0.x releases. Upstream then tests `0.10.1` as an
  upgrade source for the 0.12 line. Never pass through `0.11.0` or `0.11.1`.

  Guardians in an Iroh federation must coordinate the 0.12 upgrade because
  the new P2P transport is wire-incompatible with older releases. Consensus
  pauses while neither old nor new guardians have quorum; upstream's
  [networking guidance](https://github.com/fedimint/fedimint/blob/v0.12.1/docs/networking.md)
  does not support rolling back across this transport boundary. Separately,
  once the transitional Iroh 1.0 client identity is advertised, its listener
  must remain enabled and reachable and downgrading to a binary without it is
  unsupported.

  Fedimint 0.12 also stops encrypting newly written guardian configuration
  files. Legacy encrypted configurations remain readable, but operators must
  protect filesystem and backup access to the data directory. Gateway
  operators should upgrade promptly because `0.12.1` fixes security issues in
...
- `postgresqlPackages.anonymizer` has been updated from 2.4.1 to 3.2.2. The extension must be dropped and recreated, and masking no longer runs as a superuser. See the [upgrade notes](https://postgresql-anonymizer.readthedocs.io/en/stable/UPGRADE/) for details. `pg-dump-anon` has been removed as it was removed upstream. Use `pg_dump` with a masked role instead.
...
- The JetBrains IDEs have all been moved to the top-level:
  - `jetbrains.idea` and `jetbrains.idea-oss` have been renamed to `intellij-idea` and `intellij-idea-oss` respectively.
  - `jetbrains.mps` has been renamed to `jetbrains-mps`.
  - `jetbrains.gateway` has been renamed to `jetbrains-gateway`.
  - All other IDEs have directly moved to the top-level with no prefix  (e.g. `jetbrains.pycharm` -> `pycharm`).
  - `jetbrains.plugins` remains unchanged for now.
