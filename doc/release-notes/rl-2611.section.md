...
- `ocamlPackages.buildDunePackage` now builds packages with `out` and `dev` outputs by default, unless they set `outputs` or override `installPhase`.
  The OCaml libraries are installed into `dev`, and `out` only contains executables, man pages and documentation, which removes build-time only OCaml dependencies from the closure of tools like `patdiff`.
