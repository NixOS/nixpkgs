...
- The JetBrains IDEs have all been moved to the top-level (e.g. `jetbrains.idea` -> `idea`). MPS and Gateway require a `jetbrains-` prefix (e.g. `jetbrains.mps` -> `jetbrains-mps`).
