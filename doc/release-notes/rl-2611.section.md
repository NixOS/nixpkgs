...
- `fetchFromGitea`, `fetchFromForgejo`, and `fetchFromCodeberg`'s `netrcImpureEnvVars` for private repository fetching has changed its default from
  ```nix
  [
  ...
  ```nix
  [
    "NIX_GITEA_PRIVATE_USERNAME"
    "NIX_GITEA_PRIVATE_PASSWORD"
  ]
  ```
  due to the introduction of the unified fetcher `fetchFromGitProvider` and its argument `providerName`.

  Arguments controlling such default includes `providerName`, `varPrefix`, and `varBase`.
  See `fetchFromGitProvider` for how they interact with `netrcImpureEnvVars`.
