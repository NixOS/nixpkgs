...
- `fetchPnpmCache` has been added as an alternative to `fetchPnpmDeps`. It uses
`mitm-cache` with a custom fetcher, instead of relying on pnpm's store format.
See [pnpm `fetchPnpmCache` section](#javascript-fetchPnpmCache) of the
manual for details.
