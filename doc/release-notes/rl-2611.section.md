...
- `genSecretsReplacement` have been added which generates a config file with secrets in any format (YAML, json, INI, etc.) using a nix function generator like `toJSON`. `genJqSecretsReplacement` is now implemented using `genSecretsReplacement`.
