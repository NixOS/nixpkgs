...
- `pdfium` is now built from source instead of packaging prebuilt binaries. `pdfium-binaries` has been renamed to `pdfium`. `pdfium-v8` enables V8 but not XFA; if you need XFA support, use `pdfium-full`. `pdfium-binaries-v8` has been renamed to `pdfium-full`.
