...
- `anthy` has been replaced with `anthy-unicode` as original anthy is no longer maintained. `anthy-unicode` supports XDG Base Directory specification. To enable it on existing installations, move `~/.anthy` to `$XDG_CONFIG_HOME/anthy`.
