...
- `buildDartApplication` and `buildFlutterApplication` now enable `strictDeps` and `structuredAttrs`.
  The `extraWrapProgramArgs` argument now expects a list rather than a string.
