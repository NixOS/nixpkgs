...
- `buildNpmPackage` now supports a `packageJSON` attribute (`null` by default) to optionally provide a path to `package.json` and correctly enabling rebuilds when the file changes.
