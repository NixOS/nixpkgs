...
- `command-not-found` package will be enabled by default if the system-wide package set's sources contains `programs.sqlite`. It is also by default stateless which means activating a new system generation is likely required to use a different database.
