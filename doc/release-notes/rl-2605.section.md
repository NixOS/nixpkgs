...
- `anthy` has been replaced with `anthy-unicode` as original anthy is no longer maintained. Note that `anthy-unicode` supports XDG Base Directory specification, and will migrate `~/.anthy` to `$XDG_CONFIG_HOME/anthy`. If you back up custom dictionaries or rely on this directory in other way, please update it.
