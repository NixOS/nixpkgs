...
- To help ensure reproducibility, `phpPackages.buildComposerProject` and `phpPackages.buildComposerProject2` no longer generate `.bat` files, even on Windows and Windows Subsystem for Linux machines. This can result to a change in packages' `vendorHash` if the `vendorHash` was generated on Windows/WSL.
