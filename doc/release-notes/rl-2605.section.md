...
- `lxgw-{wenkai{,-tc,-screen},neoxihei,fusionkai}` have been replaced by individual font packages under the namespace `lxgw-fonts`. The full list of font packages provided by `lxgw-fonts` can be found at the [lxgw's fonts list](https://github.com/lxgw/lxgw/blob/main/fonts.md). Configuration changes are required, see build output.
