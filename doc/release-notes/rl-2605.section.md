...
- `nodejs` is now a simple wrapper for `nodejs-slim`+`nodejs-slim.npm`+`nodejs-slim.corepack`, meaning it is no longer possible to reference or override its attributes or outputs (e.g. `nodejs.libv8` must be replaced with `nodejs-slim.libv8` or `nodejs.slim.libv8`, `nodejs_22.nativeBuildInputs` with `nodejs-slim_22.nativeBuildInputs` or `nodejs_22.slim.nativeBuildInputs`, etc.).
