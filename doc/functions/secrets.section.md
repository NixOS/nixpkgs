...
- a generator function `a -> string` to serialize the configuration from Nix to the wanted format,
- the location where the final configuration with embedded secrets should live,
- extra options, relevant to specific fomats, to fine-tune their behaviour and output.

And that the functions return an attrset containing at least a `script` attribute.

The nix expression representing the configuration is an arbitrary value of atoms, nested lists and attrsets - which is fully compatible
...
```nix
{
  mysecret = "<some unique placeholder>";
}
```
...
The bash script returning by the function in the `script` attribute is used to replace all those placeholders
...
Assuming the file `/run/secrets/path/to/secret` is readable on the target host and contains
...
in
{
  options.services.myservice = {
    settings = mkOption {
      ...
      description = ''
        All attributes of form "<name>._secret = "/path/to/file";` are considered secrets
        and will be replaced by placeholders before being stored in the nix store.
        The secrets will then be embedded in the file on the target host by reading the
        secret files at the location given in the `_secret` attribute. It is assumed
        those files will be populated with an external tool which safely transport secrets.
      '';
      default = { };
    };
  };
...
The `gen*SecretsReplacement` functions can be configured to return an attrset
