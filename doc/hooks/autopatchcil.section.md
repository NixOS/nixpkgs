...
As the hook needs information for the host where the package will be ran on, there's a required environment variable called `autoPatchcilRuntimeId` which should be filled in with the RID (Runtime Identifier) of the machine where the output will be run on. If you're using `buildDotnetModule`, this will be automatically set to `if runtimeId != null then runtimeId else systemToDotnetRid stdenvNoCC.hostPlatform.system` for you if not provided.
