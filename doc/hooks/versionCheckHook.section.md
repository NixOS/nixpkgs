...
- `versionCheckProgram`: The full path to the program that should print the `${version}` string. Defaults roughly to `${placeholder "out"}/bin/${mainProgram}`, where `mainProgram` is the first non-null value out of `env.mainProgram`, `meta.mainProgram` and `pname`, in that order. Using `$out` in the value of this variable won't work, as environment variables from this variable are not expanded by the hook. Hence using `placeholder` is unavoidable.
