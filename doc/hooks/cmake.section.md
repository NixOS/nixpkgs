...
We choose `ON` and `OFF` as the canonical CMake boolean values, consistent to CMake's JSON-reading behaviour and existing Nixpkgs Function Library functions.
Values from the `stdenv.mkDerivation`'s `cmakeEntries` argument are automatically canonicalized during `${prePhases[@]}` when `__structuredAttrs` is `true`.
...
The `cmakeEntries` value is serialised into `-D` flags with a Bash function `concatCMakeEntryFlagsTo`.
Additional Bash helper functions facilitate value referencing between `cmakeEntries` and other Bash variables/condition.
