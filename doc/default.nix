...
in pkgs.stdenv.mkDerivation {
  ...
  buildPhase = let
    # pkgs.substitute doesn't work with multiline substitutions
    pythonSectionSubstituted = let
      src = ./languages-frameworks/python.section.md;
      fromVar = "python-interpreter-table";
      toFile = pkgs.callPackage ./doc-support/python-interpreter-table {};
    in pkgs.runCommand "python.section.substituted.md" { inherit (pkgs) stdenv; } ''
      source $stdenv/setup

      cp ${src} $out
      substituteInPlace $out --replace-fail "@${fromVar}@" "$(cat ${toFile})"
    '';
  in ''
    # Substitute autogenerated content
    cp ${pythonSectionSubstituted} ./languages-frameworks/python.section.md
