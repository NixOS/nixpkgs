...
- When also loaded, the []{#ssec-gnome-hooks-glib} `glib` setup hook will populate `GSETTINGS_SCHEMAS_PATH` which `wrapGAppsHook` will then prepend to `XDG_DATA_DIRS`.
