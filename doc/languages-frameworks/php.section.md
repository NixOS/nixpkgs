...
You can access the `php` and `composerRepository` attributes from a package
constructed using the `buildComposerProject` builder.

To retrieve the `php` attribute used during the project's build, refer to the
`php` attribute. This can be accessed by using `<derivation>.php`. Likewise, to
access the `composerRepository` directory generated by the builder, use the
`<derivation>.composerRepository` attribute.
