...
::: {.note}
Derivations are nested under the `cuda` attribute due to an OfBorg quirk where evaluation failures (in this case, due to the presence of unfree licenses) cause the enclosing attribute set to be discarded, preventing discovery, evaluation, and building of the other attributes in the set.
:::
