...
3. Additional packages are defined using `build-asdf-system` in `packages.nix`
4. CL implementations are enhanced with the `pkgs` and `withPackages` attributes (`makeLisp`)
5. Packages are overridden individually using `overrideLispAttrs`
6. Packages are woven into a new scope using `packageOverlays`
