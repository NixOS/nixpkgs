...
published for that release is included. The generated metadata lives in
`deps.toml` alongside the compiler, and is regenerated by running the
`update.sh` script in the same directory:

```
$ cd pkgs/development/compilers/chicken/6/
$ ./update.sh
```
...
`overrides.nix` in the same directory; the entries there are keyed by egg name
and applied automatically by `eggDerivation`.
