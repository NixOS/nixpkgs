...
* `nugetDeps` has to be used to specify the NuGet dependency file. Unfortunately, these cannot be deterministically fetched without a lockfile. A script to fetch these is available as `passthru.fetch-deps`. This file can also be generated manually using [`nuget2nix`](https://github.com/winterqt/nuget2nix) tool, which is available in nixpkgs.
* `nugetConfig` is used to specify the project's `NuGet.config` file, which is used to specify external package repositories used by the package.
