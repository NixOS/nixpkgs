...
* `nugetSha256` is used to specify the hash of the generated Nix-compatible lockfile containing all NuGet dependencies.
  ...
  This requires that no floating version ranges are requested upstream, as we cannot deterministically fetch those. If any unstable version range is requested, an error will be thrown and you will need to use `nugetDeps` instead. This option is mutually exclusive with the `nugetDeps` attribute.
* `nugetDeps` is used to specify a manually generated Nix-compatible lockfile, in case we cannot deterministically do so.

  A script to generate this file is available as `passthru.fetch-deps`. It can also be generated manually using `nuget-to-nix` tool, which is available in Nixpkgs. If the argument is a derivation, it will be used directly and assume it has the same output as `mkNugetDeps`. This option is mutually exclusive with the `nugetSha256` attribute.
