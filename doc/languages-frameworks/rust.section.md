...
where they are known to differ. But there are ways to customize the argument:
 ...
 - To pass a completely custom target, define
   ...
   Note that some tools like Cargo uses the file name of the target JSON.
   Therefore, do not use `./path/to/target-spec.json` directly, because it will
   be renamed by Nix. Instead, place it a directory and use
   `"${./path/to/dir}"/target-spec.json"`.
