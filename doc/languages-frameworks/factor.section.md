...
connections. It also comes with the Fuel libary for Emacs that provides an
...
- `guiSupport` can be deactivated when graphics support is not necessary in the
  development environment (e.g., when developing with the Emacs Fuel IDE, but
  also see pre-configured factor packages below).
  reducing the number of the runtime dependencies.
...
Factor applications are built using factor's `deploy` facility with the
...
This is added to `all-packages.nix` as any other application would be.
