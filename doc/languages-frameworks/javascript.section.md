...
Bun based projects use a `bun.lock` file instead of a `package-lock.json` to pin dependencies. Nixpkgs provides a hooks below to for install dependencies via `bun.configHook`, which configures the project to install dependencies through `bunDeps` before building the project.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  bunDeps = bun.fetchDeps {
    inherit (finalAttrs) pname version src;
    hash = "..."; # clear this and generate a new one
  };
})
```
...
In such cases, you'd want to use `bun --filter=<bun workspace name> build` to build your project, as `npmHooks.npmBuildHook` probably won't work. A `buildPhase` based on the following example will probably fit most workspace projects:
...
If you require changing a Bun configuration setting (such as `--dry-run` or similar),
