...
`importNpmLock.buildNodeModules` is a Nix function that requires the following optional arguments:
...
It returns a derivation with a pre-built `node_modules` directory, as imported by `importNpmLock`.
