...
- `modPostBuild`: Bash commands to run after the build of the go-modules executes `go mod vendor`, and before calculating fixed output derivation's `vendorHash` (or `vendorSha256`). Note that if you need this hook, you probably need to update `vendorHash` (or `vendorSha256`) attribute when changing it.
...
Many attributes [controlling the build phase hooks](#variables-controlling-the-build-phase) are respected by both `buildGoModule` and `buildGoPackage`. Note that `buildGoModule` reads the following attributes also when building the `vendor/` go-modules fixed output derivation as well:
