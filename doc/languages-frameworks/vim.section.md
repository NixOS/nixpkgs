...
## The updater script for Vim plugins {#the-updater-script-for-vim-plugins}

Since the updater script in your system nixpkgs is probably outdated, you should instead run

```sh
# bash/zsh
alias vim-plugins-updater="$(nix-build -A vimPluginsUpdater)/bin/vim-plugins-updater"
# fish
alias vim-plugins-updater "$(nix-build -A vimPluginsUpdater)/bin/vim-plugins-updater"
```

Note that by default this will apply only until you exit the session, you definitely shouldn't paste this into your `.bashrc`/`.zshrc`.

## Adding new plugins to nixpkgs {#adding-new-plugins-to-nixpkgs}

Nix expressions for Vim plugins are stored in [pkgs/applications/editors/vim/plugins](https://github.com/NixOS/nixpkgs/tree/master/pkgs/applications/editors/vim/plugins). For the vast majority of plugins, Nix expressions are automatically generated by running [`vim-plugins-updater`](#the-updater-script-for-vim-plugins). This creates a [generated.nix](https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/editors/vim/plugins/generated.nix) file based on the plugins listed in [vim-plugin-names](https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/editors/vim/plugins/vim-plugin-names).

When the vim plugins updater detects an nvim-treesitter update, it also runs [`nvim-treesitter/update.py $(nix-build -A vimPlugins.nvim-treesitter)`](https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/editors/vim/plugins/utils/update.py) to update the tree sitter grammars for `nvim-treesitter`.
...
Sometimes plugins require an override that must be changed when the plugin is updated. This can cause issues when Vim plugins are auto-updated but the associated override isn't updated. For these plugins, the override should be written so that it specifies all information required to install the plugin, and running [`vim-plugins-updater`](#the-updater-script-for-vim-plugins) doesn't change the derivation for the plugin. Manually updating the override is required to update these types of plugins. An example of such a plugin is `LanguageClient-neovim`.

To add a new plugin, run `vim-plugins-updater add "[owner]/[name]"`. **NOTE**: This script automatically commits to your git repository. Be sure to check out a fresh branch before running.
...
```sh
vim-plugins-updater --github-token mytoken # or set GITHUB_TOKEN environment variable
```

Alternatively, set the number of processes to a lower count to avoid rate-limiting.

```sh
vim-plugins-updater --proc 1
```

To update only specific plugins, list them after the `update` command:

```sh
vim-plugins-updater update "nvim-treesitter" "mini.nvim" "mini-nvim"
```
