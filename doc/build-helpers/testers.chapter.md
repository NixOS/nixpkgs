...
- Invoke the `.online` attribute with [`nix run`](https://nix.dev/manual/nix/stable/command-ref/new-cli/nix3-run) ([experimental](https://nix.dev/manual/nix/stable/contributing/experimental-features#xp-feature-nix-command)). This runs outside the sandbox, and checks that both internal and external hyperlinks are correct.
  Example:
