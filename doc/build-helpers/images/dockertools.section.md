...
::: {.warning}
Deprecated in favor of `streamLayeredImage` + `writeImageStream`.
:::
...
::: {.warning}
Deprecated in favor of `streamNixShellImage` + `writeImageStream`.
:::
...
: Extra directories whose contents will be added to the generated image.
  ...
  If set to `null`, some convenience contents are included:

  - `/bin/sh`
    - Symlink to `${bashInteractive}/bin/bash`.
  - `/usr/bin/env`
