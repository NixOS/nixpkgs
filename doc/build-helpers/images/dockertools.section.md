...
: A bash script that will run as root inside a VM that contains the existing layers of the base image and the new generated layer (including the files from `copyToRoot`).
  ...
  :::{.caution}
  Using this attribute requires the `kvm` device to be available, see [`system-features`](https://nix.dev/manual/nix/stable/command-ref/conf-file.html#conf-system-features).
...
For this function, you specify a [store path](https://nix.dev/manual/nix/stable/store/store-path) or a list of store paths to be added to the image, and the functions will automatically include any dependencies of those paths in the image.
...
: The hash of the image after it is downloaded.
  Internally, this is passed to the [`outputHash`](https://nix.dev/manual/nix/stable/language/advanced-attributes#adv-attr-outputHash) attribute of the resulting derivation.
...
Because of this, using this function requires the `kvm` device to be available, see [`system-features`](https://nix.dev/manual/nix/stable/command-ref/conf-file.html#conf-system-features).
:::
...
: The path to the `bash` binary to use as the shell.
  ...
  This can be seen as an equivalent of the `NIX_BUILD_SHELL` [environment variable](https://nix.dev/manual/nix/stable/command-ref/nix-shell.html#environment-variables) for {manpage}`nix-shell(1)`.
