...
Nix packages are most commonly shared between machines using [HTTP, SSH, or S3](https://nix.dev/manual/nix/stable/package-management/sharing-packages.html), but a flat-file binary cache can still be useful in some situations.
...
The more idiomatic way to work with flat-file binary caches is via the [nix-copy-closure](https://nix.dev/manual/nix/stable/command-ref/nix-copy-closure.html) command.
