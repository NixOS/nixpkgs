...
Following functions help with filling out `version` and `update` fields. For many packages it should be enough to specify only `meta.identifiers.cpeParts = lib.meta.cpePatchVersionInUpdateWithVendor vendor version` to make CPE available.

#### `lib.meta.cpeFullVersionWithVendor` {#lib-meta-cpeFullVersionWithVendor}

Takes `vendor` and `version` arguments.
Copies `vendor` and `version` to the output, and sets `update` to `*`.
...
#### `lib.meta.cpePatchVersionInUpdateWithVendor` {#lib-meta-cpePatchVersionInUpdateWithVendor}

Takes `vendor` and `version` arguments.
When `version` matches `X.Y.Z` where all parts are numerical, sets `version` and `update` fields to `X.Y` and `Z`. Example:
