...
- When a package indicates a minimum supported version, also called the deployment target, you can set it in your derivation using `darwinMinVersionHook`.

  The deployment target controls how Darwin handles availability and access to some APIs. In most cases, if a deployment target is newer than the first availability of an API, that API will be linked directly. Otherwise, the API will be weakly linked and checked at runtime. On aarch64-darwin, the minimum version cannot be lower than 11.0. If a lower version is set, 11.0 will be used instead.
...
- Many of the SDK libraries are built from source (typically corresponding to the source releases for the latest release of macOS). Several of these are propagated to your package automatically. They can be accessed via the `darwin` package set.
...
- The legacy SDKs packages are still available in the `darwin` package set under their existing names, but all packages in these SDKs (frameworks, libraries, etc) have been replaced with stubs containing a readme advising you to read this documentation for how to update your derivation to use the new pattern. Note that the new SDK pattern uses the name `apple-sdk` to better align with nixpkgs naming conventions.
