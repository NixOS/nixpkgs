...
* Confirm the kernels by either booting them directly on hardware or at least smoke-testing with `nixosTests.kernel-generic`.
* File a PR against `batch-with-kernel-updates`.
...
* Merge into `batch-with-kernel-updates`.
* File as PR from `batch-with-kernel-updates` against `master`.
