...
with subtest("Logs are collected"):
    ...
    _, echo_request_packets = machine.execute(
        "tcpdump -r /var/log/ulogd.pcap icmp[0] == 8 and host 127.0.0.1"
    )
    expected, actual = 5 * 2, len(echo_request_packets.splitlines())
    assert expected == actual, (
        f"Expected {expected} ICMP request packets from pcap, got: {actual}"
    )
    _, echo_reply_packets = machine.execute(
        "tcpdump -r /var/log/ulogd.pcap icmp[0] == 0 and host 127.0.0.1"
    )
    expected, actual = 5 * 2, len(echo_reply_packets.splitlines())
    assert expected == actual, (
        f"Expected {expected} ICMP reply packets from pcap, got: {actual}"
    )
    ...
    expected, actual = 5 * 2, len(echo_request_packets.splitlines())
    assert expected == actual, (
        f"Expected {expected} ICMP request packets from logfile, got: {actual}"
    )
