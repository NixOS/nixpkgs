# The secrets CLI needs to call Nix at runtime. This would usually fail inside a
# NixOS test, as the VM has no network access.
#
# This module takes a NixOS configuration, collects every secrets-related
# derivation, and returns their combined closures. Said closure can later be
# added to `system.extraDependencies`.
{ pkgs, configuration }:
let
  inherit (pkgs) lib;
  evaluated = pkgs.nixos-secrets.jsonify {
