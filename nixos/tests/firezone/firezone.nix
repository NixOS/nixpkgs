    ...
    testScript =
      ...
      ''
        ...
        with subtest("Connect gateway"):
            ...
            # Assert both ipv4 and ipv6 were assigned and both relay ips are known
            gateway.wait_until_succeeds("journalctl --since -2m --unit firezone-gateway.service --grep 'Updated allocation.*srflx_ip4.*Some.*srflx_ip6.*Some.*relay_ip4.*Some.*relay_ip6.*Some'", timeout=30)
