...

{
  ...
  testScript = with pkgs.dockerTools; ''
    ...
    with subtest("Ensure environment variables are inherited"):
      # Check parent has variable
      docker.succeed(
          "docker load --input='${inheritance_check}'",
          "docker run --rm ${inheritance_check.imageName} bash -c 'echo \$FOO' | grep foo",
          "docker rmi ${inheritance_check.imageName}",

          # Check child has variable of parent and self
          "docker load --input='${inheritance_check2}'",
          "docker run --rm ${inheritance_check2.imageName} bash -c 'echo \$FOO' | grep foo",
          "docker run --rm ${inheritance_check2.imageName} bash -c 'echo \$BAR' | grep bar",
          "docker rmi ${inheritance_check2.imageName}",

          # Check child can owerride variables
          "docker load --input='${inheritance_check3}'",
          "docker run --rm ${inheritance_check3.imageName} bash -c 'echo \$FOO' | grep baz",
          "docker run --rm ${inheritance_check3.imageName} bash -c 'echo \$BAR' | grep bar",
          "docker rmi ${inheritance_check3.imageName}",
      )
  '';
})
