  ...
  testScript =
    ''
      ...
      with subtest("squeezelite player successfully connects to slimserver"):
          ...
          machine.wait_until_succeeds("journalctl -u squeezelite.service | grep 'slimproto:.* connected'")
          player_mac = machine.wait_until_succeeds("journalctl -eu squeezelite.service | grep 'sendHELO:.* mac:'").strip().split(" ")[-1]
