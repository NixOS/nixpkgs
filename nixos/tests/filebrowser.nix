  ...
  in {
    ...
    nodes = {
      ...
      socket = {
        services.filebrowser = {
          enable = true;
          address = "/var/lib/filebrowser/filebrowser.sock";
        };
      };
    };
    ...
    testScript = ''
      ...
      with subtest("check if it works via unix socket"):
        socket.wait_for_unit("filebrowser.service")
        socket.succeed("curl -sSf --unix-socket /var/lib/filebrowser/filebrowser.sock http://localhost | grep '<!doctype html>'")
    '';
  })
