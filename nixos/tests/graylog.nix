...
{ lib, pkgs, ... }:
{
  ...
  testScript =
    ...
    ''
      machine.start()
      machine.execute("echo \"${passwordSecret}\" > /run/graylog-passwordsecret && chmod 400 /run/graylog-passwordsecret")
      machine.execute("echo \"${rootPasswordSha2}\" > /run/graylog-rootpasswordsha2 && chmod 400 /run/graylog-rootpasswordsha2")
      machine.wait_for_unit("graylog.service")

      line = machine.wait_until_succeeds(
        "journalctl -o cat -u graylog.service | grep 'Initial configuration is accessible at 127.0.0.1:9000'"
      )

      import re

      m = re.search(r"password '([^']+)'", line)
      assert m, f"Could not extract password from: {line}"
      ...
      machine.succeed("curl -sSL http://127.0.0.1:9000/")

      machine.succeed("curl 'http://localhost:9000/api/ca/create'"
        + " -X POST"
        + f" -u 'admin:{password}'"
        + " -H 'Accept: application/json'"
        + " -H 'Content-Type: application/json'"
        + " --data-raw '{\"organization\":\"Graylog CA\"}'")

      machine.succeed("curl 'http://localhost:9000/api/renewal_policy'"
        + " -X POST"
        + f" -u 'admin:{password}'"
        + " -H 'Accept: application/json'"
        + " -H 'Content-Type: application/json'"
        + " --data-raw '{\"mode\":\"Automatic\",\"certificate_lifetime\":\"P30D\"}'")

      machine.succeed("curl 'http://localhost:9000/api/status/skip-config'"
        + " -X POST"
        + f" -u 'admin:{password}'"
        + " -H 'Accept: application/json'"
        + " -H 'Content-Type: application/json'")
