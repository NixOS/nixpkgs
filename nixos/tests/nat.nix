      ...
      environment.systemPackages = [
        (pkgs.writeScriptBin "check-connection"
          ''
            ...
            function test_http() { [[ `timeout 3 curl $ADDRESS` == "$HOSTNAME" ]] || echo "hello world" 1>&2; }
  ...
  in
  {
    ...
    testScript =
      ...
      in ''
        ...
        # We assume we are isolated from layer 2 attacks or are securely configured by disabling (like disabling forwarding by default)
        ...
        ${lib.optionalString (!nftables) ''
          ...
          server.succeed("check-connection ${clientIp} client expect-failure")
        ''}
        server.fail("timeout 3 curl http://${routerIp}:8080")
        router.fail("timeout 3 curl http://${routerIp}:8080")
