...
let
  ...
  makeRadicleTest = encryptPrivateKey: makeTest {
    name = "radicle";
    ...
    nodes = {
      seed =
        { pkgs, config, ... }:
        {
          ...
          systemd.services.creds =
            if encryptPrivateKey then
              {
                enable = encryptPrivateKey;
                ...
                serviceConfig.ExecStart = "${
                  pkgs.writeShellApplication {
                    ...
                    text = ''
                      ...
                      systemd-creds encrypt ${seed-ssh-keys.snakeOilEd25519PrivateKey} /etc/credstore-encrypted/key.cred
                    '';
                  }
                }/bin/creds";
              }
          ...
          services.radicle = {
            ...
            privateKeyFile =
              if encryptPrivateKey then
                "key.cred:/etc/credstore-encrypted/key.cred"
              else
                seed-ssh-keys.snakeOilEd25519PrivateKey;
...
in
{
  unencryptedCreds = makeRadicleTest false;
  encryptedCreds = makeRadicleTest true;
}
