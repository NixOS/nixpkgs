import ./make-test-python.nix ({ lib, ... }: let
  ...
  # The commit hash for stable VS Code in Nixpkgs
  rev = "c722ca6c7eed3d7987c0d5c3df5c45f6b15e77d1";
  ...
  vscode-server = pkgs.srcOnly {
    ...
    src = pkgs.fetchurl {
      ...
      sha256 = "16fh0ysnxa3ag0cygv4a0p5ld3rn1ala1v03f11c56lqjgr65cfc";
    };
  };
in {
  ...
  in {
    server = { ... }: {
      ...
      virtualisation.additionalPaths = [ pkgs.nodejs-12_x pkgs.nodejs-14_x ];
    };
    client = { ... }: {
  ...
  in ''
    ...
    with subtest("fails to connect when server has the wrong Node installed"):
      server.succeed("nix-env -i ${pkgs.nodejs-12_x}")
    ...
    with subtest("connects when server has the correct Node installed"):
      ...
      server.succeed("kill $(pgrep -f vscode-server)")
