...
in {
  ...
  nodes = {
    ...
    client = { nodes, pkgs, ... }: {
      ...
      virtualisation.additionalPaths = with pkgs;
        map (pkg: writeReferencesToFile pkg.drvPath) [
          curl gitMinimal cacert_with_extras
        ];
  ...
  testScript = { nodes, ... }:
    let
      ...
      createTokenReadRepo = pkgs.writeText "create-project-token-read-repo.json" (builtins.toJSON {
        id = aliceProjectId;
        name = "test_read_repo";
      ...
      createTokenReadApi = pkgs.writeText "create-project-token-read-api.json" (builtins.toJSON {
        id = aliceProjectId;
        name = "test_read_api";
        scopes = [ "read_api" ];
        access_level = 20;  # "Reporter"
