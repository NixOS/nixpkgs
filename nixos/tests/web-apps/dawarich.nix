{ ... }:
{
  ...
  testScript = ''
    ...
    api_key = machine.succeed("""
      dawarich-rails runner "puts User.find_by(email: 'demo@dawarich.app').api_key"
    """).strip()

    res = machine.succeed(f"""
      curl -f -H 'Authorization: Bearer {api_key}' http://localhost/api/v1/users/me
    """)
    ...
    machine.succeed(f"""
      curl -f -H 'Authorization: Bearer {api_key}' --json '{json.dumps(example_point)}' http://localhost/api/v1/points
    """)

    res = machine.succeed(f"""
      curl -f -H 'Authorization: Bearer {api_key}' http://localhost/api/v1/points
    """)
    ...
    machine.succeed("""
      dawarich-rails runner "Import::WatcherJob.perform_now"
    """)
    # job runs in the background so doesn't update immediately
    res = machine.wait_until_succeeds(f"""
      curl -f -H 'Authorization: Bearer {api_key}' http://localhost/api/v1/points | grep '"id":2'
    """)
