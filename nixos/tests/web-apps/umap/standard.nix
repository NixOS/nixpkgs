...
in
{
  ...
  testScript =
    ...
    let
      ...
      configFile = nodes.machine.systemd.services.umap.environment.UMAP_SETTINGS;
      package = nodes.machine.services.umap.package;
    in
    ''
      import json
      import re
      ...
      with subtest("The static stamp is keyed on the package and the settings file"):
          stamp = machine.succeed("cat /var/lib/umap/.static-stamp")
          assert "${package}" in stamp, f"package missing from stamp: {stamp}"
          assert "${configFile}" in stamp, f"settings file missing from stamp: {stamp}"
      ...
      with subtest("A stamp that no longer matches collects the statics again"):
          machine.succeed("echo stale > /var/lib/umap/.static-stamp")
          machine.systemctl("restart umap.service")
          machine.wait_for_unit("umap.service")
          machine.wait_for_file("/run/umap/umap.sock")
          machine.fail("test -e ${staticRoot}/sentinel")
          assert "${configFile}" in machine.succeed("cat /var/lib/umap/.static-stamp")
          machine.succeed("curl -sSfL -o /dev/null http://localhost/static/umap/img/marker.svg")
    '';
}
