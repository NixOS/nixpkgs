  ...
  nodes.machine =
    { pkgs, ... }:
    {
      services.sharkey = {
        ...
        environmentFiles = [ "/run/sharkey/environment" ];
        settings = {
  ...
  testScript =
    ...
    ''
      ...
      with subtest("Setting up Meilisearch API key and index"):
          ...
          machine.succeed(f"mkdir /run/sharkey; echo 'MK_CONFIG_MEILISEARCH_APIKEY={create_index_result["key"]}' > /run/sharkey/environment")
