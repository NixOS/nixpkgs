...
import ./make-test-python.nix (
  { pkgs, lib, ... }:
  {
    ...
    nodes = {
      ...
      router =
        { config, ... }:
        {
          ...
          boot.extraModulePackages = with config.boot.kernelPackages; [ dpdk-kmods ];
          ...
          services.vpp.instances.main = {
            ...
            config = {
              ...
              plugins = {
                # Disable all plugins, selectively enable necessary ones
                plugin.default.disable = true;
                plugin."dpdk_plugin.so".enable = true;
                plugin."ping_plugin.so".enable = true;
              };
    ...
    testScript = ''
      ...
      # make sure VPP initialized vlan1 correctly
      # the sleep is necessary since startupConfig executes *after* the service starts
      router.sleep(2)
      router.succeed("vppctl show int addr | grep -A1 vlan1 | grep -q fd01::1/64")
      router.succeed("vppctl show int addr | grep -A1 vlan2 | grep -q fd02::1/64")

      # wait for server, make sure httpd is reachable
