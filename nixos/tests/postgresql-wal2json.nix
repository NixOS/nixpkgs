...
let
  ...
  makePostgresqlWal2jsonTest = postgresqlPackage: let
  in makeTest {
...
in if postgresql == null then builtins.listToAttrs (map makeTestAttribute (builtins.attrNames (import ../../pkgs/servers/sql/postgresql pkgs)))
# but if postgresql is set, we're being made as a passthru test for a specific postgres + wal2json version, just run one
else makePostgresqlWal2jsonTest "postgresql" postgresql
