...

{
  ...
  testScript = ''
    ...
    machine.succeed("curl --fail http://localhost:14318/health")
    machine.succeed("systemctl show --property=Environment --value otelite.service | grep -F 'OTELITE_RETENTION_DAYS=30'")
