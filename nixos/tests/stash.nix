    ...
    nodes.machine =
      { config, ... }:
      {
        services.stash = {
          ...
          username = "test";
          password.output = config.testing.hardcoded-secret."stash-password".output;

          jwtSecretKey.output = config.testing.hardcoded-secret."jwt_secret_key".output;
          sessionStoreKey.output = config.testing.hardcoded-secret."session_store_key".output;
        ...
        testing.hardcoded-secret."stash-password" = {
          input = config.services.stash.password.input;
