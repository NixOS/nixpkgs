...
parser.add_argument(
    ...
    dest="language",
    type=str,
    help="Name of the scripting language",
    metavar="LANG",
    required=True,
)
parser.add_argument(
    ...
    dest="content_file",
    type=pathlib.Path,
    help="read script contents from FILE",
    metavar="FILE",
    required=True,
)
parser.add_argument(
    ...
    dest="input_file",
    type=pathlib.Path,
    help="read script arguments from FILE",
    metavar="FILE",
    required=True,
)
...
if "python" in args.language:
    ...
    with http_client.open(python_version_req) as response:
        assert 200 == response.status, (
            f"Failure {response.status}: Update global instance python version."
        )
...
with http_client.open(script_request) as response:
    assert 201 == response.status, (
        f"Failure {response.status}: Create {args.language} script"
    )
...
if "bash" not in args.language:
    for i in [1, 2, 3, 4, 5, 6]:
        ...
        with http_client.open(script_request) as response:
            try:
                assert 200 == response.status, (
                    f"Failure {response.status}: Retrieve {args.language} deployment status"
                )
...
with http_client.open(request) as response:
    assert 201 == response.status, (
        f"Failure {response.status}: Run {args.language} script"
    )
