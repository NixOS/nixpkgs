  ...
  makeHostNameTest = hostName: domain: fqdnOrNull:
    ...
    makeTest {
      ...
      testScript = { nodes, ... }: ''
        ...
        assert "${lib.optionalString (domain != null) domain}" == machine.succeed("dnsdomainname").strip()
        assert (
            "${hostName}"
            == machine.succeed(
        ...
        fqdn_and_host_name = "${lib.optionalString (domain != null) "${hostName}.${domain} "}${hostName}"
        assert (
            fqdn_and_host_name
