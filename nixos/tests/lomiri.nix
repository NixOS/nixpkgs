...
in {
  ...
  nodes.machine = { config, ... }: {
    ...
    environment = {
      ...
      variables = {
        # To track what content-hub is doing behind the scenes
    ...
    in [
      # Help with OCR
      "d ${config.users.users.${user}.home}/.config 0700 ${user} users - -"
      "d ${config.users.users.${user}.home}/.config/terminal.ubports 0700 ${user} users - -"
      "L ${config.users.users.${user}.home}/.config/terminal.ubports/customized.colorscheme - - - - ${terminalColors}"
      "L ${config.users.users.${user}.home}/.config/terminal.ubports/terminal.ubports.conf - - - - ${terminalConfig}"
    ];
  };
