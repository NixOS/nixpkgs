...
in
{
    ...
    agent =
      { pkgs, ... }:
      {
        ...
        services.nezha-agent = {
          ...
          clientSecretFile = pkgs.writeText "fakeagentsecret" "fakeagentsecret";
        };
      };
    ...
    dashboard =
      { pkgs, ... }:
      {
        ...
        services.nezha = {
          ...
          agentSecretFile = pkgs.writeText "fakeagentsecret" "fakeagentsecret";
        };
      };
  };
  ...
  testScript = ''
    ...
    with subtest("Verify connection and uuid"):
        uuid = agent.succeed("uuidgen --md5 -n @dns -N ${agent_host}")
        # remove unprintable characters
        uuid = "".join([char for char in uuid if char.isprintable()])
        result = json.loads(agent.succeed(f"""
          curl --fail -X GET --header 'Authorization: Bearer {token}' \
            'http://dashboard.test/api/v1/server'
        """))
        assert uuid == result['data'][0]['uuid']
  '';
}
