  ...
  { pkgs, ... }:
  {
      ...
      server =
        ...
        }:
        {
          ...
          specialisation = {
            ...
            rfc42settings.configuration =
              { ... }:
              {
                services.ejabberd.settings = {
                  access_rules = {
    ...
    testScript =
      ...
      ''
        ...
        with subtest("config file"):
          server.succeed(
            '${nodes.server.system.build.toplevel}/specialisation/configfile/bin/switch-to-configuration test',
          )
        ...
        with subtest("rfc42 settings"):
          server.succeed(
            '${nodes.server.system.build.toplevel}/specialisation/rfc42settings/bin/switch-to-configuration test',
          )
          server.wait_for_unit("ejabberd.service")
