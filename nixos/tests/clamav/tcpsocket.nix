# Test ClamAV with TCP sockets enabled.
...
{ lib, pkgs, ... }:
{
  ...
  testScript = ''
    ...
    # Check connection over TCP works
    machine.succeed("sed '/LocalSocket/d' /etc/clamav/clamd.conf > tcp.conf")
    machine.succeed("clamdscan -c tcp.conf /opt/ok")
    machine.fail("clamdscan -c tcp.conf /opt/testfile")
