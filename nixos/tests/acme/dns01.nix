{ config, lib, pkgs, ... }: let
  ...
  dnsScript = pkgs.writeShellScript "dns-hook.sh" ''
    ...
    if [ "$1" = "present" ]; then
      ${pkgs.curl}/bin/curl --data '{"host": "'"$2"'", "value": "'"$3"'"}' http://dnsserver.test:8055/set-txt
    else
      ${pkgs.curl}/bin/curl --data '{"host": "'"$2"'"}' http://dnsserver.test:8055/clear-txt
    fi
  '';
in {
  name = "dns01";
  ...
  nodes = {
    ...
    dnsserver = { nodes, ... }: {
      networking = {
        firewall.allowedTCPPorts = [ 8055 53 ];
  ...
  testScript = { ... }: ''
    ...
    with subtest("Boot and acquire a new cert"):
        ...
        check_domain(client, cert, "toodeep.nesting." + cert, fail=True)
        check_domain(client, cert, "whatever." + cert)
  '';
}
