import ./make-test-python.nix (
  { pkgs, lib, ... }:
    ...
    let
      ...
      mkRadicaleProps = tag: pkgs.writeText "Radicale.props" (builtins.toJSON {
        inherit tag;
      });

      writeLines = name: eol: lines: pkgs.writeText name (lib.concatMapStrings (l: "${l}${eol}") lines);

      prodid = "-//NixOS test//EN";
      dtstamp = "20231129T194743Z";

      writeICS = { uid, summary, dtstart, dtend }: writeLines "${uid}.ics" "\r\n" [
        "BEGIN:VCALENDAR"
        "VERSION:2.0"
        "PRODID:${prodid}"
        "BEGIN:VEVENT"
      ...
      writeVCF = { uid, name, displayName, email }: writeLines "${uid}.vcf" "\r\n" [
