...
let
  testScriptPreamble =
    ...
    ''
      ...
      def check_generation(generation: int, tries_left=0, tries_failed=0) -> list[str]:
          conf_files = machine.succeed(
              f"grep --files-with-matches 'version Generation {generation} NixOS' /boot/loader/entries/nixos-*.conf"
          ).split("\n")
    ...
    { pkgs, lib, ... }:
    {
      ...
      testScript =
        ...
        ''
          ...
          conf_files = check_generation(1)
          machine.succeed(
              f"grep -q 'title NixOS (something)' {" ".join(conf_files)}"
          )
          machine.succeed(
              f"grep 'sort-key something' {" ".join(conf_files)}"
          )
      ...
      nodes = {
        machine =
          { nodes, lib, ... }:
          {
            ...
            specialisation."+something+-+that+-+breaks-parsing+-+".configuration = {
              boot.loader.systemd-boot.sortKey = "something";
            };
          };
      };
