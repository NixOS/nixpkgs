...
in {
  ...
  matrix.implementation.choice.networkd.extraConfig = { ... }: {
    _module.args.networkd = true;
  };
  matrix.implementation.choice.scripted.extraConfig = { ... }: {
    _module.args.networkd = false;
  };
  matrix.testcase.choice = {
    loopback.extraConfig = {
      nodes.client = { pkgs, ... }: {
