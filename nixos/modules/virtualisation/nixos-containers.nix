...
stateDirectory = "/var/lib/${configurationPrefix}containers";
  ...
  serviceDirectives = cfg: {
    ...
    RuntimeDirectory = lib.optional cfg.ephemeral "${configurationDirectory}/%i";
...

{
  ...
  config = mkIf (config.boot.enableContainers) (let
    ...
    warnings = flatten [
      (optional (config.virtualisation.containers.enable && versionOlder config.system.stateVersion "20.09") ''
        Enabling both boot.enableContainers & virtualisation.containers on system.stateVersion < 20.09 is unsupported.
      '')
    ];
  ...
  in {
          ...
          if containerConfig.autoStart then
            {
              ...
              restartTriggers = [
                ...
                config.environment.etc."${configurationDirectory}/${name}.conf".source
              ];
    ...
    environment.etc =
      ...
      in mapAttrs' (name: cfg: nameValuePair "${configurationDirectory}/${name}.conf"
      { text =
          ''
