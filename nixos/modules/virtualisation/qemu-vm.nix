...
let
  ...
  efiPrefix = if pkgs.stdenv.hostPlatform.isx86 then
...
in {
  ...
  config = {
    ...
    in mkVMOverride (cfg.fileSystems // {
      ...
      "/boot" = mkIf cfg.useBootLoader
        ...
        {
          device = "${
              lookupDriveDeviceName "boot" cfg.qemu.drives
            }2"; # 2 for e.g. `vdb2`, as created in `bootDisk`
          fsType = "vfat";
