...
in {
  options = {
    hardware.nvidia = {
      enable = lib.mkEnableOption (lib.mdDoc ''
        install nvidia device drivers and files.
        This does not integrate with Wayland or X11.
        NVIDIA devices will be exposed for use, i.e. for CUDA.
      '');
  ...
  in lib.mkMerge [
    # Only include driver dependencies
    (lib.mkIf enabled {
      assertions = [
        {
    ...
    (lib.mkIf hasNvidiaVideoDriver {
      assertions = [
        {
          assertion = primeEnabled -> hasNvidiaVideoDriver;
          message = ''
            PRIME requires integration with X11 and/or Wayland.
          '';
        }
      ...
      # hasNvidiaVideoDriver implies enabled, for backwards compatibility
      ...
      services.xserver.drivers =
        ...
        ++ (lib.optional hasNvidiaVideoDriver {
          name = "nvidia";
      ...
      boot = {
        kernelModules = lib.optionals config.services.xserver.enable ["nvidia" "nvidia_modeset" "nvidia_drm"];
