...
let
  ...
  toOpenSSLMultiVal = name: list: listToAttrs (
    imap1
      (i: val: {
  ...
  toOpenSSLMultiVals = attrs: pipe attrs [
    (mapAttrsToList
      (name: value:
        if (isList value)
        then (toOpenSSLMultiVal name value)
        else { ${name} = value; }
      )
    )
    mergeAttrsList
  ...
  config = {
    ...
    scripts = {
      ...
      key = pkgs.writeShellScript "${name}-mkkey" (
        ...
        ''
        else abort "Unknown key type: ${elemAt (attrNames key) 0}"
      );
