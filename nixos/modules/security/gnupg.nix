    ...
    keyring = lib.mkOption {
      ...
      type = types.attrsOf (types.submodule ({name, config, ...}: {
        options.passwordGpg = lib.mkOption {
          ...
          description = ''
            Encrypted password of an OpenPGP key.
            Set to null to send only the public part.
          '';
          default = cfg.store + "/keygrip/${name}.gpg";
        ...
        options.ssh = lib.mkOption {
          ...
          default = [ "ssh" "root@${networking.hostName}.${networking.domain}" ];
          defaultText = "ssh root@\${config.networking.hostName}.\${config.networking.domain}";
        };
              ...
              while IFS= read -r line; do
                case "$line" in
                  ("gpg-connect-agent: no gpg-agent running in this session")
                    ...
                    exit 0;;
                  ("S KEYINFO ${name} "?" "?" "?" 1 "*)
                    echo >&2 "gpg-agent: keygrip ${name}: already in cache, not sending."
                    exit 0;;
                  (OK)
