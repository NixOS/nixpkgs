...
with lib; let
  ...
  certToConfig = cert: data: let
    ...
    protocolOpts =
      ...
      then ["--http" "--http.port" data.listenHTTP]
  ...
  inheritableModule = isDefaults: {config, ...}: let
    defaultAndText = name: default: {
      ...
      defaultText =
        if isDefaults
        then default
        else literalExpression "config.security.acme.defaults.${name}";
    };
  in {
    options = {
      ...
      enableDebugLogs =
        mkEnableOption "debug logging for this certificate"
        // {
  ...
  config = mkMerge [
    (mkIf (cfg.certs != {}) {
      ...
      warnings = filter (w: w != "") (mapAttrsToList (cert: data:
        if data.extraDomains != "_mkMergedOptionModule"
        then
          ''
