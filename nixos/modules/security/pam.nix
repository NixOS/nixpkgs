...
let
  ...
  motd = if config.users.dynamicMotd
    then "/run/motd.dynamic" # generated by pam_motd.so
    else (pkgs.writeText "motd" config.users.motd);

  motdScript = optionals config.users.dynamicMotd
     [ { source = pkgs.writeText "update-motd" ''
           #!${pkgs.bash}/bin/bash
           ${config.users.motd};
         '';
         target = "update-motd";
...

{
  ...
  options = {
    ...
    users.dynamicMotd = mkOption {
      default = false;
      type = types.bool;
      description = "Enable dynamic message of the day updates.";
    };
