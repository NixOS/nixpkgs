        ...
        rules =
          ...
          in
          {
            account = autoOrderRules [
              ...
              {
                name = "slurm";
                enable = config.security.pam.services."${name}".slurm.enable;
                control = "required";
                modulePath = "${pkgs.slurm}/lib/security/pam_slurm.so";
              }
              {
                name = "slurm_adopt";
                enable = config.security.pam.services."${name}".slurm.adopt.enable;
                control = "required";
                modulePath = "${pkgs.slurm}/lib/security/pam_slurm_adopt.so";
                settings = config.security.pam.services."${name}".slurm.adopt.settings;
              }
