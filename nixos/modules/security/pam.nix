  ...
  pamOpts = { config, name, ... }: let cfg = config; in let config = parentConfig; in {
    ...
    options = {
      ...
      fprintAuthSkipLidClose = lib.mkOption {
        default = config.services.fprintd.authSkipLidClose;
    ...
    config = {
      ...
      in {
        ...
        auth = autoOrderRules ([
          ...
          (let lid = pkgs.writeShellScript "lid.sh" "${pkgs.gnugrep}/bin/grep -q closed /proc/acpi/button/lid/LID0/state && exit 1; true"; in { name = "fprintd-lid"; enable = cfg.fprintAuthSkipLidClose; control = "[success=ignore default=1]"; modulePath = "${pkgs.linux-pam}/lib/security/pam_exec.so"; args = ["quiet" "${lid}"]; })
