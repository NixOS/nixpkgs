        ...
        options = {
          ...
          space_left = lib.mkOption {
            ...
            description = ''
              ...
              {option}`space_left_action`. If the value of {option}`space_left` is specified as a whole number, it is interpreted as an absolute size in megabytes
              ...
              {option}`log_file` is 2 gigabytes in size, and {option}`space_left` is set to 25%, then the audit daemon sets {option}`space_left` to approximately 500 megabytes.
