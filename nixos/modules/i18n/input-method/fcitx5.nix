...
let
  ...
  mkFcitxKeyValue = lib.flip lib.generators.mkKeyValueDefault "=" {
    mkValueString = v:
      if true == v then "True"
  ...
  config = mkIf (im.enabled == "fcitx5") {
    ...
    environment.etc =
      ...
      lib.attrsets.mergeAttrsList [
        (optionalFile "config" (lib.generators.toINI { mkKeyValue = mkFcitxKeyValue; }) cfg.settings.globalOptions)
        (optionalFile "profile" (lib.generators.toINI { mkKeyValue = mkFcitxKeyValue; }) cfg.settings.inputMethod)
        (lib.concatMapAttrs
          (name: value: optionalFile
            "conf/${name}.conf"
            (lib.generators.toINIWithGlobalSection { mkKeyValue = mkFcitxKeyValue; })
            value)
          cfg.settings.addons)
      ];
