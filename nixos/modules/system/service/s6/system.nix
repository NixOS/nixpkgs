...
let
  ...
  serviceInfo =
    ...
    in
    {
      ...
      finishScript = lib.optionalString (
        service.process.type != "oneshot"
  ...
  oneshotScripts = mapAttrs (name: info: pkgs.writeText "s6-oneshot-${name}" info.oneshotScript) oneshotInfos;
  upFiles = mapAttrs (name: info: pkgs.writeText "s6-up-${name}" "${shell} ${oneshotScripts.${name}}") oneshotInfos;
  ...
  '' + concatMapStrings (name: (
      ...
      ''
        ...
        ${if info.type == "oneshot" then "cp ${upFiles.${name}} $out/${name}/up\n" else "cp ${runScripts.${name}} $out/${name}/run && chmod 0755 $out/${name}/run\n"}
        echo '${info.type}' > $out/${name}/type
        ${lib.optionalString (info.type == "longrun" && info.finishScript != "") "cp ${finishScripts.${name}} $out/${name}/finish && chmod 0755 $out/${name}/finish\n"}
        ${lib.optionalString (info.notificationFd != "") "echo '${info.notificationFd}' > $out/${name}/notification-fd\n"}
        ${concatMapStrings (d: "ln -sfn ../../${d} $out/${name}/dependencies.d/${d}\n") info.deps}
      ''
    )) (builtins.attrNames infos));

  s6RcDb = pkgs.runCommand "s6-rc-db" {
...
modularServiceConfiguration = lib.services.configure {
    ...
    extraRootModules = [
      (configDataPathModule { baseDir = "/etc/s6/system-services"; })
    ];
  };
in
{
  ...
  options.system.services = mkOption {
    description = ''
      A collection of NixOS modular services configured as s6 services.
    '';
