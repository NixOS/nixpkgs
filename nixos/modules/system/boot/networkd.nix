      ...
      sectionLink = checkUnitConfig "Link" [
        ...
        (assertValueOneOf "RequiredForOnline" (boolValues ++ (
          let
            ...
            operationalStateRanges = lib.concatLists (imap0 (i: min: map (max: "${min}:${max}") (drop i operationalStates)) operationalStates);
          in
  ...
  commonOptions = visible: {
    ...
    units = lib.mkOption {
      ...
      type = with lib.types; attrsOf (submodule (
        { name, config, ... }:
        { options = lib.mapAttrs (_: x: x // { internal = true; }) concreteUnitOptions;
          config = {
