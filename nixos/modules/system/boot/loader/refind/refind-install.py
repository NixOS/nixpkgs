...
def generate_config_entry(
    profile: str, gen: str, special: bool, group_name: str
) -> str:
    ...
    if len(specialisation_list) > 0:
        ...
        entry += config_entry(
            False, boot_spec, f"NixOS {group_name} Generation {gen}", str(time)
        )
    return entry
...
def install_bootloader() -> None:
    ...
    dest_path = os.path.join(
        ...
        "efi",
        "boot" if config("efiRemovable") else "refind",
        boot_file,
    )
    ...
    if config("canTouchEfiVariables"):
        if config("efiRemovable"):
            ...
            if matches := re.findall(
                r"Boot([0-9a-fA-F]{4})\*? rEFInd", efibootmgr_output
            ):
