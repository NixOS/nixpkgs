...
let
  ...
  systemdBootBuilder = pkgs.substituteAll rec {
    ...
    inherit (cfg) consoleMode graceful editor installDeviceTree;
...
in {
  ...
  options.boot.loader.systemd-boot = {
    ...
    installDeviceTree = mkOption {
      ...
      description = ''
        Install the devicetree blob specified by
        `config.hardware.deviceTree.name` to the ESP and instruct systemd-boot
        to pass this DTB to linux.
      '';
    };
