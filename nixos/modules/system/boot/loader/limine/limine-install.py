...
def get_copied_path_uri(path: str, target: str) -> str:
  ...
  if can_use_direct_paths:
    result = f'uuid://{root_fs_uuid}{path}'
  else:
...
def config_entry(levels: int, toplevel: str, label: str, comment: str, kernel_path: str, kernel_params: str, init: str, initrd_path: Optional[str], initrd_secrets: Optional[str]) -> str:
  entry = '/' * levels + label + '\n'
  entry += f'comment: {comment}\n'
  entry += 'protocol: linux\n'
  entry += 'kernel_path: ' + get_kernel_uri(kernel_path) + '\n'
  entry += ('cmdline: ' + 'init=' + init + ' ' + kernel_params).strip() + '\n'
  if initrd_path:
    entry += f'module_path: ' + get_kernel_uri(initrd_path) + '\n'
  ...
  if initrd_secrets:
    ...
    if os.system(initrd_secrets + " " + initrd_secrets_path_temp) != 0:
...
def generate_config_entry(profile: str, gen: str) -> str:
  boot_spec = json.load(open(get_system_path(profile, gen) + '/boot.json', 'r'))

  kernel_path = boot_spec['org.nixos.bootspec.v1']['kernel']
  initrd_path = boot_spec['org.nixos.bootspec.v1'].get('initrd')
  initrd_secrets = boot_spec['org.nixos.bootspec.v1'].get('initrdSecrets')
  init_path = boot_spec['org.nixos.bootspec.v1']['init']
  kernel_params = ' '.join(boot_spec['org.nixos.bootspec.v1']['kernelParams'])

  entry = config_entry(2, boot_spec['org.nixos.bootspec.v1']['toplevel'], f'Generation {gen}', boot_spec['org.nixos.bootspec.v1']['label'], kernel_path, kernel_params, init_path, initrd_path, initrd_secrets)
  for spec in boot_spec['org.nixos.specialisation.v1']:
    spec_boot_spec = boot_spec['org.nixos.specialisation.v1'][spec]

    kernel_path = spec_boot_spec['org.nixos.bootspec.v1']['kernel']
    initrd_path = spec_boot_spec['org.nixos.bootspec.v1'].get('initrd')
    initrd_secrets = spec_boot_spec['org.nixos.bootspec.v1'].get('initrdSecrets')
    init_path = spec_boot_spec['org.nixos.bootspec.v1']['init']
    kernel_params = ' '.join(spec_boot_spec['org.nixos.bootspec.v1']['kernelParams'])

    entry += config_entry(2, spec_boot_spec['org.nixos.bootspec.v1']['toplevel'], f'Generation {gen}-{spec}', spec_boot_spec['org.nixos.bootspec.v1']['label'], kernel_path, kernel_params, init_path, initrd_path, initrd_secrets)
  return entry
...
def main():
  ...
  if os.path.exists(limine_dir + '/kernels'):
    print(f'nuking {limine_dir}/kernels')
    shutil.rmtree(limine_dir + '/kernels')

  os.makedirs(limine_dir + "/kernels")
  ...
  if config('installBios'):
    ...
    limine_deploy_args = [limine_binary, 'bios-install', disk_device]
    ...
    if device == 'nodev':
      print("note: boot.loader.limine.installBios is set, but device is set to nodev, only the stage 2 bootloader will be installed.", file=sys.stderr)
      return
    else:
      try:
