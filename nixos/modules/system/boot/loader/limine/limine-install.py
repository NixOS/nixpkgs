...
def generate_config_entry(profile: str, gen: Optional[str], spec: Optional[str]) -> str:
  ...
  return textwrap.dedent(f'''//{entry_name}
    protocol: linux
  ...
  if config('canTouchEfiVariables'):
    ...
    if config('hostArchitecture', 'bits') == 32:
      if cpu_family == 'x86':
        ...
        raise Exception(f'Unsupported 32-bit CPU family: {cpu_family}')
    elif config('hostArchitecture', 'bits') == 64:
      if cpu_family == 'x86':
        boot_file = 'BOOTX64.EFI'
      else:
        raise Exception(f'Unsupported 64-bit CPU family: {cpu_family}')
    else:
      raise Exception(f'Unsupported CPU bits: {config("hostArchitecture", "bits")}')
    ...
    raise Exception("Limine can only be installed on EFI systems")
