...
def get_dest_path(path: str, target: str) -> str:
    ...
    return os.path.join(limine_dir, target, dest_file)
...
def bootjson_to_bootspec(bootjson: dict) -> BootSpec:
    ...
    xen = None
    if 'org.xenproject.bootspec.v1' in bootjson:
...
def main():
    ...
    limine_xen_dir = os.path.join(limine_dir, 'xen')
    if os.path.exists(limine_xen_dir):
        print(f'nuking {limine_xen_dir}')
