  ...
  mergeAnswer = winners: locs: defs:
    ...
    in
      if defs == [ ] then abort "This case should never happen."
        ...
        if winner == [ ]
        then abort "Give a valid list of winner"
        else
          if inter == [ ]
          then mergeOneOption locs defs
          else
            if findWinner values winner
            then
              winner
            else
    ...
    in
      if (val == "") then "\"\""
        ...
        then val
          ...
          if all isNumber (stringToCharacters val)
          then val
          else
            if substring 0 2 val == "0x"
            then val
