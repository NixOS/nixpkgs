...

{
  ...
  options = {
    ...
    boot.initrd.network.tailscale.enable = mkOption {
      type = types.bool;
      default = false;
      description = lib.mdDoc ''
        Starts an Tailscale client during initrd boot. It can be used to e.g.
    ...
    boot.initrd.network.tailscale.configuration = mkOption {
  ...
  config = mkIf (config.boot.initrd.network.enable && cfg.enable) {
    ...
    # Add kernel modules needed for Tailscale
    boot.initrd.kernelModules = [ "tun" "tap" ];

    # Add Tailscale and ip binaries to the initrd
    ...
    boot.initrd.secrets = {
      "/etc/tailscale.ovpn" = cfg.configuration;
    };

    # openvpn --version would exit with 1 instead of 0
    boot.initrd.extraUtilsCommandsTest = ''
      $out/bin/tailscale --show-gateway
    '';
