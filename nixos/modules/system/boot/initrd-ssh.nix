...

{
  ...
  imports =
    map (opt: lib.mkRemovedOptionModule ([ "boot" "initrd" "network" "ssh" ] ++ [ opt ]) ''
  ...
  config = let
    ...
    initrdKeyPath = path: if lib.isString path
      then path
  ...
  in lib.mkIf enabled {
    ...
    boot.initrd.network.postCommands = lib.mkIf (!config.boot.initrd.systemd.enable) ''
      ...
      mkdir -p /root/.ssh
      ${lib.concatStrings (map (key: ''
        echo ${lib.escapeShellArg key} >> /root/.ssh/authorized_keys
      '') cfg.authorizedKeys)}
      ${lib.concatStrings (map (keyFile: ''
    ...
    boot.initrd.systemd = lib.mkIf config.boot.initrd.systemd.enable {
      ...
      users.root.shell = lib.mkIf (config.boot.initrd.network.ssh.shell != null) config.boot.initrd.network.ssh.shell;
