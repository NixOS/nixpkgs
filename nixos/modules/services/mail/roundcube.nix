  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.roundcube-setup = mkMerge [
      ...
      })
      {
        ...
        script = let
          postgresBinary = if cfg.database.host == "localhost" then "psql" else "${pkgs.postgresql}/bin/psql";
          psql = "${lib.optionalString (!localDB) "PGPASSFILE=${cfg.database.passwordFile}"} ${postgresBinary} ${lib.optionalString (!localDB) "-h ${cfg.database.host} -U ${cfg.database.username} "} ${cfg.database.dbname}";
        in
        ''
