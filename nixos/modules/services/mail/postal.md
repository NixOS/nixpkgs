...
in {
  networking.firewall.allowedTCPPorts = [ 443 465 ];
  ...
  services.nginx = {
    ...
    virtualHosts.${config.services.postal.domain} = {
      enableACME = true;
      forceSSL = true;
      locations."/".proxyPass = "http://localhost:${toString config.services.postal.config.web_server.default_port}";
    };
  };
...
`SMTP_PASSWORD` environment variable, and ensure your SMTP
settings are correct (`services.postal.config.smtp`).

You can now test sending mail from the CLI:

```sh
su postal -c "postal test-app-smtp you@example.com"
```
