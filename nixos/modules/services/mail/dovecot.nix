...
let
  ...
  sievePlugins = concatStringsSep " " (
    cfg.sieve.plugins
    ++ optional (
      cfg.imapsieve.mailbox != [ ] && versionOlder cfg.package.version "2.4"
    ) "sieve_imapsieve"
...
in
{
  ...
  options.services.dovecot2 = {
    ...
    imapsieve.mailbox = mkOption {
      default = [ ];
      description = "Configure Sieve filtering rules on IMAP actions, this option is no longer used starting from Dovecot version 2.4.";
      type = types.listOf (
        types.submodule (
          { config, ... }:
          {
  ...
  config = mkIf cfg.enable {
    ...
    services.dovecot2.settings =
      ...
      {
        dovecot_config_version = mkIf (versionAtLeast cfg.package.version "2.4") (
          mkDefault cfg.package.version
        );
        dovecot_storage_version = mkIf (versionAtLeast cfg.package.version "2.4") (
          mkDefault cfg.package.version
        );
        ...
        if versionAtLeast cfg.package.version "2.4" then
          {
            ...
            sieve_script_bin_path = "${baseDir}/sieve/%{user|username|lower}";
    ...
    assertions = [
      {
        assertion = versionAtLeast cfg.package.version "2.4" -> cfg.imapsieve.mailbox == [ ];
      ...
      {
        assertion =
          versionAtLeast cfg.package.version "2.4"
          ->
