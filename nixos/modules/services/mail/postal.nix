...
in
{
            ...
            tls_certificate_path = mkOption {
              ...
              defaultText = literalExpression ''
                if config.${opt.enableACME} then
                  ...
                  null;
              '';
              description = "The path to the SMTP server's TLS certificate. When not using [](#opt-services.postal.enableACME), you are responsible for restarting `postal-smtp.service` after the certificate is renewed.";
            };
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    (mkIf cfg.enableACME {
      security.acme.certs.${cfg.settings.postal.smtp_hostname} = {
        group = config.users.users.${cfg.user}.group;
        reloadServices = [ "postal-smtp.service" ];
      };
