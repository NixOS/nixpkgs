--[[--
- open the `stalwart-cli apply` .ndjson input (arg #1) and stalwart json schema (path in arg #3)
...
- write to the output file (path in arg #2)
--]]--
...
local json_object_mt = { __jsontype = "object" }
...
do
	...
	schema = json.decode((assert(fi:read("*a"), "failed to read schema file")))
...
do
	...
	local function check_id_ty_match(parent, id, got, expected)
		...
		do
			-- sometimes id referent types are like "Account/Group", which means they expect to get a group account, so we split that here
	...
	local function recursearch(deps, ty, val, parent)
		...
		elseif beh.kind == "list" then
			if jty(val) ~= "array" then return err(parent, "expected `%s` to be an array of `%s` (got `%s`)", nameof(beh), beh.of, jty(val)) end
			-- if we don't do this, empty tables are encoded as arrays, which aren't valid
			local new = setmetatable({}, json_object_mt)
			for i, elem in pairs(val) do
				new[tostring(i - 1)] = recursearch(deps, beh.of, elem, parent .. "[" .. i .. "]") or elem
			end
			return new
		elseif beh.kind == "set" then
			if jty(val) ~= "array" then return err(parent, "expected `%s` to be a unique array of `%s` (got `%s`)", nameof(beh), nameof(beh.of), jty(val)) end
			-- see above
			local new = setmetatable({}, json_object_mt)
			for i, elem in pairs(val) do
				local current = parent .. "[" .. i .. "]"
				elem = recursearch(deps, beh.of, elem, current) or elem
				if new[elem] then err(current, "duplicate set key `%s`", elem) end
				new[elem] = true
			end
			return new
		...
		elseif beh.kind == "obj" then
			...
			for k, v in pairs(val) do
				...
				val[k] = recursearch(deps, vty, v, parent .. "." .. k) or v
			end
	...
	for line in input:gmatch("([^\n]+)") do
		...
		if jty(op) ~= "object" then err("<line " .. ln .. ">", "expected line to be an object (got `%s`)", jty(op))
		elseif jty(op["@type"]) ~= "string" then err("<line " .. ln .. ">", "expected `@type` field to be a string (got `%s`)", jty(op))
		elseif jty(op.object) ~= "string" then err("<line " .. ln .. ">", "expected `object` field to be a string (got `%s`)", jty(op))
		elseif jty(op.value) ~= "object" then err("<operation: " .. op["@type"] .. " " .. op.object .. ">", "expected `value` field to be an object (got `%s`)", jty(op.value))
		elseif not behavior_map[op.object] then err("<operation: " .. op["@type"] .. " " .. op.object .. ">", "unknown object type `%s`", op.object)
		else
			local deps = {}
			if op["@type"] == "update" then recursearch(deps, op.object, op.value, "<" .. op.object .. ">")
			else
				local match_fields
				...
				if op.scope ~= nil and op.scope ~= json.null then recursearch(deps, op.object, op.scope, "<" .. op.object .. " scope>") end
			end
	...
	for id, waiters in pairs(awaiting_definition) do
		for _, waiter in ipairs(waiters) do err(waiter.parent, "undefined id: %s", id) end
	end
