...
let
  ...
  pythonEnv = pkgs.python37.withPackages (ps: [ps.mailman]);

  mailmanExe = with pkgs; stdenv.mkDerivation {
    name = "mailman-" + python37Packages.mailman.version;
    unpackPhase = ":";
    installPhase = ''
      mkdir -p $out/bin
  ...
  mailmanCfg = ''
    ...
    [paths.fhs]
    bin_dir: ${pkgs.python37Packages.mailman}/bin
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.mailman = {
      ...
      preStart = "install -d -o mailman -g mailman -m 750 /var/lib/mailman";
      serviceConfig = {
