  ...
  options.services.nominatim = {
    enable = lib.mkOption {
      ...
      description = ''
        ...
        Further nginx configuration can be done by
        adapting `services.nginx.virtualHosts.<name>`. See
        [](#opt-services.nginx.virtualHosts) for further information.
      '';
    };
    ...
    ui = {
      enable = lib.mkOption {
        ...
        default = false;
        description = ''
          Whether to enable nominatim UI.
        '';
      };
    ...
    database = {
      host = lib.mkOption {
        ...
        default = "localhost";
        description = ''
          Host of the postgresql server. If this is not set to `localhost`,
          you have to create the postgresql users and database yourself, with
      ...
      apiUser = lib.mkOption {
        ...
        description = ''
          Postgresql database user with read-only permissions used for web API.
        '';
      };
      ...
      passwordFile = lib.mkOption {
        ...
        default = null;
        description = ''
          Password file used for the web API postgresql database connection.
      ...
      extraConnectionParams = lib.mkOption {
        ...
        default = null;
        description = ''
          Extra postgresql database connection parameters.
    ...
    lib.mkIf cfg.enable {
      ...
      systemd.services.nominatim-init = lib.mkIf localDb {
        ...
        script = ''
          ...
          db_exists=$(${pkgs.postgresql}/bin/psql -U postgres -tAc "$sql")
