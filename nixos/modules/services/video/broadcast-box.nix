...
in
{
  options.services.broadcast-box = {
    ...
    web = {
      ...
      openFirewall = mkOption {
        ...
        default = false;
        description = ''
          Whether to open the HTTP server port and, if enabled, the HTTPS
          redirect server port in the firewall.
        '';
      };
    };
    ...
    openFirewall = mkOption {
      ...
      default = false;
      description = ''
        Whether to open WebRTC traffic ports in the firewall. Randomly selected
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = lib.allUnique (tcpPorts ++ webPorts);
        message = ''
    ...
    systemd.services.broadcast-box = {
      ...
      serviceConfig =
        let
          priviledgedPort = lib.any (p: p > 0 && p < 1024) (udpPorts ++ tcpPorts ++ webPorts);
        in
        {
