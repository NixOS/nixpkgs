...
let
  ...
  confFile = pkgs.writeText "nntp-proxy.conf" ''
    ...
    proxy:
    {
      ...
      verbose = "${lib.toUpper cfg.verbosity}";
...

{
  ...
  options = {
    ...
    services.nntp-proxy = {
      enable = lib.mkEnableOption "NNTP-Proxy";
      ...
      upstreamMaxConnections = lib.mkOption {
        type = lib.types.int;
        default = 20;
        description = ''
      ...
      verbosity = lib.mkOption {
        type = lib.types.enum [ "error" "warning" "notice" "info" "debug" ];
        default = "info";
        example = "error";
        description = ''
          Verbosity level
