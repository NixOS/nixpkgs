...
rec {
  ...
  mkAttrsOfParams = params :
    mkAttrsOf params (lib.types.submodule {options = paramsToOptions params;});
  ...
  mkAttrsOf = param : option : description : {
    ...
    render = single (attrs:
      (paramsToRenderedStrings attrs
        (lib.mapAttrs (_n: _v: param) attrs)));
  };

  mkPrefixedAttrsOfParams = params :
    mkPrefixedAttrsOf params (lib.types.submodule {options = paramsToOptions params;});
  ...
  mkPrefixedAttrsOf = p : option : description : {
    ...
    render = prefix: attrs:
      let prefixedAttrs = lib.mapAttrs' (name: lib.nameValuePair "${prefix}-${name}") attrs;
      in paramsToRenderedStrings prefixedAttrs
           (lib.mapAttrs (_n: _v: p) prefixedAttrs);
  };
  ...
  mkPostfixedAttrsOfParams = params : description : {
    ...
    render = postfix: attrs:
      let postfixedAttrs = lib.mapAttrs' (name: lib.nameValuePair "${name}-${postfix}") attrs;
      in paramsToRenderedStrings postfixedAttrs
           (lib.mapAttrs (_n: _v: params) postfixedAttrs);
  };
