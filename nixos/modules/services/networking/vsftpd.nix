...

{
  ...
  options = {
    ...
    services.vsftpd = {
      ...
      userlistFile = lib.mkOption {
        ...
        defaultText = lib.literalExpression ''pkgs.writeText "userlist" (lib.concatMapStrings (x: "''${x}\n") cfg.userlist)'';
        description = ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    # If you really have to access root via FTP use lib.mkOverride or userlistDeny
    # = false and whitelist root
    services.vsftpd.userlist = lib.optional cfg.userlistDeny "root";

    systemd = {
      tmpfiles.rules = lib.optional cfg.anonymousUser
