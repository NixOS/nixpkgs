...
in
{
	...
	options.services.clarissa =
	{
		enable = mkOption
		{
			type = types.bool;
			default = true;
			description = "Whether to enable the network census daemon.";
		};
	...
	config = mkIf cfg.enable
	{
		systemd.services.clarissa =
		{
			...
			documentation = [ "man:clarissa(8)" "man:clar_count(1)" "https://gitlab.com/evils/clarissa" ];
