...
let
  ...
  totalIni = "/var/lib/${cfg.stateDir}/sabnzbd.ini";
in
  ...
  options = {
    services.sabnzbd = {
      ...
      secretsFile = mkOption {
        type = types.nullOr types.path;
        description = ''
          Path to an ini file containing confidential settings such as credentials.
          Settings here will be merged with the rest of the configuration (with
          the secret settings taking precedence in case of conflicts).
                ...
                type =
                  types.coercedTo
                    (types.enum [
                      ...
                      "api (full)"
                      "api+web (auth needed)"
                      "api+web (locally no auth)"
                    ])
                    (
                      val:
                      if val == "none" then
                        0
                      else if val == "api (add nzbs)" then
                        1
                      else if val == "api (no config)" then
                        2
                      else if val == "api (full)" then
                        3
                      else if val == "api+web (auth needed)" then
                        4
                      # if val == "api+web (locally no auth)" then
                      else
                        5
                    )
                    (
              ...
              type = types.submodule {
                ...
                options = {
                  ntfosd_enable = mkOption {
                    type = types.bool;
                    description = "Whether to enable NotifyOSD alerts.";
                    default = false; # does not make sense in a server environment
                  };
                };
              };
            };
                      ...
                      type =
                        types.coercedTo
                          ...
                          (
                            ...
                            if val == "strict" then
                              3
                            else if val == "allow injection" then
                              2
                            else
                              0
                          )
    ...
    assertions = [
      {
        ...
        message = ''
          sabnzbd.configFile and sabnzbd.settings are mutually exclusive,
          please set configFile to null or settings to {}
        '';
      }
    ];
    ...
    systemd.services.sabnzbd =
      ...
      in
      {
        ...
        serviceConfig = {
          ...
          ExecStart = "${lib.getBin cfg.package}/bin/sabnzbd -d -f ${totalIni}";
        };
        preStart = ''
          set -euo pipefail
          ...
          install -D -m ${if cfg.allowConfigWrite then "600" else "400"} \
            -o '${cfg.user}' -g '${cfg.group}' /dev/stdin '${totalIni}'
        '';
      };
