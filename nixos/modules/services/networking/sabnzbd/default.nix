  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.sabnzbd =
      let
        ...
        optionalWriteableConfigInclusion = optionalString cfg.allowConfigWrite ''[ -e '${sabnzbdIniPath}' ] && files=('${sabnzbdIniPath}' "''${files[@]}")'';
      in
      {
        ...
        serviceConfig = {
          ...
          ExecStart = "${lib.getBin cfg.package}/bin/sabnzbd -d -f ${sabnzbdIniPath}";
        };
        preStart = ''
          set -euo pipefail

          files=('${publicSettingsIni}' ${secretIniQuoted})
          ...
          install -D -m ${if cfg.allowConfigWrite then "600" else "400"} \
            -o '${cfg.user}' -g '${cfg.group}' /dev/stdin '${sabnzbdIniPath}'
        '';
      };
