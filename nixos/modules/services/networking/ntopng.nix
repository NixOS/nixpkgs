...

{
  ...
  imports = [
    (lib.mkRenamedOptionModule [ "services" "ntopng" "http-port" ] [ "services" "ntopng" "httpPort" ])
  ];
  ...
  options = {
    ...
    services.ntopng = {
      ...
      redis.address = lib.mkOption {
        type = lib.types.str;
        example = lib.literalExpression "config.services.redis.ntopng.unixSocket";
        description = ''
          Redis address - may be a Unix socket or a network host and port.
        '';
      };

      redis.createInstance = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = lib.optionalString (lib.versionAtLeast config.system.stateVersion "22.05") "ntopng";
        description = ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.ntopng.redis.address =
      lib.mkIf createRedis config.services.redis.servers.${cfg.redis.createInstance}.unixSocket;

    services.redis.servers = lib.mkIf createRedis {
      ${cfg.redis.createInstance} = {
        enable = true;
        user = lib.mkIf (cfg.redis.createInstance == "ntopng") "ntopng";
      };
    };
    ...
    systemd.services.ntopng = {
      ...
      requires = lib.optional createRedis redisService;
      after = [ "network.target" ] ++ lib.optional createRedis redisService;
