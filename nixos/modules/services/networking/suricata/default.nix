...
, ...
  ...
  inherit (lib) mkEnableOption mkPackageOption mkOption types literalExpression filterAttrsRecursive concatStringsSep strings lists mkIf;
in {
  ...
  options.services.suricata = {
    ...
    configFile = mkOption {
      ...
      default = pkgs.writeTextFile {
        ...
        text = ''
          %YAML 1.1
          ---
          ${builtins.readFile (yaml.generate "suricata-settings-raw.yaml" (filterAttrsRecursive (name: value: value != null) cfg.settings))}
        '';
      };
      description = ''
        The suricata configuration file, usually you don't want to override the default value
        and just use the settings options. If you want to include extra configuration to the file
    ...
    disabledRules = mkOption {
      ...
      default = [ "2270000" "2270001" "2270002" "2270003" "2270004" ];
      description = ''
  ...
  config = let
    ...
    in unique (
      map (e: e.interface) (
        ...
        ++ (optionals (cfg.settings.dpdk != null && cfg.settings.dpdk.interfaces != null) cfg.settings.dpdk.interfaces)
    ...
    systemd.services = {
      suricata-update = {
        ...
        script = let
          ...
          enabledSourcesCmds = map (src: "${python.interpreter} ${pkg}/bin/suricata-update enable-source ${src}") cfg.enabledSources;
        in ''
          ${concatStringsSep "\n" enabledSourcesCmds}
        ...
        serviceConfig = {
          ...
          ReadWritePaths = ["/var/lib/suricata" cfg.settings."default-rule-path"];
        };
      };
      suricata = {
