...
in {
  ...
  options = {
    ...
    outputs = mkOption {
      type = with types; nullOr (listOf (attrsOf (submodule {
        freeformType = yaml.type;
        options = {
      ...
      default = null;
      # TODO enable fast and eve-log by default if possible
    ...
    logging = {
      "default-log-level" = mkOption {
        type =  types.enum [ "error" "warning" "notice" "info" "perf" "config" "debug" ];
        default = "notice";
        description = ''
      ...
      type = with types; nullOr (submodule {
        options = {
          "error-policy" = mkOption {
            type = types.enum [ "drop-flow" "pass-flow" "bypass" "drop-packet" "pass-packet" "reject" "ignore" ];
            default = "ignore";
            description = ''
          ...
          protocols = mkOption {
            type = with types; nullOr (attrsOf (submodule {
              ...
              options = {
                enabled = mkOption {
                  type = types.enum [ "yes" "no" "detection-only" ];
                  default = "no";
                  description = ''
    ...
    "host-mode" = mkOption {
      type = types.enum [ "router" "sniffer-only" "auto" ];
      default = "auto";
      description = ''
    ...
    "exception-policy" = mkOption {
      type = types.enum [ "auto" "drop-packet" "drop-flow" "reject" "bypass" "pass-packet" "pass-flow" "ignore" ];
      default = "auto";
      description = ''
