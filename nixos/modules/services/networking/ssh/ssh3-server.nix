{ pkgs, config, lib, ... }:
let
  inherit (lib) mkEnableOption mkOption mkPackageOptionMD mkIf types;
...
in
  {
    options.services.ssh3-server = {
      ...
      package = mkPackageOptionMD pkgs "ssh3" { };
      ...
      domain = mkOption {
        type = types.str;
        description = lib.mdDoc "Domain name for this server";
      };

      certificate = {
        publicKeyPath = mkOption {
          type = types.path;
          description = lib.mdDoc "Path to the public key of the certificate, often a full chain";
          default = "${acmeDirectory}/fullchain.pem";
          defaultText = lib.literalExpression "(ACME directory for this domain)/fullchain.pem";
        };
        privateKeyPath = mkOption {
          type = types.path;
          description = lib.mdDoc "Path to the private key of the certificate";
    ...
    config = mkIf cfg.enable {
      ...
      let
        listenParameters = lib.concatStringsSep " " (map ({ addr, port }: "-bind ${addr}:${toString port}") cfg.listenAddresses);
        certificateParameters = "-cert ${cfg.certificate.publicKeyPath} -key ${cfg.certificate.privateKeyPath}";
      in
      {
        ...
        serviceConfig = {
          ExecStart = "${cfg.package}/bin/ssh3-server ${listenParameters} ${certificateParameters} -url-path ${cfg.urlPath}";
          KillMode = "process";
          Restart = "always";
          Type = "simple";
        };
