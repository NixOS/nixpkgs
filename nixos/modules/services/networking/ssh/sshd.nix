...
in {
  ...
  config = mkIf cfg.enable {
            ...
            flip concatMapStrings cfg.hostKeys (k: ''
              if ! [ -s "${k.path}" ]; then
                  ssh-keygen \
                    ...
                    ${
                if k ? bits
                then "-b ${toString k.bits}"
                else ""
              } \
                    ${
                if k ? rounds
                then "-a ${toString k.rounds}"
                else ""
              } \
                    ${
                if k ? comment
                then "-C '${k.comment}'"
                    ...
                    ${
                if k ? openSSHFormat && k.openSSHFormat
                then "-o"
                else ""
              } \
                    -f "${k.path}" \
    ...
    services.openssh.authorizedKeysFiles = ["%h/.ssh/authorized_keys" "%h/.ssh/authorized_keys2" "/etc/ssh/authorized_keys.d/%u"];
