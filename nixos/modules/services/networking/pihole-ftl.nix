  ...
  config = mkIf cfg.enable {
    assertions = [
...

      {
        ...
        message = ''
          services.pihole-ftl.settings.webserver.api.cli_pw must be true when lists are set in services.pihole-ftl.lists.
          The ephemeral CLI password must be enabled in order for domain lists to be loaded automatically on startup.
        '';
      }
    ];
    ...
    services.pihole-ftl.settings = lib.mkMerge [
      ...
      (lib.optionalAttrs cfg.useDnsmasqConfig {
        misc.dnsmasq_lines = lib.pipe dnsmasqConf [
          builtins.readFile
      ...
      pihole-ftl-log-deleter = mkIf cfg.queryLogDeleter.enable {
        ...
        script =
          ...
          ''
            ...
            ${pkgs.systemd}/bin/systemctl reload ${config.systemd.services.pihole-ftl.name}
          '';
      };
    };
