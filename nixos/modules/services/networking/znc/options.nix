...

{
  ...
  config = lib.mkIf cfg.useLegacyConfig {
    ...
    services.znc.config = let
      ...
      lib.mkDefault = lib.mkOverride 900;
    in {
      ...
      User.${c.userName} = {
        ...
        Network = lib.mapAttrs (name: net: {
          ...
          Chan = lib.optionalAttrs net.hasBitlbeeControlChannel { "&bitlbee" = lib.mkDefault {}; } //
            lib.listToAttrs (map (n: lib.nameValuePair "#${n}" (lib.mkDefault {})) net.channels);
          extraConfig = if net.extraConf == "" then lib.mkDefault null else net.extraConf;
        }) c.networks;
