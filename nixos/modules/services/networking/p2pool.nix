{
  config,
  lib,
  pkgs,
  ...
}:

let
  cfg = config.services.p2pool;
in

{
  options = {
    services.p2pool = {
      enable = lib.mkEnableOption "Monero P2Pool node daemon";

      dataDir = lib.mkOption {
        type = lib.types.str;
        default = "/var/lib/monero";
        description = ''
          The directory where p2pool stores its data files.
        '';
      };

      mini = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = ''
          Whether to mine on P2Pool Mini (true) or Main (false).
        '';
      };

      host = lib.mkOption {
        type = lib.types.str;
        default = "127.0.0.1";
        description = ''
          The IP address of your Monero node.
        '';
      };

      logLevel = lib.mkOption {
        type = lib.types.int;
        default = 2;
        description = ''
          Log level for the P2Pool node. (Between 0 - 6)
        '';
      };

      rpcPort = lib.mkOption {
        type = lib.types.int;
        default = 18081;
        description = ''
          Monero daemon RPC API port.
        '';
      };

      zmqPort = lib.mkOption {
        type = lib.types.int;
        default = 18083;
        description = ''
          Monero daemon ZMQ pub port.
        '';
      };

      socks5.enable = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = ''
          Whether to connect to a SOCKS5 proxy for outgoing connections.
        '';
      };

      socks5.ip = lib.mkOption {
        type = lib.types.str;
        default = "127.0.0.1";
        description = ''
          IP address of the SOCKS5 proxy to connect to.
        '';
      };

      socks5.port = lib.mkOption {
        type = lib.types.int;
        default = 9050;
        description = ''
          The port number of the SOCKS5 proxy.
        '';
      };

      walletAddress = lib.mkOption {
        type = lib.types.str;
        default = "";
        description = ''
          The Monero wallet address used for mining payouts.
        '';
      };

      environmentFile = lib.mkOption {
        type = lib.types.str;
        default = null;
        example = "/var/lib/p2pool/p2pool.env";
        description = ''
          Path to an EnvironmentFile for the p2pool service as defined in {manpage}`systemd.exec(5)`.

          Secrets may be passed to the service by specifying placeholder variables in the Nix config
          and setting values in the environment file.

          Example:

          ```
          # In environment file:
          WALLET_ADDRESS=888tNkZrPN6JsEgekjMnABU4TBzc2Dt29EPAvkRxbANsAnjyPbb3iQ1YBRk1UXcdRsiKc9dhwMVgN5S9cQUiyoogDavup3H
          ```

          ```
          # Service config
          services.p2pool.walletAddress = "$WALLET_ADDRESS";
          ```
        '';
      };
    };
  };

  config = lib.mkIf cfg.enable {
    users.users.p2pool = {
      isSystemUser = true;
      group = "p2pool";
      description = "P2Pool node user";
      home = cfg.dataDir;
      createHome = true;
    };

    users.groups.p2pool = { };

    systemd.sockets.p2pool = {
      description = "P2Pool Command Socket";
      socketConfig = {
        SocketUser = "p2pool";
        SocketGroup = "p2pool";
        ListenFIFO = "/run/p2pool/p2pool.control";
        RemoveOnStop = true;
        DirectoryMode = "0755";
        SocketMode = "0666";
      };
    };

    systemd.services.p2pool = {
      description = "P2Pool node";
      after = [
        "network.target"
        "monero.service"
        "system-modules-load.target"
      ];
      wants = [
        "network.target"
        "monero.service"
        "system-modules-load.target"
      ];
      wantedBy = [ "multi-user.target" ];
      script =
        ''
          ${lib.getExe' pkgs.p2pool "p2pool"} \
            --data-dir ${cfg.dataDir} \
            --rpc-port ${builtins.toString cfg.rpcPort} \
            --zmq-port ${builtins.toString cfg.zmqPort} \
            --host ${cfg.host} \
        ''
        + "--loglevel ${builtins.toString cfg.logLevel}"
        + "${if cfg.mini then " \\\n --mini" else ""}"
        + "${
          if cfg.socks5.enable then
            " \\\n--socks5 ${cfg.socks5.ip}:${builtins.toString cfg.socks5.port}"
          else
            ""
        }"
        + "${if (cfg.walletAddress != "") then " \\\n--wallet ${cfg.walletAddress}" else ""}";

      serviceConfig = {
        Type = "exec";
        User = "p2pool";
        Group = "p2pool";
        Restart = "always";
        WorkingDirectory = cfg.dataDir;
        TimeoutStop = 60;

        EnvironmentFile = lib.mkIf (cfg.environmentFile != null) [ cfg.environmentFile ];

        StandardInput = "socket";
        StandardOutput = "journal";
        StandardError = "journal";

        Sockets = [ "p2pool.socket" ];
      };
    };

    assertions = [
      {
        assertion = cfg.logLevel >= 0 && cfg.logLevel <= 6;
        message = ''
          The log level needs to be between 0 and 6.
        '';
      }
      {
        assertion = cfg.walletAddress != "";
        message = ''
          A wallet address must be specified.
        '';
      }
    ];
  };

  meta.maintainers = with lib.maintainers; [ JacoMalan1 ];
}
