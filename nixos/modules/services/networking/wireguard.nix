...
let
  ...
  peerOpts = {
    ...
    options = {
      ...
      publicKeyFile = mkOption {
        example = "/var/lib/wireguard/pubkey";
        type = with types; nullOr path;
        default = null;
        description = "File containing the base64 public key of the peer.";
      };
  ...
  generatePeerUnit = { interfaceName, interfaceCfg, peer }:
    let
      peerName = if peer.publicKey != null then peer.publicKey else toString peer.publicKeyFile;
      ...
      serviceName = peerUnitServiceName interfaceName peerName dynamicRefreshEnabled;
    in
    nameValuePair serviceName
      {
        ...
        script =
          let
            wg_setup = concatStringsSep " " (
              [ ''${wg} set ${interfaceName} peer "${publicKeyArg}"'' ]
        ...
        postStop =
          ...
          in
          ''
            ${wg} set "${interfaceName}" peer "${publicKeyArg}" remove
            ${route_destroy}
          '';
      };
...

{
  ...
  config = mkIf cfg.enable (
    ...
    in
    {
      ...
      assertions = builtins.concatLists (attrValues (
        mapAttrs
          (name: value:
            let
              interfaceAssertions = [
              ...
              peerAssertions = builtins.concatLists (map
                (peer:
                  let
                  ...
                  [
                    {
                      ...
                      message = ''networking.wireguard.interfaces.${name} peer "${peerName}" has both publicKey and publicKeyFile set, but only one can be used.'';
                    }
                    {
                      assertion = (peer.presharedKey == null) || (peer.presharedKeyFile == null);
                      message = ''networking.wireguard.interfaces.${name} peer "${peerName}" has both presharedKey and presharedKeyFile set, but only one can be used.'';
                    }
                  ])
                value.peers);
            in
            interfaceAssertions ++ peerAssertions
          )
          cfg.interfaces));
