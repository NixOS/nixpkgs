...
let
  ...
  interfaceOpts = { ... }: {
    ...
    options = {
      ...
      preSetup = lib.mkOption {
        example = lib.literalExpression ''"''${pkgs.iproute2}/bin/ip netns add foo"'';
        default = "";
        type = with lib.types; coercedTo (listOf str) (lib.concatStringsSep "\n") lines;
        description = ''
      ...
      preShutdown = lib.mkOption {
        example = lib.literalExpression ''"''${pkgs.iproute2}/bin/ip netns del foo"'';
        default = "";
        type = with lib.types; coercedTo (listOf str) (lib.concatStringsSep "\n") lines;
        description = ''
      ...
      postShutdown = lib.mkOption {
        example = lib.literalExpression ''"''${pkgs.openresolv}/bin/resolvconf -d wg0"'';
        default = "";
        type = with lib.types; coercedTo (listOf str) (lib.concatStringsSep "\n") lines;
      ...
      peers = lib.mkOption {
        ...
        type = with lib.types; listOf (submodule peerOpts);
      };
      ...
      interfaceNamespace = lib.mkOption {
        default = null;
        type = with lib.types; nullOr str;
        example = "init";
  ...
  peerOpts = self: {
    ...
    options = {
      ...
      name = lib.mkOption {
        ...
        defaultText = lib.literalExpression "publicKey";
        example = "bernd";
        type = lib.types.str;
      ...
      dynamicEndpointRefreshSeconds = lib.mkOption {
        default = null;
        defaultText = lib.literalExpression "config.networking.wireguard.interfaces.<name>.dynamicEndpointRefreshSeconds";
        example = 5;
        type = with lib.types; nullOr int;
        description = ''
      ...
      persistentKeepalive = lib.mkOption {
        ...
        type = with lib.types; nullOr int;
        example = 25;
        description = ''This is lib.optional and is by default off, because most
  ...
  generateKeyServiceUnit = name: values:
    ...
    lib.nameValuePair "wireguard-${name}-key"
      {
  ...
  peerUnitServiceName = interfaceName: peerName: dynamicRefreshEnabled:
    let
      refreshSuffix = lib.optionalString dynamicRefreshEnabled "-refresh";
    in
    ...
    in lib.nameValuePair serviceName
      {
        description = "WireGuard Peer - ${interfaceName} - ${peer.name}"
          + lib.optionalString (peer.name != peer.publicKey) " (${peer.publicKey})";
        ...
        script = let
          wg_setup = lib.concatStringsSep " " (
            [ ''${wg} set ${interfaceName} peer "${peer.publicKey}"'' ]
            ++ lib.optional (psk != null) ''preshared-key "${psk}"''
            ++ lib.optional (peer.endpoint != null) ''endpoint "${peer.endpoint}"''
            ++ lib.optional (peer.persistentKeepalive != null) ''persistent-keepalive "${toString peer.persistentKeepalive}"''
            ++ lib.optional (peer.allowedIPs != []) ''allowed-ips "${lib.concatStringsSep "," peer.allowedIPs}"''
          );
          route_setup =
            lib.optionalString interfaceCfg.allowedIPsAsRoutes
              (lib.concatMapStringsSep "\n"
                (allowedIP:
                  ''${ip} route replace "${allowedIP}" dev "${interfaceName}" table "${interfaceCfg.table}" ${lib.optionalString (interfaceCfg.metric != null) "metric ${toString interfaceCfg.metric}"}''
                ) peer.allowedIPs);
        in ''
          ${wg_setup}
          ${route_setup}

          ${lib.optionalString (dynamicEndpointRefreshSeconds != 0) ''
        ...
        postStop = let
          route_destroy = lib.optionalString interfaceCfg.allowedIPsAsRoutes
            (lib.concatMapStringsSep "\n"
              (allowedIP:
    ...
    lib.nameValuePair "wireguard-${name}"
      {
        ...
        script = lib.concatStringsSep "\n" (
          lib.optional (!config.boot.isContainer) "modprobe wireguard || true"
          ++ [
            values.preSetup
            ''${ipPreMove} link add dev "${name}" type wireguard''
          ]
          ++ lib.optional (values.interfaceNamespace != null && values.interfaceNamespace != values.socketNamespace) ''${ipPreMove} link set "${name}" netns "${ns}"''
          ++ lib.optional (values.mtu != null) ''${ipPostMove} link set "${name}" mtu ${toString values.mtu}''
          ++ (map (ip:
          ...
          ++ [
            ...
            ++ lib.optional (values.listenPort != null) ''listen-port "${toString values.listenPort}"''
            ++ lib.optional (values.fwMark != null) ''fwmark "${values.fwMark}"''
            ))
  ...
  nsWrap = cmd: src: dst:
    let
      nsList = lib.filter (ns: ns != null) [ src dst ];
      ns = lib.last nsList;
    in
      if (lib.length nsList > 0 && ns != "init") then ''ip netns exec "${ns}" "${cmd}"'' else cmd;
in

{
  ...
  options = {
    ...
    networking.wireguard = {
      ...
      enable = lib.mkOption {
        ...
        defaultText = lib.literalExpression "config.${opt.interfaces} != { }";
        example = true;
      };
      ...
      interfaces = lib.mkOption {
        ...
        type = with lib.types; attrsOf (submodule interfaceOpts);
      };
  ...
  config = lib.mkIf cfg.enable (let
    all_peers = lib.flatten
      (lib.mapAttrsToList (interfaceName: interfaceCfg:
  ...
  in {
    ...
    systemd.services = lib.mkIf (!cfg.useNetworkd) (
      (lib.mapAttrs' generateInterfaceUnit cfg.interfaces)
      // (lib.listToAttrs (map generatePeerUnit all_peers))
      // (lib.mapAttrs' generateKeyServiceUnit
      (lib.filterAttrs (name: value: value.generatePrivateKeyFile) cfg.interfaces)));

      systemd.targets = lib.mkIf (!cfg.useNetworkd) (lib.mapAttrs' generateInterfaceTarget cfg.interfaces);
    }
  );
