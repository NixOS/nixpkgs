...
let
  ...
  peerOpts = { config, ... }: {
    ...
    options = {
      ...
      endpointsUpdater = {
        enable = mkEnableOption ''
          receiving the enpoint of other peers known by this peer,
          which must have <option>networking.wireguard.peersAnnouncing</option> enabled
          in its own configuration.

          This is especially useful to punch-through a NAT
          by establishing a direct peer-to-peer connectivity:
          the peers behind a NAT initiate a connection
          to the <option>networking.wireguard.listenPort</option>
          of the announcing peer, they receive from this peer the endpoints to reach
          the other peers behind a NAT, and except for symmetric NAT,
          UDP hole punching enables them to establish a direct WireGuard tunnel between them.
        '';
        addr = mkOption {
          type = types.str;
        ...
        refreshSeconds = mkOption {
          type = with types; int;
          example = 10;
          defaultText = "<option>networking.wireguard.persistentKeepalive</option> or 25 if null";
          description = ''
    ...
    config = {
      endpointsUpdater = {
        ...
        refreshSeconds = mkDefault (if config.persistentKeepalive == null then 25 else config.persistentKeepalive);
      };
    };
    ...
    nameValuePair "wireguard-${interfaceName}-endpoints-updater-${keyToUnitName peer.publicKey}"
      {
        ...
        script = ''
          ...
          while true; do
            # Set stdin to netcat's stdout
            exec < <(exec ${pkgs.netcat-gnu}/bin/nc </dev/null --tcp --wait="$wait" \
              "${peer.endpointsUpdater.addr}" \
              "${toString peer.endpointsUpdater.port}")
