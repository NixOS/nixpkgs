...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.jibri = {
      ...
      + ''
        ${lib.getExe pkgs.jdk11_headless} -Djava.util.logging.config.file=${./logging.properties-journal} -Dconfig.file=${configFile} -jar ${pkgs.jibri}/opt/jitsi/jibri/jibri.jar --config /var/lib/jibri/jibri.json
      '';
