    ...
    type = submodule {
      ...
      options = {
        ...
        IPv6_rpfilter = mkOption {
          ...
          description = ''
            ...
            - strict: Performs "strict" filtering as per RFC 3704. This check
              verifies that the in ingress interface is the same interface that
              would be used to send a packet reply to the source. That is,
              ingress == egress.
            - loose: Performs "loose" filtering as per RFC 3704. This check only
              ...
              interface; even if it's not the same one on which the packet arrived.
            - strict-forward: This is almost identical to "loose", but does not
              perform RPF for packets targeted to the host (INPUT).
            - loose-forward: This is almost identical to "loose", but does not
              perform RPF for packets targeted to the host (INPUT).
        ...
        IndividualCalls = mkOption {
          type = bool;
          description = ''
            Whether to use individual -restore calls to apply changes to the
            firewall. The use of individual calls increases the time that is
        ...
        FirewallBackend = mkOption {
          ...
          description = ''
            The firewall backend implementation. This applies to all firewalld
        ...
        RFC3964_IPv4 = mkOption {
          type = bool;
          description = ''
            Whether to filter IPv6 traffic with 6to4 destination addresses
            that correspond to IPv4 addresses that should not be routed over
            the public internet.
          '';
          default = true;
        };
        StrictForwardPorts = mkOption {
          type = bool;
          description = ''
            If enabled, the generated destination NAT (DNAT) rules will NOT
            accept traffic that was DNAT'd by other entities, e.g. docker.
            Firewalld will be strict and not allow published container ports
            until they're explicitly allowed via firewalld. If set to no, then
            docker (and podman) integrates seamlessly with firewalld. Published
            container ports are implicitly allowed.
          '';
          default = false;
        };
        NftablesFlowtable = mkOption {
          ...
          description = ''
            This may improve forwarded traffic throughput by enabling nftables
            flowtable. It is a software fastpath and avoids calling nftables
            rule evaluation for data packets. Its value is a space separate
            list of interfaces.
          '';
          default = "off";
        };
        ...
        NftablesTableOwner = mkOption {
          type = bool;
          description = ''
            If enabled, the generated nftables rule set will be owned
            exclusively by firewalld. This prevents other entities from
            mistakenly (or maliciously) modifying firewalld's rule set. If you
            intend to modify firewalld's rules, set this to false.
          '';
          default = true;
        };
      };
