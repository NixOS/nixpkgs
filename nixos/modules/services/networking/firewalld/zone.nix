    ...
    type = attrsOf (submodule {
      options = {
        ...
        ingressPriority = mkOption {
          ...
          description = "Priority for inbound traffic. Lower values have higher priority.";
          default = null;
        };
        egressPriority = mkOption {
          type = nullOr ints.s16;
          description = "Priority for outbound traffic. Lower values have higher priority.";
          default = null;
        };
        interfaces = mkOption {
        ...
        icmpBlockInversion = mkOption {
          type = bool;
          description = ''
            Whether to invert the icmp block handling.  Only enabled ICMP types
            are accepted and all others are rejected in the zone.
          '';
          default = false;
        };
        forward = mkOption {
          type = bool;
          description = ''
            Whether to enable intra-zone forwarding.  When enabled, packets
            will be forwarded between interfaces or sources within a zone, even
            if the zone's target is not set to ACCEPT.
          '';
          default = false;
        };
        short = mkOption {
