...
let
  ...
  interfaceOpts = { ... }: {
    options = {
      ...
      preUp = lib.mkOption {
        ...
        default = "";
        type = with lib.types; coercedTo (listOf str) (concatStringsSep "\n") lines;
        description = ''
  ...
  generateUnit = name: values:
    assert lib.assertMsg (values.configFile != null || ((values.privateKey != null) != (values.privateKeyFile != null))) "Only one of privateKey, configFile or privateKeyFile may be set";
    assert lib.assertMsg (values.generatePrivateKeyFile == false || values.privateKeyFile != null) "generatePrivateKeyFile requires privateKeyFile to be set";
    let
      ...
      postUp =
            lib.optional (values.privateKeyFile != null) "wg set ${name} private-key <(cat ${values.privateKeyFile})" ++
            (lib.concatMap (peer: lib.optional (peer.presharedKeyFile != null) "wg set ${name} peer ${peer.publicKey} preshared-key <(cat ${peer.presharedKeyFile})") values.peers) ++
            lib.optional (values.postUp != "") values.postUp;
      postUpFile = if postUp != [] then writeScriptFile "postUp.sh" (lib.concatMapStringsSep "\n" (line: line) postUp) else null;
      ...
      configDir = pkgs.writeTextFile {
        ...
        '' +
        lib.optionalString (values.table != null) "Table = ${values.table}\n" +
        lib.optionalString (values.mtu != null) "MTU = ${toString values.mtu}\n" +
        lib.optionalString (values.privateKey != null) "PrivateKey = ${values.privateKey}\n" +
        lib.optionalString (values.listenPort != null) "ListenPort = ${toString values.listenPort}\n" +
        lib.optionalString (generateKeyScriptFile != null) "PreUp = ${generateKeyScriptFile}\n" +
        lib.optionalString (preUpFile != null) "PreUp = ${preUpFile}\n" +
        lib.optionalString (postUpFile != null) "PostUp = ${postUpFile}\n" +
        lib.optionalString (preDownFile != null) "PreDown = ${preDownFile}\n" +
        lib.optionalString (postDownFile != null) "PostDown = ${postDownFile}\n" +
        lib.concatMapStringsSep "\n" (peer:
          assert lib.assertMsg (!((peer.presharedKeyFile != null) && (peer.presharedKey != null))) "Only one of presharedKey or presharedKeyFile may be set";
          "[Peer]\n" +
          "PublicKey = ${peer.publicKey}\n" +
          lib.optionalString (peer.presharedKey != null) "PresharedKey = ${peer.presharedKey}\n" +
          ...
          lib.optionalString (peer.allowedIPs != []) "AllowedIPs = ${lib.concatStringsSep "," peer.allowedIPs}\n"
        ) values.peers;
      };
      configPath =
    ...
    lib.nameValuePair "wg-quick-${name}"
      {
        ...
        wantedBy = lib.optional values.autostart "multi-user.target";
        ...
        script = ''
          ...
          ${lib.optionalString (values.configFile != null) ''
...
in {
  ...
  config = lib.mkIf (cfg.interfaces != {}) {
    ...
    systemd.services = lib.mapAttrs' generateUnit cfg.interfaces;
