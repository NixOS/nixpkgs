...
let
  inherit (lib) types;
  inherit (lib.attrsets) filterAttrs mapAttrs mapAttrs' mapAttrsToList nameValuePair;
  ...
  generateNetdev = name: interface: nameValuePair "40-${name}" {
  ...
  refreshEnabledInterfaces = filterAttrs (name: interface: interface.dynamicEndpointRefreshSeconds != 0) cfg.interfaces;

  generateRefreshTimer = name: interface: nameValuePair "wireguard-dynamic-refresh-${name}" {
  ...
  generateRefreshService = name: interface: nameValuePair "wireguard-dynamic-refresh-${name}" {
...
in
{
  ...
  config = mkIf (cfg.enable && cfg.useNetworkd) {
    ...
    #
    # preSetup, postSetup, and postShutdown may be possible, but networkd is
    # not likely to support script hooks like this directly. See:
    ...
    assertions = concatLists (
      flip mapAttrsToList cfg.interfaces (name: interface: [
