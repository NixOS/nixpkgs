...
let
  ...
  # check if we need to enable capabilities.
  checkPorts = x: lib.findFirst (x: x < 1024) false x; 

  # collect low ports. if not null, we need capabilities to bind to lower
  # ports.
  lowPortsBool = if (checkPorts (map getPort cfg.listenAddresses)) != false then true else false;
...

{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.stubby = {
      ...
      serviceConfig = {
        AmbientCapabilities = "${optionalString lowPortsBool "CAP_NET_BIND_SERVICE"}";
        CapabilitiesBoundingSet = "${optionalString lowPortsBool "CAP_NET_BIND_SERVICE"}";
