...
in
{
  options.services.gokapi = {
    enable = lib.mkEnableOption "Lightweight selfhosted Firefox Send alternative without public upload. AWS S3 supported.";

    mutableSettings = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Allow changes to the program config made by the program to persist between restarts. If disabled all required values must be set using nix, and all changes to config format over application updates must be resolved by user.";
    };
    ...
    environment = lib.mkOption {
      ...
      default = { };
      description = "Environment variables to be set for the gokapi service. Can use systemd specifiers. For full list see https://gokapi.readthedocs.io/en/latest/advanced.html#environment-variables";
    };
