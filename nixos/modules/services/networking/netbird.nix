...
in
{
  ...
  options.services.netbird = {
    ...
    ui.enable = mkOption {
      type = bool;
      default = config.services.xserver.displayManager.sessionPackages != [ ];
      defaultText = literalExpression ''config.services.xserver.displayManager.sessionPackages != [ ]'';
      description = ''
                  ...
                  pkgs.stdenv.mkDerivation {
                    ...
                    installPhase = concatStringsSep "\n" [
                      ...
                      (optionalString cfg.ui.enable ''
                        ...
                        makeWrapper ${lib.getExe cfg.ui.package} "$out/bin/netbird-ui-${name}" \
                        ...
                        substitute ${cfg.ui.package}/share/applications/netbird.desktop \
                          ...
                          --replace-fail '${lib.getExe cfg.ui.package}' "$out/bin/netbird-ui-${name}"
                      '')
                    ];
                  };
              };
  ...
  config = mkMerge [
    (mkIf cfg.enable (
      let name = "wt0"; client = cfg.clients."${name}"; in {
        ...
        environment.systemPackages = [
          (pkgs.runCommand "${client.wrapper}-as-default" { } ''
            mkdir -p "$out/bin"
    ...
    ))
    {
      ...
      environment.etc = toClientAttrs (client: nameValuePair "${client.name}/config.d/50-nixos.json" {
        text = builtins.toJSON client.config;
        mode = "0444";
      });

      systemd.services = toClientAttrs (client: nameValuePair client.name {
    ...
    (mkIf (hardenedClients != { }) {
      ...
      users.users = toHardenedClientAttrs (client: nameValuePair client.name {
        isSystemUser = true;
        home = "/var/lib/${client.name}";
        group = client.name;
      });

      systemd.services = toHardenedClientAttrs (client: nameValuePair client.name (mkIf client.hardened {
        serviceConfig = {
