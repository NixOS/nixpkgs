...
in
{
  options.services.routedns = {
    enable = lib.mkEnableOption "RouteDNS - DNS stub resolver, proxy and router";
    ...
    configFile = lib.mkOption {
      ...
      type = lib.types.path;
      example = lib.literalExpression ''"''${pkgs.routedns}/cmd/routedns/example-config/use-case-1.toml"'';
      description = "Path to RouteDNS TOML configuration file.";
    };

    package = lib.mkPackageOption pkgs "routedns" { };
  };
  ...
  config = lib.mkIf cfg.enable {
    systemd.services.routedns = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${lib.getBin cfg.package}/bin/routedns -l 4 ${cfg.configFile}";
      };
    };
  };
