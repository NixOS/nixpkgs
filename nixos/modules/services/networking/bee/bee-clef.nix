  ...
  options = {
    services.bee-clef = {
      ...
      swarm = mkOption {
        ...
        default = config.services.bee.swarm;
        description = ''
          Which named swarm the bee nodes should join. This will affect the default value of the `--chainid` argument of the `clef` binary.
          Specifying the `mainnet` value will result in `100` (xDai mainnet), and `testnet` will result in `5` (Ethereum's Goerli testnet)).
        '';
      };
      ...
      chainId = mkOption {
        ...
        description = ''
          ...
          Unless you know what you're doing, you should rather set the `config.services.bee.settings.swarm` variable instead that will affect the default of this option.
        '';
      };
