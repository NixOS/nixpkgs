  ...
  options = {
    services.bee = {
      ...
      swarm = mkOption {
        ...
        default = "testnet";
        description = ''
          Which swarm the bee node(s) should join. This will affect the default values of `chain-id`, `network-id`, and `swap-endpoint`.
        '';
      };
    ...
    in
      mkIf (0 < cfg.instanceCount) {
        ...
        services.bee.settings = rec {
          swarm                = lib.mkDefault "testnet";
          password-file        = lib.mkDefault "/var/lib/bee/password";
          clef-signer-enable   = lib.mkDefault true;
          mainnet              = lib.mkIf (swarm == "mainnet") true;
          network-id           = lib.mkIf (isString swarm) {
            testnet = 10;
            mainnet = 1;
          }.${cfg.settings.swarm};
        };
