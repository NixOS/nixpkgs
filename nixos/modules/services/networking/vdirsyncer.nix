...
let
  ...
  toIniJson = with lib.generators; toINI {
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    systemd.services = lib.mapAttrs' (name: cfg': lib.nameValuePair "vdirsyncer@${name}" (
    ...
    systemd.timers = lib.mapAttrs' (name: cfg': lib.nameValuePair "vdirsyncer@${name}" {
