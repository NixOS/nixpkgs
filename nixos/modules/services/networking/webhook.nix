...
let
  ...
  hookType = lib.types.submodule ({ name, ... }: {
  ...
  hookFiles = lib.mapAttrsToList (name: hook: hookFormat.generate "webhook-${name}.json" [ hook ]) cfg.hooks
           ++ lib.mapAttrsToList (name: hook: pkgs.writeText "webhook-${name}.json.tmpl" "[${hook}]") cfg.hooksTemplated;
    ...
    systemd.services.webhook = {
      ...
      script = let
        args = [ "-ip" cfg.ip "-port" (toString cfg.port) "-urlprefix" cfg.urlPrefix ]
            ++ lib.concatMap (hook: [ "-hooks" hook ]) hookFiles
