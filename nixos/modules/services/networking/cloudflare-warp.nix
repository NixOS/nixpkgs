...
in
{
  options.services.cloudflare-warp = {
    enable = mkEnableOption "Cloudflare Zero Trust client daemon";

    package = mkPackageOption pkgs "cloudflare-warp" { };

    rootDir = mkOption {
      type = types.str;
      default = "/var/cloudflare-warp";
      description = ''
    ...
    openFirewall = mkOption {
      ...
      default = true;
      description = ''
        Open the `udpPort` port in the firewall.
      '';
    };
  };
  ...
  meta.maintainers = with maintainers; [ treyfortmuller ];
}
