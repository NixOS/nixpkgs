...
in
{
    ...
    services.opengist = {
      ...
      settings = lib.mkOption {
        type = lib.types.submodule {
          ...
          options = {
            ...
            "http.git-enabled" = lib.mkOption {
              default = false;
              description = "Enable or disable git operations (clone, pull, push) via HTTP.";
              type = lib.types.bool;
              example = true;
            };
            ...
            "ssh.git-enabled" = lib.mkOption {
              default = false;
              description = "Enable or disable git operations (clone, pull, push) via SSH.";
              type = lib.types.bool;
              example = true;
            };
        ...
        description = ''
          ...
          nix store.
          Use <option>services.opengist.environmentFiles</option> for secrets.
        '';
      };
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.opengist = {
      ...
      serviceConfig = {
        Type = "simple";
        ExecStart = "${cfg.package}/bin/opengist --config ${configurationFile}";
        Restart = "on-failure";
        RestartSec = 300;
