...
in
{
  options.services.authentik = {
    ...
    secretsFile = mkOption {
      ...
      description = ''
        Path of a file with extra environment variables to be loaded from disk. This file is not added to the nix store, so it can be used to pass secrets to authentik. Refer to the [documentation](https://docs.goauthentik.io/docs/installation/configuration) for options.
