...
let
  ...
  policy = (lib.types.enum [ "allow" "block" "reject" "keep" "apply-policy" ]);
...
in
{
  ...
  options = {
    services.usbguard = {
      ...
      implicitPolicyTarget = lib.mkOption {
        type = lib.types.enum [ "allow" "block" "reject" ];
        default = "block";
        description = ''
      ...
      insertedDevicePolicy = lib.mkOption {
        type = lib.types.enum [ "block" "reject" "apply-policy" ];
        default = "apply-policy";
        description = ''
  ...
  imports = [
    (lib.mkRemovedOptionModule [ "services" "usbguard" "IPCAccessControlFiles" ] "The usbguard module now hardcodes IPCAccessControlFiles to /var/lib/usbguard/IPCAccessControl.d.")
    (lib.mkRemovedOptionModule [ "services" "usbguard" "auditFilePath" ] "Removed usbguard module audit log files. Audit logs can be found in the systemd journal.")
    (lib.mkRenamedOptionModule [ "services" "usbguard" "implictPolicyTarget" ] [ "services" "usbguard" "implicitPolicyTarget" ])
  ];
}
