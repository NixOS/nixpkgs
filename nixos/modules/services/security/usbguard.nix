    ...
    services.usbguard = {
      ...
      ruleFile = lib.mkOption {
        ...
        description = ''
          ...
          For more details see {manpage}`usbguard-daemon.conf(5)` and
          {manpage}`usbguard-rules.conf(5)`.
        '';

      };
      ruleFolder = lib.mkOption {
        type = lib.types.nullOr lib.types.path;
        default = "/var/lib/usbguard/rules.d";
        example = "/run/secrets/usbguard-rules.d";
        description = ''
          This tells the USBGuard daemon from which directory to load the policy
          rule set.

          The files in this directory can be changed manually or via the IPC
          interface assuming they have the right file permissions.
      ...
      rules = lib.mkOption {
        ...
        description = ''
          ...
          its policy rule set from the options configured in `services.usbguard.ruleFile`
          and `services.usbguard.ruleFolder`.
    ...
    systemd.services = {
      usbguard = {
        ...
        preStart = ''
          ...
          [ -d "${cfg.ruleFolder}" ] || mkdir -p ${cfg.ruleFolder}
        '';
