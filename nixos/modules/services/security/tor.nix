...
with lib; let
  ...
  mkValueString = k: v:
    ...
    then optionalString (v.transports != []) "${concatStringsSep "," v.transports} exec ${v.exec}"
  ...
  genTorrc = settings:
    ...
    # Not necesssary, but prettier rendering
      if
        elem k ["AutomapHostsSuffixes" "DirPolicy" "ExitPolicy" "SocksPolicy"]
        && v != []
...
in {
  ...
  options = {
    services.tor = {
      ...
      enableGeoIP =
        mkEnableOption ''          use of GeoIP databases.
      ...
      relay = {
        ...
        onionServices = mkOption {
          ...
          type = types.attrsOf (types.submodule ({
            ...
            config = {
              path = mkDefault ((if config.secretKey == null
              then stateDir
              else runDir)
              + "/onion/${name}");
      ...
      settings = mkOption {
        ...
        type = types.submodule {
          ...
          options.SOCKSPort = mkOption {
            ...
            default =
              if cfg.settings.HiddenServiceNonAnonymousMode == true
              then [{port = 0;}]
              else [];
      ...
      allowedTCPPorts =
        concatMap (
          o:
            if isInt o && o > 0
            then [o]
            else if o ? "port" && isInt o.port && o.port > 0
            then [o.port]
            else []
        ) (flatten [
          cfg.settings.ORPort
      ...
      serviceConfig = {
        ...
        ExecStartPre = [
          ...
          + pkgs.writeShellScript "ExecStartPre" (concatStringsSep "\n" (flatten (
            ...
            cfg.relay.onionServices
            ++ mapAttrsToList (name: onion: imap0 (i: prvKeyPath: let
