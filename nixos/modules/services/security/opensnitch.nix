...
let
  ...
  predefinedRules = lib.flip lib.mapAttrs cfg.rules (name: cfg: {
...
in {
        ...
        type = lib.types.submodule {
          ...
          options = {
            ...
            ProcMonitorMethod = lib.mkOption {
              type = lib.types.enum [ "ebpf" "proc" "ftrace" "audit" ];
              description = ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.opensnitch.settings = lib.mapAttrs (_: v: lib.mkDefault v) (builtins.fromJSON (builtins.unsafeDiscardStringContext (builtins.readFile "${pkgs.opensnitch}/etc/opensnitchd/default-config.json")));
    ...
    systemd = {
      ...
      services.opensnitchd = {
        ...
        preStart = lib.mkIf (cfg.rules != {}) (let
          rules = lib.flip lib.mapAttrsToList predefinedRules (file: content: {
        ...
        in ''
          ...
          # `cfg.rules`).
          find ${cfg.settings.Rules.Path} -type l -lname '${builtins.storeDir}/*' ${lib.optionalString (rules != {}) ''
