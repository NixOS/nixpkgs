...
in
{
    ...
    provision = {
      ...
      groups = mkOption {
        ...
        type = types.attrsOf (types.submodule (groupSubmod: {
          ...
          config.members = concatLists (flip mapAttrsToList cfg.provision.persons (person: personCfg:
  ...
  config = mkIf (cfg.enableClient || cfg.enableServer || cfg.enablePam) {
    assertions = let
      ...
      entitiesByName = foldl' (acc: { type, name }:
        acc // {
    ...
    in
      ...
      ++ flip mapAttrsToList (filterPresent cfg.provision.persons) (person: personCfg:
        assertGroupsKnown "services.kanidm.provision.persons.${person}.groups" personCfg.groups
      )
      ++ flip mapAttrsToList (filterPresent cfg.provision.groups) (group: groupCfg:
