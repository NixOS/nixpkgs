...
let
  ...
  genTorrc = settings:
    ...
    settings));
  torrc = pkgs.writeText "torrc-${serviceName}" (
...
{
  imports = [
    (mkRenamedOptionModule [ "services" "${serviceName}" "client" "dns" "automapHostsSuffixes" ] [ "services" "${serviceName}" "settings" "AutomapHostsSuffixes" ])
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "dns" "isolationOptions" ] "Use services.${serviceName}.settings.DNSPort instead.")
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "dns" "listenAddress" ] "Use services.${serviceName}.settings.DNSPort instead.")
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "privoxy" "enable" ] "Use services.privoxy.enable and services.privoxy.enableTor instead.")
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "socksIsolationOptions" ] "Use services.${serviceName}.settings.SOCKSPort instead.")
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "socksListenAddressFaster" ] "Use services.${serviceName}.settings.SOCKSPort instead.")
    (mkRenamedOptionModule [ "services" "${serviceName}" "client" "socksPolicy" ] [ "services" "${serviceName}" "settings" "SocksPolicy" ])
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "transparentProxy" "isolationOptions" ] "Use services.${serviceName}.settings.TransPort instead.")
    (mkRemovedOptionModule [ "services" "${serviceName}" "client" "transparentProxy" "listenAddress" ] "Use services.${serviceName}.settings.TransPort instead.")
    (mkRenamedOptionModule [ "services" "${serviceName}" "controlPort" ] [ "services" "${serviceName}" "settings" "ControlPort" ])
    (mkRemovedOptionModule [ "services" "${serviceName}" "extraConfig" ] "Plese use services.${serviceName}.settings instead.")
    (mkRenamedOptionModule [ "services" "${serviceName}" "hiddenServices" ] [ "services" "${serviceName}" "relay" "onionServices" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "accountingMax" ] [ "services" "${serviceName}" "settings" "AccountingMax" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "accountingStart" ] [ "services" "${serviceName}" "settings" "AccountingStart" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "address" ] [ "services" "${serviceName}" "settings" "Address" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "bandwidthBurst" ] [ "services" "${serviceName}" "settings" "BandwidthBurst" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "bandwidthRate" ] [ "services" "${serviceName}" "settings" "BandwidthRate" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "bridgeTransports" ] [ "services" "${serviceName}" "settings" "ServerTransportPlugin" "transports" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "contactInfo" ] [ "services" "${serviceName}" "settings" "ContactInfo" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "exitPolicy" ] [ "services" "${serviceName}" "settings" "ExitPolicy" ])
    (mkRemovedOptionModule [ "services" "${serviceName}" "relay" "isBridge" ] "Use services.${serviceName}.relay.role instead.")
    (mkRemovedOptionModule [ "services" "${serviceName}" "relay" "isExit" ] "Use services.${serviceName}.relay.role instead.")
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "nickname" ] [ "services" "${serviceName}" "settings" "Nickname" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "port" ] [ "services" "${serviceName}" "settings" "ORPort" ])
    (mkRenamedOptionModule [ "services" "${serviceName}" "relay" "portSpec" ] [ "services" "${serviceName}" "settings" "ORPort" ])
  ];
    ...
    services.${serviceName} = {
      ...
      relay = {
        enable = mkEnableOption ''relaying of Tor traffic for others.
          ...
          <option>services.${serviceName}.relay.role</option>
          and
          <option>services.${serviceName}.settings.ORPort</option>
          options'';
          ...
          type = types.attrsOf (types.submodule ({name, config, ...}: {
                ...
                options = {
                  ...
                  clientNames = mkOption {
                    ...
                    description = ''
                      ...
                      <xref linkend="opt-services.${serviceName}.settings.HidServAuth"/>.
                    '';
                  };
                };
              }));
            ...
            options.settings = mkOption {
              ...
              type = types.submodule {
                ...
                options.HiddenServiceSingleHopMode = mkOption {
                  description = descriptionGeneric "HiddenServiceSingleHopMode";
                  type = types.bool;
                  default = serviceName == "tor-insecure";
                };
      ...
      settings = mkOption {
        ...
        type = types.submodule {
          ...
          options.HiddenServiceNonAnonymousMode = mkOption {
            description = descriptionGeneric "HiddenServiceNonAnonymousMode";
            type = types.bool;
            default = serviceName == "tor-insecure";
          };
  ...
  config = mkIf cfg.enable {
    ...
    users.groups.${serviceName}.gid = config.ids.gids.${serviceName};
    users.users.${serviceName} =
      { description = "Tor Daemon User (${serviceName})";
        createHome  = true;
        home        = stateDir;
        group       = "${serviceName}";
        uid         = config.ids.uids.${serviceName};
      };

    services.${serviceName}.settings = mkMerge [
    ...
    systemd.services.${serviceName} = {
      description = "Tor Daemon (${serviceName})";
      path = [ pkgs.tor ];
      ...
      serviceConfig = {
          ...
          ("+" + pkgs.writeShellScript "ExecStartPre" (concatStringsSep "\n" (flatten (["set -eu"] ++
            mapAttrsToList (name: onion:
              optional (onion.authorizedClients != []) ''
                ...
                install -d -o ${serviceName} -g ${serviceName} -m 0700 ${escapeShellArg onion.path} ${escapeShellArg onion.path}/authorized_clients
              '' ++
              imap0 (i: pubKey: ''
                echo ${pubKey} |
                install -o ${serviceName} -g ${serviceName} -m 0400 /dev/stdin ${escapeShellArg onion.path}/authorized_clients/${toString i}.auth
              '') onion.authorizedClients ++
              optional (onion.secretKey != null) ''
                install -d -o ${serviceName} -g ${serviceName} -m 0700 ${escapeShellArg onion.path}
                key="$(cut -f1 -d: ${escapeShellArg onion.secretKey})"
                case "$key" in
                 ("== ed25519v"*"-secret")
                  install -o ${serviceName} -g ${serviceName} -m 0400 ${escapeShellArg onion.secretKey} ${escapeShellArg onion.path}/hs_ed25519_secret_key;;
            ...
            mapAttrsToList (name: onion: imap0 (i: prvKeyPath:
              ...
              install -o ${serviceName} -g ${serviceName} -m 0700 /dev/stdin \
        ...
        RuntimeDirectory = [
          ...
          "${serviceName}/ClientOnionAuthDir"
        ];
        ...
        StateDirectory = [
          ...
          flatten (mapAttrsToList (name: onion:
            optional (onion.secretKey == null) "${serviceName}/onion/${name}"
          ) cfg.relay.onionServices);
        ...
        UMask = "0066";
        BindPaths = [ stateDir ];
        BindReadOnlyPaths = [ storeDir "/etc" ]; # TODO? remove /etc
