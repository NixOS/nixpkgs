...

{
  ...
  options = {
    services.fail2ban-dashboard = {
      ...
      cacheLocation = lib.mkOption {
        type = lib.types.path;
        description = "The path of the folder where fail2ban-dashboard caches data in";
        default = "/var/lib/fail2ban-dashboard/cache";
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.fail2ban-dashboard = {
      ...
      serviceConfig = {
        Restart = "always";
        ExecStart = "${cfg.package} --port ${cfg.port} --cache-dir ${cfg.cacheLocation}";
