...
let
  ...
  mkJailConfig = name: attrs:
    ...
    lib.optionalAttrs (attrs.filter != null) { filter = if (builtins.isString lib.filter) then lib.filter else name; } //
    attrs.settings;

  mkFilter = name: attrs: lib.nameValuePair "fail2ban/filter.d/${name}.conf" {
...

{
  ...
  imports = [
    (lib.mkRemovedOptionModule [ "services" "fail2ban" "daemonConfig" ] "The daemon is now configured through the attribute set `services.fail2ban.daemonSettings`.")
    (lib.mkRemovedOptionModule [ "services" "fail2ban" "extraSettings" ] "The extra default configuration can now be set using `services.fail2ban.jails.DEFAULT.settings`.")
  ];
  ...
  options = {
    services.fail2ban = {
      ...
      jails = lib.mkOption {
        ...
        '';
        type = with lib.types; attrsOf (either lines (submodule ({ name, ... }: {
          options = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    } // (lib.mapAttrs' mkFilter (lib.filterAttrs (_: v: v.filter != null && !builtins.isString v.filter) attrsJails));
    ...
    services.fail2ban.jails = lib.mkMerge [
      {
        DEFAULT.settings = (lib.optionalAttrs cfg.bantime-increment.enable
          ({ "bantime.increment" = cfg.bantime-increment.enable; } // (lib.mapAttrs'
            (name: lib.nameValuePair "bantime.${name}")
      ...
      (lib.mkIf config.services.openssh.enable {
        sshd.settings.port = lib.mkDefault (lib.concatMapStringsSep "," builtins.toString config.services.openssh.ports);
      })
    ];
