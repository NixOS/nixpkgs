...
let
  ...
  specPaths = map dirOf (lib.concatMap (spec:
...
in
{
  options.services.certmgr = {
    ...
    specs = lib.mkOption {
      ...
      '';
      type = with lib.types; attrsOf (either path (submodule {
        options = {
          ...
          action = lib.mkOption {
            type = addCheck str (x: cfg.svcManager == "command" || lib.elem x ["restart" "reload" "nop"]);
            default = "nop";
    ...
    svcManager = lib.mkOption {
      default = "systemd";
      type = lib.types.enum [ "circus" "command" "dummy" "openrc" "systemd" "sysv" ];
      description = ''
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = !lib.any (lib.hasAttrByPath [ "authority" "auth_key" ]) (lib.attrValues cfg.specs);
        message = ''
