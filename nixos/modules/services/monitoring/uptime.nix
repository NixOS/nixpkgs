...
in {
  ...
  config = mkMerge [ (mkIf cfg.enableWebService {
    systemd.services.uptime = {
      ...
      serviceConfig.ExecStart = "${lib.getExe pkgs.nodejs} ${pkgs.nodePackages.node-uptime}/lib/node_modules/node-uptime/app.js";
    };
    ...
    systemd.services.uptime-monitor = {
      ...
      serviceConfig.ExecStart = "${lib.getExe pkgs.nodejs} ${pkgs.nodePackages.node-uptime}/lib/node_modules/node-uptime/monitor.js";
    };
  }) ];
}
