...
let
  ...
  serverArgs = lib.cli.toGNUCommandLineShell { } (
    ...
    // lib.optionalAttrs (cfg.server.oidc.issuerUrl != null) {
      oidc-issuer-url = cfg.server.oidc.issuerUrl;
    }
    // lib.optionalAttrs (cfg.server.oidc.clientId != null) {
      oidc-client-id = cfg.server.oidc.clientId;
    }
    // lib.optionalAttrs (cfg.server.oidc.clientSecret != null) {
      oidc-client-secret = cfg.server.oidc.clientSecret;
    }
    // lib.optionalAttrs (cfg.server.oidc.providerName != null) {
      oidc-provider-name = cfg.server.oidc.providerName;
    }
    // lib.optionalAttrs (cfg.server.oidc.redirectUrl != null) {
      oidc-redirect-url = cfg.server.oidc.redirectUrl;
    }
    ...
    server = lib.mkOption {
      ...
      type = lib.types.submodule {
        options = {
          ...
          oidc = lib.mkOption {
            description = "NetVisor server OIDC configuration.";
            default = { };
            type = lib.types.submodule {
              options = {
                issuerUrl = lib.mkOption {
                  ...
                  default = null;
                  description = "OIDC provider's issuer URL (must end with `/`.";
                };
                ...
                providerName = lib.mkOption {
                  type = lib.types.nullOr lib.types.str;
                  default = null;
                  description = "Display name shown in UI (e.g., `Authentik`, `Keycloak`).";
                };
                redirectUrl = lib.mkOption {
                  type = lib.types.nullOr lib.types.str;
                  default = null;
                  description = "URL from OIDC provider for authentication redirect.";
                };
              };
            };
          };
