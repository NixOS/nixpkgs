...
let
  ...
  nodeConf =
    pkgs.writeText "munin-node.conf"
      ''
        log_level 3
        log_file Sys::Syslog
        port 4949
  ...
  pluginConf =
    pkgs.writeText "munin-plugin-conf"
      ''
        [hddtemp_smartctl]
        user root
        group root
  ...
  extraPluginDir =
    internAndFixPlugins "munin-extra-plugins.d"
      internOnePlugin
      nodeCfg.extraPlugins;

  extraAutoPluginDir =
    internAndFixPlugins "munin-extra-auto-plugins.d"
      internManyPlugins
      (
