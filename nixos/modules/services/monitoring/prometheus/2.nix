...
let
  ...
  writePrettyJSON = name: x:
    pkgs.runCommand name { } ''
      cat '${pkgs.writeText "${name}-raw.json" (builtins.toJSON x)}' | ${pkgs.jq}/bin/jq . > $out
    '';
  ...
  promConfig = {
    ...
    alerting.alertmanagers = [ { static_configs = [ { targets = cfg.alertmanagerURL; } ]; } ];
  ...
  cmdlineArgs = cfg.extraFlags ++ [
    ...
    "--storage.tsdb.retention=${cfg.retention}"
  ...
  options = {
    services.prometheus2 = {
      ...
      rules = mkOption {
        ...
        default = [];
        description = ''
          Alerting and/or Recording rules to evaluate at runtime as attribute
          sets. These will be converted to YAML.
        '';
      };
      ...
      ruleFiles = mkOption {
        ...
        default = [];
        description = ''
          Any additional rules files (YAML format) to include in this configuration.
        '';
      };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.prometheus = {
      ...
      serviceConfig = {
        ...
        ReadOnlyDirectories = "/";
        ReadWriteDirectories = [ "/proc/self" cfg.dataDir ];
      };
    };
  };
}
