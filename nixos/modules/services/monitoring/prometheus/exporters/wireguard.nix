...
in
{
  ...
  extraOpts = {
    ...
    withLatestHandshakeDelay = mkOption {
      type = types.bool;
      default = false;
      description = ''
        Whether or no to add the `wireguard_latest_handshake_delay_seconds` metric that
        automatically calculates the seconds passed since the last handshake.
      '';
    };

    withPrependSudo = mkOption {
      type = types.bool;
      default = false;
      description = ''
    ...
    serviceConfig = {
      ...
      ExecStart = ''
        ${pkgs.prometheus-wireguard-exporter}/bin/prometheus_wireguard_exporter \
          ...
          ${optionalString cfg.withLatestHandshakeDelay "-d true"} \
          ${optionalString cfg.withPrependSudo "-a true"} \
