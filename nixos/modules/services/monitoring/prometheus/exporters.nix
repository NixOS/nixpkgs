...
let
  ...
  mkExporterConf = { name, conf, serviceOpts }:
    let enableDynamicUser = serviceOpts.serviceConfig.DynamicUser or true;
    in mkIf conf.enable {
...
in {
  ...
  config = mkMerge ([{
    assertions = [
      ...
      {
        assertion = cfg.mail.enable -> ((cfg.mail.configFile == null)
      ...
      assertion = cfg.${exporter}.firewallFilter != null
