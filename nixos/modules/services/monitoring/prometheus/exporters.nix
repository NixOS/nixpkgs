...
in
{
  ...
  config = mkMerge ([{
    assertions.services.prometheus.exporters = {
      ...
      scaphandre.kernelModuleInterlRaplCommonIsEnabled = {
    ...
    (lib.listToAttrs
    (map
      (exporter: lib.nameValuePair exporter {
    ...
    warnings = {
      services.prometheus.exporters.idrac.configurationPathOverrides = {
        condition = config.services.prometheus.exporters.idrac.enable && config.services.prometheus.exporters.idrac.configurationPath != null;
        message = ''
