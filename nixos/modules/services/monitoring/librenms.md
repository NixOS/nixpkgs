...
It can happen that the database doesn't get initialized for some reason.
To fix this you manually have to run the following commands once.

Seed the database:

```bash
librenms-artisan db:seed --force --no-interaction
```
