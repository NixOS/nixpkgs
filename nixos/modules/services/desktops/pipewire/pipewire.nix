...
let
  ...
  configs = {
    ...
    pipewire-pulse =
      recursiveUpdate defaults.pipewire-pulse cfg.config.pipewire-pulse;
  };
in {
  ...
  options = {
    services.pipewire = {
      ...
      jack = { enable = mkEnableOption "JACK audio emulation"; };

      pulse = { enable = mkEnableOption "PulseAudio server emulation"; };
    };
  };
