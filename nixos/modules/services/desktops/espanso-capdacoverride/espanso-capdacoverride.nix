...
let
  ...
  # Espanso requires the CAP_DAC_OVERRIDE capability. One can create a wrapper binary with this capability
  # using the `config.security.wrappers.<name>` framework. However, this is not enough: the capability is
  # required by a worker process of espanso created by forking `/proc/self/exe`, which points to the
  # executable **without** the CAP_DAC_OVERRIDE capability. Thus, we inject a wrapper library into espanso
  ...
  wrapperLib = stdenv.mkDerivation {
    ...
    postPatch = ''
      substitute ${wrapperLibSource} lib.c --subst-var-by to "${capDacOverrideWrapperDir}/espanso-capdacoverride"
