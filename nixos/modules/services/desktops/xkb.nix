...
let
  ...
  xkb_patched = pkgs.xorg.xkeyboardconfig_custom {
    layouts = config.services.xkb.extraLayouts;
  };
in
{
  ...
  config = lib.mkIf cfg.enable {
    environment = {
      etc."X11/xkb".source = config.services.xkb.dir;
      # runtime override supported by multiple libraries e. g. libxkbcommon
      # https://xkbcommon.org/doc/current/group__include-path.html
      sessionVariables.XKB_CONFIG_ROOT = config.services.xkb.dir;
    };

    services.xkb.dir = lib.mkIf (cfg.extraLayouts != { }) "${xkb_patched}/etc/X11/xkb";
