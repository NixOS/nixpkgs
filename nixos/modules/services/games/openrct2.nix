...
let
  ...
  mkService = name: park:
  {
    ...
    serviceConfig = {
      ExecStart = ''${cfg.package}/bin/openrct2-cli host "${park.dataDir}/${name}.park" --headless "--user-data-path=${park.dataDir}" ${if cfg.gamePath == "" then "" else  ''"--rct2-data-path=${cfg.gamePath}"''} ${if cfg.rct1Path == "" then "" else ''"--rct1-data-path=${cfg.rct1Path}"''} --port ${toString park.port}'';
      Restart = "always";
      User = cfg.user;
...
in {
  ...
  config = mkIf config.services.openrct2.enable {
      ...
      users.groups.openrct2 = mkIf (cfg.group == "openrct2") {
        gid = config.ids.gids.openrct2;
      };

      systemd.services = mapAttrs' (name: value: nameValuePair "openrct2-${name}" (mkService name value)) cfg.parks;
