...
let
  ...
  isMysqlAtLeast57 =
    (lib.getName mysql == lib.getName pkgs.mysql57) && (builtins.compareVersions mysql.version "5.7" >= 0);
  ...
  settingsFile = pkgs.writeText "my.cnf" (
    generators.toINI { listsAsDuplicateKeys = true; } cfg.settings + optionalString (cfg.extraOptions != null) "[mysqld]\n${cfg.extraOptions}"
  );
in
{
  imports = [
  ...
  config = mkIf config.services.mysql.enable {
    ...
    services.mysql.dataDir =
      mkDefault (
        if versionAtLeast config.system.stateVersion "17.09" then "/var/lib/mysql"
        else "/var/mysql");
