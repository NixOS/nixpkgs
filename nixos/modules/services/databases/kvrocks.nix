...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.kvrocks = {
      description = "Kvrocks - Distributed key value NoSQL database that uses RocksDB as storage engine and is compatible with Redis protocol";
      ...
      serviceConfig = {
        Type = "notify";
        ExecStart = "${cfg.package}/bin/kvrocks -c ${configFile}";
        Restart = "on-failure";
        RestartSec = "10s";
        User = cfg.user;
        Group = cfg.group;
