...
in
{
  ...
  config = mkIf config.services.influxdb.enable {
    ...
    systemd.services.influxdb = {
      ...
      '';
      postStart =
        let bindAddr = (ba: if hasPrefix ":" ba then "127.0.0.1${ba}" else "${ba}")(toString configOptions.http.bind-address);
        in mkBefore ''
        until ${pkgs.curl.bin}/bin/curl -s -o /dev/null ${if configOptions.http.https-enabled then "-k https" else "http"}://${bindAddr}/ping; do
          sleep 1;
        done
      '';
    };
