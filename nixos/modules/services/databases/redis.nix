        ...
        type = with lib.types; attrsOf (submodule ({ config, name, ... }: {
          options = {
            ...
            save = lib.mkOption {
              ...
              description = ''
                The schedule in which data is persisted to disk, represented as a list of lib.lists where the first lib.element represent the amount of seconds and the second the number of changes.

                If set to the empty list (`[]`) then RDB persistence will be disabled (useful if you are using AOF or don't want lib.any persistence).
              '';
            };
