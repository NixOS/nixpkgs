...
in
{
  options.services.dump1090-fa = {
    ...
    nginx = {
      addVirtualHost =
        lib.mkEnableOption "nginx virtual host for dump1090-fa exposing the web interface"
        // {
          default = true;
        };
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.nginx = lib.mkIf cfg.nginx.addVirtualHost {
      virtualHosts."${cfg.nginx.virtualHostName}" = {
        locations = {
          "/".alias = "${cfg.package}/share/dump1090/";
          "/data/".alias = "/run/${config.systemd.services.dump1090-fa.serviceConfig.RuntimeDirectory}/";
        };
      };
    };
  };
