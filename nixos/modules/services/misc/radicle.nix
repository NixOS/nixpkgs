    ...
    services.radicle = {
      enable = lib.mkOption {
        ...
        description = ''
          ...
          Enabling the service will also add a `rad-system` script to
          `environment.systemPackages`. This scripts runs the `rad` command
          inside the namespace of the `radicle-node` service.
        '';
        type = lib.types.bool;
      };
      ...
      settings = lib.mkOption {
        description = ''
          Settings with the same structure used in Radicle's `config.json` (see `rad config schema` for the JSON Schema).
        '';
        default = { };
