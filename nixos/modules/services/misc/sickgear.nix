...
in
{
  ...
  options = {
    services.sickgear = {
      ...
      user = lib.mkOption {
        type = lib.types.str;
        default = if lib.versionAtLeast config.system.stateVersion "26.11" then "sickgear" else "sickbeard";
        defaultText = lib.literalExpression ''
          if lib.versionAtLeast config.system.stateVersion "26.11" then "sickgear" else "sickbeard"
        '';
