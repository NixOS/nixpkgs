    ...
    settings = lib.mkOption {
      type = types.submodule {
        options = {
          ...
          auth = lib.mkOption {
            type = types.listOf types.str;
            description = "Add auth roles, e.g. user:pass@/dir1:rw,/dir2.";
            default = [ ];
            example = lib.literalExpression ''
              [
                "user:pass@/dir1:rw,/dir2"
                "@/dir3"
              ]
            '';
          };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.dufs =
      ...
      in
      {
        ...
        serviceConfig = {
          BindReadOnlyPaths = [ builtins.storeDir ] ++ lib.optional (!pathWritable) settings.serve-path;
    ...
    users = {
      users.dufs = lib.mkIf (cfg.user == "dufs") {
        ...
        uid = config.ids.uids.dufs;
      };
      groups.dufs = lib.mkIf (cfg.group == "dufs") { gid = config.ids.gids.dufs; };
    };
  };
