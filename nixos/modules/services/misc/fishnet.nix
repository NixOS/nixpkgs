...
with lib;
{
  ...
  options.services.fishnet = {
    enable = mkEnableOption "fishnet: distributed Stockfish analysis for lichess.org";

    package = mkPackageOption pkgs "fishnet" { };
      ...
      serviceConfig =
        ...
        in
        {
          ExecStart = "${cfg.package}/bin/fishnet --conf ${format.generate "fishnet.ini" cfg.settings} ${
            optionalString (cfg.keyfile != null) "--key-file ${escapeShellArg cfg.keyfile}"
