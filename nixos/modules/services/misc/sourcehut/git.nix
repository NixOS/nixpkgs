...
in {
  ...
  config = with scfg;
    lib.mkIf (cfg.enable && elem "git" cfg.services) {
      ...
      services = {
        cron.systemCronJobs =
          [ "*/20 * * * * ${cfg.python}/bin/gitsrht-periodic" ];
        fcgiwrap.enable = true;

        openssh.authorizedKeysCommand =
          ''/etc/ssh/gitsrht-dispatch "%u" "%h" "%t" "%k"'';
      ...
      services.sourcehut.settings = {
        ...
        "git.sr.ht".connection-string = mkDefault
        ...
        "git.sr.ht".post-update-script =
          mkDefault "${pkgs.sourcehut.gitsrht}/bin/gitsrht-update-hook";
        ...
        "git.sr.ht::dispatch".${
          ...
          "${pkgs.sourcehut.gitsrht}/bin/gitsrht-keys"
        } = mkDefault "${user}:${user}";
      };
      ...
      services.nginx.virtualHosts."git.${cfg.originBase}" = {
        ...
        locations."/static".root =
          "${pkgs.sourcehut.gitsrht}/${pkgs.sourcehut.python.sitePackages}/gitsrht";
        extraConfig = ''
              location = /authorize {
