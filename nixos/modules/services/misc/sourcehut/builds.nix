...
in {
  ...
  config = with scfg;
    let
      image_dirs = lib.lists.flatten (lib.attrsets.mapAttrsToList (distro: revs:
        lib.attrsets.mapAttrsToList (rev: archs:
          lib.attrsets.mapAttrsToList (arch: image:
            pkgs.runCommand "buildsrht-images" { } ''
              mkdir -p $out/${distro}/${rev}/${arch}
              ln -s ${image}/*.qcow2 $out/${distro}/${rev}/${arch}/root.img.qcow2
            '') archs) revs) scfg.images);
      ...
      '';
    in lib.mkIf (cfg.enable && elem "builds" cfg.services) {
      users = {
        users = {
          "${user}" = {
            isSystemUser = true;
      ...
      systemd = {
        tmpfiles.rules = [ "d ${statePath} 0755 ${user} ${user} -" ]
        ...
        services = {
          ...
          buildsrht-worker = {
            ...
            serviceConfig.ExecStart =
              "${pkgs.sourcehut.buildsrht}/bin/builds.sr.ht-worker";
          };
        };
      };
      ...
      services.sourcehut.settings = {
        ...
        "builds.sr.ht".connection-string = mkDefault
      ...
      services.nginx.virtualHosts."logs.${cfg.originBase}" =
        if scfg.enableWorker then {
          listen = with builtins;
            ...
            in [{
              addr = elemAt address 0;
              port = lib.toInt (elemAt address 2);
            }];
      ...
      services.nginx.virtualHosts."builds.${cfg.originBase}" = {
        ...
        locations."/static".root =
          "${pkgs.sourcehut.buildsrht}/${pkgs.sourcehut.python.sitePackages}/buildsrht";
      };
    };
}
