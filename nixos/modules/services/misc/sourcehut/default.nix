...
let
          ...
          lib.mapAttrs
            (
              ...
              if
                ...
                || lib.head srvMatch == srv # Include sections for the service being configured
              then
                v
              # Enable Web links and integrations between services.
              else if lib.tail srvMatch == [ null ] && cfg.${lib.head srvMatch}.enable then
                {
                  inherit (v) origin;
  ...
  commonServiceSettings = srv: {
    ...
    migrate-on-upgrade = lib.mkEnableOption "automatic migrations on package upgrade" // {
      default = true;
    };
...
in
{
  options.services.sourcehut = {
      ...
      type = lib.types.submodule {
        ...
        options.objects = {
          ...
          s3-secret-key = lib.mkOption {
            ...
            default = null;
            apply = lib.mapNullable (s: "<" + toString s);
          };
        };
        options.webhooks = {
        ...
        options."builds.sr.ht" = commonServiceSettings "builds" // {
          allow-free = lib.mkEnableOption "nonpaying users to submit builds";
          redis = lib.mkOption {
        ...
        options."lists.sr.ht" = commonServiceSettings "lists" // {
          allow-new-lists = lib.mkEnableOption "creation of new lists";
        ...
        options."meta.sr.ht" =
          ...
          // {
            ...
            welcome-emails = lib.mkEnableOption "sending stock sourcehut welcome emails after signup";
          };
        ...
        options."meta.sr.ht::billing" = {
          enabled = lib.mkEnableOption "the billing system";
        ...
        options."meta.sr.ht::settings" = {
          registration = lib.mkEnableOption "public registration";
    ...
    builds = {
      enableWorker = lib.mkEnableOption ''
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    ...
    (lib.mkIf (cfg.builds.enable || cfg.git.enable || cfg.hg.enable) {
      ...
      systemd.tmpfiles.settings."10-sourcehut-gitsrht" = lib.mkIf cfg.git.enable (lib.mkMerge [
            ...
            services.sourcehut.settings = lib.mkMerge [
              {
                ...
                "git.sr.ht::dispatch"."/usr/bin/buildsrht-keys" =
                  lib.mkDefault "${cfg.builds.user}:${cfg.builds.group}";
              }
              (lib.mkIf cfg.builds.enableWorker {
      ...
      in
      {
        ...
        mainService = lib.mkMerge [
          baseService
          {
            ...
            preStart = lib.mkIf (lib.versionOlder config.system.stateVersion "22.05") (lib.mkBefore ''
        ...
        extraServices.listssrht-process = {
          serviceConfig = {
            ...
            ExecStart =
              ...
              + lib.concatStringsSep " " cfg.lists.process.extraArgs;
    ...
    (import ./service.nix "pages" {
      ...
      mainService =
        ...
        in
        {
          ...
          '';
          serviceConfig = {
            ExecStart = lib.mkForce "${pkgs.sourcehut.pagessrht}/bin/pages.sr.ht -b ${cfg.listenAddress}:${toString cfg.pages.port}";
          };
        };
    })
