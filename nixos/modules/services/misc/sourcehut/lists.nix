...
in {
  ...
  config = with scfg;
    lib.mkIf (cfg.enable && elem "lists" cfg.services) {
      users = {
        users = {
          "${user}" = {
            isSystemUser = true;
        ...
        services = {
          ...
          listssrht-process = {
            ...
            serviceConfig = {
              ...
              Restart = "always";
              ExecStart =
                "${cfg.python}/bin/celery -A ${drv.pname}.process worker --loglevel=info";
            };
          };
      ...
      services.sourcehut.settings = {
        ...
        "lists.sr.ht".connection-string = mkDefault
        ...
        "lists.sr.ht".webhooks =
          mkDefault "redis://${rcfg.bind}:${toString rcfg.port}/2";
        # The redis connection used for the celery worker
        "lists.sr.ht".redis =
          mkDefault "redis://${rcfg.bind}:${toString rcfg.port}/4";
        # Network-key
        ...
        "lists.sr.ht::worker".reject-url =
          mkDefault "https://man.sr.ht/lists.sr.ht/etiquette.md";
      ...
      services.nginx.virtualHosts."lists.${cfg.originBase}" = {
        ...
        locations."/static".root =
          "${pkgs.sourcehut.listssrht}/${pkgs.sourcehut.python.sitePackages}/listssrht";
      };
    };
}
