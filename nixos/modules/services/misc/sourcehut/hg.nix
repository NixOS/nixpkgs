...
in {
  ...
  config = with scfg;
    lib.mkIf (cfg.enable && elem "hg" cfg.services) {
      ...
      services = {
        cron.systemCronJobs =
          [ "*/20 * * * * ${cfg.python}/bin/hgsrht-periodic" ]
          ++ optional cloneBundles
          "0 * * * * ${cfg.python}/bin/hgsrht-clonebundles";

        openssh.authorizedKeysCommand =
          ''/etc/ssh/hgsrht-dispatch "%u" "%h" "%t" "%k"'';
      ...
      services.sourcehut.settings = {
        ...
        "hg.sr.ht".connection-string = mkDefault
        ...
        "hg.sr.ht".changegroup-script =
          mkDefault "${cfg.python}/bin/hgsrht-hook-changegroup";
        ...
        "hg.sr.ht::dispatch"."/run/current-system/sw/bin/hgsrht-keys" =
          mkDefault "${user}:${user}";
      };
      ...
      services.nginx.virtualHosts."hg.${cfg.originBase}" = {
        ...
        locations."/static".root =
          "${pkgs.sourcehut.hgsrht}/${pkgs.sourcehut.python.sitePackages}/hgsrht";
      };
    };
}
