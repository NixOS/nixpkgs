...
in {
  ...
  config = with scfg;
    lib.mkIf (cfg.enable && elem "meta" cfg.services) {
      assertions = [{
        assertion = with cfgIni."meta.sr.ht::billing";
          enabled == "yes"
          -> (stripe-public-key != null && stripe-secret-key != null);
        message =
          "If meta.sr.ht::billing is enabled, the keys should be defined.";
      }];
      ...
      services.cron.systemCronJobs =
        [ "0 0 * * * ${cfg.python}/bin/metasrht-daily" ];
      services.postgresql = {
        authentication = ''
        ...
        services = {
          metasrht =
            import ./service.nix { inherit config pkgs lib; } scfg drv iniKey {
              ...
              preStart = ''
                ...
                ${concatMapStringsSep "\n\n" (attr: ''
                  if ! test -e "${statePath}/${attr}.oauth" || [ "$(cat ${statePath}/${attr}.oauth)" != "${
                    cfgIni."${attr}".oauth-client-id
                  }" ]; then
                    ...
                    printf "%s" "${
                      cfgIni."${attr}".oauth-client-id
                    }" > "${statePath}/${attr}.oauth"
                  fi
                '') (builtins.attrNames (filterAttrs (k: v:
                  !(hasInfix "::" k) && builtins.hasAttr "oauth-client-id" v
                  && v.oauth-client-id != null) cfg.settings))}
              '';
          ...
          metasrht-api =
            import ./service.nix { inherit config pkgs lib; } scfg drv iniKey {
              ...
              serviceConfig.ExecStart =
                "${pkgs.sourcehut.metasrht}/bin/metasrht-api -b :${
                  toString (port + 100)
                }";
            };
      ...
      services.sourcehut.settings = {
        ...
        "meta.sr.ht".connection-string = mkDefault
        ...
        "meta.sr.ht".webhooks =
          mkDefault "redis://${rcfg.bind}:${toString rcfg.port}/6";
        ...
        "meta.sr.ht::settings".onboarding-redirect =
          mkDefault "https://meta.${cfg.originBase}";
      ...
      services.nginx.virtualHosts."meta.${cfg.originBase}" = {
        ...
        locations."/static".root =
          "${pkgs.sourcehut.metasrht}/${pkgs.sourcehut.python.sitePackages}/metasrht";
      };
    };
}
