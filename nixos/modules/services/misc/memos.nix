...
in
{
  options.services.memos = {
    ...
    user = lib.mkOption {
      ...
      description = ''
        ...
        ::: {.note}
        If changing the default value, **You** are responsible of creating the corresponding user with {option}`users.users`.
        :::
      '';
      default = "memos";
    };
    ...
    group = lib.mkOption {
      ...
      description = ''
        ...
        ::: {.note}
        If changing the default value, **You** are responsible of creating the corresponding group with {option}`users.groups`.
        :::
      '';
      default = "memos";
    };
    ...
    dataDir = lib.mkOption {
      ...
      description = ''
        ...
        ::: {.note}
        It will be automatically created with the permissions of {option}`services.memos.user` and {option}`services.memos.group`.
        :::
      '';
    };
    ...
    environmentFile = lib.mkOption {
      ...
      description = ''
        ...
        ::: {.note}
        By default, generated from {option}`services.memos.settings`.
        :::
      '';
      example = "/var/lib/memos/memos.env";
      default = envFileFormat.generate "memos.env" cfg.settings;
      defaultText = ''
        generated from {option}`services.memos.settings`
      '';
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.memos.settings = lib.mkDefault {
      MEMOS_MODE = "prod";
      MEMOS_ADDR = "127.0.0.1";
      MEMOS_PORT = "5230";
