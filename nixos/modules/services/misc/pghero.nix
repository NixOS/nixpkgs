...
in
{
  options.services.pghero = {
    enable = lib.mkEnableOption (lib.mdDoc "PgHero service");
    package = lib.mkPackageOptionMD pkgs "pghero" { };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.pghero = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${lib.getExe cfg.package} --bind-to-activated-sockets only ${lib.escapeShellArgs cfg.extraArgs}";
        Restart = "always";
