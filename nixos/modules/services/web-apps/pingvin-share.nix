...
let
  ...
  inherit (lib) mkOption mdDoc mkEnableOption mkIf mkPackageOptionMD types;
in

rec {
  options = {
    services.pingvin-share = {
      enable = mkEnableOption (mdDoc "Pingvin Share, a self-hosted file sharing platform.");
      ...
      nginx = {
        enable = mkEnableOption (mdDoc ''
          a Nginx reverse proxy for Pingvin Share.
        '');
      };
    };
  };
