...
let cfg = config.services.keycloak;
in {
  ...
  config = let
    ...
    createLocalPostgreSQL = databaseActuallyCreateLocally && cfg.database.type
      == "postgresql";
    createLocalMySQL = databaseActuallyCreateLocally && cfg.database.type
      == "mysql";
    ...
    keycloakConfig' = builtins.foldl' lib.recursiveUpdate {
      ...
      "socket-binding-group=standard-sockets"."socket-binding=http".port =
        cfg.httpPort;
      ...
      "subsystem=datasources"."data-source=KeycloakDS" = {
        max-pool-size = "20";
        user-name = if databaseActuallyCreateLocally then
          "keycloak"
        else
      ...
      (lib.optionalAttrs (cfg.database.type == "postgresql") {
        "subsystem=datasources" = {
          ...
          "data-source=KeycloakDS" = {
            connection-url = "jdbc:postgresql://${cfg.database.host}:${
                builtins.toString cfg.database.port
              }/keycloak";
            driver-name = "postgresql";
            "connection-properties=ssl".value =
              lib.boolToString cfg.database.useSSL;
      ...
      (lib.optionalAttrs (cfg.database.type == "mysql") {
        "subsystem=datasources" = {
          ...
          "data-source=KeycloakDS" = {
            connection-url = "jdbc:mysql://${cfg.database.host}:${
                builtins.toString cfg.database.port
              }/keycloak";
            driver-name = "mysql";
            "connection-properties=useSSL".value =
              lib.boolToString cfg.database.useSSL;
            "connection-properties=requireSSL".value =
              lib.boolToString cfg.database.useSSL;
            "connection-properties=verifyServerCertificate".value =
              lib.boolToString cfg.database.useSSL;
            ...
            "connection-properties=trustCertificateKeyStoreUrl".value =
              "file:${mySqlCaKeystore}";
            "connection-properties=trustCertificateKeyStorePassword".value =
              "notsosecretpassword";
          });
        };
      })
      (lib.optionalAttrs
        (cfg.sslCertificate != null && cfg.sslCertificateKey != null) {
          "socket-binding-group=standard-sockets"."socket-binding=https".port =
            cfg.httpsPort;
          "core-service=management"."security-realm=UndertowRealm"."server-identity=ssl" =
            {
              keystore-path =
                "/run/keycloak/ssl/certificate_private_key_bundle.p12";
              keystore-password = "notsosecretpassword";
            };
          "subsystem=undertow"."server=default-server"."https-listener=https".security-realm =
            "UndertowRealm";
        })
      cfg.extraConfig
    ];
    ...
    */
        ...
        writeAttributes = path: set:
          let
            ...
            prefixExpression = string:
              let match = (builtins.match ''"\$\{.*}"'' string);
              in if match != null then "expression " + string else string;

            writeAttribute = attribute: value:
              let type = builtins.typeOf value;
              in if type == "set" then
                let names = builtins.attrNames value;
                in builtins.foldl' (text: name:
                  text + (writeAttribute "${attribute}.${name}" value.${name}))
                "" names
              ...
              '' else if builtins.elem type [ "string" "path" "bool" ] then
                ...
                in ''
                  ...
                  }) of ${path}:read-attribute(name="${attribute}")
                    ${path}:write-attribute(name=${attribute}, value=${value'})
                  end-if
                ''
              else
                throw "Unsupported type '${type}' for path '${path}'!";
          in lib.concatStrings (lib.mapAttrsToList
            (attribute: value: (writeAttribute attribute value)) set);
          ...
          let
            makeArg = attribute: value:
              ...
              in if type == "set" then
                ...
                throw
                "Unsupported type '${type}' for attribute '${attribute}'!";
          in lib.concatStringsSep ", " (lib.mapAttrsToList makeArg set);
        ...
        */
        recurse = state: node:
          let
            ...
            isPath = name:
              let value = lib.getAttrFromPath (path ++ [ name ]) attrs;
              in if (builtins.match ".*([=]).*" name) == [ "=" ] then
                if builtins.isAttrs value || value == null then
                  true
                else
                  throw "Parsing path '${
                    lib.concatStringsSep "." (path ++ [ name ])
                  }' failed: JBoss attributes cannot contain '='!"
              else
                false;
          ...
          in state // {
            text = state.text + (if nodeValue != null then
              ''
    ...
    jbossCliScript =
      pkgs.writeText "jboss-cli-script" (mkJbossScript keycloakConfig');
  ...
  in lib.mkIf cfg.enable {
    ...
    in {
      ...
      path = with pkgs; [ cfg.package openssl replace-secret ];
      environment = {
      ...
      serviceConfig = {
        ...
        in [
          "+${
            pkgs.writeShellScript "keycloak-start-pre-full-privileges"
            startPreFullPrivileges
          }"
