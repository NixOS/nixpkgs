  ...
  config = let
    ...
    mkJbossScript = attrs: let
      ...
      writeAttributes = path: set: let
        ...
        writeAttribute = attribute: value: let
          type = typeOf value;
        in
          if type == "set"
          then
          ...
          then
            ...
            ''
          else if elem type ["string" "path" "bool"]
          then
            let
              value' =
                if type == "bool"
       ...
       */
      makeArgList = set: let
        makeArg = attribute: value: let
          type = typeOf value;
        in
          if type == "set"
          then "${attribute} = { " + (makeArgList value) + " }"
          else if elem type ["string" "path" "bool"]
          then
            "${attribute} = ${
              if type == "bool"
          ...
          then ""
          else throw "Unsupported type '${type}' for attribute '${attribute}'!";
      in
       ...
       */
      recurse = nodePath: nodeValue: let
        nodeContent =
          if isAttrs nodeValue && nodeValue._type or "" == "order"
          then nodeValue.content
          else nodeValue;
        isPath = name: let
        ...
        in
          if (match ".*([=]).*" name) == ["="]
          then
            if isAttrs value || value == null
            then true
            else throw "Parsing path '${concatStringsSep "." (nodePath ++ [name])}' failed: JBoss attributes cannot contain '='!"
          else false;
        jbossPath = "/" + concatStringsSep "/" nodePath;
        children =
          if !isAttrs nodeContent
          then {}
          else nodeContent;
        subPaths = filter isPath (attrNames children);
        getPriority = name: let
          value = children.${name};
        in
          if value._type or "" == "order"
          then value.priority
          else 1000;
