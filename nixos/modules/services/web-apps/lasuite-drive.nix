...
in
{
  ...
  config = mkIf cfg.enable {
    warnings = mkIf (cfg.enableNginx && !(hasSuffix "/" cfg.s3Url)) [
      ''
        services.lasuite-drive.s3Url does not finish with a trailing /.
              This will potentially lead nginx to do bad requests.''
    ];
    ...
    systemd.services.lasuite-drive = {
      ...
      serviceConfig = {
        BindReadOnlyPaths = "${cfg.backendPackage}/share/static:/var/lib/lasuite-drive/static";
