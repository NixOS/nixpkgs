  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      }
      {
        ...
        message = ''
          CREDS_KEY, CREDS_IV, JWT_SECRET, and JWT_REFRESH_SECRET must be defined in `services.librechat.credentials` and point to locations of files on the host.
    ...
    # TODO: can we get the port using nix? where is the port even defined, what if the user change it?
