...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.immichApiKeyFile != null;
        message = "services.immich-kiosk.immichApiKeyFile must be set";
      }
    ];
    ...
    systemd.services.immich-kiosk = {
      ...
      script = ''
        ...
        ${lib.optionalString (cfg.settings != { }) ''
          export KIOSK_CONFIG_FILE="$RUNTIME_DIRECTORY/config.yaml"
        ''}
        exec ${lib.getExe cfg.package}
      '';

      preStart = lib.optionalString (cfg.settings != { }) ''
        cp ${format.generate "config.yaml" { kiosk = cfg.settings; }} "$RUNTIME_DIRECTORY/config.yaml"
      '';
