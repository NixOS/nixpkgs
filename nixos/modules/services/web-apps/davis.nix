...
let
  ...
  davisEnvVars = lib.generators.toKeyValue {
    mkKeyValue = lib.flip lib.generators.mkKeyValueDefault "=" {
      mkValueString = v:
        if builtins.isInt     v then toString v
  ...
  in
    lib.mkIf cfg.enable {
      ...
      services.davis.config =
        {
          APP_ENV = "prod";
          CACHE_DIRECTORY = "${cfg.dataDir}/var/cache";
          LOG_DIRECTORY = "${cfg.dataDir}/var/log";
        ...
        // (
          ...
          else {MAILER_DSN._secret = mail.dsnFile;}
        )
        // (
          if db.createLocally
          then {
            DATABASE_URL =
              ...
              then "postgres://${user}@localhost/${db.name}?host=/run/postgresql&charset=UTF-8"
              else if mysqlLocal
      ...
      systemd.services.davis-env-setup = {
        ...
        before = [ "phpfpm-davis.service" "davis-db-migrate.service" ];
        ...
        restartTriggers = [ cfg.package davisEnv ];
        script = ''
          # error handling
          set -euo pipefail
      ...
      systemd.services.davis-db-migrate = {
        ...
        after = lib.optional mysqlLocal "mysql.service" ++ lib.optional pgsqlLocal "postgresql.service" ++ [ "davis-env-setup.service" ];
        requires = lib.optional mysqlLocal "mysql.service" ++ lib.optional pgsqlLocal "postgresql.service" ++ [ "davis-env-setup.service" ];
        wantedBy = [ "multi-user.target" ];
        serviceConfig =
          defaultServiceConfig
          // {
      ...
      systemd.services.phpfpm-davis.after = [ "davis-env-setup.service" "davis-db-migrate.service" ];
      systemd.services.phpfpm-davis.requires =
        [ "davis-env-setup.service" "davis-db-migrate.service" ]
