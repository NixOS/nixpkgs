...
let
  ...
  configFile = pkgs.writeText "ihatemoney.cfg" ''
    ...
    SQLALCHEMY_DATABASE_URI = '${
      if cfg.backend == "sqlite" then
        "sqlite:////var/lib/ihatemoney/ihatemoney.sqlite"
      else
        "postgresql:///${db}"
    }'
    ...
    ADMIN_PASSWORD = r"${
      toString cfg.adminHashedPassword # toString null == ""
    }"
...
in {
  options.services.ihatemoney = {
    enable = mkEnableOption
      "ihatemoney webapp. Note that this will set uwsgi to emperor mode";
    backend = mkOption {
    ...
    adminHashedPassword = mkOption {
      ...
      default = null;
      description =
        "The hashed password of the administrator. To obtain it, run <literal>ihatemoney generate_password_hash</literal>";
    };
    ...
    enableDemoProject =
      mkEnableOption "access to the demo project in ihatemoney";
    enablePublicProjectCreation =
      mkEnableOption "permission to create projects in ihatemoney by anyone";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [ "d /var/lib/ihatemoney 770 ${user} ${group}" ];
    users = {
      users.${user} = {
        isSystemUser = true;
        inherit group;
