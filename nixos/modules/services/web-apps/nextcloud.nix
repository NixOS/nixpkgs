...
let
  ...
  nextcloudGreaterOrEqualThan = lib.versionAtLeast overridePackage.version;
  nextcloudOlderThan = lib.versionOlder overridePackage.version;
...
in
{
        ...
        services.nginx.virtualHosts.${cfg.hostName} = {
          ...
          locations = {
            ...
            "~ \\.php(?:$|/)" = {
              ...
              extraConfig = ''
                ...
                rewrite ^/(?!index|remote|public|cron|core\/ajax\/update|status|ocs\/v[12]|updater\/.+|oc[s${
                  lib.optionalString (!ocmProviderIsNotAStaticDirAnymore) "m"
            ...
            "~ ^\\/(?:updater|ocs-provider${
              lib.optionalString (!ocmProviderIsNotAStaticDirAnymore) "|ocm-provider"
            })(?:$|\\/)".extraConfig =
              ''
                try_files $uri/ =404;
                index index.php;
  ...
  meta.maintainers = lib.teams.nextcloud;
}
