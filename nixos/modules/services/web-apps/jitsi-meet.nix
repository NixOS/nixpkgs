...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.jitsi-meet-init-secrets = {
      ...
      before = [ "jicofo.service" "jitsi-videobridge2.service" ] ++ (lib.optional cfg.prosody.enable "prosody.service") ++ (lib.optional cfg.jigasi.enable "jigasi.service");
      serviceConfig = {
        Type = "oneshot";
        UMask = "027";
        User = "root";
      ...
      script = let
        secrets = [ "jicofo-component-secret" "jicofo-user-secret" "jibri-auth-secret" "jibri-recorder-secret" ] ++ (lib.optionals cfg.jigasi.enable [ "jigasi-user-secret" "jigasi-component-secret" ]) ++ (lib.optional (cfg.videobridge.passwordFile == null) "videobridge-secret");
      in
      ''
      ...
      virtualHosts.${cfg.hostName} = {
        ...
        let
          templatedJitsiMeet = pkgs.runCommand "templated-jitsi-meet" { } ''
            ...
            cp -R . $out
            cp ${overrideJs "${pkgs.jitsi-meet}/config.js" "config" (lib.recursiveUpdate defaultCfg cfg.config) cfg.extraConfig} $out/config.js
    ...
    services.jitsi-meet.config = lib.recursiveUpdate
