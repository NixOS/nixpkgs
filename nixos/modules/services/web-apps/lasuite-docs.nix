...
in
{
      ...
      type = types.submodule {
        ...
        options = {
          ...
          REDIS_URL = mkOption {
            ...
            default =
              if cfg.redis.createLocally then
                "unix://${config.services.redis.servers.lasuite-docs.unixSocket}?db=1"
              else
                null;
          ...
          CELERY_BROKER_URL = mkOption {
            ...
            default =
              if cfg.redis.createLocally then
                "redis+socket://${config.services.redis.servers.lasuite-docs.unixSocket}?db=2"
              else
                null;
  ...
  config = mkIf cfg.enable {
    systemd.services.lasuite-docs = {
      ...
      preStart = ''
        ln -sf ${cfg.backendPackage}/share/static /var/lib/lasuite-docs/static
