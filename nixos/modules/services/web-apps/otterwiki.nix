    ...
    { config, ... }@moduleArgs:
    {
      options = {
        ...
        repository = lib.mkOption {
          ...
          description = "Path to the Git repository for the wiki content.";
        };
...
in
{
  ...
  config = {
    services.gunicorn.instances = lib.mapAttrs (name: inst: {
      inherit (inst) process socket;
      app.package = pkgs.otterwiki;
    }) cfg.instances;
    ...
    systemd.services = mapToUnits (inst: {
      ...
      serviceConfig = {
        ...
        PrivateNetwork = false; # needed to send emails
