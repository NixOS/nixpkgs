  ...
  startupScript =
    ...
    pkgs.writeScript "booklore-env" ''
      ...
      ${lib.strings.concatStringsSep "\n" (
        lib.attrsets.mapAttrsToList (key: path: "export ${key}=$(cat \"${path}\")") cfg.secretFiles
      )}
      ${cmd}
    '';
...
in
{
  options = {
    services.booklore = {
      ...
      bookDir = mkOption {
        description = "Storage path of the book library. Needs to exists already if not using the default path.";
