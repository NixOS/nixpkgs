...
in {
  ...
  config = lib.mkIf cfg.enable {
    services.peering-manager = {
      ...
      '';
      # TODO think about adding a oidcSecretFile option to use secrets
      # + lib.optionalString (cfg.oidcSecretFile != null) ''
      #  with open("${cfg.oidcSecretFile}", "r") as file:
      #    OIDC_RP_CLIENT_SECRET = file.readline()
      #'';

      plugins = lib.mkIf (cfg.enableLdap || cfg.enableOidc) (ps: [
        (lib.mkIf cfg.enableLdap ps.django-auth-ldap)
        (lib.mkIf cfg.enableOidc ps.mozilla-django-oidc)
      ]);
    };
