...
let
  ...
  // lib.optionalAttrs (cfg.redis.host != null && !cfg.redis.enableUnixSocket) { DB_HOST = cfg.database.host; }
  // lib.optionalAttrs (cfg.redis.port != null && !cfg.redis.enableUnixSocket) { REDIS_PORT = toString(cfg.redis.port); }
    ...
    assertions = [
      {
        ...
        message = ''
          <option>services.mastodon.redis.host</option> and <option>services.mastodon.redis.port</option>
            needs to be set if <option>services.mastodon.redis.createLocally</option> is not enabled.
        '';
      }
      {
