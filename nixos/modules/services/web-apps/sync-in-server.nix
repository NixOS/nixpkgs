...
in {
  options.services.sync-in = {
    ...
    package = lib.mkOption {
      ...
      description = "Package for Sync-in";
    };
    ...
    runtimePath = lib.mkOption {
      type = lib.types.str;
      default = "/var/run/sync-in-server";
      description = ''
        Runtime path for sync-in-server.
      '';
    };
  ...
  config = lib.mkIf cfg.enable (
    ...
    in {
      ...
      systemd.tmpfiles.rules = [
        ...
        "d ${cfg.runtimePath} 0750 ${cfg.user} ${cfg.group}"
        "d ${cfg.files.dataPath} 0750 ${cfg.user} ${cfg.group}"
      ...
      systemd.services.sync-in-server = {
        ...
        preStart = ''
          ...
          if ["$DB_PASS" -eq "" ]; then
            DB_PASS=`pwgen 16 1`
            echo $DB_PASS > ${cfg.mysql.passwordFile}
          fi
          if ["$ADMIN_PASS" -eq "" ]; then
            ADMIN_PASS=`pwgen 16 1`
            echo $DB_PASS > ${cfg.mysql.passwordFile}
          fi
          if ["$ACCESS_TOKEN" -eq "" ]; then
            ACCESS_TOKEN=`${pkgs.pwgen}/bin/pwgen 32 1`
            echo $ACCESS_TOKEN > ${cfg.auth.token.access.secretFile}
          fi
          if ["$REFRESH_TOKEN" -eq "" ]; then
            REFRESH_TOKEN=`${pkgs.pwgen}/bin/pwgen 32 1`
            echo $REFRESH_TOKEN > ${cfg.auth.token.refresh.secretFile}
          fi

          cp ${configFile} ${cfg.runtimePath}/environment.yaml
          chmod ug+w ${cfg.runtimePath}/environment.yaml

          sed -i "s|__DB_PASSWORD__|$DB_PASS|g" ${cfg.runtimePath}/environment.yaml
          sed -i "s|__LDAP_PASSWORD__|$LDAP_PASS|g" ${cfg.runtimePath}/environment.yaml

          sed -i "s|__MAIL_PASS__|$MAIL_PASS|g" ${cfg.runtimePath}/environment.yaml
          sed -i "s|__LDAP_PASSWORD__|$LDAP_PASS|g" ${cfg.runtimePath}/environment.yaml
          sed -i "s|__OIDC_PASSWORD__|$OIDC_PASS|g" ${cfg.runtimePath}/environment.yaml
          sed -i "s|__ACCESS_TOKEN__|$ACCESS_TOKEN|g" ${cfg.runtimePath}/environment.yaml
          sed -i "s|__REFRESH_TOKEN__|$REFRESH_TOKEN|g" ${cfg.runtimePath}/environment.yaml
          sed -i -E '/ecretFile:|asswordFile:|assFile:|: null$/d' ${cfg.runtimePath}/environment.yaml

          cp ${./drizzle.js} ${cfg.runtimePath}/drizzle.js
          chown ${cfg.user}:${cfg.group} ${cfg.runtimePath}/drizzle.js

          sed -i "s|\./release/sync-in-server/server/|${cfg.package}/lib/release/sync-in-server/server/|g" ${cfg.runtimePath}/drizzle.js
          sed -i "s|__PASSWORD__|$DB_PASS|g" ${cfg.runtimePath}/drizzle.js
          sed -i "s|__USER__|$DB_USER|g" ${cfg.runtimePath}/drizzle.js
          sed -i "s|__HOST__|$DB_HOST|g" ${cfg.runtimePath}/drizzle.js
          sed -i "s|__PORT__|$DB_PORT|g" ${cfg.runtimePath}/drizzle.js
          sed -i "s|__NAME__|$DB_NAME|g" ${cfg.runtimePath}/drizzle.js
          ...
          sed "s|/etc/sync-in/drizzle.js|${cfg.runtimePath}/drizzle.js|g" ${cfg.package}/bin/sync-in-migrate-db >  ${cfg.applications.files.dataPath}/sync-in-migrate-db
