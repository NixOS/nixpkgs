...
in
{
  options.services.node-red = {
    ...
    package = lib.mkPackageOption pkgs [ "nodePackages" "node-red" ] { };
    ...
    configFile = lib.mkOption {
      ...
      defaultText = lib.literalExpression ''"''${package}/lib/node_modules/node-red/settings.js"'';
      description = ''
      ...
      serviceConfig = lib.mkMerge [
        {
          User = cfg.user;
          Group = cfg.group;
          ExecStart = "${cfg.package}/bin/node-red ${pkgs.lib.optionalString cfg.safe "--safe"} --settings ${cfg.configFile} --port ${toString cfg.port} --userDir ${cfg.userDir} ${lib.concatStringsSep " " (lib.mapAttrsToList (name: value: "-D ${name}=${value}") cfg.define)}";
          PrivateTmp = true;
          Restart = "always";
