...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      "d ${cfg.settings.jelu.database.path} 0750 ${cfg.user} ${cfg.group} - -"
      "d ${cfg.settings.jelu.files.images} 0750 ${cfg.user} ${cfg.group} - -"
      "d ${cfg.settings.jelu.files.imports} 0750 ${cfg.user} ${cfg.group} - -"
    ];
