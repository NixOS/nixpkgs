...
let
  ...
  backupRsyncCommand = "${pkgs.rsync}/bin/rsync -rlt --del --perms";
  restoreRsyncCommand = "${backupRsyncCommand} --chmod=u+rwX --chown=nextcloud:nextcloud";
  dbBackupRestore =
    if cfg.config.dbtype == "sqlite"
    then
      {
    ...
    else if cfg.config.dbtype == "pgsql"
    then
      let
      ...
      in
      {
        ...
        restoreCommand = ''
          ${setPassword}
          sudo -E -u nextcloud ${pg_restore} ${commonFlags} --clean -d ${cfg.config.dbname} $1/nextcloud-backup/${backupFileName}
        '';
      }
    else if cfg.config.dbtype == "mysql"
    then
      let
    ...
    else throw "Unknown nextcloud database type ${cfg.config.dbtype}";
  checkBackup = ''
    for f in \
  ...
  '';
  backupScript = pkgs.writeScriptBin "nextcloud-backup" ''
    #! ${pkgs.runtimeShell}
    set -Eeuo pipefail
    ...
    echo "Nextcloud Backup Metadata" > $1/nextcloud-backup/metadata.txt
    echo "Backup Time: $(date --rfc-3339=seconds)" >> $1/nextcloud-backup/metadata.txt
    echo "Nextcloud Version: ${cfg.package.version}" >> $1/nextcloud-backup/metadata.txt
    echo "Backup Format Version: 1" >> $1/nextcloud-backup/metadata.txt
    echo "Database Type: ${cfg.config.dbtype}" >> $1/nextcloud-backup/metadata.txt
    ${checkBackup}
    echo "Nextcloud backup completed."
  '';
  restoreScript = pkgs.writeScriptBin "nextcloud-restore" ''
    #! ${pkgs.runtimeShell}
    set -Eeuo pipefail
    if [ "$#" != "1" ] || [ "$1" = "-h" ] || [ "$1" = "--help" ] ;then
    ...
    NEXTCLOUD_MAJOR_VERSION=$(grep "^Nextcloud Version:" "$1/nextcloud-backup/metadata.txt" | sed -r "s/Nextcloud Version: ([0-9]+)\..*/\1/")
    NEXTCLOUD_DB_TYPE=$(grep "^Database Type:" "$1/nextcloud-backup/metadata.txt" | sed -r "s/Database Type: //")
    BACKUP_FORMAT_VERSION=$(grep "^Backup Format Version:" "$1/nextcloud-backup/metadata.txt" | sed -r "s/Backup Format Version: //")
    # A few sanity checks to make sure that it's possible to restore the backup.
    if [ ! "$BACKUP_FORMAT_VERSION" = "1" ] ;then
      # In case there are backwards-incompatible changes to this script in the future
      echo "Unknown backup format version $BACKUP_FORMAT_VERSION, aborting."
      exit 1
    fi
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    environment.systemPackages = [ backupScript restoreScript ];
  };
}
