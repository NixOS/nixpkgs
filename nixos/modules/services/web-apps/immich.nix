...
in
{
  options.services.immich = {
      ...
      type = lib.types.nullOr (
        lib.types.submodule {
          ...
          options = {
            ...
            server.externalDomain = lib.mkOption {
              type = lib.types.str;
              default = "";
    ...
    redis = {
      enable = lib.mkEnableOption "a redis cache for use with immich" // {
        default = true;
      };
      host = lib.mkOption {
        type = lib.types.str;
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.immich-server = {
      ...
      serviceConfig = commonServiceConfig // {
        ...
        SupplementaryGroups = lib.mkIf (cfg.redis.enable && isRedisUnixSocket) [
          config.services.redis.servers.immich.group
        ];
      };
    };

    systemd.services.immich-machine-learning = lib.mkIf cfg.machine-learning.enable {
    ...
    users.users = lib.mkIf (cfg.user == "immich") {
      immich = {
        name = "immich";
        group = cfg.group;
        isSystemUser = true;
    ...
    users.groups = lib.mkIf (cfg.group == "immich") { immich = { }; };
