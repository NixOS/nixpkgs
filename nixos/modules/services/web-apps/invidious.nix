  ...
  serviceConfig = {
    systemd.services.invidious = {
      ...
      + lib.optionalString generateHmac ''
        configParts+=("$(${lib.getExe pkgs.jq} -R '{"hmac_key":.}' <"${generatedHmacKeyFile}")")
      ''
      ...
      + lib.optionalString (cfg.database.host != null) ''
        configParts+=("$(${lib.getExe pkgs.jq} -R '{"db":{"password":.}}' ${lib.escapeShellArg cfg.database.passwordFile})")
      ''
      ...
      + ''
        export INVIDIOUS_CONFIG="$(${lib.getExe pkgs.jq} -s 'reduce .[] as $item ({}; . * $item)' <<<"''${configParts[*]}")"
