...
let
  ...
  settingsType = submodule {
    ...
    options = {
      ...
      jwt_secret_key = mkOption {
        type = str;
        description = ''
          secret used to sign JWT tokens. 64 bytes long string.

          ::: {.warning}
            This secret is stored in the WORLD-READABLE Nix store
          ::
        '';
      };
      session_store_key = mkOption {
        type = str;
        description = ''
          secret for session store. 64 bytes long string.
  ...
  settingsDefault = {
    blobs_path = "${cfg.dataDir}/blobs";
    cache = "${cfg.dataDir}/cache";
    database = "${cfg.dataDir}/stash-go.sqlite";
    generated = "${cfg.dataDir}/generated";
    plugins_path = if (cfg.mutablePlugins) then "${cfg.dataDir}/plugins" else cfg.plugins;
    scrapers_path = if (cfg.mutableScrapers) then "${cfg.dataDir}/scrapers" else cfg.scrapers;
...
in
{
  meta.maintainers = with lib.maintainers; [ DrakeTDL ];
  ...
  options = {
    services.stash = {
      ...
      dataDir = mkOption {
        ...
        description = ''
          The directory where Stash stores its files.

          ::: {.note}
          If left as the default value this directory will automatically be created
          before the Stash starts, otherwise you are responsible for ensuring
        ...
        defaultText = literalExpression ''
          {
            ...
            plugins_path = if (services.stash.mutablePlugins) then "${dataDir}/plugins" else services.stash.plugins;
            scrapers_path = if (services.stash.mutableScrapers) then "${dataDir}/scrapers" else services.stash.scrapers;
        ...
        ExecStartPre = writers.writeBash "stash-setup.bash" (
          ''
            ...
            if [[ ! -z "${toString cfg.mutableSettings}" || ! -f ${cfg.dataDir}/config.yml ]]; then
              install -T ${settingsFile} ${cfg.dataDir}/config.yml
            fi
          ''
