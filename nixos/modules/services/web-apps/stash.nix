  ...
  options = {
    services.stash = {
      ...
      passwordFile = mkOption {
        description = ''
          The path to a file containing a string hashed with Bcrypt

          ::: {.note}
            {option}`services.stash.settings.username` must also be used for Stash auth to be activated.
          :::
        '';
        type = nullOr path;
        default = null;
    ...
    assertions = [
      {
        assertion =
          (
            cfg.settings.username or null != null
        ...
        message = "You must set either both username and passwordFile or password, or neither.";
      }
    ];
      ...
      serviceConfig = {
        ...
        ExecStartPre = writers.writeBash "stash-setup.bash" (
          ''
            ...
            env \
              jwt_secret_key=$(cat ${cfg.jwtSecretKeyFile}) \
              session_store_key=$(cat ${cfg.sessionStoreKeyFile}) \
              ${optionalString (cfg.passwordFile != null) "password=$(cat ${cfg.passwordFile})"} \
              ${getExe yq-go} '
                .jwt_secret_key = strenv(jwt_secret_key) |
                .session_store_key = strenv(session_store_key) |
                (
                    strenv(password) as $pass |
                    with(select($pass != ""); .password = $pass)
                )
              ' ${settingsFile} > ${dataDir}/config.yml
