...
let
  inherit (lib)
    mkDefault mkEnableOption mkForce mkIf mkMerge mkOption types maintainers
    recursiveUpdate;
  inherit (lib)
    any attrValues concatMapStrings concatMapStringsSep flatten
    literalExpression;
  inherit (lib)
  ...
  dokuwikiLocalConfig = hostName: cfg:
    pkgs.writeText "local-${hostName}.php" ''
      <?php
  ...
  dokuwikiPluginsLocalConfig = hostName: cfg:
    pkgs.writeText "plugins.local-${hostName}.php" ''
      <?php
      ${cfg.pluginsConfig}
    '';
  ...
  pkg = hostName: cfg:
    pkgs.stdenv.mkDerivation rec {
      ...
      installPhase = ''
        ...
        ln -s ${
          dokuwikiPluginsLocalConfig hostName cfg
        } $out/share/dokuwiki/plugins.local.php

        # symlink acl
        ln -s ${
          dokuwikiAclAuthConfig hostName cfg
        } $out/share/dokuwiki/acl.auth.php

        # symlink additional plugin(s) and templates(s)
        ${concatMapStringsSep "\n" (template:
          "ln -s ${template} $out/share/dokuwiki/lib/tpl/${template.name}")
        cfg.templates}
        ${concatMapStringsSep "\n" (plugin:
          "ln -s ${plugin} $out/share/dokuwiki/lib/plugins/${plugin.name}")
        cfg.plugins}
      '';
    };
  ...
  siteOpts = { config, lib, name, ... }: {
    options = {
      ...
      aclFile = mkOption {
        ...
        default = if (config.aclUse && config.acl == null) then
...
in {
      ...
      assertions = flatten (mapAttrsToList (hostName: cfg: [
        {
          ...
          message =
            "Either services.dokuwiki.sites.${hostName}.acl or services.dokuwiki.sites.${hostName}.aclFile is mandatory if aclUse true";
        }
        {
          assertion = cfg.usersFile != null -> cfg.aclUse != false;
          message =
            "services.dokuwiki.sites.${hostName}.aclUse must must be true if usersFile is not null";
        }
      ]) eachSite);

      warnings = mapAttrsToList (hostName: _:
        ''
          services.dokuwiki."${hostName}" is deprecated use services.dokuwiki.sites."${hostName}"'')
        (oldSites cfg);
...

    {
      systemd.tmpfiles.rules = flatten (mapAttrsToList (hostName: cfg:
        [
          ...
          "d ${
            stateDir hostName
          }/media_attic 0750 ${user} ${webserver.group} - -"
          "d ${
            stateDir hostName
          }/media_meta 0750 ${user} ${webserver.group} - -"
  ...
  meta.maintainers = with maintainers; [ _1000101 onny ];
}
