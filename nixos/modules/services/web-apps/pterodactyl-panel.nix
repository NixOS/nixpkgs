...
let
  ...
  setupScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      php ${cfg.finalPackage}/artisan migrate --seed --force
      php ${cfg.finalPackage}/artisan optimize:clear
    '';
  };
...
in
{
  options.services.pterodactyl.panel = {
    ...
    package = lib.mkPackageOption pkgs "pterodactyl-panel" { };

    finalPackage = lib.mkOption {
      type = lib.types.package;
      visible = false;
      readOnly = true;
      description = "Resulting customized pterodactyl-panel package.";
    };
  ...
  config = lib.mkIf cfg.enable {
    services.pterodactyl.panel.finalPackage = cfg.package.override {
      dataDir = cfg.dataDir;
    };
    ...
    systemd.services.pteroq = {
      ...
      serviceConfig = cfgService // {
        ExecStart = "${lib.getExe cfg.phpPackage} ${cfg.finalPackage}/artisan queue:work --queue=high,standard,low --sleep=3 --tries=3";
        Restart = "always";
      };
    };
    ...
    systemd.services.pterodactyl-panel-cron = {
      ...
      serviceConfig = cfgService // {
        Type = "oneshot";
        ExecStart = "${lib.getExe cfg.phpPackage} ${cfg.finalPackage}/artisan schedule:run";
      };
    };
