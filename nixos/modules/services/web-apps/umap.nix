...
let
  ...
  staticDir =
    if
      builtins.hasAttr "UMAP_CUSTOM_STATICS" cfg.settings
      || builtins.hasAttr "UMAP_PICTOGRAMS_COLLECTIONS" cfg.settings
    then
    ...
    nginx.resolver = lib.mkOption {
      ...
      default =
        if config.services.resolved.enable then
          ...
          lib.concatStringsSep " " config.networking.nameservers;
