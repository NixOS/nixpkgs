...
let
  ...
  finalPackage = cfg.package.overrideAttrs (old: {
    ...
    prePatch =
      ...
      in
      ''
        ${pkgs.yq}/bin/yq -s '.[0] * .[1]' open_web_calendar/default_specification.yml ${nixosSpec} > newspec.json
        mv newspec.json open_web_calendar/default_specification.yml
      '';
  });
  inherit (finalPackage) python;
  pythonEnv = python.buildEnv.override { extraLibs = [ (python.pkgs.toPythonModule finalPackage) ]; };
