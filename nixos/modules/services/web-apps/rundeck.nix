...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = [
      "L+ /var/log/rundeck - - - - ${cfg.dataDir}/var/logs"
    ];
        ...
        ExecStart = toString (
          pkgs.writeShellScript "start-rundeck" ''
            ...
            if [ ! -f ${cfg.dataDir}/.ssh/id_rsa ]; then
              umask 0077
              ${lib.getExe' pkgs.openssh "ssh-keygen"} -t rsa -b 4096 -N "" -f ${cfg.dataDir}/.ssh/id_rsa
              chmod 644 ${cfg.dataDir}/.ssh/id_rsa.pub
              chown ${cfg.user}:${cfg.group} ${cfg.dataDir}/.ssh/id_rsa ${cfg.dataDir}/.ssh/id_rsa.pub
            fi
