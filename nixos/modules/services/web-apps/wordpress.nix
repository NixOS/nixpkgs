...
let
  ...
  in
    if lib.isString v then lib.escapeShellArg v
    # NOTE: If any value contains a , (comma) this will not get escaped
    else if lib.isList v && lib.any lib.strings.isCoercibleToString v then lib.escapeShellArg (lib.concatMapStringsSep "," toString v)
  ...
  siteOpts = { lib, name, config, ... }:
    {
      options = {
        ...
        plugins = lib.mkOption {
          type = with lib.types; coercedTo
            ...
            (l: warn "setting this option with a list is deprecated"
              lib.listToAttrs (map (p: lib.nameValuePair (p.name or (throw "${p} does not have a name")) p) l))
            (attrsOf path);
          default = {};
          description = ''
        ...
        themes = lib.mkOption {
          ...
          defaultText = lib.literalExpression "{ inherit (pkgs.wordpressPackages.themes) twentytwentythree; }";
          description = ''
