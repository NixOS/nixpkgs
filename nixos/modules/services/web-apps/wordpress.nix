...
let
  ...
  inherit (lib)
    filterAttrs mapAttrs mapAttrs' mapAttrsToList nameValuePair optional
  ...
  wpConfig = hostName: cfg:
    pkgs.writeText "wp-config-${hostName}.php" ''
      <?php
  ...
  config = mkIf (eachSite != { }) (mkMerge [
    {
      ...
      warnings = mapAttrsToList (hostName: _:
        ''
          services.wordpress."${hostName}" is deprecated use services.wordpress.sites."${hostName}"'')
        (oldSites cfg);
