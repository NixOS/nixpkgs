  ...
  options = {
    services.linkding = {
      ...
      extraConfigFile = mkOption {
        ...
        default = null;
        description = lib.mdDoc ''
          Path to a file with extra linkding config options file. You can use it to set sensitive
          options like {env}`LD_SUPERUSER_PASSWORD`.
          See [the options documentation](https://github.com/sissbruecker/linkding/blob/master/docs/Options.md)
          for available options.
      ...
      database.engine = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          `localhost`, will add systemd dependencies on `postgresql.service`, but the postgres user
          and database are not created automatically.
        '';
      };
