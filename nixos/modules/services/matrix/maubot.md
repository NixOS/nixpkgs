...
1. If you want to use PostgreSQL instead of SQLite, do this:
...
1. If you plan to expose your Maubot interface to the web, do something
   like this:
...
1. Optionally, set `services.maubot.pythonPackages` to a list of python3
   packages to make available for Maubot plugins.

1. Optionally, set `services.maubot.plugins` to a list of Maubot
...
1. Start Maubot at least once before doing the following steps (it's
   necessary to generate the initial config).

1. If your PostgreSQL connection requires a password, add
...
1. To create a user account for logging into Maubot web UI and
...
1. Optional: if you want to be able to register new users with the
...
1. Restart Maubot after editing `/var/lib/maubot/config.yaml`, and
