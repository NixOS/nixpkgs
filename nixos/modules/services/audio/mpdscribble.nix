  ...
  config = mkIf cfg.enable {
    systemd.services.mpdscribble = {
      ...
      serviceConfig = {
        User = cfg.user;
        Group = cfg.group;
        ExecStart = "${pkgs.mpdscribble}/bin/mpdscribble --no-daemon --conf ${mpdscribbleConf}";
        PermissionsStartOnly = true;
      };
    };

    users.extraUsers = optionalAttrs (cfg.user == "mpdscribble") (singleton {
      inherit uid;
      name  = cfg.user;
      group = cfg.group;
    });

    users.extraGroups = optionalAttrs (cfg.group == "mpdscribble") (singleton {
      gid = gid;
    });
  };
