...
in
{
    ...
    castopod = {
      ...
      nginx = {
        virtualHost = lib.mkOption {
          ...
          example = {
            serverName = "castopod.example.org";
            forceSSL = true;
            enableACME = true;
          };
          description = lib.mdDoc ''
  ...
  config = lib.mkIf cfg.enable {
    services.castopod.settings =
      let
        sslEnabled = with cfg.nginx.virtualHost; addSSL || forceSSL || onlySSL || enableACME || useACMEHost != null;
        baseURL = "http${lib.optionalString sslEnabled "s"}://${cfg.nginx.virtualHost.serverName}";
      in
    ...
    services.nginx = {
      ...
      virtualHosts."${cfg.nginx.virtualHost.serverName}" = lib.mkMerge [
        ...
        {
          # https://code.castopod.org/adaures/castopod/-/blob/main/docker/production/web-server/nginx.conf
