...
let
  ...
  mpdConf = pkgs.writeTextFile {
    ...
    derivationArgs = {
      expectScript = ''
        spawn ${lib.getBin pkgs.buildPackages.mpd}/bin/mpd --no-daemon "$env(out)"
        expect {
    ...
    checkPhase = ''
      ${lib.getBin pkgs.buildPackages.expect}/bin/expect -f "$expectScriptPath"
    '';
  };
  ...
  options = {
    ...
    services.mpd = {
      ...
      openFirewall = lib.mkOption {
        type = lib.types.bool;
        default = builtins.elem cfg.settings.bind_to_address [
          "any"
          "0.0.0.0"
        ];
        defaultText = lib.literalExpression ''
          builtins.elem config.services.mpd.settings.bind_to_address ["any" "0.0.0.0"]
        '';
