...
let
  ...
  configFile = pkgs.runCommand "ci-broker.yaml" { nativeBuildInputs = [ pkgs.yq-go ]; } (
    ''
      yq '(.. | select(kind=="map" and length==1 and (keys[0]|match("^!"))!=null)) |= (.[] tag=keys[0] | .[])
          | ((.. | select(tag=="!!str" and match("^!")!=null)) |= (. tag=. | ""))' \
        ${settingsFormat.generate "ci-broker.yaml" cfg.settings} > $out
    ''
