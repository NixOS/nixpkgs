        ...
        if [ ! -e "${cfg.userStoragePath}/config.json" ]; then
          ...
          jq_filter=$(cat <<EOF
            .bridge = (.bridge // {}) * {
              ...
              ${optionalString (
                cfg.bridge.advertiser != null
              ) '',"advertiser": "${cfg.bridge.advertiser}"''}
              ...
              ${optionalString (
                cfg.bridge.manufacturer != null
              ) '',"manufacturer": "${cfg.bridge.manufacturer}"''}
              ${optionalString (cfg.bridge.model != null) '',"model": "${cfg.bridge.model}"''}
              ${optionalString (
                cfg.bridge.disableIpc != null
              ) '',"disableIpc": ${toString cfg.bridge.disableIpc}''}
            } |
            .platforms |= map(
              if .platform == "config" then
                . * {
                  ...
                  ${optionalString (
                    cfg.ui.loginWallpaper != null
                  ) '',"loginWallpaper": "${cfg.ui.loginWallpaper}"''}
                  ${optionalString (
