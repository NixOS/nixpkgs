  ...
  config =
    ...
    mkIf (cfg.servers != { }) {
      assertions = mapAttrsToList (
        server: options: {
          assertion = options.useTransformers -> options.initialPromt == false;
          message = "wyoming-faster-whisper/${server}: Configuring `initialPrompt` is currently incompatible together with `useTransformers`.";
        }
      );
