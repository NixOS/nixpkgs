...

{
  ...
  config = lib.mkIf cfg.enable {
    services.xserver.windowManager.session = lib.singleton {
      ...
      start = ''
        ...
        SXHKD_SHELL=/bin/sh ${cfg.sxhkd.package}/bin/sxhkd ${lib.optionalString (cfg.sxhkd.configFile != null) "-c \"${cfg.sxhkd.configFile}\""} &
  ...
  imports = [
   ...
   (lib.mkRemovedOptionModule [ "services" "xserver" "windowManager" "bspwm" "startThroughSession" ]
     "bspwm package does not provide bspwm-session anymore.")
   (lib.mkRemovedOptionModule [ "services" "xserver" "windowManager" "bspwm" "sessionScript" ]
     "bspwm package does not provide bspwm-session anymore.")
  ];
}
