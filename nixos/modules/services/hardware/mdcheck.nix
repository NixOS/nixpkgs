...
in
{
  options.services.mdcheck = {
    enable = mkEnableOption ''
      mdcheck timers that run regular full scrubs of the md devices.
      These processes can also cause high I/O utilization;
      the configuration option `boot.kernel.sysctl."dev.raid.speed_limit_max"`
    ...
    checkDuration = mkOption {
      ...
      description = ''
        ...
        be spent on the scrub operation before stopping.
        The default configuration is to scrub for 6 hours, then pause.
  ...
  meta.maintainers = with lib.maintainers; [ sbruder ];
}
