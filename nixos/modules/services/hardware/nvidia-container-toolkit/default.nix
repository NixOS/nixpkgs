...

{
  imports = [
    ...
    (lib.mkRenamedOptionModule
      ...
      [ "hardware" "nvidia-container-toolkit" "suppress-nvidia-driver-assertion" ]
    )
  ];
  ...
  options =
    ...
    in
    {
      ...
      hardware.nvidia-container-toolkit = {
        ...
        suppress-nvidia-driver-assertion = lib.mkOption {
          default = false;
          type = lib.types.bool;
          description = ''
    ...
    assertions = [
      {
        assertion =
          ...
          || config.hardware.nvidia-container-toolkit.suppress-nvidia-driver-assertion;
        message = ''`nvidia-container-toolkit` requires nvidia drivers: set `hardware.nvidia.datacenter.enable`, add "nvidia" to `services.xserver.videoDrivers`, or set `hardware.nvidia-container-toolkit.suppress-nvidia-driver-assertion` if the driver is provided by another NixOS module (e.g. from NixOS-WSL)'';
      }
      {
        assertion =
