...
let
  dockerPackageVersion = config.hardware.nvidia-container-toolkit.dockerPackage.version;
in
{
  imports = [
  ...
  config = lib.mkMerge [
    (lib.mkIf config.virtualisation.docker.enableNvidia {
      ...
      virtualisation.docker = {
        ...
        package = (
          config.hardware.nvidia-container-toolkit.dockerPackage.overrideAttrs (old: {
            buildInputs = old.buildInputs ++ [ pkgs.makeWrapper ];
            installPhase =
              old.installPhase
              + ''
                wrapProgram $out/bin/dockerd \
                  --prefix PATH : "${lib.getOutput "tools" config.hardware.nvidia-container-toolkit.package}/bin"

                wrapProgram $out/libexec/docker/docker \
                  --prefix PATH : "${lib.getOutput "tools" config.hardware.nvidia-container-toolkit.package}/bin"
              '';
          })
        );
    ...
    (lib.mkIf config.hardware.nvidia-container-toolkit.enable {
      ...
      virtualisation = {
        ...
        docker = {
          daemon.settings = lib.mkIf (lib.versionAtLeast dockerPackageVersion "25") {
            features.cdi = true;
          };

          rootless.daemon.settings = lib.mkIf (lib.versionAtLeast dockerPackageVersion "25") {
            features.cdi = true;
          };
        };
      };
