...
let
  ...
  mkVHostConf = hostOpts:
    let
      ...
      sslCertDir =
        ...
        then config.security.acme.certs.${hostOpts.hostName}.directory
        else
          if hostOpts.useACMEHost != null
          then config.security.acme.certs.${hostOpts.useACMEHost}.directory
  ...
  ''}
    ...
    ${let
      mkModule = module:
          if isString module
          then { name = module; path = "${pkg}/modules/mod_${module}.so"; }
          else
              if isAttrs module
