...
let
  ...
  dependentCertNames = filter (cert: certs.${cert}.dnsProvider == null && !certs.${cert}.tlsMode) vhostCertNames; # those that might depend on the HTTP server
  independentCertNames = filter (cert: certs.${cert}.dnsProvider != null || certs.${cert}.tlsMode) vhostCertNames; # those that don't depend on the HTTP server
  ...
  vhosts = concatStringsSep "\n" (mapAttrsToList (vhostName: vhost:
    let
        ...
        acmeLocation = optionalString ((vhost.enableACME || vhost.useACMEHost != null) && certs.${acmeName}.dnsProvider == null && !certs.${acmeName}.tlsMode)
  ...
  config = mkIf cfg.enable {
    ...
    security.acme.certs = let
      ...
      in nameValuePair vhostConfig.serverName {
        ...
        webroot = if certs.${vhostConfig.serverName}.tlsMode then null else mkOverride (if hasRoot then 1000 else 2000) vhostConfig.acmeRoot;
