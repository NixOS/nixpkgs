...
let
  ...
  formatKeyValue =
    ...
    if (v == null) then
      ...
      toOption indent n (if v then "on" else "off")
    else if isString v then
      toOption indent n v
    else if isList v then
      # we need to go from 'n = [1 2]' to 'n=1; n=2;'
      # so call formatKeyValue with name & value, which is constructed by
      # mapping over v to get something like [{name=n; value=1;} {name=n; value=2;}]
      lib.lists.map (attrs: formatKeyValue indent attrs.name attrs.value) (
        lib.lists.imap0 (i: _: {
          name = n;
          value = lib.lists.elemAt v i;
        }) v
      )
      ...
      concatStringsSep "\n" (
        ...
        ++ lib.lists.flatten (mapAttrsToList (formatKeyValue "${indent}    ") v)
        ++ [ "${indent}}" ]
      )
    else
      throw (traceSeq v "services.nginx.settings: unexpected type");

  toNginxConf =
    conf: concatStringsSep "\n" (lib.lists.flatten (lib.mapAttrsToList (formatKeyValue "") conf));
in

{
  options = {
    services.nginx = {
      ...
      settings = mkOption {
        description = ''
          RFC42-style settings option that provides lower level access to the nginx config.
          Using this option bypasses all other nginx options.
        '';
        default = { };
        type =
          let
      ...
      generatedConfigFile = mkOption {
        description = "nginx config file generated by parsing the `settings` option";
        defaultText = "";
        default = pkgs.writers.writeNginxConfig "nginx.conf" (toNginxConf cfg.settings);
        readOnly = true;
      };
