...
let
  ...
  acmeVHosts = lib.filter (hostOpts: hostOpts.useACMEHost != null) virtualHosts;
    ...
    in
      ''
        ${hostOpts.hostName} ${lib.concatStringsSep " " hostOpts.serverAliases} {
          ${lib.optionalString (hostOpts.listenAddresses != [ ]) "bind ${lib.concatStringsSep " " hostOpts.listenAddresses}"}
          ${lib.optionalString (hostOpts.useACMEHost != null) "tls ${sslCertDir}/cert.pem ${sslCertDir}/key.pem"}
          log {
            ${hostOpts.logFormat}
          }
  ...
  acmeHosts = lib.unique (lib.catAttrs "useACMEHost" acmeVHosts);

  mkCertOwnershipAssertion = import ../../../security/acme/mk-cert-ownership-assertion.nix;
in
{
  imports = [
    (lib.mkRemovedOptionModule [ "services" "caddy" "agree" ] "this option is no longer necessary for Caddy 2")
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.caddy = {
      ...
      serviceConfig = let
        runOptions = ''--config ${configPath} ${lib.optionalString (cfg.adapter != null) "--adapter ${cfg.adapter}"}'';
      in {
        # Override the `ExecStart` line from upstream's systemd unit file by our own:
        # https://www.freedesktop.org/software/systemd/man/systemd.service.html#ExecStart=
        # If the empty string is assigned to this option, the list of commands to start is reset, prior assignments of this option will have no effect.
        ExecStart = [ "" ''${cfg.package}/bin/caddy run ${runOptions} ${lib.optionalString cfg.resume "--resume"}'' ];
    ...
    security.acme.certs =
      let
        certCfg = map (useACMEHost: lib.nameValuePair useACMEHost {
