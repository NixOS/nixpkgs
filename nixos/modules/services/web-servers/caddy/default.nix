...
let
  ...
  acmeEnabledVhosts = lib.filter (hostOpts: hostOpts.useACMEHost != null) virtualHosts;
  vhostCertNames = lib.unique (map (hostOpts: hostOpts.useACMEHost) acmeEnabledVhosts);
...
in
{
  imports = [
    ...
    (lib.mkRenamedOptionModule [ "services" "caddy" "ca" ] [ "services" "caddy" "acmeCA" ])
    (lib.mkRenamedOptionModule [ "services" "caddy" "config" ] [ "services" "caddy" "extraConfig" ])
  ];
  ...
  options.services.caddy = {
    ...
    virtualHosts = lib.mkOption {
      type = with lib.types; attrsOf (submodule (import ./vhost-options.nix { inherit cfg; }));
      default = { };
      example = lib.literalExpression ''
        {
          "hydra.example.com" = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.caddy = {
      ...
      startLimitBurst = 10;
      reloadTriggers = lib.optional cfg.enableReload cfg.configFile;
      restartTriggers = lib.optional (!cfg.enableReload) cfg.configFile;

      serviceConfig =
        let
          runOptions = ''--config ${configPath} ${
            lib.optionalString (cfg.adapter != null) "--adapter ${cfg.adapter}"
          }'';
        in
        {
          ...
          ExecStart = [
            ""
            ''${cfg.package}/bin/caddy run ${runOptions} ${lib.optionalString cfg.resume "--resume"}''
          ];
          ...
          StateDirectory = lib.mkIf (cfg.dataDir == "/var/lib/caddy") [ "caddy" ];
          LogsDirectory = lib.mkIf (cfg.logDir == "/var/log/caddy") [ "caddy" ];
          Restart = "on-failure";
    ...
    users.users = lib.optionalAttrs (cfg.user == "caddy") {
      caddy = {
        group = cfg.group;
    ...
    users.groups = lib.optionalAttrs (cfg.group == "caddy") {
    ...
    security.acme.certs =
      let
        certCfg = map (
          certName:
          lib.nameValuePair certName {
            group = lib.mkDefault cfg.group;
            reloadServices = [ "caddy.service" ];
          }
        ) vhostCertNames;
      in
      lib.listToAttrs certCfg;
