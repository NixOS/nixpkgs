...
in
{
  ...
  options.services.git-pages = {
    ...
    nginx.virtualHosts = lib.mkOption {
      default = { };
      # A deferred module gives the definitions to `services.nginx` unchanged.
      # A submodule gives its evaluated value, which also contains the defaults
      # and the options that have no value.
      ...
      description = ''
        ...
        other virtual host settings here. Each value takes the options of
        {option}`services.nginx.virtualHosts.<name>`. Set this and nginx replaces the Caddy
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.nginx = lib.mkIf (cfg.nginx.virtualHosts != { } && pagesPort != null) {
      ...
      virtualHosts = lib.mapAttrs (
        _name: vhost:
        # A function, because the nginx virtual host submodule reads an
        # attribute set as configuration only, and `imports` is not an option.
        { ... }:
        {
          imports = [ vhost ];
          locations."/" = {
