  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      }
      {
        ...
        message = "'services.traefik.extraFiles' requires the dynamic file provider to be set to a directory";
      }
      {
        assertion = cfg.group != "docker";
        message = "Setting the primary group to 'docker' will cause files (such as those generated by 'services.traefik.extraFiles') to be owned by the group 'docker', which may be a security risk. Use 'services.traefik.supplementaryGroups' instead.";
      }
      {
        assertion = cfg.static.file != null -> cfg.plugins == [ ];
        message = "'services.traefik.static.file' and 'services.traefik.plugins' are set, but 'plugins' is only compatible with 'services.traefik.static.settings'";
      }
    ];

    warnings =
      optional (!all id (map (plugin: plugin._isTraefikPlugin or false) cfg.plugins)) ''
        ...
        - ${
          concatMapStringsSep "\n- " (badPlugin: badPlugin.outPath) (
            filter (plugin: plugin._isTraefikPlugin or false) cfg.plugins
          )
        }
      ''
      ++
    ...
    services.traefik.static.settings = mkMerge [
      ...
      (mkIf (cfg.plugins != [ ]) {
        experimental.localPlugins = listToAttrs (
          map (plugin: nameValuePair plugin.plugin { inherit (plugin) moduleName; }) cfg.plugins
        );
      })
    ];
      ...
      (mkIf (cfg.dynamic.dir != null) (
        ...
        // (mapAttrs' (
          ...
          nameValuePair "${cfg.dynamic.dir}/_nixos-${name}.yml" {
            "L+" = {
              mode = "0444";
              argument = writeText name (toJSON value.settings);
            };
          }
        ) cfg.extraFiles)
      ))
