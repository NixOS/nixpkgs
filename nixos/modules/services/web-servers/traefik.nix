...
let
  ...
  ## eg. isDefault "static.settings" == (cfg.static.settings == opt.static.settings.default)
  isDefault =
    attrPathStr:
    let
...
in
{
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = (!(isDefault "static.file")) -> isDefault "static.settings";
        message = ''
    ...
    warnings =
      ...
      ++ optional (!(isDefault "dynamic.settings")) ''
    ...
    services.traefik = mkIf (isDefault "static.file") {
      ...
      static.settings = mkIf (cfg.dynamic.dir != null || !(isDefault "dynamic.file")) {
        providers.file = {
