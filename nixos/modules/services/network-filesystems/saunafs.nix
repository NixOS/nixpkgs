...
let
  ...
  sfsUser = if cfg.runAsUser then "saunafs" else "root";
  ...
  options = {
    services.saunafs = {
      ...
      runAsUser = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Run daemons as user saunafs instead of root.";
      };
    ...
    lib.mkIf (cfg.client.enable || cfg.master.enable || cfg.metalogger.enable || cfg.chunkserver.enable)
      {
        ...
        warnings = [ (lib.mkIf (!cfg.runAsUser) "Running saunafs services as root is not recommended.") ];

        # Service settings
        services.saunafs = {
          master.settings = lib.mkIf cfg.master.enable {
            WORKING_USER = sfsUser;
        ...
        users =
          lib.mkIf (cfg.runAsUser && (cfg.master.enable || cfg.metalogger.enable || cfg.chunkserver.enable))
            {
              users.saunafs = {
                isSystemUser = true;
        ...
        systemd.tmpfiles.rules =
          lib.optional cfg.master.enable "d ${cfg.master.settings.DATA_PATH} 0700 ${sfsUser} ${sfsUser} -"
          ++ lib.optional cfg.metalogger.enable "d ${cfg.metalogger.settings.DATA_PATH} 0700 ${sfsUser} ${sfsUser} -"
          ++ lib.optional cfg.chunkserver.enable "d ${cfg.chunkserver.settings.DATA_PATH} 0700 ${sfsUser} ${sfsUser} -";
