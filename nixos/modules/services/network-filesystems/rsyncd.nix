...
let
  ...
  rsyncdCfg = ""
    ...
    + flip concatMapStrings cfg.modules (m: "[${m.name}]\n\tpath = ${m.path}\n"
      + optionalString (m.comment != "") "\tcomment = blah" #${m.comment}\n"
      + m.extraConfig
      + "\n"
    );
  ...
  options = {
	...
	description = "IP address the daemon will listen on.";
      ...
      extraConfig = mkOption {
        type = types.lines;
	default = "";
	description = "Lines of configuration to add to rsyncd globally.";
      };
	  ...
	  path = mkOption {
	    ...
	    description = "Directory to make available in this module.";
          ...
          extraConfig = mkOption {
            type = types.lines;
	    default = "";
            description = "Lines of configuration to add to this module.";
          };
	};
      };
    };
