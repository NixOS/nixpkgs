...
with lib; let
  ...
  mkBackupScript = cfg:
    ''
      ...
      archiveName="${
        if cfg.archiveBaseName == null
        then ""
        else cfg.archiveBaseName + "-"
      }$(date ${cfg.dateFormat})"
  ...
  mkActivationScript = name: cfg: let
  ...
  mkUsersConfig = name: cfg: {
    users.${cfg.user} = {
      openssh.authorizedKeys.keys = (map (mkAuthorizedKey cfg false) cfg.authorizedKeys
