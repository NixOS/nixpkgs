...
in
{
  options.services.restic.fileBackups = lib.mkOption {
    description = ''
      Periodic backups to create with Restic.

      Each instance of the attrset is a provider for the [filebackup](#opt-contracts.filebackup) contract.
    '';
  ...
  config = lib.mkMerge [
    ...
    }
    {
      services.restic.backups =
        let
          mkBackupConfig =
            ...
            {
              user = cfg.input.user;
              paths = cfg.input.sourceDirectories;
              backupPrepareCommand = lib.concatStringsSep "\n" cfg.input.beforeBackupHooks;
              backupCleanupCommand = lib.concatStringsSep "\n" cfg.input.afterBackupHooks;
              exclude = cfg.input.excludePatterns;
            }
            // cfg.providerOptions;
        in
        lib.mapAttrs mkBackupConfig config.services.restic.fileBackups;
      ...
      environment.systemPackages =
        let
          mkRestoreScript =
            ...
            pkgs.writeShellApplication {
              name = "restic-backups-${name}-restore";
              text = ''
                if [ "$1" = "snapshots" ]; then
        ...
        in
        lib.mapAttrsToList mkRestoreScript config.services.restic.fileBackups;
    }
  ];
}
