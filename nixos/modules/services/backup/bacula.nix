    ...
    options = {
      enable = mkOption {
        ...
        description = lib.mdDoc ''
          Specifies if TLS should be enabled if this is false TLS will be
          completely disabled, even if tls.require is true.
        '';
      };
      require = mkOption {
        ...
        default = null;
        description = lib.mdDoc ''
          Require TLS or TLS-PSK encryption. This directive is ignored unless
          one of TLS Enable or TLS PSK Enable is set to true.
          If TLS is not required while TLS or TLS-PSK are enabled, then the
          Bacula component will connect with other components either with or
          without TLS or TLS-PSK

          If TLS or TLS-PSK is enabled and TLS is required, then the Bacula
      ...
      certificate = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          not be specified in a client context if TLS Verify Peer is
          set to `false` in the corresponding server context.
        '';
      };
      key = mkOption {
        type = types.str;
        description = lib.mdDoc ''
          The full path and filename of a PEM encoded TLS
          private key. It must correspond to the TLS certificate.
        '';
      };
      verifyPeer = mkOption {
        type = types.nullOr types.bool;
        default = null;
        description = lib.mdDoc ''
          Verify peer certificate. Instructs server to request
          and verify the client's X.509 certificate. Any client certificate
          signed by a known-CA will be accepted. Additionally, the client's
          X509 certificate Common Name must meet the value of the Address
          directive. If the TLSAllowed CN configuration directive is used,
          the client's x509 certificate Common Name must also correspond to
          one of the CN specified in the TLS Allowed CN directive. This directive
          is valid only for a server and not in client context. Standard from Bacula is 'yes'.
         '';
      };
      allowedCN = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          is valid for a server and in a client context. If this directive is
          specified, the peer certificate will be verified against this list.
          In the case this directive is configured on a server side, the allowed
          CN list will not be checked if TLS Verify Peer is set to false.
        '';
      };
      caCertificateFile = mkOption {
        ...
        Multiple certificates are permitted in the file. One of TLS CA Certificate
        File or TLS CA Certificate Dir are required in a server context, unless TLS
        verifyPeer is set to no, and are always required in a client context.
        '';
      };
    };
  };
