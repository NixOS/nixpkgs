...
let
  ...
  credentials = {
    sshKey = "net.openoid.syncoid.sshKey";
  };
in
{
  ...
  options.services.syncoid = {
    ...
    sshKey = lib.mkOption {
      ...
      description = ''
        ...
        (as an absolute file path or Systemd credential name).
            ...
            options = {
              ...
              sshKey = lib.mkOption {
                ...
                description = ''
                  ...
                  can use a different credential name.
                '';
              };
        ...
        lib.mkMerge [
          {
            ...
            serviceConfig = {
              ...
              ExecStart = lib.escapeShellArgs (
                ...
                ++ lib.optionals (c.sshKey != null) [
                  "--sshkey"
                  "\${CREDENTIALS_DIRECTORY}/${credentials.sshKey}"
                ]
                ++ c.extraArgs
                ++ [
                  "--sendoptions"
            ...
            // lib.optionalAttrs (cfg.sshKey != null) (
              if lib.types.path.check cfg.sshKey then
                {
                  LoadCredential = [ "${credentials.sshKey}:${cfg.sshKey}" ];
                }
              else
                {
                  ImportCredential = [ "${cfg.sshKey}:${credentials.sshKey}" ];
                }
            );
          }
          cfg.service
