...
let
  cfg = config.services.libvirtBackup;
    ...
    assertions = [
      {
        ...
        message = "'services.libvirtBackup.vms' must either be null for all VMs or a non-empty list of VM configurations";
      }
      {
        assertion = config.virtualisation.libvirtd.enable;
        message = "libvirtd must be enabled to use libvirtBackup";
      }
    ];

    systemd = {
      timers.libvirt-backup = {
