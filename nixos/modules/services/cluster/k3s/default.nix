...
let
      ...
      , ...
        ...
        options = {
          enable = lib.mkOption {
            type = lib.types.bool;
            default = true;
            description = lib.mdDoc "Whether this manifest file should be generated.";
          };
          ...
          source = lib.mkIf (config.content != null) (
            let
              ...
              mkSource = value:
                ...
                then pkgs.concatText name' (lib.concatMap (x: [ yamlDocSeparator (mkYaml docName x) ]) value)
                else mkYaml name' value;
            in
  ...
  linkChartEntry =
    ...
    in
    name: value: "${pkgs.coreutils-full}/bin/ln -sfn ${value} ${chartDir}/${mkTarget (builtins.baseNameOf name)}";

  activateK3sContent = pkgs.writeShellScript "link-k3s-content" ''
    ${lib.optionalString (builtins.length enabledManifests > 0) "${pkgs.coreutils-full}/bin/mkdir -p ${manifestDir}"}
    ${lib.optionalString (cfg.charts != {}) "${pkgs.coreutils-full}/bin/mkdir -p ${chartDir}"}
    ${lib.optionalString (builtins.length cfg.images > 0) "${pkgs.coreutils-full}/bin/mkdir -p ${imageDir}"}
...
in
{
  ...
  config = mkIf cfg.enable {
    warnings = (lib.optional (cfg.role != "server" && cfg.manifests != { })
    ...
    assertions = [
      {
        assertion = cfg.role != "server" -> (cfg.configPath != null || cfg.serverAddr != "");
        message = "serverAddr or configPath (with 'server' key) should be set if role is 'agent'";
      }
      {
        assertion = cfg.role != "server" -> cfg.configPath != null || cfg.tokenFile != null || cfg.token != "";
