...
let
  ...
  inherit (lib) concatStrings concatStringsSep mapAttrsToList concatMap attrByPath
    mkIf mkMerge mkEnableOption mkOption concatMapStringsSep types mkPackageOption;
      ...
      mkIf serviceOptions.enable (mkMerge [
        {
          systemd = {
            services."druid-${name}" = {
              ...
              path = [ cfg.package serviceOptions.jdk ];
              script =
                let
          ...
          networking.firewall.allowedTCPPorts = mkIf
            ((attrByPath [ "openFirewall" ] false serviceOptions))
            allowedTCPPorts;
          users = {
            users.druid = {
...
in
{
  options.services.druid = {
    package = mkPackageOption pkgs "druid" { };
  ...
  config = mkMerge [
    ...
    (druidServiceConfig rec {
      ...
      extraConfig.services.druid.historical.internalConfig."druid.segmentCache.locations" = ''
        ...
        ) cfg.historical.segmentLocations )}]
      '';
    })
    (druidServiceConfig rec {
      name = "middleManager";
      tmpDirs = [ "/var/log/druid/indexer" ]
      ...
      extraConfig = {
        services.druid.middleManager.internalConfig = {
          ...
          "druid.indexer.runner.javaOpts" =
            (attrByPath [ "druid.indexer.runner.javaOpts" ] "" cfg.middleManager.config) + " -Dlog4j.configurationFile=file:${cfg.log4j}";
        };
