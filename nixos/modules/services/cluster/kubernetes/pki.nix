  ...
  config = lib.mkIf cfg.enable (
    ...
    in
    {
      ...
      systemd.services.cfssl.preStart =
        let
          cfsslDataDir = config.services.cfssl.dataDir;
        in
        lib.mkIf (top.apiserver.enable) (
          lib.concatStringsSep "\n" [
            "set -e"
            (lib.optionalString cfg.genCfsslCACert ''
              if [ ! -f "${cfg.caCertPathPrefix}.pem" ]; then
                ${pkgs.cfssl}/bin/cfssl genkey -initca ${csrCA} | \
                  ${pkgs.cfssl}/bin/cfssljson -bare ${cfg.caCertPathPrefix}
              fi
            '')
            (lib.optionalString cfg.genCfsslAPICerts ''
              if [ ! -f "${cfsslDataDir}/cfssl.pem" ]; then
                ${pkgs.cfssl}/bin/cfssl gencert -ca "${cfg.caCertPathPrefix}.pem" -ca-key "${cfg.caCertPathPrefix}-key.pem" ${csrCfssl} | \
                  ${pkgs.cfssl}/bin/cfssljson -bare ${cfsslCertPathPrefix}
              fi
            '')
      ...
      systemd.services.kube-addon-manager = lib.mkIf top.addonManager.enable (
        lib.mkMerge [
          {
