...
let
  ...
  taintOptions = with lib.types; { name, ... }: {
    options = {
      key = lib.mkOption {
        ...
        default = name;
        defaultText = lib.literalMD "Name of this submodule.";
        type = str;
      };
      value = lib.mkOption {
  ...
  taints = lib.concatMapStringsSep "," (v: "${v.key}=${v.value}:${v.effect}") (lib.mapAttrsToList (n: v: v) cfg.taints);
in
{
  imports = [
  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      ...
      systemd.services.kubelet = {
        ...
        serviceConfig = {
          ...
          ExecStart = ''${top.package}/bin/kubelet \
            ...
            ${lib.optionalString (cfg.nodeIp != null)
              "--node-ip=${cfg.nodeIp}"} \
            --pod-infra-container-image=pause \
            ${lib.optionalString (cfg.manifests != {})
              "--pod-manifest-path=/etc/${manifestPath}"} \
            ${lib.optionalString (taints != "")
      ...
      services.kubernetes.kubelet.hostname =
        lib.mkDefault (lib.toLower config.networking.fqdnOrHostName);
    ...
    (lib.mkIf (cfg.enable && cfg.manifests != {}) {
      environment.etc = lib.mapAttrs' (name: manifest:
