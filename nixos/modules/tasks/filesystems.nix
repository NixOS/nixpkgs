  ...
  fileSystemOpts = {config, ...}: {
    ...
    config = let
      defaultFormatOptions =
        ...
        if (builtins.substring 0 3 config.fsType) == "ext"
        then "-F"
        # -q needed for non-interactive operations
        else if config.fsType == "jfs"
        then "-q"
        # (same here)
        else if config.fsType == "reiserfs"
        then "-q"
        else null;
    in {
  ...
  config = {
    ...
    in [
      ...
      }
      {
        ...
        in "Mountpoint '${fs.mountPoint}': 'autoResize = true' is not supported for 'fsType = \"${fs.fsType}\"':${
          if fs.fsType == "auto"
          then " fsType has to be explicitly set and"
          else ""
