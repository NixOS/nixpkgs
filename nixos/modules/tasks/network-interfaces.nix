    ...
    imports =
      ...
      [
        ...
        (mkMergedOptionModule
          ...
          (cfg: with cfg;
          optional
            (defined ipAddress && defined prefixLength) { address = ipAddress; prefixLength = prefixLength; }
          )
        )
        (mkMergedOptionModule
          ...
          (cfg: with cfg;
          optional
            (defined ipv6Address && defined ipv6PrefixLength) { address = ipv6Address; prefixLength = ipv6PrefixLength; }
          )
        )
...
in
{
  ...
  config = {
    ...
    boot.kernel.sysctl = {
      ...
      flip
        concatMap
        (filter (i: i.proxyARP) interfaces)
    ...
    systemd.services = {
      ...
      let
        deviceDependency =
          if (config.boot.isContainer || i.name == "lo")
          then [ ]
