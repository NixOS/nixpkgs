...
in
{
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.boot.vdo.enable {
      ...
      services.lvm.package = lib.mkOverride 999 pkgs.lvm2_vdo; # this overrides mkDefault
    ...
    (lib.mkIf (cfg.dmeventd.enable || cfg.boot.thin.enable) {
      boot.initrd.systemd.contents."/etc/lvm/lvm.conf".text =
        lib.optionalString (config.boot.initrd.services.lvm.enable && cfg.boot.thin.enable) (
