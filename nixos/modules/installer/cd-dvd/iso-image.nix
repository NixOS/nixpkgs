...
with lib; let
   ...
   */
  buildMenuAdditionalParamsGrub2 = config: additional: let
    finalCfg = {
  ...
  syslinuxTimeout =
    ...
    then 0
    else max (config.boot.loader.timeout * 10) 1;
  ...
  targetArch =
    if pkgs.stdenv.isi686 || config.boot.loader.grub.forcei686
    then "ia32"
