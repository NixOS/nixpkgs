...
my $networkingDhcpConfig = generateNetworkingConfig();
...
# Interface configurations based on
# - https://github.com/systemd/systemd/blob/main/network/89-ethernet.network.example
# - https://github.com/systemd/systemd/blob/main/network/80-wifi-station.network.example
sub generateNetworkingConfig {
    my $config = <<EOF;
  # Use systemd-networkd to enable dynamic network configuration using
  # DHCP, DHCPv6 and Router Advertisements on every physical ethernet and
  # wireless client interface.
  systemd.network = {
    enable = true;
    networks = {
      # Use lower indices for additional profiles, since networkd
      # only ever applies the first matching configuration profile
      # to an interface.
      "99-autoconfig-ether" = {
        matchConfig = {
          Kind = "!*";
...
EOF
        ...
        my $networkingConfig = generateNetworkingConfig();
