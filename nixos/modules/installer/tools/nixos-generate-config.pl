...
sub generateNetworkingConfig {
    my $config = <<EOF;
  # Use systemd-networkd to enable DHCP (IPv4) and Router
  # Advertisements (IPv6) on every ethernet and wireless interface.
  systemd.network = {
    enable = true;
    networks = {
      # Use lower numbers for additional profiles, since networkd
      # processes these alphanumerically and every interface will only
      # ever use its first match.
      "99-autoconfig-ether" = {
        matchConfig.Type = [
          "ether"
        ];
