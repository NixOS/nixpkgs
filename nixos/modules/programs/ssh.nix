...
let
  ...
  knownHostsFiles = [ "/etc/ssh/ssh_known_hosts" ]
    ++ map toString cfg.knownHostsPaths
...
in
{
  ...
  options = {
    ...
    programs.ssh = {
      ...
      knownHostsFiles = mkOption {
        ...
        description = lib.mdDoc ''
          Deprecated, please use {option}`programs.ssh.knownHostsPaths` instead.
  ...
  config = {
    ...
    warnings = lib.optional (lib.length cfg.knownHostsFiles != 0)
      ''the option programs.ssh.knownHostsFiles is deprecated, please use programs.ssh.knownHostsPaths instead.'';
