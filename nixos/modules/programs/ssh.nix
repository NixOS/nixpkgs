  ...
  options = {
    ...
    programs.ssh = {
      ...
      knownHostsFiles = lib.mkOption {
        ...
        description = ''
          Files containing SSH host keys to set as global known
          hosts. `/etc/ssh/ssh_known_hosts` (which is generated by
          {option}`programs.ssh.knownHosts`) and `/etc/ssh/ssh_known_hosts2` are
          always included.
        '';
        example = lib.literalExpression ''
          [
