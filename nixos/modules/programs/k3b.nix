  ...
  options.programs.k3b = {
    enable = lib.mkOption {
      ...
      description = ''
        ...
        add `setuid` wrappers in {option}`config.security.wrapperDir`
        (`/run/wrappers/bin`) for both `cdrdao` and `cdrecord`. On first
        run you must manually configure the path of `cdrdae` and
        `cdrecord` to correspond to the appropriate paths under
        {option}`config.security.wrapperDir` in the "Setup External Programs" menu.
      '';
    };
  };
