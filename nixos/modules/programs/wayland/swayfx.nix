...
let
  cfg = config.programs.swayfx;

  wayland-lib = import ./lib.nix { inherit lib; };
in
{
  options.programs.swayfx = {
    enable = lib.mkEnableOption ''
      SwayFX, Sway-compatible wayland compositori,
      aimed to provide more visual effects to Sway, 
      such as: window blur, window shadow, window dim.
      As SwayFX is just a fork, it's shares binary name with Sway itself.
    '';

    package =
      lib.mkPackageOption pkgs "swayfx" {
        nullable = true;
        extraDescription = ''
          ...
          Set to `null` to not add any SwayFX package to your path.
          This should be done if you want to use the Home Manager SwayFX module to install SwayFX.
        '';
      }
      // {
        apply =
    ...
    wrapperFeatures = {
      base =
        lib.mkEnableOption ''
          ...
          dbus-run-session to the swayfx command''
        // {
          default = true;
        };
    ...
    extraSessionCommands = lib.mkOption {
      ...
      '';
      description = ''
        Shell commands executed just before SwayFX is started. See
    ...
    lib.mkMerge [
      {
        assertions = [
          {
            ...
            message = ''
              The extraSessionCommands for SwayFX will not be run if wrapperFeatures.base is disabled.
            '';
          }
        ];
        ...
        # DM is optional, but needed to start SwayFX with system, not manually:
