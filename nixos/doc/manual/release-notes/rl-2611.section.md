...
- `services.journald` has been migrated to the structured [](#opt-services.journald.settings.Journal) option, which accepts any `journald.conf(5)` option directly. As part of this:
  - `services.journald.extraConfig` has been removed. Set the equivalent options under `services.journald.settings.Journal` instead. For example, replace `services.journald.extraConfig = "Storage=volatile";` with `services.journald.settings.Journal.Storage = "volatile";`.
  - `services.journald.storage`, `services.journald.rateLimitInterval`, `services.journald.rateLimitBurst` and `services.journald.forwardToSyslog` have been renamed to `services.journald.settings.Journal.Storage`, `services.journald.settings.Journal.RateLimitIntervalSec`, `services.journald.settings.Journal.RateLimitBurst` and `services.journald.settings.Journal.ForwardToSyslog` respectively.
  - `services.journald.console` has been removed. Set `services.journald.settings.Journal.ForwardToConsole = true;` together with `services.journald.settings.Journal.TTYPath` instead.
  - `services.journald.audit` has been removed. Set `services.journald.settings.Journal.Audit` instead. The previous `"keep"` default corresponds to leaving the setting unset, so the generated configuration no longer emits a literal `Audit=keep` line by default.
