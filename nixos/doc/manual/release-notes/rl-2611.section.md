...
- `services.k3s` and `services.rke2` now append the chart's store-path hash to the `spec.chart` URL in the `HelmChart` CR generated by `autoDeployCharts` (e.g. `.../chart.tgz?v=<storehash>`). This makes the on-disk manifest depend on the chart package, so when the chart derivation changes, the manifest file's bytes change, `${name}` re-applies the HelmChart CR, and helm-controller upgrades the chart. Previously, swapping the chart package for a different bytes-identical derivation left the manifest byte-identical and no upgrade would be triggered. The static file server ignores the query string.
