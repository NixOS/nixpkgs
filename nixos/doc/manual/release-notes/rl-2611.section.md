...
- iwd ([`networking.wireless.iwd`](options.html#opt-networking.wireless.iwd.enable)) no longer sets `DriverQuirks.DefaultInterface` by default. The setting was previously added becaues of reported race conditions, but these should generally not occur with newer iwd versions anymore. In the unlikely case that your wireless adapter is still affected, use `networking.wireless.iwd.settings.DriverQuirks.DefaultInterface = "?*";` to restore the previous default.
