...
- Container-only NixOS tests using `systemd-nspawn` can now run on Darwin. The test framework runs the containers inside a Linux QEMU virtual machine. This requires `apple-virt` and a Linux builder, but not `uid-range` support on the Darwin builder. Interactive container tests and tests that mix containers with QEMU virtual machine nodes remain unsupported on Darwin.
