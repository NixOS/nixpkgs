...
- `services.librechat` with `enableLocalDB = true` now sets an explicit
  database name (`LibreChat`) in the default `MONGO_URI`, instead of
  relying on MongoDB's implicit fallback to the `test` database. This
  means that on existing deployments, LibreChat will appear to have no
  users or conversations after upgrading, even though the data is still
  present in the `test` database. To migrate existing data to the new
  `LibreChat` database, run (inside the LibreChat container, if
  applicable):

  ```console
  mongodump --db=test --out=/tmp/backup
  mongorestore --db=LibreChat --nsFrom='test.*' --nsTo='LibreChat.*' /tmp/backup/test
  ```
