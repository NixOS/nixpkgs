...
- All modules for standalone window managers now have a `package` option (i.e. [](#opt-services.xserver.windowManager.spectrwm.package), [](#opt-services.xserver.windowManager.leftwm.package), and so on). This allows modifying the WM packages directly instead of using an overlay.
