...
- The `mumble` and `murmur` packages no longer read the `config.mumble.jackSupport`, `config.mumble.speechdSupport` and `config.murmur.iceSupport` Nixpkgs configuration options.
  Configure these feature flags through `.override` instead, for example `mumble.override { jackSupport = true; }` or `murmur.override { iceSupport = false; }`.
