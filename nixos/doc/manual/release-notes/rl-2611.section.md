...
- [Renderdoc](https://renderdoc.org/), a graphics debugger for Vulkan, D3D11, D3D12, OpenGL, and OpenGL ES. Available as [programs.renderdoc](#opt-programs.renderdoc.enable).
...
- `services.fedimintd` has been updated for Fedimint 0.12.1. The old
  `bitcoin.rpc.url` and `bitcoin.rpc.kind` options were removed, so existing
  configurations using them fail evaluation. `bitcoin.rpc.secretFile` was
  renamed to `bitcoin.bitcoindSecretFile`. Use `bitcoin.bitcoindUrl`,
  `bitcoin.bitcoindUser`, and `bitcoin.bitcoindSecretFile` for bitcoind, or
  `bitcoin.esploraUrl` for Esplora. The password file contains only the
  password, is read at runtime, and must not be a bitcoind cookie file. URLs
  with embedded credentials and the Electrum backend are no longer supported.
  The Iroh endpoint now shares `api_ws.bind` and `api_ws.port`.
  `api_iroh.enable` selects Iroh for API and P2P during initial federation
  setup and cannot change an existing federation's persisted transport.
  Disabling it for a new TCP federation requires public `p2p.url` and
  `api_ws.url` values. The legacy Iroh UDP firewall port remains open by
  default to preserve existing Iroh federations; disable
  `api_iroh.openFirewall` for a known TCP-only federation.
  Fedimint now starts metrics for every instance; configure a distinct
  `metrics.port` for each enabled instance. Configurations using
  `environment.FM_BIND_METRICS` must instead set `metrics.bind` and
  `metrics.port`. The module also adds runtime credential options
  `passwordUiFile` and `passwordApiFile`, and the transitional Iroh 1.0 endpoint
  under `api_iroh_next`.

  A direct `0.7.1` to `0.12.1` guardian upgrade is not tested upstream. First
  upgrade to [`0.10.1`](https://github.com/fedimint/fedimint/releases/tag/v0.10.1),
  which documents compatibility with earlier 0.x releases; never pass through
  `0.11.0` or `0.11.1`. Upstream's [0.12 upgrade
  guidance](https://github.com/fedimint/fedimint/releases/tag/v0.12.0) tests
  `0.10.1` as a starting version.

  Iroh federations must coordinate the 0.12 upgrade because its guardian P2P
  transport is wire-incompatible with older releases. Consensus temporarily
  stops when neither side has quorum; upstream's [networking
  guidance](https://github.com/fedimint/fedimint/blob/v0.12.1/docs/networking.md)
  does not support rollback across that transport boundary. Separately, for
  eligible Iroh federations set `api_iroh_next.openFirewall = true` (or allow
  UDP on its configured port externally) before its new client identity is
  advertised. After advertisement, the endpoint must remain enabled and
  reachable and downgrading to a binary without it is unsupported.

  Fedimint 0.12 no longer encrypts newly written guardian configuration files.
  Legacy encrypted configurations remain readable, but protect filesystem and
  backup access to each instance's data directory.
