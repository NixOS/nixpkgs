...
- `services.librechat.enableLocalDB` is deprecated in favor of {option}`services.librechat.localDB.enable`, and will be removed in a future release. A new {option}`services.librechat.localDB.name` option also lets you choose the database name LibreChat's local MongoDB instance uses. It defaults to `test`, matching MongoDB's previous implicit behavior, so existing deployments are unaffected and require no action. If you'd like to move to a properly named database instead (e.g. `LibreChat`), set {option}`services.librechat.localDB.name` to `"LibreChat"`. MongoDB does not rename an existing database automatically, so migrate your existing data first:

  ```
  mongodump --db=test --out=/tmp/backup
  mongorestore --db=LibreChat /tmp/backup/test
  ```
