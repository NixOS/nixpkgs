...
- The `services.ocis` module and the `ocis_5-bin` package have been removed because they are outdated and no longer maintained in Nixpkgs. OCIS requires a strict upgrade sequence; do not skip the required intermediate versions when migrating existing data.
