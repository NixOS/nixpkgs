...
<!-- To avoid merge conflicts, consider adding your item at an arbitrary place in the list instead. -->
- `networking.wireless` (wpa_supplicant) now warns at build time when a
  network combines an `ext:`-referenced [](#opt-networking.wireless.networks._name_.pskRaw)
  with an SAE or FT-SAE (WPA3) auth protocol. wpa_supplicant only resolves
  such secrets for the classic (non-H2E) SAE exchange, not when deriving
  the SAE Hash-to-Element password element, so authentication to access
  points that require H2E (mandatory on the 6 GHz band) fails with
  `SAE: Could not find PT for group …` and repeatedly
  disconnects/reassociates. Use [](#opt-networking.wireless.extraConfigFiles)
  with a literal password for affected networks instead.
