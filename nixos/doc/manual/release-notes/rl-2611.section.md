...
- The Elastic stack (`elasticsearch`, `logstash`, and the beats: `auditbeat`, `filebeat`, `heartbeat`, `metricbeat`, `packetbeat`) has been updated from 7.17.x to 8.19.x. Elasticsearch does not support upgrading directly from 7.x to 9.x, so 8.x is shipped as a stepping stone for users who need to migrate their indices. The `ingest-attachment`, `repository-s3`, and `repository-gcs` plugins were merged into Elasticsearch core in 8.0 and no longer ship separately, so the corresponding `elasticsearchPlugins.*` attributes now throw.
