...
- The cloud image builders (Amazon EC2, Azure, Google Compute, DigitalOcean, Linode, OCI and OpenStack) now share a single `virtualisation.configFile` option to control the `/etc/nixos/configuration.nix` installed into generated images. The builder-specific options `virtualisation.googleComputeImage.configFile`, `virtualisation.digitalOceanImage.configFile` and `virtualisation.linodeImage.configFile` continue to work unchanged and take precedence when set, but are deprecated and will be removed in a future release; setting them emits a warning.
