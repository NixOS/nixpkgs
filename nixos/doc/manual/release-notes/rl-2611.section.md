...
- The k3s and rke2 modules now manage manifests declaratively through a linkFarm in the Nix store,
  symlinked into a `nixos/` subdirectory under the manifests directory. Previously, individual manifests
  were symlinked via systemd-tmpfiles. Old symlinks remaining in
  `/var/lib/rancher/${distro}/server/manifests/` should be removed manually after upgrading.
