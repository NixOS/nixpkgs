...
- The `NIX_XDG_DESKTOP_PORTAL_DIR` environment variable is no longer used in the `xdg-desktop-portal` package and is therefore no longer set in `xdg.portal` module.
  ...
  If you were relying on `NIX_XDG_DESKTOP_PORTAL_DIR`, migrate by either:
  - Configuring portals declaratively via `xdg.portal.config` or
    `xdg.portal.configPackages`.
  - Moving your custom portals to a directory on `XDG_DATA_DIRS`.
