...
- `services.qui.secretFile` has been renamed to `services.qui.settings.sessionSecretFile` and is now optional. When unset, qui generates its own session secret. When set, the path now must be absolute and outside the Nix store.
