...
- The `code-cursor` package attribute has been renamed to `cursor` to match upstream branding and `pname`.
  Compatibility aliases `code-cursor`, `code-cursor-fhs`, and `code-cursor-fhsWithPackages` remain with warnings.
