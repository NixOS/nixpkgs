...
- [Gitolite](https://gitolite.com/) module now supports declarative configuration via {option}`services.gitolite.repos`, {option}`services.gitolite.keys`, {option}`services.gitolite.groups`, and {option}`services.gitolite.extraConfig`.  When these options are used, gitolite is managed entirely through NixOS without requiring the gitolite-admin.git repository.  Available as [services.gitolite](#opt-services.gitolite.enable).
