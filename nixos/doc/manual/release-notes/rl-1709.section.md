...
- Modules can now be disabled by using [ disabledModules](https://nixos.org/manual/nixpkgs/stable/#sec-replace-modules), allowing another to take it's place. This can be used to import a set of modules from another channel while keeping the rest of the system on a stable release.
