...
- `nodePackages.bower`, `nodePackages.bower2nix`, and `fetchbower` have been removed. Bower has been unmaintained for several years now, and so it's reasonable to drop, as no packages within nixpkgs depend on it anymore.
