...
- `gnome.nautilus` was renamed to `nautilus` and is now expected to be managed through [](#opt-programs.nautilus.enable) NixOS option. Additionally, binary Nautilus extensions will no longer be picked up from [](#opt-environment.systemPackages), they need to be installed through [](#opt-programs.nautilus.extensions). (This does not apply to `nautilus-python` scripts, which should be installed through as before.)
