...
- The build helper `buildLinux` now provides a structural `passthru.tests` for its result Linux kernel packages as an attribute set instead of a list.

  Following the fix of `<kernel>.overrideAttrs` ([\#288154](https://github.com/NixOS/nixpkgs/pull/288154)), it is now recommended to override the `passthru.tests` attribute to add additional tests. For convenience, the input argument `kernelTests` has been preserved and updated to accept an attribute set by default.

  For compatibility purposes, the previous usage of `kernelTests` as a list is still supported in this release. Test cases specified in this manner are accessible via `passthru.tests.extra-kernel-test-<0-based index>`. However, please note that this compatibility layer will be removed in future releases to ensure consistency of the `<pkg>.override` interface.
