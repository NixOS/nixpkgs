...
- The SSH client (`programs.ssh`) now has settings options to avoid use of `extraConfig`, named
  [`programs.ssh.hostSettings`](#opt-programs.ssh.hostSettings) and [`programs.ssh.matchSettings`](#opt-programs.ssh.matchSettings).
