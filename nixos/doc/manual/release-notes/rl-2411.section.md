...
- Legacy package `stalwart-mail_0_6` was dropped, please note the
  ...
  before changing the package to `pkgs.stalwart` in
  [`services.stalwart.package`](#opt-services.stalwart.package).
...
- `services.stalwart` now runs under the `stalwart` system user
  ...
  `/var/lib/private/stalwart-mail` to `/var/lib/stalwart`, and to
  change the ownership of the directory and its content to `stalwart`.

- `services.stalwart` now uses RocksDB as the default storage backend
