...
- Legacy package `globalprotect-openconnect` 1.x and related module
  `services.globalprotect` were dropped. Two new packages -- `globalprotect-openconnect_2` and
  `globalprotect-openconnect_2_withGui` resulting from the 2.x version of the GlobalProtect-openconnect project
  -- are added in its place. The GUI version has an unfree license, and the regular version is under
  `gpl3` license. See [homepage](https://github.com/yuezk/GlobalProtect-openconnect) for more info.
