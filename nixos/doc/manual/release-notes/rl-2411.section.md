...
- `programs.chromium` now defaults to adding a chromium binary to the path, this might not be desireble when also configuring chromium
  with home-manager as two binaries would be in the system then. You can turn this behavior off by setting `programs.chromium.package = null`
