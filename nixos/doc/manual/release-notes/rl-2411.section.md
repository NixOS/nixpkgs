...
- Caddy can now be built with external plugins by overriding `externalPlugins` and `vendorHash` arguments like this:
  ...
  To get the necessary SRI hash of the vendored dependencies, set `vendorHash = "";`. The first build will fail, but it will produce the correct `vendorHash` in the error message.
