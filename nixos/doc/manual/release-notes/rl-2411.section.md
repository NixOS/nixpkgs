...
- Some Authelia options have been replaced or removed
  - `services.authelia.instances.<name>.settings.server.{host,port}` default values have been removed and replaced with a default value for `services.authelia.instances.<name>.settings.server.address` as this setting supersedes the former ones.
