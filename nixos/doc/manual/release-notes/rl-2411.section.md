...
- Docker now defaults to `docker_25`. This version of Docker has support for features such as the Container Device Interface (CDI), removing the need for runtime wrappers in order to expose specialized hardware to containers.
