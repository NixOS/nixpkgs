...
- Node.js headers in `nodejs` package are now in a separate `include` output.
  Existing Nix code that sets `npm_config_nodedir=${nodejs}` and
  `--nodedir=${nodejs}` should be updated to use `lib.getInclude nodejs`.
