...
- The setting [`services.xorg.config`](options.html#opt-services.xorg.config) now has a changed order, so that if the user sets this option, their config is rendered _after_ NixOS's default Xorg config.
  ...
  There may be unlikely user configurations that rely on the user config being overridden. If you have such a config, use `services.xorg.config = lib.mkBefore ...` on your config to invoke the old behaviour.
