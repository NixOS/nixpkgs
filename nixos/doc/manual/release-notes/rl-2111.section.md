...
- Zfs: `latestCompatibleLinuxPackges` is now exported on the zfs package. One can use `boot.kernelPackages = config.boot.zfs.package.latestCompatibleLinuxPackges;` to always track the latest compatible kernel with a given version of zfs.
