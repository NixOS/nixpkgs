...
- `services.wordpress.$name.database.name` default value has changed from `wordpress` to `wordpress-${name}` on NixOS installations where `system.stateVersion` is `21.11` or higher.
  ...
  The theme and plugin directory names don't include the version any more on NixOS installations where `system.stateVersion` is `21.11` or higher. This breaks the theme as wordpress isn't able to find it any more.

  To fix this you first need to change directory into the `/nix/store` wordpress directory.

  Before changing anything in the database you should really make a database backup. If you mistype anything you WILL break your database irrevocably. You can make a backup with:
  `nix-shell -p wp-cli --command 'sudo -u wordpress -- wp db export /tmp/$(date +%s)'`
  Remember to move the file to a persistent location.

  The following steps only work when the theme was never loaded before without the version suffix because overriding the values will fail otherwise. In that case you would get an error like `Duplicate entry 'theme_mods_twentytwenty'` which means you would need to delete that old value and rename the `theme_mods_twentytwenty-1.5` one to it.
  ```bash
    nix-shell -p wp-cli --command 'sudo -u wordpress -- wp db search twentytwenty'
    nix-shell -p wp-cli --command 'sudo -u wordpress -- wp db search twentytwenty-1.5'
    nix-shell -p wp-cli --command 'sudo -u wordpress -- wp search-replace "twentytwenty-1.5" "twentytwenty"'
  ```
  It is recommended to verify that all places that are found using the search command are fine to be replaced.
