...
- Mattermost 6 is now available, and can be selected with `services.mattermost.package`
  using the `mattermost_6` attribute. The default is still Mattermost 5, as this is
  Mattermost's extended support release. If you upgrade from a previous Mattermost
  version to 6, migrations may take a while. See the [upgrade notes](https://docs.mattermost.com/install/self-managed-changelog.html#release-v6.2-feature-release).
