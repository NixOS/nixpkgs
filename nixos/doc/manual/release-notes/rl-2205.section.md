...
- [polymc](https://github.com/PolyMC/PolyMC), a free, open source launcher for Minecraft.
...
- `multimc` has been replaced with `multimc-bin`, which is a self-updating
  package that uses official binaries from upstream. Users with existing
  installations need not take any steps, as the new package stores data in the
  same locations as the previous one.
