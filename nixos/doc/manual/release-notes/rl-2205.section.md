...
- `multimc` has been replaced with a self-updating package which uses official
  binaries from upstream. Users with existing installations need not take any
  steps, as the new package stores data in the same locations as the previous
  one.
