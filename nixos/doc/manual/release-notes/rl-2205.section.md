...
- `services.tahoe.nodes.<name>.sftpd.accounts.url` has been removed following the removal of the corresponding feature from Tahoe-LAFS.

- The configuration files generated by `services.tahoe` are no longer linked into `/etc`.
...
- The options [services.tahoe.nodes.\<name\>.settings](#opt-services.tahoe.nodes._name_.package) and [services.tahoe.introducers.\<name\>.settings](#opt-services.tahoe.introducers._name_.package) were added, allowing complete control of the Tahoe-LAFS configuration file.

- Tahoe-LAFS processes configured using [services.tahoe](#opt-services.tahoe.nodes) now run as dedicated static users instead of as root.
