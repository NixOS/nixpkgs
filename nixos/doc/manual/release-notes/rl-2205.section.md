...
- `buildDunePackage` now defaults to using Dune 2 and the option `useDune2` is therefore deprecated and a warning has been added. In its place `useDune1` is added for packages that doesn't work with  Dune 2.
  During the deprecation period `useDune2` will be inversed and used as `useDune1`, eg. `useDune2 = false` will be treated as `useDune1`.
