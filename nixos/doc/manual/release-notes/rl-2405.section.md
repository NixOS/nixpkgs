...
- `services.traefik` now also processes the dynamic configuration file generated from `services.traefik.dynamicConfigOptions`
  or `services.traefik.dynamicConfigFile` with `envsubst` and the provided `services.traefik.environmentFiles`. Previously,
  that was only done to the static configuration file.
