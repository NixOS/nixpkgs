...
- Configuring PipeWire and WirePlumber via `environment.etc` is no longer supported due to limitations of `environment.etc` and how `services.pipewire.configPackages` and `services.pipewire.wireplumber.configPackages` were implemented .
  - PipeWire configuration should now use `services.pipewire.extraConfig`. Packages providing configuration files can now be specified in `services.pipewire.configPackages`.
  - WirePlumber is only configurable via `services.pipewire.wireplumber.configPackages` until [#292115](https://github.com/NixOS/nixpkgs/pull/292115) lands. For example, to replace a config file `/etc/wireplumber/main.lua.d/41-set-celluloid-default-volume.lua` please use
    ```nix
    pkgs.writeTextDir "share/wireplumber/main.lua.d/41-set-celluloid-default-volume.lua" ''
      -- Your config file contents here
    ''
    ```
