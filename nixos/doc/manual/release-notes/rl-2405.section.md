...
- Default `CONFIG_ARCH_MMAP_RND_BITS` are set to the maximum to mitigate [kernel >=5.18 ASLR regression](https://zolutal.github.io/aslrnt/).  This may lead to programs breaking, specially the ones running on limited virtual address space. To get the previous behavior, check the minimum value with `zcat /proc/config.gz | grep "ARCH_MMAP_RND_BITS_MIN"` and set `boot.kernel.sysctl."vm.mmap_rnd_bits"` to the minimum.
