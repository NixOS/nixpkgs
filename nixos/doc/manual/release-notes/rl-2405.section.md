...
- `services.jackett` now listens on `localhost` by default. In order to listen on all interfaces set `services.jackett.listenOnAllInterfaces = true`. This option will override any option specified in the Web UI or via manually editing the config file. The port is now controlled via the `services.jackett.port` option. If you have customized the port you will need to manually configure this value or it will override the port you have selected.
