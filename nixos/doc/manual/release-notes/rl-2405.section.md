...
- The build helper `buildLinux` now provides a structural `passthru.tests` for its result Linux kernel packages as an attribute set instead of a list.
  ...
  For compatibility purpose, the previous usage of `kernelTests` as a list is still supported in this release. Test cases specified in this manner is accessible via `passthru.tests.extra-kernel-test-<0-based index>`. However, please note that this compatibility layer will be removed in future releases to ensure consistency of the `<pkg>.override` interface.
