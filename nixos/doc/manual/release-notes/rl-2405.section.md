...
- `docker` from ver. 25.x the device mapper driver has been deprecated and removed - https://docs.docker.com/storage/storagedriver/device-mapper-driver/ Therefore the `devicemapper` option have been removed from `virtualisation.docker.storageDriver`. Warning: changing the storage driver will cause any existing containers and images to become inaccessible.
