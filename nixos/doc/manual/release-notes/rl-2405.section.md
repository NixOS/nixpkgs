...
- `writeTextFile`'s input arguments was changed as follows:
  - `preInstall`, `postInstall` are now added to specify commands to run before/after text file writing.
  - `checkPhase` is now renamed to `installCheckPhase` to emphasize the execution order (`echo ... > "$out"` -> `postInstall` -> `installCheckPhase`).
  - `checkPhase` becomes an alias to `installCheckPhase` when only the former is specefied, with an eval-time warning about the rename.

  `writeShellApplication`, a build helper based on `writeTextFile`, also adopt the above interface changes. Its `installCheckPhase` defaults to run `preInstallCheck` hooks at the beginning and `postInstallCheck` hooks at the end, which used to be `preCheck` and `postCheck` hooks.

  Its derived build helper, `writeShellApplication`, now also use `installCheckPhase` instead of `checkPhase`.
