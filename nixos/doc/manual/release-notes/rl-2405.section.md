...
- Passing the `stdenv` attribute to build helper `buildPythonPackage` and `buildPythonApplication` is now warned against, in favor of `buildPython*.override { stdenv = ...;}`.
  ...
  For out-of-tree projects that needs to support Nixpkgs revisions across this change, use the following expression to determine if `buildPythonPackage.override { stdenv = ...; }` functionality is available:

  ```nix
  buildPythonPackage?override
  && (lib.functionArgs buildPythonPackage.override)?stdenv
  ```
