...
- Users now default to a primary group named the same as the user, and with group id same as user id.
  Before it was "users" with group id 100 for normal users and "" (empty) for system users -- which meant system users required explicitly setting the primary group.
  This change gives better control over permissions/file sharing and aligns NixOS with other distros like Arch Linux/Debian/Fedora/Ubuntu.
  The new functionality is gated behind an option called `users.solitaryByDefault`, which defaults to true for `system.stateVersion >= 24.05`.
