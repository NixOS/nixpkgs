...
- `security.pam.services.<name>` now has an option `fprintAuthAfterPassword` to change the order of fingerprint and password authentication. Combined with `allowNullPassword`, this allows use of a password if you cannot `Ctrl+C` fingerprint authentication (due to a lack of a shell).
