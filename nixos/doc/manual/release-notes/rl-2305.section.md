...
- `openjdk` from version 11 and above is not build with `openjfx` (i.e.: JavaFX) support by default anymore. You can re-enable it by overriding with `enableJavaFX` set to `true`, e.g.: `openjdk11.override { enableJavaFX = true; };`.
