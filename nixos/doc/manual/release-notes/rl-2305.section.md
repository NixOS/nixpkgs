...
- CoreDNS can now be built with external plugins by overriding `externalPlugins` and `vendorSha256` arguments like this:
```
services.coredns = {
  enable = true;
  package = pkgs.coredns.override {
    externalPlugins = ["fanout"];
    vendorSha256 = "<sha256 hash>";
  };
};
```
To get the necessary sha256 hash, set `vendorSha256 = lib.fakeHash;`. The build will fail and produce your sha256 hash in the error message. You should copy that hash and use it for your `vendorSha256`.
Please note! If you use this feature, you will not receive updates to CoreDNS when you upgrade your system. To update CoreDNS, you'll have to update `vendorSha256`; for that, set `vendorSha256 = lib.fakeHash;` again and follow the steps above.
