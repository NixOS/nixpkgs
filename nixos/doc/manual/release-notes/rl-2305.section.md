...
### libgcc is now packaged separately, may affect downstream packages {#sec-release-23.05-migration-libgcc}

Nixpkgs now puts `libgcc` in its own package, as most distributions
do.  If you maintain a Nix expression outside of nixpkgs you may
encounter link failures mentioning `-lgcc_s`, especially if you are
using `autoPatchelf`.

#### What do the link failures look like? {#sec-release-23.05-migration-libgcc-what}

It depends on what compiler and linker you're using.  Here are a few
examples:

```
/nix/store/f4qnwzv6y0nq8lix33jr5ykkyybs6fxf-binutils-2.40/bin/ld: warning: libgcc_s.so.1, needed by ../x86_64-unknown-linux-gnu/libgo/.libs/libgo.so, not found (try using -rpath or -rpath-link)
/nix/store/f4qnwzv6y0nq8lix33jr5ykkyybs6fxf-binutils-2.40/bin/ld: warning: libgcc_s.so.1, needed by ../x86_64-unknown-linux-gnu/libgo/.libs/libgo.so, not found (try using -rpath or -rpath-link)
```

```
searching for dependencies of /nix/store/svym0wxqk6qp7dc7dxq3c5apw83jqqm6-cargo-1.70.0-nightly-2023-03-10-x86_64-unknown-linux-gnu/bin/cargo
    libgcc_s.so.1 -> not found!
```

#### How can I fix the link failures? {#sec-release-23.05-migration-libgcc-how}

If you experience link failures like the one above, please try the
following:

1. (Preferred): add `(stdenv.cc.cc.libgcc or null)` to the
   `buildInputs` of your expression.

2. Add `(glibc.cc.libgcc or null)` to the `buildInputs` of your
   expression.

3. Change any use of `autoPatchelfHook` to
   `autoPatchelfHookWithLibgcc`.

#### Why was libgcc moved to its own package? {#sec-release-23.05-migration-libgcc-why}

Previously, nixpkgs copied `libgcc_s.so` out of the package which
builds it (`gcc`) and into an unrelated package (`glibc`).  This
copying caused serious [contamination problems](https://github.com/NixOS/nixpkgs/pull/170857#issuecomment-1170558348) which prevented
`stdenv` from "bootstrapping past" ABI changes like [aarch64
`-moutline-atomics`](https://github.com/NixOS/nixpkgs/issues/108111) and [powerpc64le IEEE-standard `long
double`](https://github.com/NixOS/nixpkgs/pull/170215).  Moving `libgcc_s.so` into its own package was an
essential part of a [larger project](https://github.com/NixOS/nixpkgs/pull/209870) to remove these kinds of
artificial constraints on our bootstrap sequence.

Complete technical details can be found in commit
[7553d0fe29801938bcb280bb324b579ef9016aea](https://github.com/NixOS/nixpkgs/pull/209870/commits/7553d0fe29801938bcb280bb324b579ef9016aea).

#### Why can't this be fixed automatically in nixpkgs? {#sec-release-23.05-migration-libgcc-why2}

Glibc `dlopen()`s `libgcc_s` in order to implement [`pthread`
cancellation](https://github.com/NixOS/nixpkgs/pull/210112#issuecomment-1379542735), by unwinding the stack.  Glibc also implements the
dynamic linker on GNU/Linux systems like NixOS.  For reasons which
remain somewhat vague, glibc has a [special and unique mechanism](https://sourceware.org/legacy-ml/libc-help/2013-11/msg00026.html)
for loading `libgcc_s`, which is completely different from and more
restrictive than the ordinary library-loading mechanism.

Because `dlopen()` is used, *the fact that `libc.so` depends on
`libgcc_s.so` is not expressed in the ELF file structure of any of
these libraries*, nor is the dependency detectable by Nix's
reference scanner.  Since this dependency is hidden so well from Nix
and nixpkgs, our usual tooling (`autoPatchelf`, etc) cannot "see"
this dependency.

Some discussion of the tradeoffs involved in trying to auto-detect
this can be found [here](https://github.com/NixOS/nixpkgs/issues/225963#issuecomment-1519381058).  At the moment all of the possible
solutions are somewhat fragile.  Ideally glibc would expose this
linkage dependency in its ELF file structure like all other
libraries do.
