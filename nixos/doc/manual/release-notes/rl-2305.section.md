...
- `i686-linux` NixOS tests requiring a system image (i.e. boot-related, etc.) are broken until [LKL](https://lkl.github.io/) is fixed on i686-linux or someone introduces a slowpath in `make-disk-image` for creating images without `cptofs` (fully in VM for example) which would only depend on QEMU then.
