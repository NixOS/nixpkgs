...
- `lf` introduces a few breaking changes in [version r29](https://github.com/gokcehan/lf/releases/tag/r29). Mainly that configuration options can now use `set option true/false`, instead of using `set option` and `set nooption`, although this is still valid. Using `set option on/off` no longer works and will throw an error; and using `set option!` isn't affected. And env variable `VISUAL` is checked before `EDITOR`.
