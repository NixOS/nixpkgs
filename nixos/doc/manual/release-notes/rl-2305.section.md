...
- Following Fedora's behaviour, `systemd-oomd` now monitors the system slice by default, using the memory pressure policy. This can be reverted by setting `systemd-oomd.enableSystemSlice` to `false`, or disabling the daemon completely with `systemd-oomd.enable`. Monitoring of all user slices can also be enabled by setting `systemd-oomd.enableUserSlices` to `true`, but as `systemd-oomd` acts on a per-cgroup level, applications will need to spawn processes into separate cgroups (e.g. with `systemd-run`) or use a desktop environment (e.g. GNOME, KDE) that does this for them.
