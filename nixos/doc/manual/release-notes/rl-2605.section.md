...
- `services.vikunja` has been updated to 1.0.0 with breaking changes. Notable ones for NixOS users:
  - CORS is now enabled by default and requires `services.vikunja.settings.service.publicurl` (or CORS explicitly disabled), otherwise the service will not start.
  - API routes changed and OpenID providers config switched from list to map; existing integrations/configs may need adjustments.
  - SQLite relative paths now resolve under `service.rootpath`; absolute paths remain unaffected.
