...
- `services.phpfpm.pools.<name>.phpOptions`, `services.phpfpm.phpOptions` and
  `services.httpd.phpOptions` are now attribute sets instead of strings.
   Supplying the PHP configuration as strings is still supported but will get
