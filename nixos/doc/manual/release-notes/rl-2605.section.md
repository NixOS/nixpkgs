...
- `winePackages` has been restructured.

  Previously, `winePackages` was parameterized by wine build (e.g., `wine32`, `wineWow64`),
  and `wineWowPackages`, `wineWow64Packages`, etc. were separate package sets.
  Now, `winePackages` contains all variants,
  with each variant (e.g., `winePackages.stable`, `winePackages.full`) providing all builds as attributes (`.wine32`, `.wine64`, `.wineWow`, `.wineWow64`).
  For example, `wineWowPackages.stable` should be replaced with `winePackages.stable.wineWow`.
  The old `wineWowPackages`, `wineWow64Packages`, and `wine64Packages` are deprecated aliases that emit warnings.

  The new structure is better because different releases may not support all the same builds.
  For example, the recently-deprecated old "WoW" mode could be removed in newer versions entirely, but we would still want to expose it in older versions.

  Additionally, the ability to `.override { wineBuild = "..."; wineRelease = "..."; }` has been removed;
  use the new attribute structure to select the desired build and release directly.
