...
- `p7zip` has been modified to symlink to `_7zz` instead of it's original source from [p7zip-project/p7zip](https://github.com/p7zip-project/p7zip). This has been done since `p7zip` is no longer the only Linux (and Unix) implementation; `_7zz` is the official 7-zip release and sees more development. Additionally `p7zip` has been dropped by several other distributions.
