...
- The system.nix file has been added has an alternative entry point to configuration.nix (and flake.nix) that allows to configure NixOS without using `nix-channel`.
  ...
  If present, a system.nix file in the current directory will be used instead.

  Note: before this change `nixos-rebuild` and `nixos-install` would load a default.nix file in the current directory.
  This file is now deprecated and support for it will be removed in the future.
  This was done to avoid confusion and prevent accidentally loading default.nix files that are unrelated to NixOS.
