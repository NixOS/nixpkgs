...
- `services.overseerr` has been removed as the `overseerr` and `jellyseerr` projects were merged under `seerr`; you can migrate the existing services to `services.seerr`. Notable breaking changes:
  - systemd service name changed accordingly.
  - Default config directory moved from `/var/lib/overseerr` to `/var/lib/seerr/`.
