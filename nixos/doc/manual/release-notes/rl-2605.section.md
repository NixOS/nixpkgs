...
- `flameshot` now uses `xdg-desktop-portal` for screenshotting. This is a breaking change for users on window managers without portal support with Screenshot capabilities, as well as if the `xdg.portal` configuration is not set up correctly. See https://github.com/flameshot-org/flameshot/releases/tag/v14.0.rc1 and https://wiki.nixos.org/wiki/Flameshot.
