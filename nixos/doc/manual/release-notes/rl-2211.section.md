...
- The [`pkgs.dockerTools.pullImage`](https://nixos.org/manual/nixpkgs/stable/#ssec-pkgs-dockerTools-fetchFromRegistry) function drop support
  for `arch` and `os` parameters in favor of Nixpkgs cross compilation. One way to migrate is to change `pkgs.dockerTools.pullImage` to
  `pkgs.pkgsCross.<platform>.dockerTools.pullImage` where `<platform>` corresponds to the correct parameters used before as in the example below.

  Before

  ```nix
  {
    myDockerImage = pkgs.dockerTools.pullImage {
      imageName = "nixos/nix";
      imageDigest =
  ...
  ```nix
  {
    myDockerImage = pkgs.pkgsCross.armv7l-hf-multiplatform.dockerTools.pullImage {
  ...
  For more information on how to obtain `pkgs` for any combinations of parameters,
  see [the official nix guide about cross compilation](https://nixos.org/guides/cross-compilation.html).
