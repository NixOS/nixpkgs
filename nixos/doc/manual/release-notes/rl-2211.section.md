...
- The installation device profile (e.g. used for the ISO installer) has now a friendlier way to discover and connect to wifi networks from the command line via `nmtui connect` & `nmtui`
