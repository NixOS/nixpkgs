...
- The `services.quassel` module has been refactored and extended to provide more
  options. The existing options `dataDir`, `certificateFile` and `requireSSL`
  have been moved to `services.quassel.settings.dataDir`,
  `services.quassel.settings.ssl.certFile` and
  `services.quassel.settings.ssl.required` respectively. Database and
  authentication settings can now be set declarative when
  `services.quassel.settings.useDeclarativeConfig` is enabled.

- Removed the `quassel` user and group of the `services.quassel` module. The
  module now uses a systemd DynamicUser with the same name. This required moving
  the default `services.quassel.settings.dataDir` to `/var/lib/quassel` to use
  the systemd StateDirectory. You may need to move your data to the new location
  or set a custom dataDir.
