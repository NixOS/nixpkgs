...
## New NixOS Configuration Modules {#sec-release-23.11-new-modules}

### Hardware {#sec-release-23.11-new-hardware-options}

[hardware.infiniband.enable](#opt-hardware.infiniband.enable)
: `hardware/infiniband.nix` adds infiniband subnet manager support using an [opensm](https://github.com/linux-rdma/opensm) systemd-template service, instantiated on card guids. The module also adds kernel modules and cli tooling to help administrators debug and measure performance.

[hardware.tuxedo-rs.enable](#opt-hardware.tuxedo-rs.enable)
: [tuxedo-rs](https://github.com/AaronErhardt/tuxedo-rs), Rust utilities for interacting with hardware from TUXEDO Computers.

### Networking {#sec-release-23.11-new-networking-options}

[networking.jool.enable](#opt-networking.jool.enable)
: [Jool](https://nicmx.github.io/Jool/en/index.html), a kernelspace NAT64 and SIIT implementation, providing translation between IPv4 and IPv6.

### Programs {#sec-release-23.11-new-program-options}

[programs.ns-usbloader](#opt-programs.ns-usbloader.enable)
: [NS-USBLoader](https://github.com/developersu/ns-usbloader/), an all-in-one tool for managing Nintendo Switch homebrew.

[programs.river](#opt-programs.river.enable)
: [river](https://github.com/riverwm/river), a dynamic tiling wayland compositor.

[programs.wayfire](#opt-programs.wayfire.enable)
: [wayfire](https://wayfire.org), A modular and extensible wayland compositor.

### Services {#sec-release-23.11-new-service-options}

[services.acme-dns](#opt-services.acme-dns.enable)
: [acme-dns](https://github.com/joohoi/acme-dns), a limited DNS server to handle ACME DNS challenges easily and securely.

[services.anuko-time-tracker](#opt-services.anuko-time-tracker.enable)
: [Anuko Time Tracker](https://github.com/anuko/timetracker), a simple, easy to use, open source time tracking system.
...
: [Castopod](https://castopod.org/), an open-source hosting platform made for podcasters who want to engage and interact with their audience.

[services.ebusd](#opt-services.ebusd.enable)
: [ebusd](https://ebusd.eu), a daemon for handling communication with eBUS devices connected to a 2-wire bus system (“energy bus” used by numerous heating systems).
...
: [GoToSocial](https://gotosocial.org/), an ActivityPub social network server, written in Golang.

[services.guacamole-server](#opt-services.guacamole-server.enable) and [services.guacamole-client](#opt-services.guacamole-client.enable)
: [Apache Guacamole](https://guacamole.apache.org/), a cross-platform, clientless remote desktop gateway.

[services.hddfancontrol](#opt-services.hddfancontrol.enable)
: [hddfancontrol](https://github.com/desbma/hddfancontrol), a service to regulate fan speeds based on hard drive temperature.
...
[services.mautrix-whatsapp.enable](#opt-services.mautrix-whatsapp.enable)
...
: [MCHPRS](https://github.com/MCHPR/MCHPRS), a multithreaded Minecraft server built for redstone.

[services.mobilizon.enable](#opt-services.mautrix-whatsapp.enable)
...
: [netclient](https://github.com/gravitl/netclient), an automated WireGuard® Management Client.

[services.nncp](#opt-services.nncp.caller.enable)
: [NNCP](http://www.nncpgo.org/). Added nncp-daemon and nncp-caller services. Configuration is set with [programs.nncp.settings](#opt-programs.nncp.settings).

[services.osquery.enable](#opt-services.osquery.enable)
: [osquery](https://www.osquery.io/), a SQL powered operating system instrumentation, monitoring, and analytics.

[services.pgbouncer](#opt-services.pgbouncer.enable)
...
: [Prometheus MySQL exporter](https://github.com/prometheus/mysqld_exporter), a MySQL server exporter for Prometheus.

[services.sitespeed-io](#opt-services.sitespeed-io.enable)
: [sitespeed-io](https://sitespeed.io), a tool that can generate metrics (timings, diagnostics) for websites.

[services.stalwart-mail](#opt-services.stalwart-mail.enable)
: [stalwart-mail](https://stalw.art), an all-in-one email server (SMTP, IMAP, JMAP).

[services.trust-dns](#opt-services.trust-dns.enable)
: [trust-dns](https://trust-dns.org/), a Rust based DNS server built to be safe and secure from the ground up.

[services.typesense](#opt-services.typesense.enable)
: [Typesense](https://github.com/typesense/typesense), a fast, typo-tolerant search engine for building delightful search experiences.

[services.zitadel](#opt-services.zitadel.enable)
: [ZITADEL](https://zitadel.com), a turnkey identity and access management platform.

### Systemd {#sec-release-23.11-new-systemd-options}

[systemd.sysupdate](opt-systemd.sysupdate)
: [systemd-sysupdate](https://www.freedesktop.org/software/systemd/man/systemd-sysupdate.html), atomically updates the host OS, container images, portable service images or other sources.
