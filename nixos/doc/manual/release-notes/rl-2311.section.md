...
- All functions have been removed from `{build,host,target}Platform` attrsets in order to ensure that equality behaves predictably on them.
  - If you were using `machinePlatform.canExecute binaryPlatform`, use `lib.systems.canExecute machinePlatform binaryPlatform` instead.
  - If you were using `binaryPlatform.{emulator,emulatorAvailable} pkgs`, use `lib.systems.{emulator,emulatorAvailable} binaryPlatform pkgs` instead.
