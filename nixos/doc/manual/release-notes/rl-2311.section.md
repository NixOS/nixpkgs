...
- `virtualisation.containers` now supports customizing journal linking behavior using `virtualisation.containers.<name>.linkJournal` option. To preserve backwards compatibility, this option is set to `try-guest` on non-`ephemeral` containers and is unset on ephemeral containers.

For ephemeral containers, it is now possible to set it to `try-host` to preserve logs for ephemeral containers.
