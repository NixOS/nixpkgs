...
- Emacs was upgraded from version 28 to 29. This includes many major additions, most notably `tree-sitter` support (enabled by default) and the pgtk variant, which is available under the attribute `emacs-pgtk`.
