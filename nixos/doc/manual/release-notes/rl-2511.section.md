...
- `services.minio` removed deprecated credential options: `accessKey`, `secretKey` and `rootCredentialsFile`. Use `environmentFile` instead. Further the `--config-dir` parameter was deprecated by minio and thus the `configDir` option has been remove. Use the new `extraEnvironment` option to configure minio.
