...
- The `services.postgresql` module now sets up a systemd unit `postgresql.target`, which other services should depend on instead of `postgresql.service`. Only `postgresql.target` will have ensured users and run the initial setup script. Additionally, it will wait for a read-write connection after a restore from backup, while `postgresql.service` will only wait for a read-only connection during recovery.
