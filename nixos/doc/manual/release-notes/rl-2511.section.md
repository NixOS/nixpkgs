...
- The `pkgs.uptime-kuma` package now throws a warning regarding migration to the v2.x.x series. Migration can be completed by setting `services.uptime-kuma.package` to `pkgs.uptime-kuma_2`.
  Additionally, the logic for the module's default package is now as follows:
  - If [`services.uptime-kuma.package`](#opt-services.uptime-kuma.package) is specified explicitly, this package will be installed (**recommended**)
  - If [`system.stateVersion`](#opt-system.stateVersion) is >=25.11, `pkgs.uptime-kuma_2` will be installed by default.
  - Else, `pkgs.uptime-kuma_1` will be installed by default.
  - Please note that an upgrade from v1 to v2 might require manual intervention and can take a few hours to complete. **A backup is highly recommended.**
