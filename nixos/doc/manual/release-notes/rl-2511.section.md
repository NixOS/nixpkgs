...
- The `services.caddy.admin` option will now overwrite admin API settings provided in `services.caddy.globalConfig`, and it is a prerequisite to `services.caddy.enableReload`. Caddy admin API will be enabled by default under Caddy's data directory and set to a Unix socket named `admin.sock`. This will transitively enable `services.caddy.enableReload`.
