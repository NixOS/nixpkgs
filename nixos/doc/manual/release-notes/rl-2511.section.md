...
- All modules for standalone window managers now have a `package` option (i.e. [](#opt-services.xserver.windowManager.spectrwm.package) and so on). This allows to modify the WM packages directly instead of using an overlay.
