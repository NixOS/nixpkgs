...
- `null` values now occur in `environment.variables` and `environment.sessionVariables`. This trade-off was made in order to allow self-references in these options, avoiding infinite recursions. This trade-off is documented in [lazyAttrsOf](https://nixos.org/manual/nixos/unstable/#sec-option-types-composed) and [filterAttrs](https://nixos.org/manual/nixpkgs/unstable/#function-library-lib.attrsets.filterAttrs).

  This change is associated with the error messages such as:
  ```
  … while evaluating definitions from `/path/to/some-configuration-aka-module.nix`:
  ...
  It can typically be resolved by handling / filtering the `null` values from the aforementioned options.
  - Instead of `config.environment.variables ? FOO`, use `config.environment.variables.FOO or null != null`.
  - Instead of `filterAttrs (n: v: n == "FOO") config.environment.variables`, use `filterAttrs (n: v: n == "FOO" && v != null) config.environment.variables` (in that order).
  - Instead of `intersectAttrs { FOO = throw "unused"; } config.environment.variables`, use `filterAttrs (n: v: v != null) (intersectAttrs { FOO = null; } config.environment.variables)` (in that order).
  - Same for `config.environment.sessionVariables`.
