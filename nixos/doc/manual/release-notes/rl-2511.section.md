...
- All modules for standalone window managers now have a `package` option (i.e. `services.xserver.windowManager.spectrwm.package` etc). This allows to modify the WM packages directly instead of using an overlay.
