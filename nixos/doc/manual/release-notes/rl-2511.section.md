...
- The [PAM module for libpwquality](https://github.com/libpwquality/libpwquality/) is available for use. One can enable it globally using the [security.pwquality.pam.enable](#opt-security.pwquality.pam.enable) or per service. Configuration of pwquality can be done globally ([](#opt-security.pwquality.settings)), globally for just PAM ([](#opt-security.pwquality.pam.settings)), or per service ([](#opt-security.pam.services._name_.pwquality.settings)).
