...
- Various changes to the `postfix` module may require changes to your configuration:
  - Settings in [services.postfix.config](#opt-services.postfix.config) can be removed from the rendered config by setting them to `null` or an empty list, depending on the option type.
