...
- [{option}`programs.ssh.startAgent`](#opt-programs.ssh.startAgent), has switched from using `gnome-keyring`, to `gcr_4`. This is due to ssh-agent support being deprecated in the upstream `gnome-keyring` project. See the [relevant upstream PR](https://gitlab.gnome.org/GNOME/gcr/-/merge_requests/67) for more details.
  - {option}`services.ssh.startAgent` will itself be deprecated soon. The new [{option}`services.gnome.gcr-ssh-agent`](#opt-services.gnome.gcr-ssh-agent.enable) options should be used instead.
