...
- `omnissa-horizon-client` has been added as the rebrand of `vmware-horizon-client`. Changes currently are just file renames but could be more substantial going forwards. `vmware-horizon-client` will remain for this release and it's eventual removal shall appear in future release notes.
