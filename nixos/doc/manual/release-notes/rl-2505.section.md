...
- The concept of desync was added to the `autoUpgrade` module. This allows people who use channels to update their system even if some of their packages do not build with the latest channel version. This works especially nicely with declarative containers. See [`system.autoUpgrade.desync`](options.html#opt-system.autoUpgrade.desync) for more details.
