...
- The `nixos-generate-config` script now looks for hardware devices in a different order when generating `hardware-configuration.nix`, prioritizing devices under the `/dev/disk/by-label/*` glob over `/dev/disk/by-uuid/*` or `/dev/mapper/*`. This makes the manual installation process more robust (e.g. when porting config from a VM), but might break certain automated installation.
