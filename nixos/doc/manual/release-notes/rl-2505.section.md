...
- The default package for the `redis` service got changed to the drop-in replacement `valkey`. The `redis` package can still used by configuring [`services.redis.package`](#opt-services.redis.package).
