...
Some programs require their configuration file to embed secrets. Care
must be taken for those secrets to not end up as cleartext in the nix
store. Nix supports does support this use case for nix-representable
file formats.
...
### Options which are secrets {#sec-settings-secrets}

Although one should prefer passing secrets to applications in other
ways, as environment variables for example, sometimes upstream programs
are stubborn and they only accept secrets embedded in their
configuration file.

Nix supports such cases by allowing some `settings` attributes to be
considered secrets. Those values never end up in cleartext in the nix
store. The onus falls on the user to make the secrets available at
predefined paths on the target host. Nix will then take care of
constructing the final configuration file with secrets embedded.

This all can be achieved by combining the aforementioned `pkgs.format.*`
functions and the `gen*SecretsReplacement` functions. See the
[nixpkgs manual](https://nixos.org/nixpkgs/manual#sec-secrets) for more
details on all the arguments those functions accept.

An attribute is then considered a secret if instead of a value, it
contains another attribute set with the `_secret` attr:
...
where the cleartext secret will be available from on the target host.

The `gen*SecretsReplacement` function then replaces all occurrences of
such an attrset with a uniquely identified placeholder string whose
...
From this new representation, the `gen*SecretsReplacement` functions
generate the configuration file in the expected output format and this
gets stored in the nix store. For example in JSON format:
...
Assuming the file `/run/secrets/path/to/secret` is readable on the
target host and contains `my super secret`, the final configuration file
will be:
...
Here we extend the above example allowing for secrets and add the
required machinery to make this all work.

::: {#ex-settings-secrets .example}
### Using secrets in program settings

All attributes of form `"<name>._secret = "/path/to/file";` are
considered secrets and will be replaced by placeholders before being
stored in the nix store. The secrets will then be embedded in the file
on the target host by reading the secret files at the location given in
the `_secret` attribute. It is assumed those files will be populated
with an external tool which safely transport secrets.

::: {.warning}
This example will not work for systemd services that set a user different
from root, like with `DynamicUser=true` or `User=foo`. In those cases,
the example with `LoadCredential` in the next section should be used.
:::

```nix
let
  # We chose /run instead of /etc to avoid storing the
  # configuration file with secrets on the disk, as /run is
  # usually an in-memory filesystem.
  secretsReplacement = utils.genJqSecretsReplacement {
    loadCredential = true;
  } cfg.settings "/run/foo/myconfiguration.json";
in

{
  ...
  config = {
    # We do not rely on environment.etc."foo.json".source
    # to place the configuration file in the correct location,
    # we use the systemd service's preStart option which allows
    # to run a script.
...
::: {#ex-settings-secrets-user .example}
### Using secrets in program settings for non-root users

For systemd services with a user other than root, the secrets will
usually not be readable and must be loaded instead through systemd's
`LoadCredential` feature.

Building up on the previous example, we just need to set the
`loadCredential` attribute in the `genJqSecretsReplacement` function and
add `secretsReplacement.credentials` to the systemd service.

```nix
let
  secretsReplacement =
    utils.genJqSecretsReplacement
      # Add loadCredential = true
      { loadCredential = true; }
      cfg.settings
      "/run/foo/myconfiguration.json";
in
...
### Using structured secrets in program settings {#sec-settings-structured-secrets}

By default, the secrets are embedded as strings in the configuration
file. How a string is defined is dependent on the actual file format.
For example for json it is a quoted string and for key-value it is just
a value. But what remains constant is that the secret will be quoted and
any possibly problematic syntax in the secret will be escaped as to not
cause any syntax error.

It is also possible to embed secrets that are themselves valid syntax
for the particular configuration format. Of course, the user must take
care of formatting the secret to make the resulting configuration file
valid. No help is provided by nix for ensuring this. This is
configurable independently per secret.

::: {#ex-settings-structured-secret .example}
### Using a structured secret
As an example, if the file `"/path/to/secret"` contains the JSON document:
...
would generate a script that outputs the following JSON file at
`/path/to/output.json`:
