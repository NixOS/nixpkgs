...
- `lib`: Utility functions for convenience, or special interactions with the format.
  This attribute is optional. It may contain inside a `types` attribute
  containing types specific to this format.

- `generate` _`filename jsonValue`_: A function that can render a value of the format to a file. Returns
  a file path.
