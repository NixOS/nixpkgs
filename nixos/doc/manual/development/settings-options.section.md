...
- `pkgs.formats.ini` { _`listsAsDuplicateKeys`_ ? false, _`listToValue`_ ? null, \.\.\. }
  ...
  The type of the input is an _attrset_ of sections; key-value pairs where
  the key is the section name and the value is the corresponding content
  which is also an _attrset_ of key-value pairs for the actual key-value
...
- `pkgs.formats.iniWithGlobalSection` { _`listsAsDuplicateKeys`_ ? false, _`listToValue`_ ? null, \.\.\. }
  ...
  It returns a set with INI-specific attributes `type` and `generate`
  as specified [below](#pkgs-formats-result).
  The type of the input is an _attrset_ of the structure
  `{ sections = {}; globalSection = {}; }` where _sections_ are several
  sections as with _pkgs.formats.ini_ and _globalSection_ being just a single
...
- `pkgs.formats.xml` { format ? "badgerfish", withHeader ? true}
  ...
  - `"badgerfish"`
    : Uses [badgerfish](http://www.sklar.com/badgerfish/) conversion.
