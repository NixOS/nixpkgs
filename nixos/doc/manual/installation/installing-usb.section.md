...
1. Find the corresponding device with `lsblk`. You can distinguish them by
   their size.
1. Make sure all partitions on the device are properly unmounted. Replace `sdX`
...
1. Plug in the USB flash drive.
1. Find the corresponding device with `diskutil list`. You can distinguish them
   by their size.
1. Make sure all partitions on the device are properly unmounted. Replace `diskX`
