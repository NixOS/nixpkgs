  ...
  utils = rec {
    ...
    */
    genJqSecretsReplacement = genSecretsReplacement {
      generator = (pkgs.formats.json { }).generate;
      escape_style = "json";
    };
    ...
    /*
      ...
      :::{.info}
      This function expects the secrets to be readily available on the
      target host at pre-established locations. An out of band tool is
      required to transport the secrets.

      Further documentation can be found
      [in the nixpkgs manual](https://nixos.org/nixpkgs/manual/index.html#sec-secrets).
      :::

      Takes a config file generator, some options for fine tuning, the
      config file expressed in nix - an arbitrary value of nested lists
      and attrsets - and a file path where the config file will be
      created on the target host.

      This function will store in the nix store a version of the given
      config file with secrets replaced by uniquely identifiable
      placeholders. It then generates a bash snippet that takes the
      config file in the nix store and creates the config file in the
      wanted location with all placeholders replaced with the decrypted
      secrets.

      By default, this function assumes the user running the bash script
      can access all files containing the required secrets. If that's
      not the case, the `loadCredential` attribute can be set to `true`
      to make use of
      ...
      : 1\. Generator attrset with two attrs:
        ...
        - `generator`: `String -> a -> String` the function generating
          the config file. It takes two arguments, the path where the
          file should be created and an arbitrary value of nested lists
          and attrsets, with secrets replaced by placeholders. It must
          produce a file in the expected format (json, yaml, etc.) in
          the nix store. It returns a string which is the path to the
          config file.

        - `escape_style`: a string which currently can only be `json`
          which describes how the decrypted secret should be sanitized
          to be embedded into the config file. For example, this tells
          to escape the quote `"` character by adding a backslash when
          replacing a placeholder in a JSON string with a secret
          containing a quote. Secrets that should not be quoted (with
          `quote = false`) will not be escaped.

      `options`

      : 2\. Options on how to parse the configuration file

        - attr: The name of the attribute that will be identify a value
          as identifying a secret, defaults to `_secret`.

        - loadCredential: A boolean determining whether the script
          should load secrets directly (false) or load them from
          $CREDENTIALS_DIRECTORY (true), defaults to `false`. In the
          latter case the output attribute set will contain a
          `.credentials` attr with the necessary credential list that
          can be passed to systemd's `LoadCredential=` option.

      `configuration`

      : 3\. Arbitrary value of nested attrset or list. The `generator`
        function will be applied to it after all secrets - the attrsets
        containing the `attr` attribute (which is `_secret` by default)
        - are replaced by uniquely identified placeholders.

        Additional attributes can be added next to `attr` to further
        customize how the secret whould be embedded:

        - `quote`: If `quote = true`, the default, the content of the
          secret file will be quoted and embedded following the
          `escape_style`.

          Otherwise, if `quote = false`, the content of the secret file
          will be outputted as-is, without quotes. Care should then be
          taken to make sure the format is compatible with the one from
          the `generator`. This allows for structured secrets.
      ...
      : Attrset containing:
        - `script`: The bash script that creates the final configuration
          with embedded secrets in the expected location by replacing
          all placeholders with the secrets in the configuration file
          stored in the nix store.
        - `credentials`: The credential list to give to systemd's
          `LoadCredential=`. Only available when the `loadCredential`
          attribute is set to `true` in the `options` attrset.
      ...
      Assuming the file `/path/to/secret` exists and contains
      `topsecretpassword1234`, the following function call:

      ```nix
      genSecretsReplacement {
        escape_style = "json";
        generator = (pkgs.formats.json { }).generator;
      } { } {
        example = [
          {
      ...
      Assuming the file `/path/to/secret` exists and contains an actual
      JSON valid value:
    ...
    genSecretsReplacement =
      ...
      let
        ...
        setWithSecretsReplaced = mapAttrsRecursiveCond' (as: !(isAttrs as && hasAttr attr as)) (
          ...
          if (isAttrs item && hasAttr attr item) then
            sanitizePath ".${concatMapStringsSep "." (n: ''"${n}"'') path}"
          else
            item
        ) set;
        ...
        sanitizePath =
          path:
          "@_secret_${
            lib.stringAsChars (c: if builtins.match "[@a-zA-Z0-9_.#=!-]" c != null then c else "") path
          }@";
      ...
      in
      {
        script = ''
          ...
          umask 007
          cp "${generator "${output}.template" setWithSecretsReplaced}" '${output}'
          chmod ug+w '${output}'
        ''
        + (concatStringsSep "\n" (
          imap1 (index: name: ''
            ${pkgs.replace-secret}/bin/replace-secret \
              "${sanitizePath name}" \
              ${credentialPath index name} \
              '${output}' \
              --quote_style=${escape_style} \
              ${optionalString secrets.${name}.quote "--quote"}
          '') (attrNames secrets)
        ));
