...
config ? null, # Nixpkgs, for qemu, lib and more
pkgs, lib, # !!! See comment about args in lib/modules.nix
specialArgs ? { }, # NixOS configuration to add to the VMs
extraConfigurations ? [ ] }:
            ...
            let
              ...
              networkConfig = {
                ...
                virtualisation.qemu.options = let
                  qemu-common =
                    import ../lib/qemu-common.nix { inherit lib pkgs; };
