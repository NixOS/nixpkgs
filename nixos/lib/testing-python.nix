...
with pkgs; rec {
  ...
  setupDriverForTest = {
    ...
    in
      with builtins;
        ...
        else "nixos-test-driver-${testName}";
  ...
  in
    if lib.length invalidNodeNames > 0
    then
      throw ''
  ...
  makeTest = {
    ...
    in
      passMeta (runTests {inherit driver pos driverInteractive;});
  in
    test
    // {
  ...
  abortForFunction = functionName:
    abort ''      The ${functionName} function was
