...
rec {
  ...
  pythonTestDriver = { qemu_pkg ? pkgs.qemu_test, enableOCR ? false }:
    let
    ...
    in stdenv.mkDerivation {
      ...
      installPhase = ''
        ...
        wrapProgram $out/bin/nixos-test-driver \
          --argv0 ${name} \
          --prefix PATH : "${
            lib.makeBinPath [ qemu_pkg vde2 netpbm coreutils socat ]
          }" \
          ${
  ...
  runTests = { driver, pos }:
    stdenv.mkDerivation {
      ...
      passthru = driver.passthru // { inherit driver; };
  ...
  setupDriverForTest = { testScript, testName, nodes, qemu_pkg ? pkgs.qemu_test
    , enableOCR ? false, skipLint ? false, passthru ? { } }:
    let
      ...
      if testNameLen > maxTestNameLen then
        abort ("The name of the test '${testName}' must not be longer than ${
      ...
      nodeHostNames =
        let nodesList = map (c: c.config.system.name) (lib.attrValues nodes);
    ...
    in if lib.length invalidNodeNames > 0 then
      ...
      lib.warnIf skipLint "Linting is disabled" (runCommand testDriverName {
        ...
        preferLocalBuild = true;
        passthru = passthru // { inherit nodes; };
      } ''
        mkdir -p $out/bin
  ...
  makeTest = { testScript, enableOCR ? false, name ? "unnamed"
      # Skip linting (mainly intended for faster dev cycles)
    , skipLint ? false, passthru ? { }, # For meta.position
    pos ? # position used in error messages and for meta.position
      (if t.meta.description or null != null then
        builtins.unsafeGetAttrPos "description" t.meta
      else
        builtins.unsafeGetAttrPos "testScript" t), ... }@t:
    let
      nodes = qemu_pkg:
        let
          testScript' =
            # Call the test script with the computed nodes.
            if lib.isFunction testScript then
              testScript { nodes = nodes qemu_pkg; }
            else
              testScript;
        ...
        in build-vms.buildVirtualNetwork
      ...
      test = let
        passMeta = drv:
          drv // lib.optionalAttrs (t ? meta) {
            meta = (drv.meta or { }) // t.meta;
          };
      in passMeta (runTests { inherit driver pos; });

    in test // { inherit test driver driverInteractive nodes; };

  abortForFunction = functionName:
    abort ''
      The ${functionName} function was
