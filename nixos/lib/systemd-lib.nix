    ...
    config = {
      unitConfig =
        ...
        // optionalAttrs (config ? restartTriggers && config.restartTriggers != [])
          { X-Restart-Triggers = "${pkgs.writeText "X-Restart-Triggers" (builtins.unsafeDiscardStringContext (toString config.restartTriggers))}"; }
        // optionalAttrs (config ? reloadTriggers && config.reloadTriggers != [])
          { X-Reload-Triggers = "${pkgs.writeText "X-Reload-Triggers" (builtins.unsafeDiscardStringContext (toString config.reloadTriggers))}"; }
