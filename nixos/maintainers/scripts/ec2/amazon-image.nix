...
let
  ...
  amiBootMode =
    if config.ec2.efi then "uefi" else "uefi-preferred";
in {
  ...
  config.boot.kernelParams = let
    timeout =
      if versionAtLeast config.boot.kernelPackages.kernel.version "4.15" then
        "4294967295"
      else
        "255";
  ...
  config.system.build.amazonImage = let
    ...
    extBuilder = import ../../../lib/make-disk-image.nix {
      ...
      fsType = "ext4";
      partitionTableType =
        if pkgs.stdenv.hostPlatform.isAarch64 then "efi" else "hybrid";
