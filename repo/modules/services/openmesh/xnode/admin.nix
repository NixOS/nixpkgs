  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.openmesh-xnode-admin = {
      ...
      serviceConfig = {
        ...
        ExecStartPre = ''-${lib.getExe pkgs.git} clone --branch feature/import-agent-config-dynamically-testing https://github.com/openmesh-network/xnodeos ${cfg.stateDir}/xnodeos'';
        ExecCondition = ''-${lib.getExe pkgs.git} pull --branch feature/import-agent-config-dynamically-testing https://github.com/openmesh-network/xnodeos ${cfg.stateDir}/xnodeos'';
        Restart = "always";
